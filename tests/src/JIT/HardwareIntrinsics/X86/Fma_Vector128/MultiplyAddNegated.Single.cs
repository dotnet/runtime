...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    {
        private static void MultiplyAddNegatedSingle()
        {
    ...
    public sealed unsafe class SimpleTernaryOpTest__MultiplyAddNegatedSingle
    {
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleTernaryOpTest__MultiplyAddNegatedSingle testClass)
            {
        ...
        public SimpleTernaryOpTest__MultiplyAddNegatedSingle()
        {
            Succeeded = true;
        ...
        {
            var result = typeof(Fma).GetMethod(nameof(Fma.MultiplyAddNegated), new Type[] { typeof(Vector128<Single>), typeof(Vector128<Single>), typeof(Vector128<Single>) })
        ...
        private void ValidateResult(Single[] firstOp, Single[] secondOp, Single[] thirdOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!Succeeded)
            {
                Console.WriteLine($"{nameof(Fma)}.{nameof(Fma.MultiplyAddNegated)}<Single>(Vector128<Single>, Vector128<Single>, Vector128<Single>): {method} failed:");
