...
usage()
{
  ...
  echo "  --arch (-a)                     Target platform: x86, x64, arm, armv6, armel, arm64, s390x or wasm."
...
while [[ $# > 0 ]]; do
  ...
  case "$opt" in
     ...
     -arch|-a)
      ...
      case "$passedArch" in
        x64|x86|arm|armv6|armel|arm64|s390x|wasm)
          arch=$passedArch
          ;;
        *)
          echo "Unsupported target architecture '$2'."
          echo "The allowed values are x86, x64, arm, armv6, armel, arm64, s390x, and wasm."
          exit 1
          ;;
      esac
      shift 2
