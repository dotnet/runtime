...
if /I [%XHARNESS_COMMAND%] == [test] (
    ...
    if [%JS_ENGINE_ARGS%] == [] (
        set "JS_ENGINE_ARGS=--browser-arg^=--js-flags^=--stack-trace-limit^=1000 --browser-arg^=-–enable-precise-memory-info --browser-arg^=--disable-hang-monitor --browser-arg^=--enable-logging --browser-arg^=--v^=2 --browser-arg^=--log-level^=INFO --browser-arg^=--log-file^=%XHARNESS_OUT%\chrome.log"
