...
if [[ -n "$wasm_runtime_loc" ]]; then
    ...
    #rsync -aq --progress $source_directory/artifacts/bin/dotnet-workload $wasm_dotnet_path/dotnet-workload
    #cp -R $rce_directory/bin/wasm/dotnet-workload $wasm_dotnet_path/dotnet-workload
    ...
    if [[ "$wasmaot" == "true" ]]; then
        extra_benchmark_dotnet_arguments="$extra_benchmark_dotnet_arguments --wasmEngine /home/helixbot/.jsvu/$javascript_engine --cli $$HELIX_CORRELATION_PAYLOAD/dotnet-wasm/dotnet-workload/dotnet --aotcompilermode wasm --buildTimeout 3600"
    else
        extra_benchmark_dotnet_arguments="$extra_benchmark_dotnet_arguments --wasmEngine /home/helixbot/.jsvu/$javascript_engine --cli $$HELIX_CORRELATION_PAYLOAD/dotnet-wasm/dotnet-workload/dotnet"
    fi
fi
