...
if /i "%__Arch%" == "wasm" (
    ...
    if /i "%__Os%" == "wasi" (
        ...
        set __CmakeGenerator=Ninja
        set __ExtraCmakeParams=%__ExtraCmakeParams% -DCLR_CMAKE_TARGET_OS=wasi -DCLR_CMAKE_TARGET_ARCH=wasm "-DWASI_SDK_PREFIX=!WASI_SDK22_PATH!" "-DCMAKE_TOOLCHAIN_FILE=!__repoRoot!/src/native/external/wasi-sdk-p2.cmake" "-DCMAKE_SYSROOT=!WASI_SDK22_PATH!share/wasi-sysroot" "-DCMAKE_CROSSCOMPILING_EMULATOR=!EMSDK_NODE! --experimental-wasm-bigint --experimental-wasi-unstable-preview1"
    )
) else (
