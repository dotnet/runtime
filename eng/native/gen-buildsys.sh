...
if [[ "$CLR_CMAKE_SKIP_PLATFORM_CACHE" != "1" ]]; then
    # Check for pipeline-cached tryrun file (generated by CI via extract-tryrun-cache.sh)
    ...
    elif [[ "$target_os" == "osx" && "$host_arch" == "arm64" && -f "$scriptroot/tryrun.osx-arm64.cmake" ]]; then
        # Fall back to checked-in cache for osx-arm64
