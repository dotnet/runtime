...
if [[ "$CLR_CMAKE_USE_PLATFORM_CACHE" == "1" ]]; then
    __pipeline_cache="$reporoot/artifacts/cmake-tryrun-cache/tryrun.${target_os}-${host_arch}.cmake"
    if [[ -f "$__pipeline_cache" ]]; then
        echo "Using pipeline-cached tryrun cache: $__pipeline_cache"
        cmake_extra_defines="-C $__pipeline_cache $cmake_extra_defines"
    fi
fi
