...
if [[ "$host_arch" == "wasm" ]]; then
    if [[ "$target_os" == "browser" ]]; then
        ...
        cmake_extra_defines=("$cmake_extra_defines" "-DCLR_CMAKE_TARGET_OS=wasi" "-DCLR_CMAKE_TARGET_ARCH=wasm" "-DWASI_SDK_PREFIX=$WASI_SDK22_PATH" "-DCMAKE_TOOLCHAIN_FILE=$WASI_SDK22_PATH/share/cmake/wasi-sdk.cmake" "-DCMAKE_SYSROOT=${WASI_SDK22_PATH}share/wasi-sysroot" "-DCMAKE_CROSSCOMPILING_EMULATOR=node --experimental-wasm-bigint --experimental-wasi-unstable-preview1")
    else
