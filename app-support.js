...
// FIXME: process args before all this
...
let loadScript2 = undefined;
if (typeof WScript !== "undefined") { // Chakra
    loadScript2 = function (file) {
        return Promise.resolve(WScript.LoadScriptFile(file));
    };
    ...
    loadScript2 = function (file) {
        return Promise.resolve(import(file));
    }
}
else if (typeof globalThis.load !== 'undefined') {
    loadScript2 = function (file) {
        return Promise.resolve(globalThis.load(file));
    }
}
else {
    throw new Error("Unknown environment, can't figure out a `loadScript2` to use");
}
globalThis.loadScript2 = loadScript2;
