...
public class EmbeddingApiTests
{
    ...
    {
        var obj = new object();
        var handle1 = CoreCLRHostWrappers.gchandle_new_v2(obj, false);
        Assert.That(handle1, Is.Not.EqualTo(0));
        var result = CoreCLRHostWrappers.gchandle_get_target_v2(handle1);
        Assert.That(obj, Is.EqualTo(result));

        var obj2 = new object();
        var handle2 = CoreCLRHostWrappers.gchandle_new_v2(obj2, true);
        Assert.That(handle2, Is.Not.EqualTo(0));
        var result2 = CoreCLRHostWrappers.gchandle_get_target_v2(handle2);
    ...
    {
        var result = CoreCLRHostWrappers.object_isinst(instance, type);
    ...
    {
        Assert.That(CoreCLRHostWrappers.object_get_class(Activator.CreateInstance(type)!), Is.EqualTo(type));
    }
    ...
    {
        var result = CoreCLRHostWrappers.unity_array_new_3d(arrayType, size0, size1, size2);
