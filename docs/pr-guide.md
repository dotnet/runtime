...
### Additional information:
  ...
  * In the rare case the license/cla check fails to register a response, it can be rerun by issuing a GET request to `https://cla.dotnetfoundation.org/check/{org}/{repo}?pullRequest={pr_number}`. A successful response may be a redirect to `https://github.com`.
