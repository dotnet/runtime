...
docker run --rm \
  ...
  ./build.sh --subset clr --cross --configuration Checked
```
...
- `./build.sh --subset clr --cross --configuration Checked`: The build command to run in the repo. In this case, we want to build the *Clr* subset in the *Checked* configuration with the cross compilation option.
