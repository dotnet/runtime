...
- `-framework|-f` identifies the target framework for the build. Possible values include `net12.0` (currently the latest .NET version) or `net481` (the latest .NET Framework version). (msbuild property `BuildTargetFramework`)
