...
| - | Microsoft.Extensions.Configuration.DisableConfigurationReferences | When set to true, Configuration stops resolving `$ref(...)` values and reference resolution is trimmed away. Transitional escape hatch while the feature settles; expected to be removed once it has, so it has no MSBuild property. |
