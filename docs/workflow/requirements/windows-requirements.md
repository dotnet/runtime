...
- The build tools (CMake, Ninja and Python) can be downloaded and installed separately (see detailed instructions in the [section below](#build-tools)) or by selecting the following **Individual Components**:
  - **C++ CMake tools for Windows** (includes Ninja, but might not work on ARM machines),
...
- Install [CMake](https://cmake.org/download) for Windows. For ARM machines please use the `windows-i386` installer (`windows-x86_64` is not going to work).
...
- Since there is no official Ninja support for ARM, you can use MSBuild instead by passing `-msbuild` to `build.cmd`.
...
- Use MSBuild instead of Ninja (no official support for ARM yet).
