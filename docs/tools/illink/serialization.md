...
The ILLink tool cannot analyze the patterns typically used by reflection-based serializers. Such serializers should be annotated with `RequiresUnreferencedCodeAttribute`, and using them in a trimmed app will likely not work (or will work unpredictably). The ILLink tool will produce static analysis [warnings](https://docs.microsoft.com/dotnet/core/deploying/trimming-options#analysis-warnings) for these patterns.
...
- Preservation logic: what the ILLink tool does with the discovered types
...
- The type, or the declaring type of the member, is used
  ...
  There must be a statically discoverable reference to the type. In other words, if running the ILLink tool without the serialization heuristics removes a given type, then the heuristics will not discover it or any of its members as a serialization root.
...
For each discovered type (including root types and the recursive type graph), if the corresponding serializer is active, the ILLink tool marks the type and the following members:
...
In addition, the ILLink tool marks:
