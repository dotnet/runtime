...
[src/inc/corjit.h](/src/coreclr/inc/corjit.h)
and its implementation is in
[src/jit/ee_il_dll.cpp](/src/coreclr/jit/ee_il_dll.cpp).
...
[src/inc/corinfo.h](/src/coreclr/inc/corinfo.h). The implementation
is defined in
[src/vm/jitinterface.cpp](/src/coreclr/vm/jitinterface.cpp).
...
[src/tests/JIT/CodeGenBringUpTests/DblRoots.cs](/src/tests/JIT/CodeGenBringUpTests/DblRoots.cs)), with `DOTNET_TieredCompilation=0` and using the DblRoots_ro.csproj project to compile it:
...
([emit.h](/src/coreclr/jit/emit.h)), using the
...
* The "instruction" enumeration itemizes the different instructions available on each target, and is used as an index into the various encoding tables (e.g. `instInfo[]`, `emitInsModeFmtTab[]`) generated from the `instrs{tgt}.h` (e.g., [instrsxarch.h](/src/coreclr/jit/instrsxarch.h)).
...
[src/jit/jitgcinfo.h](/src/coreclr/jit/jitgcinfo.h) (to
differentiate it from
[src/inc/gcinfo.h](/src/coreclr/inc/gcinfo.h)), and implemented in
[src/jit/gcinfo.cpp](/src/coreclr/jit/gcinfo.cpp).
...
* Mapping of user locals to location (register or stack). This is accomplished via:
  * Struct `siVarLoc` (in [compiler.h](/src/coreclr/jit/compiler.h)) captures the location
  * `VarScopeDsc` ([compiler.h](/src/coreclr/jit/compiler.h)) captures the live range of a local variable in a given location.
...
at [jiteh.h](/src/coreclr/jit/jiteh.h) and
[jiteh.cpp](/src/coreclr/jit/jiteh.cpp), especially, for details.
...
[src/jit/compiler.hpp](/src/coreclr/jit/compiler.hpp)). If you find a
