...
* GC encoding
  * So far only JIT32_GCENCODER (used by Windows x86) and everything else
* Debug information (so far mostly the same for all targets?)
...
There are several steps to follow to port the JIT (some of which should be parallelized):
...
1. Initial bring-up.
   ...
   * Focus on the CodeGenBringUpTests (src\coreclr\tests\src\JIT\CodeGenBringUpTests), starting with the simple ones.
     These are designed such that for a test XXX.cs, there is a single interesting function XXX to compile. You
     set `COMPlus_AltJit=XXX` so the new JIT only attempts to compile that one function.
   * Use `COMPlus_JitDisasm` to see the generated code for functions, even if the code isn't run.
2. Expand test coverage.
   * Get more and more tests to run.
   * It is helpful to collect data on asserts generated by the JIT across the entire test base, and fix them in
     order of frequency (i.e., fix the most frequently occurring asserts first). Track the number of asserts, and number
     of tests with/without asserts.
3. Bring the optimizer phases on-line
...
4. Improve quality
   ...
   * Bring `GCStress` on-line. Work on `COMPlus_GCStress=4` quality. When crossgen/ngen is brought on-line, test with `COMPlus_GCStress=8`
     and `COMPlus_GCStress=C` as well.
5. Work on performance.
   * Determine a strategy for measuring and improving performance.
6. Work on platform parity.
...
* The `insGroup` and `instrDesc` the data structures used for encoding
  * It is initialized with the opcode bits, and has fields for immediates and register numbers.
  * `instrDesc`s are collected into groups
