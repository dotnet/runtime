...
|`compSupportsNoReporting(isa)` | Use to assert whether or not an instruction set is supported, or to set up the infrastructure to support `compExactlyDependsOn` and `compOpportunisticallyDependsOn`| Return whether or not an instruction set is supported. Does not report anything. 
