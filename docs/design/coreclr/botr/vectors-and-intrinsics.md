...
Some APIs exposed in System.Private.Corelib are intentionally non-deterministic across hardware and instead only ensure determinism within the scope of a single process. ISA choices that affect their results must preserve both positive and negative prerequisites in ReadyToRun code, including in CoreLib, through `compExactlyDependsOn(isa, true)`. Their method bodies must agree with expanded calls, using self-recursion or an intrinsified fallback helper where necessary, so that indirect invocation (such as via a delegate, function pointer, reflection, etc) will compute the same result.
