...
*Interop IL stubs* - The VM passes context to shared vararg PInvoke stubs and native-to-managed IL stubs in (x86) `EAX` / (AMD64) `R10` / (ARM, ARM64) `R12` (in the JIT: `REG_SECRET_STUB_PARAM`). For a vararg PInvoke stub, the context is the MethodDesc for the exact PInvoke. For a native-to-managed stub, the context is the `UMEntryThunkData` used to recover the managed target or delegate. The IL stub represents this value as an explicit `native int` parameter with a required `System.Runtime.CompilerServices.SecretStubArgument` modifier, which tells the JIT to bind the parameter to the secret stub register.
