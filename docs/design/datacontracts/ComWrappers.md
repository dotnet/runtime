...
Version 2 supports RCWs deriving from `System.Runtime.InteropServices.ComObjectBase`.
...
| --- | --- | --- | --- | --- |
| Added | `System.Runtime.InteropServices.ComObjectBase` | `_nativeObjectWrapper` | `pointer` | NativeObjectWrapper associated directly with an RCW deriving from ComObjectBase |
...
| Fully-qualified name | Module | Members read | Purpose |
| --- | --- | --- | --- |
| `System.Runtime.InteropServices.ComObjectBase` | `System.Private.CoreLib` | `_nativeObjectWrapper` | Stores the RCW association directly on objects deriving from this type |

`GetComWrappersRCWForObject` first attempts to resolve the loaded
`ComObjectBase` type using `ManagedTypeSource`. If the type is loaded, it walks
