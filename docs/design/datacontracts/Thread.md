...
| `Thread` | `GCFrame` | Pointer to the head of the thread's GCFrame (GCPROTECT) chain. Optional: when the descriptor omits the field, readers observe `null` (the contract models it as a nullable `TargetPointer?`), which means "no GCFrame chain". |
