...
For all of this section assume that we enabled the AllocationSampling events and have observed `s` such sample events were generated from a specific thread - `event_1`, `event_2`, ... `event_s`. Each `event_i` contains corresponding fields `type_i`, `size_i`, and `offset_i`. Let `u_i = size_i - offset_i`. `u_i` represents the successful trial byte + the number bytes which remained after it in the same object. Let `u` = the sum of all the `u_i`, `i` = 1 to `s`. `p` is the constant 0.00001, the probability that each trial is successful. `q` is the complement 0.99999.
...
Some tools will likely ignore sampling error entirely or perhaps adopt simple strategies where some cutoff number of samples is deemed to produce reliable estimates. However it is possible to be more rigorous than that. The distribution for the estimate above, `n_estimate` is described by a [Negative Bernoulli distribution](https://en.wikipedia.org/wiki/Negative_binomial_distribution) with `s` successes and `p` chance of success. Here is an approach to calculate a [confidence interval](https://en.wikipedia.org/wiki/Confidence_interval) estimate based on this distribution:
...
For example if we select `C=0.95`, observed 8 samples and `u=10,908` then we'd use binary search to find `CDF(345376, 8, 1/10000) ~= 0.025` and `CDF(1442255, 8, 1/10000) ~= 0.975`. Our interval estimate for the number of bytes allocated would be `[345,376 + 10,908, 1,442,255 + 10,908]`.
...
#### Estimating the total number of bytes allocated on a thread in between events
...
List<bool> PopulateTrials(CLRRandom* pRandom)
{
    ...
    for(int i = 0; i < 100; i++)
    {
        trials.Push(pRandom->NextDouble() < 0.00001);
    }
    return trials;
}
...
BigInteger PopulateTrials(CLRRandom* pRandom)
{
    ...
    while(pRandom->NextDouble() >= 0.00001)
    {
        failedTrials++;
    }
    return failedTrials;
...
{
    return floor(log(1-pRandom->NextDouble())/log(1-0.00001));
}
...
The only change we've made in the assembly helpers is doing a comparison against alloc_sampling instead of alloc_limit which should have no performance impact. Look at [JIT_TrialAllocSFastMP_InlineGetThread](https://github.com/dotnet/runtime/blob/5c8bb402e6a8274e8135dd00eda2248b4f57102f/src/coreclr/vm/amd64/JitHelpers_InlineGetThread.asm#L38) for an example of what one of these helpers looks like in assembly code.
