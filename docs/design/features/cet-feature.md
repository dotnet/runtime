...
When opted in as above, CET shadow stacks are enabled in compatibility mode. .NET 7 is not yet fully compatible with strict mode.

CET shadow stack compatibility is available to try for Windows x64 applications. Support for the feature on Linux is pending and something we will enable in upcoming .NET releases.