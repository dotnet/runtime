...
internal sealed partial class GrammarActions
{
        ...
        int initialSyntaxErrorCount)
    {
        ...
        _currentMethod.Definition.ExportOrdinal = -1;
        _currentMethod.Definition.ExportAlias = null;
        _exportDirectiveContexts.Remove(_currentMethod.Definition);
        if (hasSyntaxError || context.ordinal is null)
        {
            return;
        }

        int diagnosticCount = _diagnostics.Count;
        int ordinal = ParseInt32(context.ordinal.Start);
        string? alias = context.alias is null ? null : ParseIdentifier(context.alias.Start);
        ...
        _currentMethod.Definition.ExportOrdinal = ordinal;
        _currentMethod.Definition.ExportAlias = alias;
        _exportDirectiveContexts[_currentMethod.Definition] = context;
    }
        ...
        int initialSyntaxErrorCount)
    {
        ...
        _currentMethod.Definition.VTableEntry = 0;
        _currentMethod.Definition.VTableSlot = 0;
        _vtableEntryDirectiveContexts.Remove(_currentMethod.Definition);
        if (hasSyntaxError || context.table is null || context.slot is null)
        {
            return;
        }

        int diagnosticCount = _diagnostics.Count;
        int vtableEntry = ParseInt32(context.table.Start);
        int vtableSlot = ParseInt32(context.slot.Start);
        if (_diagnostics.Count != diagnosticCount)
        {
            return;
        }

        _currentMethod.Definition.VTableEntry = vtableEntry;
        _currentMethod.Definition.VTableSlot = vtableSlot;
        _vtableEntryDirectiveContexts[_currentMethod.Definition] = context;
    }
