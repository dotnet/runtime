...
internal class HotReloadInstanceFieldTable
{
    ...
    //
    // The whole thing is basically a ConditionalWeakTable<object,Dictionary<uint,Store>> but
    ...
    internal static Store GetInstanceFieldStore(object inst, RuntimeTypeHandle type, uint fielddef_token)
    ...
    {
        private Dictionary<uint, Store> _fields;
        ...
        public Store LookupOrAdd(RuntimeTypeHandle type, uint key)
        {
            if (_fields.TryGetValue(key, out Store? v))
                return v;
            lock (_lock)
            {
                if (_fields.TryGetValue (key, out Store? v2))
                    return v2;

                Store s = Store.Create(type);
                _fields.Add(key, s);
                return s;
            }
        }
    ...
    internal class Store
    {
        ...
        public static Store Create (RuntimeTypeHandle type)
        {
