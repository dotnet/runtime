...
trace_info_allocate_segment (gint32 index) {
	...
	static_assert (sizeof(atomic_uintptr_t) == sizeof(TraceInfo *) && ATOMIC_POINTER_LOCK_FREE == 2, "");
...
#else
	static_assert (sizeof(atomic_int) == sizeof(gint32) && ATOMIC_INT_LOCK_FREE == 2);
...
#else
	static_assert (sizeof(atomic_long) == sizeof(gint64) && ATOMIC_LONG_LOCK_FREE == 2);
...
#else
	static_assert (sizeof(long) == sizeof(atomic_long) && ATOMIC_LONG_LOCK_FREE == 2);
...
mono_jiterp_monitor_trace (const guint16 *ip, void *_frame, void *locals)
{
...
#else
		static_assert (sizeof(int) == sizeof(atomic_int) && ATOMIC_INT_LOCK_FREE == 2);
	...
	if (hit_count == mono_opt_jiterpreter_trace_monitoring_period) {
		...
		if (average_penalty <= threshold) {
			...
			static_assert (sizeof(atomic_ushort) == sizeof(guint16) && ATOMIC_SHORT_LOCK_FREE == 2);
...
mono_jiterp_increment_counter (volatile int *counter) {
	...
	*counter = result + 1;
	return result;
#else
	static_assert (sizeof (atomic_int) == sizeof(int) && ATOMIC_INT_LOCK_FREE == 2);
