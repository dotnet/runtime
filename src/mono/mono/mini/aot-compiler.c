...
add_generic_instances (MonoAotCompile *acfg)
{
	...
	if (acfg->image == mono_defaults.corlib) {
		...
		/* Add instances of EnumEqualityComparer which are created by EqualityComparer<T> for enums */
		{
			...
			ninsts = 0;
			for (int i = 0; i < MINIPAL_LENGTHOF (enum_names); ++i) {
...
get_debug_sym (MonoMethod *method, const char *prefix, GHashTable *cache)
{
	...
	prefix = mangle_symbol (prefix, adjustedPrefix, MINIPAL_LENGTHOF (adjustedPrefix));
#endif
...
codeview_parse_compiler_version (gchar *version, CodeViewCompilerVersion *data)
{
	...
	while (*version && (value < values + MINIPAL_LENGTHOF (values))) {
		if (isdigit (*version)) {
			*value *= 10;
			*value += *version - '0';
		}
...
emit_codeview_info (MonoAotCompile *acfg)
{
	...
	for (i = 0; i < acfg->nmethods; ++i) {
		...
		int ret = g_snprintf (symbol_buffer, MINIPAL_LENGTHOF (symbol_buffer), "%sme_%x", acfg->temp_prefix, i);
		if (ret > 0 && ret < MINIPAL_LENGTHOF (symbol_buffer))
...
add_preinit_got_slots (MonoAotCompile *acfg)
{
	...
	for (i = 0; i < MINIPAL_LENGTHOF (preinited_jit_icalls); ++i) {
		ji = (MonoJumpInfo *)mono_mempool_alloc0 (acfg->mempool, sizeof (MonoJumpInfo));
...
#ifndef MONO_ARCH_HAVE_INTERP_ENTRY_TRAMPOLINE
		for (int i = 0; i < MINIPAL_LENGTHOF (interp_in_static_sigs); i++)
			add_interp_in_wrapper_for_sig (acfg, *interp_in_static_sigs [i]);
#else
		for (int i = 0; i < MINIPAL_LENGTHOF (interp_in_static_common_sigs); i++)
