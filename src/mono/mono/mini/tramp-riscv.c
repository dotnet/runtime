                                     ...
                                     gboolean aot)
{
	...
	code = mono_riscv_emit_store_regarray (code, MONO_ARCH_CALLEE_SAVED_REGS, RISCV_FP, -lmf_offset + MONO_STRUCT_OFFSET (MonoLMF, gregs), FALSE);

	/* Save caller fp */
	code = mono_riscv_emit_load (code, RISCV_T0, RISCV_FP, -(gint32)sizeof(host_mgreg_t) * 2, 0);
	code = mono_riscv_emit_store (code, RISCV_T0, RISCV_FP, -lmf_offset + MONO_STRUCT_OFFSET (MonoLMF, gregs) + (MONO_ARCH_LMF_REG_FP * sizeof(host_mgreg_t)), 0);

	/* Save caller sp */
	riscv_addi (code, RISCV_T3, RISCV_FP, -frame_size);
	code = mono_riscv_emit_store (code, RISCV_T3, RISCV_FP, -lmf_offset + MONO_STRUCT_OFFSET (MonoLMF, gregs) + (MONO_ARCH_LMF_REG_SP * sizeof(host_mgreg_t)), 0);
	...
	code = mono_riscv_emit_load_regarray (code, (MONO_ARCH_ARGUMENT_REGS | (1 << RISCV_RA) | (1 << MONO_ARCH_RGCTX_REG)), RISCV_FP, -gregs_offset, FALSE);
	/* Restore fregs */
	if (!mono_arch_is_soft_float ()){
