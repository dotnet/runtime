...
mono_resolve_patch_target (MonoMethod *method, MonoDomain *domain, guint8 *code, MonoJumpInfo *patch_info, gboolean run_cctors, MonoError *error)
{
	...
	case MONO_PATCH_INFO_ADJUSTED_IID:
		...
		break;
	case MONO_PATCH_INFO_VTABLE:
		target = mono_class_vtable_full (domain, patch_info->data.klass, error);
		mono_error_assert_ok (error);
		break;
	case MONO_PATCH_INFO_DELEGATE_TRAMPOLINE: {
	...
	case MONO_PATCH_INFO_SFLDA: {
		MonoVTable *vtable = mono_class_vtable_full (domain, patch_info->data.field->parent, error);
		mono_error_assert_ok (error);
	...
	case MONO_PATCH_INFO_INTERRUPTION_REQUEST_FLAG:
		...
		break;
	case MONO_PATCH_INFO_METHOD_RGCTX: {
		MonoVTable *vtable = mono_class_vtable_full (domain, patch_info->data.method->klass, error);
		mono_error_assert_ok (error);
