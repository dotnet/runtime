...
emit_class_init (MonoCompile *cfg, MonoClass *klass)
{
	...
	if (context_used) {
		...
		MonoVTable *vtable = mono_class_vtable_full (cfg->domain, klass, &cfg->error);
		if (!is_ok (&cfg->error)) {
			mono_cfg_set_exception (cfg, MONO_EXCEPTION_MONO_ERROR);
			return;
		}
...
mini_emit_check_array_type (MonoCompile *cfg, MonoInst *obj, MonoClass *array_class)
{
	...
	if (cfg->opt & MONO_OPT_SHARED) {
		...
		if (cfg->compile_aot) {
			...
			if (!(vtable = mono_class_vtable_full (cfg->domain, array_class, &cfg->error))) {
				mono_cfg_set_exception (cfg, MONO_EXCEPTION_MONO_ERROR);
				return;
			}
...
handle_unbox_nullable (MonoCompile* cfg, MonoInst* val, MonoClass* klass, int context_used)
{
	...
	if (context_used) {
		...
		if (pass_vtable) {
			MonoVTable *vtable = mono_class_vtable_full (cfg->domain, method->klass, &cfg->error);
	...
	if (!(cfg->opt & MONO_OPT_SHARED)) {
		...
		if (method->iflags & METHOD_IMPL_ATTRIBUTE_AGGRESSIVE_INLINING) {
			if (method->klass->has_cctor) {
				MonoError error;
				vtable = mono_class_vtable_full (cfg->domain, method->klass, &error);
				if (!is_ok (&error)) {
				  ...
				  MonoInst **sp, guint8 *ip, int *inline_costs)
{
	...
	if (cmethod->klass->valuetype && mono_class_generic_sharing_enabled (cmethod->klass) &&
		...
		if (cmethod->is_inflated && mono_method_get_context (cmethod)->method_inst) {
			mono_class_vtable_full (cfg->domain, cmethod->klass, &cfg->error);
			CHECK_CFG_ERROR;
			...
			if (context_used) {
				...
				MonoVTable *vtable = mono_class_vtable_full (cfg->domain, cmethod->klass, &cfg->error);
				CHECK_CFG_ERROR;
				CHECK_TYPELOAD (cmethod->klass);
				EMIT_NEW_VTABLECONST (cfg, vtable_arg, vtable);
			}
				...
				*sp = emit_get_rgctx_method (cfg, context_used,
				...
				if (cmethod->klass->valuetype) {
					...
					*sp = alloc;
					...
					if (!cfg->compile_aot)
						vtable = mono_class_vtable_full (cfg->domain, cmethod->klass, &cfg->error);
					CHECK_CFG_ERROR;
					CHECK_TYPELOAD (cmethod->klass);
		...
		case CEE_STSFLD: {
			...
			*/
			...
			if (!context_used && !(cfg->opt & MONO_OPT_SHARED)) {
				mono_class_vtable_full (cfg->domain, klass, &cfg->error);
				CHECK_CFG_ERROR;
				CHECK_TYPELOAD (klass);
			}
			...
			if (is_special_static && ((gsize)addr & 0x80000000) == 0 && thread_ins && !(cfg->opt & MONO_OPT_SHARED) && !context_used) {
				...
				*/
				...
				if (!cfg->compile_aot)
					vtable = mono_class_vtable_full (cfg->domain, klass, &cfg->error);
				CHECK_CFG_ERROR;
				CHECK_TYPELOAD (klass);
		...
		case CEE_NEWARR: {
			...
			if (context_used) {
				...
				if (cfg->opt & MONO_OPT_SHARED) {
					...
					mono_class_vtable_full (cfg->domain, array_type, &cfg->error);
					CHECK_CFG_ERROR;
