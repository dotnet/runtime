		   ...
		   guint inline_offset, gboolean is_virtual_call)
{
	...
	if (method->wrapper_type == MONO_WRAPPER_NATIVE_TO_MANAGED) {
		...
		MonoBasicBlock *bb, *orig_cbb, *next_bb = NULL;

		/* Insert a new bblock between init_localsbb, and bblock */
		NEW_BBLOCK (cfg, bb);
		bb->real_offset = cfg->real_offset;
		init_localsbb->next_bb = bb;
		bb->next_bb = bblock;
		link_bblock (cfg, init_localsbb, bb);
		link_bblock (cfg, bb, bblock);
		mono_unlink_bblock (cfg, init_localsbb, bblock);
		...
		/* Restore the original domain at the end */
		orig_cbb = cfg->cbb;
		if (!cfg->compile_aot) {
			cfg->cbb = cfg->bb_exit;
			dreg = alloc_ireg (cfg);
			EMIT_NEW_UNALU (cfg, args [0], OP_MOVE, dreg, cfg->orig_domain_var->dreg);
			EMIT_NEW_ICONST (cfg, args [1], TRUE);
