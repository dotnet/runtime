...
mono_llvm_emit_match_exception_call (EmitContext *ctx, LLVMBuilderRef builder, gint32 region_start, gint32 region_end)
{
	...
	LLVMValueRef args[5];
	const int num_args = MINIPAL_LENGTHOF (args);
...
mono_llvm_create_aot_module (MonoAssembly *assembly, const char *global_prefix, int initial_got_size, LLVMModuleFlags flags)
{
	...
	if (emit_codeview) {
		...
		LLVMAddNamedMetadataOperand (module->lmodule, "llvm.module.flags", LLVMMDNode (codeview_option_args, MINIPAL_LENGTHOF (codeview_option_args)));
	}
	...
	if (!static_link) {
		...
		LLVMValueRef default_lib_args[MINIPAL_LENGTHOF (default_dynamic_lib_names)];
		LLVMValueRef default_lib_nodes[MINIPAL_LENGTHOF(default_dynamic_lib_names)];

		const char *default_lib_name = NULL;
		for (int i = 0; i < MINIPAL_LENGTHOF (default_dynamic_lib_names); ++i) {
		...
		LLVMAddNamedMetadataOperand (module->lmodule, "llvm.linker.options", LLVMMDNode (default_lib_args, MINIPAL_LENGTHOF (default_lib_args)));
	}
#endif
...
// flags_map cannot be zero length in MSVC, so add useless dummy entry for arm32
	...
	if (!cpu_features)
		cpu_features = MONO_CPU_INITED | (MonoCPUFeatures)mono_llvm_check_cpu_features (flags_map, MINIPAL_LENGTHOF (flags_map));
