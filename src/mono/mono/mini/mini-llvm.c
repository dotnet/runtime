...
static LLVMValueRef
mono_llvm_build_load (LLVMBuilderRef builder, LLVMTypeRef type, LLVMValueRef PointerVal,
...
emit_volatile_load (EmitContext *ctx, int vreg)
{
	...
	g_assert (ctx->address_types [vreg]);
	v = mono_llvm_build_load (ctx->builder, ctx->address_types [vreg], ctx->addresses [vreg], "", TRUE);
...
emit_volatile_store (EmitContext *ctx, int vreg)
{
	...
	if (var && var->flags & (MONO_INST_VOLATILE|MONO_INST_INDIRECT)) {
		...
		mono_llvm_build_store (ctx->builder, convert (ctx, ctx->values [vreg], type_to_llvm_type (ctx, var->inst_vtype)), ctx->addresses [vreg], TRUE);
#else
...
emit_icall_cold_wrapper (MonoLLVMModule *module, LLVMModuleRef lmodule, MonoJitICallId icall_id, gboolean aot)
{
	...
	if (aot) {
		...
		LLVMSetInitializer (tramp_var, LLVMConstIntToPtr (LLVMConstInt (LLVMInt64Type (), (guint64)(size_t)target, FALSE), el_t));
		LLVMSetLinkage (tramp_var, LLVMExternalLinkage);
...
emit_gc_pin (EmitContext *ctx, LLVMBuilderRef builder, int vreg)
{
	...
	mono_llvm_build_store (builder, convert (ctx, ctx->values [vreg], IntPtrType ()), addr, TRUE);
}
#endif
...
emit_entry_bb (EmitContext *ctx, LLVMBuilderRef builder)
{
		...
		if (var->flags & (MONO_INST_VOLATILE|MONO_INST_INDIRECT) || (mini_type_is_vtype (var->inst_vtype) && !MONO_CLASS_IS_SIMD (ctx->cfg, var->klass))) {
			...
			if (!ctx->addresses [var->dreg]) {
				if (var->flags & MONO_INST_LMF) {
					...
					ctx->address_types [var->dreg] = type_to_llvm_type (ctx, var->inst_vtype);
					g_free (name);
				}
			}
	...
	for (i = 0; i < sig->param_count; ++i) {
		...
		case LLVMArgGsharedvtFixedVtype: {
			...
			LLVMTypeRef el_t = type_to_llvm_type (ctx, ainfo->type);
			LLVMBuildStore (builder, LLVMBuildLoad2 (builder, el_t, convert (ctx, arg, LLVMPointerType (el_t, 0)), ""), ctx->addresses [reg]);
			break;
		}
		case LLVMArgGsharedvtVariable:
		...
		if (is_virtual) {
			...
			if (cfg->compile_aot) {
				...
				if (!tramp_var_addr) {
					...
					LLVMSetInitializer (tramp_var, LLVMConstIntToPtr (LLVMConstInt (LLVMInt64Type (), (guint64)(size_t)target, FALSE), tramp_var_type));
					LLVMSetLinkage (tramp_var, LLVMExternalLinkage);
	...
	for (ins = bb->code; ins; ins = ins->next) {
		...
		if (nins > 1000) {
			...
			if (!ctx->long_bb_break_var) {
				...
				mono_llvm_build_store (ctx->alloca_builder, LLVMConstInt (LLVMInt32Type (), 0, FALSE), ctx->long_bb_break_var, TRUE);
			}

			cbb = gen_bb (ctx, "CONT_LONG_BB");
			LLVMBasicBlockRef dummy_bb = gen_bb (ctx, "CONT_LONG_BB_DUMMY");

			LLVMValueRef load = mono_llvm_build_load (builder, i4_t, ctx->long_bb_break_var, "", TRUE);
			/*
			...
			mono_llvm_build_store (builder, LLVMConstInt (LLVMInt32Type (), 1, FALSE), ctx->long_bb_break_var, TRUE);
		...
		case OP_LOAD_MEM: {
			...
			if (is_unaligned)
				...
				values [ins->dreg] = mono_llvm_build_load (builder, NULL, addr, dname, is_volatile);
		...
		case OP_STORE_MEMBASE_REG: {
			...
			if (is_unaligned)
				...
				mono_llvm_build_store (builder, srcval, ptrdst, is_volatile);
			break;
		}
...
#define ARM64_ATOMIC_FENCE_FIX
		...
		case OP_SSE_MOVSS_STORE: {
			...
			mono_llvm_build_store (builder, val, addr, FALSE);
			break;
		}
		case OP_SSE2_MOVD:
		case OP_SSE2_MOVQ:
		...
		case OP_SSE_MOVHPS_STORE: {
			...
			mono_llvm_build_store (builder, val1, addr1, FALSE);
			mono_llvm_build_store (builder, val2, addr2, FALSE);
			break;
		}
...
// Shared between ARM64 and X86
		...
		case OP_ARM64_STNP_SCALAR: {
			...
			LLVMValueRef store = mono_llvm_build_store (builder, val, address, FALSE);
			if (nontemporal)
				set_nontemporal_flag (store);
			break;
		}
...
mono_llvm_emit_method (MonoCompile *cfg)
{
	...
	ctx->address_types = g_new0 (LLVMTypeRef, cfg->next_vreg);
