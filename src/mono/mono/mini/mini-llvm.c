...
#if 0
			if (LLVMTypeOf (rhs) != etype) {
				/* The vector might have a different size, so use a store+load to convert it */
				LLVMValueRef mem = build_alloca_llvm_type (ctx, etype, 16);
				emit_store (builder, rhs, convert (ctx, mem, pointer_type (LLVMTypeOf (rhs))), FALSE);
				rhs = emit_load (builder, res_type, convert (ctx, mem, pointer_type (res_type)), "", FALSE);
			}
#endif
