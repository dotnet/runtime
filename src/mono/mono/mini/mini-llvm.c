...
process_call (EmitContext *ctx, MonoBasicBlock *bb, LLVMBuilderRef *builder_ref, MonoInst *ins)
{
	...
	if (is_virtual) {
		...
		index = const_int32 ((int) (ins->inst_offset / size));
...
process_bb (EmitContext *ctx, MonoBasicBlock *bb)
{
	...
	for (ins = bb->code; ins; ins = ins->next) {
		...
		case OP_LIVERANGE_END:
			break;
		case OP_ICONST:
			values [ins->dreg] = const_int32 ((int) ins->inst_c0);
			break;
		case OP_I8CONST:
#if TARGET_SIZEOF_VOID_P == 4
		...
		case OP_COMPARE_IMM: {
			...
			if (ins->opcode == OP_ICOMPARE_IMM) {
				lhs = convert (ctx, lhs, LLVMInt32Type ());
				rhs = const_int32 ((int)ins->inst_imm);
			}
		...
		case OP_SHR_UN_IMM: {
			...
			if (spec [MONO_INST_SRC1] == 'l') {
				...
				imm = const_int32 ((int) ins->inst_imm);
			}
...
MONO_RESTORE_WARNING
		...
		case OP_LOCALLOC_IMM: {
			...
			guint32 size = (guint32)ins->inst_imm;
		...
		case OP_LOAD_MEM: {
			...
			if ((ins->opcode == OP_LOADI8_MEM) || (ins->opcode == OP_LOAD_MEM) || (ins->opcode == OP_LOADI4_MEM) || (ins->opcode == OP_LOADU4_MEM) || (ins->opcode == OP_LOADU1_MEM) || (ins->opcode == OP_LOADU2_MEM)) {
				...
				if (ins->inst_offset == 0) {
					...
					/* Unaligned load */
					index = const_int32 ((int)ins->inst_offset);
...
#define ARM64_ATOMIC_FENCE_FIX
		...
		case OP_EXTRACT_R8: {
			...
			case OP_XEXTRACT_I8:
				...
				break;
			default:
				element_ix = const_int32 ((int)(ins->inst_c0));
			}
		...
		case OP_INSERT_I1:
			values [ins->dreg] = LLVMBuildInsertElement (builder, values [ins->sreg1], convert (ctx, values [ins->sreg2], LLVMInt8Type ()), const_int32 ((int)ins->inst_c0), dname);
			break;
		case OP_INSERT_I2:
			values [ins->dreg] = LLVMBuildInsertElement (builder, values [ins->sreg1], convert (ctx, values [ins->sreg2], LLVMInt16Type ()), const_int32 ((int)ins->inst_c0), dname);
			break;
		case OP_INSERT_I4:
			values [ins->dreg] = LLVMBuildInsertElement (builder, values [ins->sreg1], convert (ctx, values [ins->sreg2], LLVMInt32Type ()), const_int32 ((int)ins->inst_c0), dname);
			break;
		case OP_INSERT_I8:
			values [ins->dreg] = LLVMBuildInsertElement (builder, values [ins->sreg1], convert (ctx, values [ins->sreg2], LLVMInt64Type ()), const_int32 ((int)ins->inst_c0), dname);
			break;
		case OP_INSERT_R4:
			values [ins->dreg] = LLVMBuildInsertElement (builder, values [ins->sreg1], convert (ctx, values [ins->sreg2], LLVMFloatType ()), const_int32 ((int)ins->inst_c0), dname);
			break;
		case OP_INSERT_R8:
			values [ins->dreg] = LLVMBuildInsertElement (builder, values [ins->sreg1], convert (ctx, values [ins->sreg2], LLVMDoubleType ()), const_int32 ((int)ins->inst_c0), dname);
			break;
		case OP_XCAST: {
		...
		case OP_SSE_UNPACKLO: {
			if (ins->inst_c1 == MONO_TYPE_R8 || ins->inst_c1 == MONO_TYPE_I8 || ins->inst_c1 == MONO_TYPE_U8) {
				...
				values [ins->dreg] = convert (ctx, shuffled, type_to_sse_type ((int)ins->inst_c1));
		...
		case OP_SSE_LOADU: {
			...
			LLVMTypeRef etype = type_to_sse_type ((int)ins->inst_c1);
			LLVMValueRef dst_vec = LLVMBuildBitCast (builder, dst_ptr, pointer_type (etype), "");
			values [ins->dreg] = mono_llvm_build_aligned_load (builder, etype, dst_vec, "", FALSE, (int)ins->inst_c0); // inst_c0 is alignment
			break;
		}
		case OP_SSE_MOVSS: {
			LLVMValueRef addr = convert (ctx, lhs, pointer_type (LLVMFloatType ()));
			LLVMValueRef val = LLVMBuildLoad2 (builder, LLVMFloatType (), addr, "");
			values [ins->dreg] = LLVMBuildInsertElement (builder, LLVMConstNull (type_to_sse_type ((int)ins->inst_c1)), val, const_int32 (0), "");
			break;
		}
		...
		case OP_SSE_STORE: {
			...
			mono_llvm_build_aligned_store (builder, rhs, dst_vec, FALSE, (int)ins->inst_c0);
			break;
		}
		...
		case OP_SSE_MOVNTPS: {
			...
			LLVMValueRef store = mono_llvm_build_aligned_store (builder, rhs, addr, FALSE, (int)ins->inst_c0);
		...
		case OP_SSE_OR: {
			...
			values [ins->dreg] = LLVMBuildBitCast (builder, vec_and, type_to_sse_type ((int)ins->inst_c1), "");
			break;
		}
		...
		case OP_SSE2_SUBS: {
			...
			case OP_SSE2_ADDS: type = (int)ins->inst_c1; break;
			case OP_SSE2_SUBS: type = (int)ins->inst_c1; is_add = FALSE; break;
			default: g_assert_not_reached ();
			}
			if (is_add) {
				switch (type) {
		...
		case OP_SSE2_PACKUS: {
			...
			values [ins->dreg] = convert (ctx,
				...
				type_to_sse_type ((int)ins->inst_c1));
			break;
		}
		...
		case OP_SSE2_PSRLDQ: {
			...
			values [ins->dreg] = convert (ctx, values [ins->dreg], type_to_sse_type ((int)ins->inst_c1));
		...
		case OP_SSSE3_ABS: {
			...
			LLVMTypeRef typ = type_to_sse_type ((int)ins->inst_c1);
		...
		case OP_SSE41_ROUNDP: {
			LLVMValueRef args [] = { lhs, const_int32 ((int)ins->inst_c0) };
		...
		case OP_SSE41_ROUNDS: {
			...
			args [0] = lhs;
			args [1] = rhs;
			args [2] = const_int32 ((int)ins->inst_c0);
		...
		case OP_SSE42_CRC64: {
			LLVMValueRef args [2];
			args [0] = lhs;
			args [1] = convert (ctx, rhs, primitive_type_to_llvm_type ((int)ins->inst_c0));
			IntrinsicId id;
			switch (ins->inst_c0) {
...
mono_llvm_emit_method (MonoCompile *cfg)
{
	...
	if (!ctx_ok (ctx)) {
		if (ctx->lmethod) {
			...
			for (int i = 0; i < ctx->phi_values->len; ++i) {
	...
	if (intrin_arm64_ovr [id] != 0) {
		...
		for (int vw = 0; vw < INTRIN_vectorwidths; ++vw) {
			for (int ew = 0; ew < INTRIN_elementwidths; ++ew) {
				...
				llvm_ovr_tag_t elem_bit = (llvm_ovr_tag_t) (INTRIN_int8 << ew);
...
emit_dbg_loc (EmitContext *ctx, LLVMBuilderRef builder, const unsigned char *cil_code)
{
	...
	if (ctx->minfo && cil_code && cil_code >= cfg->header->code && cil_code < cfg->header->code + cfg->header->code_size) {
		...
		loc = mono_debug_method_lookup_location (ctx->minfo, GSSIZE_TO_INT (cil_code - cfg->header->code));
