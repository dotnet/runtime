...
delete_image_set (MonoImageSet *set)
{
	...
	if (set->delegate_abstract_invoke_cache)
		g_hash_table_destroy (set->delegate_abstract_invoke_cache);
	if (set->runtime_invoke_direct_cache)
		g_hash_table_destroy (set->runtime_invoke_direct_cache);
	if (set->managed_wrapper_cache)
		g_hash_table_destroy (set->managed_wrapper_cache);
	if (set->native_wrapper_cache)
		g_hash_table_destroy (set->native_wrapper_cache);
	if (set->native_wrapper_aot_cache)
		g_hash_table_destroy (set->native_wrapper_aot_cache);
	if (set->native_wrapper_check_cache)
		g_hash_table_destroy (set->native_wrapper_check_cache);
	if (set->native_wrapper_aot_check_cache)
		g_hash_table_destroy (set->native_wrapper_aot_check_cache);
	if (set->native_func_wrapper_aot_cache)
		g_hash_table_destroy (set->native_func_wrapper_aot_cache);
	if (set->remoting_invoke_cache)
		g_hash_table_destroy (set->remoting_invoke_cache);
	if (set->unbox_wrapper_cache)
		g_hash_table_destroy (set->unbox_wrapper_cache);
	if (set->cominterop_invoke_cache)
		g_hash_table_destroy (set->cominterop_invoke_cache);
	if (set->cominterop_wrapper_cache)
		g_hash_table_destroy (set->cominterop_wrapper_cache);
	if (set->thunk_invoke_cache)
		g_hash_table_destroy (set->thunk_invoke_cache);
