...
mono_marshal_get_delegate_invoke_subtype (MonoMethod *method, MonoDelegate *del)
{
	...
	if (!del->target && mono_method_signature_internal (del->method)->hasthis) {
		if (!(del->method->flags & METHOD_ATTRIBUTE_VIRTUAL) && !m_class_is_valuetype (del->method->klass)) {
			if (sig->param_count == mono_method_signature_internal (del->method)->param_count + 1)
				/* Open instance delegate: first delegate arg becomes 'this' */
				return WRAPPER_SUBTYPE_NONE;
			if (sig->param_count == mono_method_signature_internal (del->method)->param_count)
				/* Closed over null: target (null) is passed as 'this' via the bound path */
				return WRAPPER_SUBTYPE_NONE;
		}
...
mono_marshal_get_delegate_invoke (MonoMethod *method, MonoDelegate *del)
{
	...
	if (del && !del->target && del->method && mono_method_signature_internal (del->method)->hasthis) {
		if (!(del->method->flags & METHOD_ATTRIBUTE_VIRTUAL) && !m_class_is_valuetype (del->method->klass) &&
		    (sig->param_count == mono_method_signature_internal (del->method)->param_count + 1 ||
		     sig->param_count == mono_method_signature_internal (del->method)->param_count)) {
			/* Open instance delegate or closed over null: the normal invoke code can handle this */
		} else {
			callvirt = TRUE;
		}
