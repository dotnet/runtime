...
retry_state_change:
	...
	case STATE_SELF_SUSPENDED:
		...
		if (!(suspend_count > 0 && suspend_count < THREAD_SUSPEND_COUNT_MAX))
			mono_fatal_with_history ("suspend_count = %d, but should be > 0 and < THREAD_SUSPEND_COUNT_MAX, for thread 0x%" G_GSIZE_FORMAT "x", suspend_count, (gsize)mono_thread_info_get_tid (info));
