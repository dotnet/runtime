...
deputy_thread_fn (void* unused_arg G_GNUC_UNUSED)
{
	...
	// because MONO_THREAD_CREATE_FLAGS_EXTERNAL_EVENTLOOP && keepalive, this will keep living in JS event loop
	return 0;
}
...
mono_wasm_create_deputy_thread (void)
{
	...
	ERROR_DECL (error);
	mono_thread_create_internal ((MonoThreadStart)deputy_thread_fn, NULL, MONO_THREAD_CREATE_FLAGS_EXTERNAL_EVENTLOOP, error);
