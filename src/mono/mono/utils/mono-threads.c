...
#if _MSC_VER
#pragma warning(disable:4189) // declaration of 'j' hides previous local declaration
#endif
...
dump_threads (void)
{
	...
	FOREACH_THREAD_SAFE_ALL (info) {
		...
		g_async_safe_printf ("--thread %p id %p [%p] (%s) state %x  %s\n", info, (void *)(gsize)mono_thread_info_get_tid (info), (void*)(size_t)info->native_handle, thread_name, info->thread_state, info == cur ? "GC INITIATOR" : "" );
#else
		g_async_safe_printf ("--thread %p id %p [%p] state %x  %s\n", info, (void *)(gsize)mono_thread_info_get_tid (info), (void*)(size_t)info->native_handle, info->thread_state, info == cur ? "GC INITIATOR" : "" );
#endif
