...
/*
Our Mach bindings have a problem in that they might need to attach the runtime after
the the user tls keys have been destroyed.

This happens when a bound object is retained by NSThread, which is released very late in the
TLS cleanup process.

At that point, attaching the runtime leaves us in no position to detach it later using TLS destructors
as pthread is done with user keys. This leaves us with a dead thread registered, which can cause
all sorts of terrible problems.

The tipical crash is when another thread is created at the exact same address of the previous one, cause
thread registration to abort due to duplicate entries.

So what do we do here?

Experimentation showns that threadDictionary is destroied after the problematic keys, so we add our dead letter
object as an aditional way to be notified of thread death.
*/
static void
