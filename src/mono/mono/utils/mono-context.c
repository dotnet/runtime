...
mono_sigctx_to_monoctx (void *sigctx, MonoContext *mctx)
{
	...
	memcpy (&mctx->gregs, &uctx->uc_mcontext.__gregs, sizeof (host_mgreg_t) * ARRAY_SIZE (mctx->gregs));
	memcpy (&mctx->fregs, &uctx->uc_mcontext.__fpregs, sizeof (double) * ARRAY_SIZE (mctx->fregs));
#endif
}
...
mono_monoctx_to_sigctx (MonoContext *mctx, void *sigctx)
{
	...
	memcpy (&uctx->uc_mcontext.__gregs, &mctx->gregs, sizeof (host_mgreg_t) * ARRAY_SIZE (mctx->gregs));
	memcpy (&uctx->uc_mcontext.__fpregs, &mctx->fregs, sizeof (double) * ARRAY_SIZE (mctx->fregs));
#endif
}
