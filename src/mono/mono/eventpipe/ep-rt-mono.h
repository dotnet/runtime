...
EP_RT_DEFINE_THREAD_FUNC (ep_rt_thread_mono_start_func)
{
	...
	if (result == 0) {
		// self schedule again
		mono_main_thread_schedule_ds_job ((background_job_cb1)ep_rt_thread_mono_start_func, (void*)thread_params);

		// don't free the thread_params and keep it for next run
		return result;
	}

#else // HOST_BROWSER && DISABLE_THREADS
...
#if defined(HOST_BROWSER) && defined(DISABLE_THREADS)
		// it will run inline and re-schedule itself if necessary
