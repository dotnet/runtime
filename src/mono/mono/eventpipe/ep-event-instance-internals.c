	...
	const uint8_t *related_activity_id)
{
	...
	ep_return_null_if_nok (ep_event != NULL);
...
ep_sequence_point_init (EventPipeSequencePoint *sequence_point)
{
	...
	sequence_point->thread_sequence_numbers.table = NULL;
	sequence_point->thread_sequence_numbers.count = 0;
...
ep_sequence_point_fini (EventPipeSequencePoint *sequence_point)
{
	...
	if (sequence_point->thread_sequence_numbers.table) {
...
const char quiet_linker_empty_file_warning_eventpipe_event_instance_internals = 0;
#endif
