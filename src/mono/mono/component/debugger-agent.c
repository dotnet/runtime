...
assembly_commands (int command, guint8 *p, guint8 *end, Buffer *buf)
{
	...
	case MDBGPROT_CMD_ASSEMBLY_GET_DEBUG_INFORMATION: {
		...
		char *ppdb_path;
		dn_vector_ptr_t /* <char*> */ pdb_checksum_hash_type = {0,};
		dn_vector_ptr_t /* <guint8*> */ pdb_checksum = {0,};
		dn_vector_ptr_init (&pdb_checksum_hash_type);
		dn_vector_ptr_init (&pdb_checksum);
		...
		{
			buffer_add_byte (buf, 0);
			dn_vector_ptr_dispose (&pdb_checksum_hash_type);
		...
		uint32_t checksum_count = dn_vector_ptr_size (&pdb_checksum_hash_type);
		buffer_add_int (buf, checksum_count);
		for (int i = 0 ; i < checksum_count; ++i) {
			char* checksum_hash_type =  *dn_vector_ptr_index_t (&pdb_checksum_hash_type, char*, i);
			buffer_add_string (buf, checksum_hash_type);
			if (!strcmp (checksum_hash_type, "SHA256"))
				buffer_add_byte_array (buf, *dn_vector_ptr_index_t (&pdb_checksum, guint8*, i), 32);
			else if (!strcmp (checksum_hash_type, "SHA384"))
				buffer_add_byte_array (buf, *dn_vector_ptr_index_t (&pdb_checksum, guint8*, i), 48);
			else if (!strcmp (checksum_hash_type, "SHA512"))
				buffer_add_byte_array (buf, *dn_vector_ptr_index_t (&pdb_checksum, guint8*, i), 64);
		}
		dn_vector_ptr_dispose (&pdb_checksum_hash_type);
		dn_vector_ptr_dispose (&pdb_checksum);
		break;
