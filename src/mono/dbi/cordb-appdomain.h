...
public:
  ...
  const char *GetClassName() { return "CordbAppDomain"; }
  ...
  HRESULT GetArrayOrPointerType(CorElementType elementType, ULONG32 nRank,
                                ICorDebugType *pTypeArg,
                                ICorDebugType **ppType);
  HRESULT
  GetFunctionPointerType(ULONG32 nTypeArgs, ICorDebugType *ppTypeArgs[],
                         ICorDebugType **ppType);
  HRESULT
  GetCachedWinRTTypesForIIDs(ULONG32 cReqTypes, GUID *iidsToResolve,
...
public:
  ...
  ULONG AddRef(void) { return (BaseAddRef()); }
  ULONG Release(void) { return (BaseRelease()); }
  const char *GetClassName() { return "CordbAppDomainEnum"; }
