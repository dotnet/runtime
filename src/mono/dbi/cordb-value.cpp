...
CordbValue::CreateBreakpoint(ICorDebugValueBreakpoint **ppBreakpoint) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbValue - CreateBreakpoint - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbValue::GetExactType(ICorDebugType **ppType) {
  ...
  if (m_pType == NULL) {
    m_pType = new CordbType(m_type, conn);
...
HRESULT STDMETHODCALLTYPE CordbReferenceValue::GetType(CorElementType *pType) {
  LOG((LF_CORDB, LL_INFO1000000,
       "CordbReferenceValue - GetType - IMPLEMENTED\n"));
  *pType = m_type;
  return S_OK;
}
...
HRESULT STDMETHODCALLTYPE CordbReferenceValue::GetSize(ULONG32 *pSize) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbReferenceValue - GetSize - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbReferenceValue::GetAddress(CORDB_ADDRESS *pAddress) {
  *pAddress = (CORDB_ADDRESS)&m_debuggerId;
  LOG((LF_CORDB, LL_INFO1000000,
       "CordbReferenceValue - GetAddress - IMPLEMENTED\n"));
  return S_OK;
}
...
CordbReferenceValue::CreateBreakpoint(ICorDebugValueBreakpoint **ppBreakpoint) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbReferenceValue - CreateBreakpoint - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbReferenceValue::GetExactType(ICorDebugType **ppType) {
  LOG((LF_CORDB, LL_INFO1000000,
       "CordbReferenceValue - GetExactType - IMPLEMENTED\n"));
  if (m_pCordbType) {
  ...
  if (m_type == ELEMENT_TYPE_SZARRAY && m_debuggerId != -1) {
    ...
    m_pCordbType =
        new CordbType(m_type, conn, NULL,
                      new CordbType((CorElementType)type_id, conn, m_pClass));
    m_pCordbType->InternalAddRef();
    m_pCordbType->QueryInterface(IID_ICorDebugType, (void **)ppType);
    return S_OK;
  }
  m_pCordbType = new CordbType(m_type, conn);
  m_pCordbType->InternalAddRef();
  m_pCordbType->QueryInterface(IID_ICorDebugType, (void **)ppType);
  return S_OK;
}
...
HRESULT STDMETHODCALLTYPE CordbReferenceValue::GetSize64(ULONG64 *pSize) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbReferenceValue - GetSize64 - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbReferenceValue::GetValue(void *pTo) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbReferenceValue - GetValue - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbReferenceValue::SetValue(void *pFrom) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbReferenceValue - SetValue - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbReferenceValue::IsNull(BOOL *pbNull) {
  if (m_debuggerId == -1)
    *pbNull = true;
  LOG((LF_CORDB, LL_INFO1000000,
       "CordbReferenceValue - IsNull - IMPLEMENTED\n"));
  return S_OK;
}
...
HRESULT STDMETHODCALLTYPE CordbReferenceValue::GetValue(CORDB_ADDRESS *pValue) {
  ...
  LOG((LF_CORDB, LL_INFO1000000,
       "CordbReferenceValue - GetValue - IMPLEMENTED\n"));
  return S_OK;
}

HRESULT STDMETHODCALLTYPE CordbReferenceValue::SetValue(CORDB_ADDRESS value) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbReferenceValue - SetValue CORDB_ADDRESS - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE
CordbReferenceValue::Dereference(ICorDebugValue **ppValue) {
  if (m_debuggerId == -1)
    return CORDBG_E_BAD_REFERENCE_VALUE;
  if (m_type == ELEMENT_TYPE_SZARRAY || m_type == ELEMENT_TYPE_ARRAY) {
    CordbArrayValue *objectValue =
        new CordbArrayValue(conn, m_pCordbType, m_debuggerId, m_pClass);
    objectValue->QueryInterface(IID_ICorDebugValue, (void **)ppValue);
  } else {
    CordbObjectValue *objectValue =
        new CordbObjectValue(conn, m_type, m_debuggerId, m_pClass);
    objectValue->QueryInterface(IID_ICorDebugValue, (void **)ppValue);
  }
  LOG((LF_CORDB, LL_INFO1000000,
       "CordbReferenceValue - Dereference - IMPLEMENTED\n"));
  return S_OK;
}

HRESULT STDMETHODCALLTYPE
CordbReferenceValue::DereferenceStrong(ICorDebugValue **ppValue) {
  LOG((LF_CORDB, LL_INFO100000,
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetSize(ULONG32 *pSize) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetSize - NOT IMPLEMENTED\n"));
  *pSize = 10;
  return S_OK;
}
...
CordbObjectValue::GetAddress(CORDB_ADDRESS *pAddress) {
  *pAddress = (CORDB_ADDRESS)&m_debuggerId;
  LOG((LF_CORDB, LL_INFO1000000,
       "CordbObjectValue - GetAddress - IMPLEMENTED\n"));
  return S_OK;
}
...
CordbObjectValue::CreateBreakpoint(ICorDebugValueBreakpoint **ppBreakpoint) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - CreateBreakpoint - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetSize64(ULONG64 *pSize) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetSize64 - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbObjectValue::GetValue(void *pTo) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetValue - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbObjectValue::SetValue(void *pFrom) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - SetValue - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetVirtualMethodAndType(
  ...
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetVirtualMethodAndType - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::CreateHandle(
  ...
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - CreateHandle - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetThreadOwningMonitorLock(
  ...
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetThreadOwningMonitorLock - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbObjectValue::GetMonitorEventWaitList(ICorDebugThreadEnum **ppThreadEnum) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetMonitorEventWaitList - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::EnumerateExceptionCallStack(
  ...
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - EnumerateExceptionCallStack - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetCachedInterfaceTypes(
  ...
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetCachedInterfaceTypes - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetCachedInterfacePointers(
    ...
    CORDB_ADDRESS *ptrs) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetCachedInterfacePointers - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbObjectValue::GetTarget(ICorDebugReferenceValue **ppObject) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetTarget - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbObjectValue::GetFunction(ICorDebugFunction **ppFunction) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetFunction - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbObjectValue::GetClass(ICorDebugClass **ppClass) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetClass - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetFieldValue(
  ...
  LOG((LF_CORDB, LL_INFO1000000,
       "CordbObjectValue - GetFieldValue - IMPLEMENTED\n"));
  if (m_debuggerId == -1)
    return S_FALSE;
  ...
  int cmdId =
      conn->SendEvent(MDBGPROT_CMD_SET_OBJECT_REF,
...
HRESULT STDMETHODCALLTYPE CordbObjectValue::GetVirtualMethod(
    mdMemberRef memberRef, ICorDebugFunction **ppFunction) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetVirtualMethod - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbObjectValue::GetContext(ICorDebugContext **ppContext) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetContext - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbObjectValue::IsValueClass(BOOL *pbIsValueClass) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - IsValueClass - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbObjectValue::GetManagedCopy(IUnknown **ppObject) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - GetManagedCopy - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbObjectValue::SetFromManagedCopy(IUnknown *pObject) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - SetFromManagedCopy - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbObjectValue::IsValid(BOOL *pbValid) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbObjectValue - IsValid - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetClass(ICorDebugClass **ppClass) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetClass - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetFieldValue(
    ICorDebugClass *pClass, mdFieldDef fieldDef, ICorDebugValue **ppValue) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetFieldValue - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetVirtualMethod(
    mdMemberRef memberRef, ICorDebugFunction **ppFunction) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetVirtualMethod - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetAddress(CORDB_ADDRESS *pAddress) {
  *pAddress = (CORDB_ADDRESS)&m_debuggerId;
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetAddress - IMPLEMENTED\n"));
  return S_OK;
}
...
CordbArrayValue::CreateBreakpoint(ICorDebugValueBreakpoint **ppBreakpoint) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - CreateBreakpoint - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetString(ULONG32 cchString,
                                                     ...
                                                     WCHAR szString[]) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetString - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}

HRESULT STDMETHODCALLTYPE CordbArrayValue::IsValid(BOOL *pbValid) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - IsValid - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbArrayValue::GetExactType(ICorDebugType **ppType) {
  m_pCordbType->QueryInterface(IID_ICorDebugType, (void **)ppType);
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetExactType - NOT IMPLEMENTED\n"));
  return S_OK;
}

HRESULT STDMETHODCALLTYPE CordbArrayValue::GetSize64(ULONG64 *pSize) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetSize64 - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::CreateHandle(
    CorDebugHandleType type, ICorDebugHandleValue **ppHandle) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - CreateHandle - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetThreadOwningMonitorLock(
    ICorDebugThread **ppThread, DWORD *pAcquisitionCount) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetThreadOwningMonitorLock - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::EnumerateExceptionCallStack(
    ICorDebugExceptionObjectCallStackEnum **ppCallStackEnum) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - EnumerateExceptionCallStack - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetCachedInterfacePointers(
    ...
    CORDB_ADDRESS *ptrs) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetCachedInterfacePointers - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbArrayValue::GetTarget(ICorDebugReferenceValue **ppObject) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetTarget - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbArrayValue::GetFunction(ICorDebugFunction **ppFunction) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetFunction - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
CordbArrayValue::GetElementType(CorElementType *pType) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetElementType - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetDimensions(ULONG32 cdim,
                                                         ULONG32 dims[]) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetDimensions - IMPLEMENTED\n"));
  dims[0] = m_nCount;
  return S_OK;
}
...
CordbArrayValue::HasBaseIndicies(BOOL *pbHasBaseIndicies) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - HasBaseIndicies - NOT IMPLEMENTED\n"));
  return S_OK;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetBaseIndicies(ULONG32 cdim,
                                                           ULONG32 indicies[]) {
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetBaseIndicies - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetElement(
  ...
  LOG((LF_CORDB, LL_INFO1000000,
       "CordbArrayValue - GetElement - IMPLEMENTED\n"));
...
HRESULT STDMETHODCALLTYPE CordbArrayValue::GetElementAtPosition(
  ...
  LOG((LF_CORDB, LL_INFO100000,
       "CordbArrayValue - GetElementAtPosition - NOT IMPLEMENTED\n"));
  return E_NOTIMPL;
}
