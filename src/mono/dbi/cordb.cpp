...
static void debugger_thread(void *m_pProcess) {
  Connection *connection = new Connection(
      (CordbProcess *)m_pProcess, ((CordbProcess *)m_pProcess)->GetCordb());
...
HRESULT Cordb::CreateProcess(LPCWSTR lpApplicationName, LPWSTR lpCommandLine,
...
HRESULT Cordb::DebugActiveProcess(DWORD id, BOOL win32Attach,
                                  ICorDebugProcess **ppProcess) {
  ...
  DWORD thread_id;
  CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)debugger_thread, m_pProcess, 0,
               &thread_id);
  return S_OK;
}
...
HRESULT Cordb::CanLaunchOrAttach(DWORD dwProcessId,
...
HRESULT
Cordb::QueryInterface(REFIID riid,
...
ReceivedReplyPacket::ReceivedReplyPacket(int error, int error_2, int id,
                                         MdbgProtBuffer *buf) {
  this->error = error;
...
void Connection::Receive() {
  while (true) {
    ...
    if (iResult == -1) {
      pCordb->GetCallback()->ExitProcess(
          static_cast<ICorDebugProcess *>(GetProcess()));
      break;
    }
    while (iResult == 0) {
      LOG((LF_CORDB, LL_INFO100000,
    ...
    if (header.len - HEADER_LENGTH != 0) {
      ...
      while (totalRead < header.len - HEADER_LENGTH) {
        iResult = socket->Receive((char *)recvbuf->p + totalRead,
    ...
    if (header.flags == REPLY_PACKET) {
      ReceivedReplyPacket *rp = new ReceivedReplyPacket(
          header.error, header.error_2, header.id, recvbuf);
...
void Connection::ProcessPacketInternal(MdbgProtBuffer *recvbuf) {
  ...
  for (int i = 0; i < nevents; ++i) {
    ...
    case MDBGPROT_EVENT_KIND_VM_START: {
      pCordb->GetCallback()->CreateProcess(
          static_cast<ICorDebugProcess *>(GetProcess()));
    } break;
    ...
    case MDBGPROT_EVENT_KIND_ASSEMBLY_LOAD: {
      ...
      int assembly_id =
          m_dbgprot_decode_id(recvbuf->p, &recvbuf->p, recvbuf->end);
      if (pCorDebugAppDomain == NULL) {
        pCorDebugAppDomain = new CordbAppDomain(this, GetProcess());
        GetProcess()->Stop(false);
        pCordb->GetCallback()->CreateAppDomain(
            static_cast<ICorDebugProcess *>(GetProcess()), pCorDebugAppDomain);
      }
      CordbAssembly *pAssembly = new CordbAssembly(
          this, GetProcess(), pCorDebugAppDomain, assembly_id);
      CordbModule *pModule = new CordbModule(
          this, GetProcess(), (CordbAssembly *)pAssembly, assembly_id);
    ...
    case MDBGPROT_EVENT_KIND_BREAKPOINT: {
      int method_id =
          m_dbgprot_decode_id(recvbuf->p, &recvbuf->p, recvbuf->end);
      int64_t offset =
          m_dbgprot_decode_long(recvbuf->p, &recvbuf->p, recvbuf->end);
      ...
      CordbFunctionBreakpoint *breakpoint =
          GetProcess()->GetBreakpointByOffsetAndFuncId(offset, method_id);
      pCordb->GetCallback()->Breakpoint(
    ...
    case MDBGPROT_EVENT_KIND_STEP: {
      ...
      pCordb->GetCallback()->StepComplete(pCorDebugAppDomain, thread,
                                          thread->GetStepper(), STEP_NORMAL);
    } break;
    default: {
...
void Connection::LoopSendReceive() {
  ...
  int cmdId = SendEvent(MDBGPROT_CMD_SET_VM,
...
void Connection::EnableEvent(MdbgProtEventKind eventKind) {
  ...
  SendEvent(MDBGPROT_CMD_SET_EVENT_REQUEST, MDBGPROT_CMD_EVENT_REQUEST_SET,
            &sendbuf);
...
void Connection::SendPacket(MdbgProtBuffer &sendbuf) {
  int iResult =
      socket->Send((const char *)sendbuf.buf, m_dbgprot_buffer_len(&sendbuf));
  if (iResult == -1) {
    return;
  }
}
...
ULONG CordbBaseMono::BaseAddRef() { return InterlockedIncrement(&m_cRef); }
