...
public class Symbolicator
{
    ...
    public Symbolicator(string? symbolsMapFile, string? symbolPatternsFile, bool throwOnMissing, ILogger logger)
    {
        _logger = logger;
        ...
        foreach (var line in File.ReadAllLines(symbolPatternsFile))
        {
            ...
            try
            {
                ...
                Console.WriteLine ($"adding regex: {line}");
                _regexes.Add(regex);
            }
    ...
    {
        if (_wasmFunctionMap.Count == 0 || _regexes.Count == 0)
            return line;
        ...
        foreach (var regex in _regexes)
        {
            ...
            if (string.Compare(newLine, line) != 0)
                return newLine;
        }
        return line;
    }
