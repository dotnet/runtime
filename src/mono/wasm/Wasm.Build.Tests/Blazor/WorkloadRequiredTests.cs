...
public class WorkloadRequiredTests : BlazorWasmTestBase
{
    ...
    //     CommandResult result;
    //     if (publish)
    //         (result, _) = BlazorPublish(new BuildProjectOptions(id, config, ExpectSuccess: false));
    //     else
    //         (result, _) = BlazorBuild(new BuildProjectOptions(id, config, ExpectSuccess: false));
