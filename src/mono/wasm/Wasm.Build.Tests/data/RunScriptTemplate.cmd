...
:: CoreCLR WBT: ensure payload-relative paths remain set for the test process.
if [%REPOSITORY_ENGINEERING_DIR%] NEQ [] (
   set REPOSITORY_ENGINEERING_DIR=%REPOSITORY_ENGINEERING_DIR%
)
if [%BROWSER_BUILD_TARGETS_DIR%] NEQ [] (
   set BROWSER_BUILD_TARGETS_DIR=%BROWSER_BUILD_TARGETS_DIR%
)
if [%WASM_APP_BUILDER_TASKS_ASSEMBLY_PATH%] NEQ [] (
   set WASM_APP_BUILDER_TASKS_ASSEMBLY_PATH=%WASM_APP_BUILDER_TASKS_ASSEMBLY_PATH%
)
if [%EMSDK_PATH%] NEQ [] (
