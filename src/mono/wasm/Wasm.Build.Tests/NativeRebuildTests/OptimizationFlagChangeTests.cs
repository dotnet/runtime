...
public class OptimizationFlagChangeTests : NativeRebuildTestsBase
{
    ...
    // public void OptimizationFlagChange(ProjectInfo buildArgs, string cflags, string ldflags, RunHost host, string id)
    // {
    //     // force _WasmDevel=false, so we don't get -O0
    //     buildArgs = buildArgs with { ProjectName = $"rebuild_flags_{buildArgs.Configuration}", ExtraBuildArgs = "/p:_WasmDevel=false" };
