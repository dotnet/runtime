...
public class BrowserHostRunner : IHostRunner
{
    ...
    public string GetXharnessArgsWindowsOS(XHarnessArgsOptions options) => $"-v trace -b Chrome --browser-arg=--lang={options.environmentLocale} --web-server-use-cop {BinaryPathArg}";  // Windows: chrome.exe --lang=locale
    public string GetXharnessArgsOtherOS(XHarnessArgsOptions options) => $"-v trace -b Chrome --locale={options.environmentLocale} --web-server-use-cop {BinaryPathArg}";                // Linux: LANGUAGE=locale ./chrome
