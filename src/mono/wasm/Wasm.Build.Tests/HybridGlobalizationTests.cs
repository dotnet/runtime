...
namespace Wasm.Build.Tests
{
    public class HybridGlobalizationTests : WasmTemplateTestsBase
    {
        ...
        public async Task AOT_HybridGlobalizationTests(Configuration config, bool aot)
            => await TestHybridGlobalizationTests(config, aot);

        [Theory]
        [BuildAndRun(aot: false)]
        [ActiveIssue("dotnet.globalization.js not found")]
        public async Task RelinkingWithoutAOT(Configuration config, bool aot)
            => await TestHybridGlobalizationTests(config, aot, isNativeBuild: true);

        private async Task TestHybridGlobalizationTests(Configuration config, bool aot, bool isNativeBuild = false)
        {
