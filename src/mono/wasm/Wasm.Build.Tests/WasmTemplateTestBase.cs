...
public abstract class WasmTemplateTestBase : BuildTestBase
{
    ...
    public async Task<string> CreateWasmTemplateProjectAsync(string id, string template = "wasmbrowser", string extraArgs = "", bool runAnalyzers = true, bool addFrameworkArg = false)
    {
        ...
        await using var cmd = new DotNetCommand(s_buildEnv, _testOutput, useDefaultArgs: false)
                .WithWorkingDirectory(_projectDir!)
                .WithTimeout(TimeSpan.FromMinutes(1));
        var res = await cmd.ExecuteWithCapturedOutputAsync($"new {template} --no-update-check {extraArgs}").ConfigureAwait(false);
