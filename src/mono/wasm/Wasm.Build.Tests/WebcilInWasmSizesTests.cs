...
public class WebcilInWasmSizesTests
{
    ...
    {
        // A prebuilt R2R image whose MVID differs from the candidate must never be staged: with cross-module
        // inlining every image shares one version bubble the runtime checks by MVID, so a stale image (same
        // assembly version, different MVID) would fail-fast at load. Use two real assemblies with distinct MVIDs.
