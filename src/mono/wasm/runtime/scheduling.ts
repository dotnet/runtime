...
if (globalThis.navigator) {
    ...
    if (nav.userAgentData && nav.userAgentData.brands) {
        isChromium = nav.userAgentData.brands.some(
            (i: any) => i.brand == "Chromium"
        );
...
export function prevent_timer_throttling(): void {
    ...
    for (
        let schedule = next_reach_time;
        schedule < desired_reach_time;
        schedule += light_throttling_frequency
    ) {
