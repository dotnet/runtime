...
export async function mono_wasm_init_diagnostics(): Promise<void> {
    if (diagnosticsInitialized)
        return;
    if (!monoWasmThreads) {
        mono_log_warn("MONO_WASM: ignoring diagnostics options because this runtime does not support diagnostics");
        return;
    } else {
...
function diagnostic_options_from_ports_spec(val: string): DiagnosticOptions | null {
    ...
    if (ports.length === 0)
        return null;
    if (ports.length !== 1) {
        mono_log_warn("MONO_WASM: multiple diagnostic ports specified, only the last one will be used");
    }
    const portSpec = ports[ports.length - 1];
    const components = portSpec.split(",");
    if (components.length < 1 || components.length > 3) {
        mono_log_warn("MONO_WASM: invalid diagnostic port specification, should be of the form <port>[,<connect>],[<nosuspend|suspend>]");
        return null;
    }
    ...
    for (let i = components.length - 1; i >= 1; i--) {
        ...
        switch (component.toLowerCase()) {
            ...
            case "connect":
                ...
                break;
            default:
                mono_log_warn(`MONO_WASM: invalid diagnostic port specification component: ${component}`);
                break;
        }
    }
    if (!connect) {
        mono_log_warn("MONO_WASM: this runtime does not support listening on a diagnostic port; no diagnostic server started");
        return null;
    }
    return {
        server: {
