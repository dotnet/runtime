...
class StatefulParser {
    ...
    receiveBuffer(buf: ArrayBuffer): void {
        ...
        if (result.success) {
            mono_log_debug("MONO_WASM: protocol-socket: got result", result);
            ...
            mono_log_warn("MONO_WASM: socket received invalid command header", buf, result.error);
    ...
    continueWithBuffer(state: PartialCommandState, moreBuf: Uint8Array): ParseResult {
        ...
        if (partialSize < buf.byteLength) {
            ...
            if (overflow) {
                mono_log_warn("MONO_WASM: additional bytes past command payload", overflow);
                if (result.success) {
...
class ProtocolSocketImpl implements ProtocolSocket {
    ...
    onMessage(this: ProtocolSocketImpl, ev: MessageEvent<ArrayBuffer | Blob | string>): void {
        const data = ev.data;
        mono_log_debug("MONO_WASM: protocol socket received message", ev.data);
        if (typeof data === "object" && data instanceof ArrayBuffer) {
            ...
            mono_log_debug("MONO_WASM: protocol socket received string message; ignoring it", ev.data);
        } else {
            assertNever(data);
        }
    }
    ...
    onArrayBuffer(this: ProtocolSocketImpl, buf: ArrayBuffer) {
        mono_log_debug("MONO_WASM: protocol-socket: parsing array buffer", buf);
    ...
    emitCommandCallback(this: this, command: BinaryProtocolCommand): void {
        mono_log_debug("MONO_WASM: protocol-socket: queueing command", command);
        queueMicrotask(() => {
            mono_log_debug("MONO_WASM: dispatching protocol event with command", command);
    ...
    addEventListener(type: string, listener: EventListenerOrEventListenerObject, options?: boolean | AddEventListenerOptions | undefined): void {
        ...
        if (type === dotnetDiagnosticsServerProtocolCommandEvent) {
            if (this.protocolListeners === 0) {
                mono_log_debug("MONO_WASM: adding protocol listener, with a message chaser");
    ...
    removeEventListener(type: string, listener: EventListenerOrEventListenerObject): void {
        if (type === dotnetDiagnosticsServerProtocolCommandEvent) {
            mono_log_debug("MONO_WASM: removing protocol listener and message chaser");
            this.protocolListeners--;
            if (this.protocolListeners === 0) {
