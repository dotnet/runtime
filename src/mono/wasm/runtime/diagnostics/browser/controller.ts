...
class ServerControllerImpl implements ServerController {
    ...
    start(): void {
        mono_log_debug("MONO_WASM: signaling the diagnostic server to start");
        this.server.postMessageToWorker(makeDiagnosticServerControlCommand("start"));
    }
    stop(): void {
        mono_log_debug("MONO_WASM: signaling the diagnostic server to stop");
        this.server.postMessageToWorker(makeDiagnosticServerControlCommand("stop"));
    }
    postServerAttachToRuntime(): void {
        mono_log_debug("MONO_WASM: signal the diagnostic server to attach to the runtime");
    ...
    onServerReply(event: MessageEvent): void {
        ...
        if (isDiagnosticMessage(d)) {
            switch (d.cmd) {
                default:
                    mono_log_warn("MONO_WASM: Unknown control reply command: ", <any>d);
                    break;
            }
        }
    }
...
export async function startDiagnosticServer(websocket_url: string): Promise<ServerController | null> {
    ...
    mono_log_info(`MONO_WASM: starting the diagnostic server url: ${websocket_url}`);
    ...
    if (result === undefined) {
        mono_log_warn("MONO_WASM: diagnostic server failed to start");
        return null;
    }
