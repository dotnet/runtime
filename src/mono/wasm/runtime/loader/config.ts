...
import { BootConfigResultfromFetchResponse } from "./blazor/BootConfig";
...
export function normalizeConfig() {
    ...
    runtimeHelpers.pthreadPoolSize = config["pthreadPoolSize"] || 4;
}
...
export async function mono_wasm_load_config(module: DotnetModuleInternal): Promise<void> {
    ...
    try {
        loaderHelpers.config["applicationEnvironment"] = loaderHelpers.config["applicationEnvironment"] ?? (loaderHelpers.config["startupOptions"] ? loaderHelpers.config["startupOptions"]["environment"] : "Production");
        ...
        if (loaderHelpers.config["startupOptions"] && loaderHelpers.config["startupOptions"].loadBootResource) {
            ...
            if (loadedAnyConfig["resources"]) {
                ...
                normalizeConfig();
                await initializeBootConfig(BootConfigResultfromFetchResponse(configResponse, loadedAnyConfig as BootJsonData, loaderHelpers.config["applicationEnvironment"]), module, loaderHelpers.config["startupOptions"]);
            } else {
