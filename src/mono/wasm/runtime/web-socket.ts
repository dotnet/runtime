...
const wasm_ws_pending_send_buffer_offset = Symbol.for(
    "wasm ws_pending_send_buffer_offset"
);
const wasm_ws_pending_send_buffer_type = Symbol.for(
    "wasm ws_pending_send_buffer_type"
);
const wasm_ws_pending_receive_event_queue = Symbol.for(
    "wasm ws_pending_receive_event_queue"
);
const wasm_ws_pending_receive_promise_queue = Symbol.for(
    "wasm ws_pending_receive_promise_queue"
);
const wasm_ws_pending_open_promise = Symbol.for("wasm ws_pending_open_promise");
const wasm_ws_pending_close_promises = Symbol.for(
    "wasm ws_pending_close_promises"
);
const wasm_ws_pending_send_promises = Symbol.for(
    "wasm ws_pending_send_promises"
);
...
export function mono_wasm_web_socket_open(
    uri: MonoString,
    subProtocols: MonoArray,
    on_close: MonoObject,
    web_socket_js_handle: Int32Ptr,
    thenable_js_handle: Int32Ptr,
    is_exception: Int32Ptr
): MonoObject | MonoString | null {
    ...
    try {
        ...
        if (!js_uri) {
            return wrap_error(
                is_exception,
                "ERR12: Invalid uri '" + uri_root.value + "'"
            );
        }
        ...
        const ws = new globalThis.WebSocket(
            js_uri,
            <any>js_subs
        ) as WebSocketExtension;
        const { promise, promise_control: open_promise_control } =
            _create_cancelable_promise();
        ...
        const local_on_close = (ev: CloseEvent) => {
            ...
            for (const close_promise_control of ws[
                wasm_ws_pending_close_promises
            ]) {
                close_promise_control.resolve();
            }

            // send close to any pending receivers, to wake them
            const receive_promise_queue =
                ws[wasm_ws_pending_receive_promise_queue];
...
export function mono_wasm_web_socket_send(
    webSocket_js_handle: JSHandle,
    buffer_ptr: MonoObject,
    offset: number,
    length: number,
    message_type: number,
    ...
    try {
        ...
        if (!ws)
            throw new Error(
                "ERR17: Invalid JS object handle " + webSocket_js_handle
            );
        ...
        const whole_buffer = _mono_wasm_web_socket_send_buffering(
            ws,
            buffer_root,
            offset,
            length,
        ...
        return _mono_wasm_web_socket_send_and_wait(
            ws,
            whole_buffer,
            thenable_js_handle
        );
...
export function mono_wasm_web_socket_receive(
    webSocket_js_handle: JSHandle,
    buffer_ptr: MonoObject,
    offset: number,
    length: number,
    response_ptr: MonoObject,
    thenable_js_handle: Int32Ptr,
    is_exception: Int32Ptr
): MonoObject | MonoString | null {
    const buffer_root = mono_wasm_new_root(buffer_ptr);
    ...
    try {
        ...
        if (!ws)
            throw new Error(
                "ERR18: Invalid JS object handle " + webSocket_js_handle
            );
        ...
        const receive_promise_control =
            promise_control as ReceivePromiseControl;
...
export function mono_wasm_web_socket_close(
    webSocket_js_handle: JSHandle,
    code: number,
    reason: MonoString,
    wait_for_close_received: boolean,
    thenable_js_handle: Int32Ptr,
    is_exception: Int32Ptr
): MonoString | MonoObject | null {
    const reason_root = mono_wasm_new_root(reason);
    try {
        const ws = mono_wasm_get_jsobj_from_js_handle(webSocket_js_handle);
        if (!ws)
            throw new Error(
                "ERR19: Invalid JS object handle " + webSocket_js_handle
            );
        ...
        if (wait_for_close_received) {
            ...
            const { task_ptr, then_js_handle } =
                _wrap_js_thenable_as_task(promise);
            // task_ptr above is not rooted, we need to return it to mono without any intermediate mono call which could cause GC
            Module.setValue(thenable_js_handle, <any>then_js_handle, "i32");
...
export function mono_wasm_web_socket_abort(
    webSocket_js_handle: JSHandle,
    is_exception: Int32Ptr
): MonoString | null {
    try {
        const ws = mono_wasm_get_jsobj_from_js_handle(
            webSocket_js_handle
        ) as WebSocketExtension;
        if (!ws)
            throw new Error(
        ...
        ws[wasm_ws_pending_receive_promise_queue].drain(
            (receive_promise_control) => {
...
function _mono_wasm_web_socket_send_and_wait(
    ws: WebSocketExtension,
...
function _mono_wasm_web_socket_on_message(
    ws: WebSocketExtension,
    event: MessageEvent
) {
    ...
    while (promise_queue.getLength() && event_queue.getLength()) {
        ...
        _mono_wasm_web_socket_receive_buffering(
            event_queue,
            promise_control.buffer_root,
            promise_control.buffer_offset,
            promise_control.buffer_length,
...
function _mono_wasm_web_socket_receive_buffering(
    ...
    if (count > 0) {
        const targetView = Module.HEAPU8.subarray(
            <any>buffer_root.value + buffer_offset,
            <any>buffer_root.value + buffer_offset + buffer_length
        );
        const sourceView = event.data.subarray(
            event.offset,
            event.offset + count
        );
...
function _mono_wasm_web_socket_send_buffering(
    ws: WebSocketExtension,
    buffer_root: WasmRoot<MonoObject>,
    buffer_offset: number,
    length: number,
    ...
    if (buffer) {
        ...
        if (length !== 0) {
            const memoryView = Module.HEAPU8.subarray(
                message_ptr,
                message_ptr + length
            );
