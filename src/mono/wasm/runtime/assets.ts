...
export function instantiate_asset(asset: AssetEntry, url: string, bytes: Uint8Array): void {
    mono_log_debug(`MONO_WASM: Loaded:${asset.name} as ${asset.behavior} size ${bytes.length} from ${url}`);
    ...
    switch (asset.behavior) {
        ...
        case "vfs": {
            ...
            if (parentDirectory) {
                mono_log_debug(`MONO_WASM: Creating directory '${parentDirectory}'`);
            ...
            mono_log_debug(`MONO_WASM: Creating file '${fileName}' in directory '${parentDirectory}'`);
...
export async function instantiate_wasm_asset(
    ...
    if (typeof WebAssembly.instantiateStreaming === "function" && contentType === "application/wasm") {
        mono_log_debug("MONO_WASM: instantiate_wasm_module streaming");
        ...
        if (ENVIRONMENT_IS_WEB && contentType !== "application/wasm") {
            mono_log_warn("MONO_WASM: WebAssembly resource does not have the expected content type \"application/wasm\", so falling back to slower ArrayBuffer instantiation.");
        }
        const arrayBuffer = await response.arrayBuffer();
        mono_log_debug("MONO_WASM: instantiate_wasm_module buffered");
...
export async function instantiate_symbols_asset(pendingAsset: AssetEntryInternal): Promise<void> {
    try {
        ...
        mono_log_info(`MONO_WASM: Error loading symbol file ${pendingAsset.name}: ${JSON.stringify(error)}`);
    }
}
...
export async function wait_for_all_assets() {
    ...
    if (runtimeHelpers.config.assets) {
        ...
        mono_log_debug("MONO_WASM: all assets are loaded in wasm memory");
    }
}
