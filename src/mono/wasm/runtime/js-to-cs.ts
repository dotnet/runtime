...
import {
    ...
    get_cs_owned_object_by_js_handle,
...
export function _js_to_mono_uri(
    should_add_in_flight: boolean,
...
export function _js_to_mono_obj(
    should_add_in_flight: boolean,
    // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
    js_obj: any
): MonoObject {
    switch (true) {
        ...
        case typeof js_obj === "number": {
            let result = null;
            if ((js_obj | 0) === js_obj) result = _box_js_int(js_obj);
            else if (js_obj >>> 0 === js_obj) result = _box_js_uint(js_obj);
            else result = _box_js_double(js_obj);
...
function _extract_mono_obj(
    should_add_in_flight: boolean,
    js_obj: any
): MonoObject {
    if (js_obj === null || typeof js_obj === "undefined") return MonoObjectNull;
    ...
    if (js_obj[js_owned_gc_handle_symbol]) {
        ...
        result = get_js_owned_object_by_gc_handle(
            js_obj[js_owned_gc_handle_symbol]
        );
        return result;
    }
    if (js_obj[cs_owned_js_handle_symbol]) {
        result = get_cs_owned_object_by_js_handle(
            js_obj[cs_owned_js_handle_symbol],
    ...
    if (!result) {
        ...
        result = corebindings._create_cs_owned_proxy(
            js_handle,
            wasm_type_id,
            should_add_in_flight ? 1 : 0
        );
    }
...
function _box_js_int(js_obj: number) {
    ...
    return cwraps.mono_wasm_box_primitive(
...
export function js_typed_array_to_array(js_obj: any) {
    ...
    if (has_backing_array_buffer(js_obj) && js_obj.BYTES_PER_ELEMENT) {
        ...
        const bufferArray = cwraps.mono_wasm_typed_array_new(
            <any>heapBytes.byteOffset,
            js_obj.length,
...
export function js_to_mono_enum(
    // eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
    js_obj: any,
    ...
    if (typeof js_obj !== "number")
        throw new Error(
            `Expected numeric value for enum argument, got '${js_obj}'`
        );

    return (<any>js_obj) | 0;
}

export function js_array_to_mono_array(
    js_array: any[],
    asString: boolean,
    should_add_in_flight: boolean
): MonoArray {
    const mono_array = asString
        ? cwraps.mono_wasm_string_array_new(js_array.length)
        : cwraps.mono_wasm_obj_array_new(js_array.length);
    ...
    thenable.then(
        (result) => {
            ...
            if (!_use_finalization_registry) {
                corebindings._release_js_owned_object_by_gc_handle(
                    tcs_gc_handle
                );
            }
        },
        (reason) => {
            corebindings._set_tcs_failure(
                tcs_gc_handle,
...
): MonoArray | MonoString {
    const js_obj = mono_wasm_get_jsobj_from_js_handle(js_handle);
    if (!js_obj) {
        return wrap_error(
            is_exception,
            "ERR06: Invalid JS object handle '" + js_handle + "'"
        );
    }
