...
export class StringDecoder {
    ...
    copy(mono_string: MonoString): string | null {
        ...
        if (mono_string === MonoStringNull) return null;
        ...
        cwraps.mono_wasm_string_get_data(
            mono_string,
            <any>ppChars,
            <any>pLengthBytes,
            <any>pIsInterned
...
export function mono_intern_string(string: string): string {
    if (string.length === 0) return mono_wasm_empty_string;
...
function _store_string_in_intern_table(
    ...
    if (!ptr)
        ...
        throw new Error(
            `non-pointer passed to _store_string_in_intern_table: ${typeof ptr}`
        );

    const internBufferSize = 8192;

    if (_interned_string_current_root_buffer_count >= internBufferSize) {
        _interned_string_full_root_buffers.push(
    ...
    if (string.length === 0 && !_empty_string_ptr) _empty_string_ptr = ptr;

    return ptr;
}

export function js_string_to_mono_string_interned(
    ...
    if (text.length === 0 && _empty_string_ptr) return _empty_string_ptr;
...
export function js_string_to_mono_string(string: string): MonoString | null {
    ...
    if (string.length === 0) return js_string_to_mono_string_interned(string);
...
function js_string_to_mono_string_new(string: string): MonoString {
    ...
    const result = cwraps.mono_wasm_string_from_utf16(
        <any>buffer,
        string.length
    );
    Module._free(buffer);
