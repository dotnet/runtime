...
let commands_received: t_CommandReceived;
...
export function mono_wasm_runtime_ready(): void {
    ...
    if ((<any>globalThis).dotnetDebugger) debugger;
    else
        console.debug(
...
export function mono_wasm_add_dbg_command_received(
    res_ok: boolean,
    id: number,
    buffer: number,
    buffer_len: number
...
function mono_wasm_malloc_and_set_debug_buffer(command_parameters: string) {
    if (command_parameters.length > _debugger_buffer_len) {
        if (_debugger_buffer) Module._free(_debugger_buffer);
        _debugger_buffer_len = Math.max(
            command_parameters.length,
            _debugger_buffer_len,
            256
        );
    ...
    _debugger_heap_bytes = new Uint8Array(
        Module.HEAPU8.buffer,
        <any>_debugger_buffer,
        _debugger_buffer_len
    );
...
export function mono_wasm_send_dbg_command_with_parms(
    ...
    valtype: number,
    newvalue: number
): t_CommandReceivedResult {
    mono_wasm_malloc_and_set_debug_buffer(command_parameters);
    cwraps.mono_wasm_send_dbg_command_with_parms(
        id,
        command_set,
        command,
        _debugger_buffer,
...
export function mono_wasm_send_dbg_command(
    id: number,
    command_set: number,
    command: number,
    command_parameters: string
    ...
    mono_wasm_malloc_and_set_debug_buffer(command_parameters);
    cwraps.mono_wasm_send_dbg_command(
        id,
        command_set,
        command,
        _debugger_buffer,
...
export function mono_wasm_get_dbg_command_info(): t_CommandReceivedResult {
    const { res_ok, res } = commands_received;
...
export function mono_wasm_call_function_on(request: CallRequest): {
    ...
    if (objId.startsWith("dotnet:cfo_res:")) {
        ...
        else throw new Error(`Unknown object id ${objId}`);
    } else {
    ...
    const fn_args =
        request.arguments != undefined
            ? request.arguments.map((a) => JSON.stringify(a.value))
            : [];
...
export function mono_wasm_get_details(
    objectId: string,
    args = {}
): t_ValueAsJsonString {
...
type t_CommandReceivedResult = {
    id: number;
    value: string;
};

type t_CommandReceived = {
    res_ok: boolean;
    res: t_CommandReceivedResult;
};

type t_ValueAsJsonString = { __value_as_json_string__: string };
