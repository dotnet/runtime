...
import {
    ...
    _js_owned_object_registry,
    _lookup_js_owned_object,
    _register_js_owned_object,
...
function _unbox_cs_owned_root_as_js_object(root: WasmRoot<any>) {
    ...
    const js_handle = corebindings._get_cs_owned_object_js_handle(
        root.value,
        0
    );
...
export function _unbox_mono_obj_root_with_known_nonprimitive_type(
    root: WasmRoot<any>,
    type: number
): any {
    ...
    switch (type) {
        ...
        case 20: {
            // clr .NET DateTime
            const dateValue = corebindings._get_date_value(root.value);
            return new Date(dateValue);
        }
        case 21: {
            // clr .NET DateTimeOffset
            const dateoffsetValue = corebindings._object_to_string(root.value);
            return dateoffsetValue;
        }
        case 22: {
            // clr .NET Uri
            const uriValue = corebindings._object_to_string(root.value);
            return uriValue;
        }
        ...
        default:
            throw new Error(
                `no idea on how to unbox object kind ${type} at offset ${
                    root.value
                } (root address is ${root.get_address()})`
            );
    }
}
...
export function _unbox_mono_obj_root(root: WasmRoot<any>): any {
    ...
    const type = cwraps.mono_wasm_try_unbox_primitive_and_get_type(
        root.value,
        runtimeHelpers._unbox_buffer
    );
    switch (type) {
        case 1: // int
        ...
        case 28: // char
            return String.fromCharCode(
                Module.HEAP32[<any>runtimeHelpers._unbox_buffer / 4]
            );
        default:
            return _unbox_mono_obj_root_with_known_nonprimitive_type(
                root,
                type
            );
    }
...
export function mono_array_to_js_array(mono_array: MonoArray): any[] | null {
    if (mono_array === MonoArrayNull) return null;
...
export function _mono_array_root_to_js_array(
    arrayRoot: WasmRoot<MonoArray>
): any[] | null {
    if (arrayRoot.value === MonoArrayNull) return null;
    ...
    try {
        ...
        for (let i = 0; i < len; ++i) {
            ...
            else res[i] = _unbox_mono_obj_root(elemRoot);
        }
        return res;
    } finally {
        elemRoot.release();
...
export function _wrap_delegate_root_as_function(
    root: WasmRoot<MonoObject>
): Function | null {
    if (root.value === MonoObjectNull) return null;
...
export function _wrap_delegate_gc_handle_as_function(
    gc_handle: GCHandle,
    after_listener_callback?: () => void
): Function {
    ...
    if (!result) {
        ...
        result = function (...args: any[]) {
            const delegateRoot = mono_wasm_new_root(
                get_js_owned_object_by_gc_handle(gc_handle)
            );
            try {
                const res = call_method(
                    result[delegate_invoke_symbol],
                    delegateRoot.value,
                    result[delegate_invoke_signature_symbol],
                    args
                );
        ...
        try {
            if (typeof result[delegate_invoke_symbol] === "undefined") {
                result[delegate_invoke_symbol] =
                    cwraps.mono_wasm_get_delegate_invoke(delegateRoot.value);
            ...
            if (
                typeof result[delegate_invoke_signature_symbol] === "undefined"
            ) {
                result[delegate_invoke_signature_symbol] =
                    mono_method_get_call_signature(
                        result[delegate_invoke_symbol],
...
export function mono_wasm_create_cs_owned_object(
    core_name: MonoString,
    args: MonoArray,
    ...
    try {
        ...
        try {
            ...
            const allocator = function (
                ...
                const tempCtor = constructor.bind.apply(
                    constructor,
                    <any>argsList
                );
...
function _unbox_ref_type_root_as_js_object(root: WasmRoot<MonoObject>) {
    ...
    // we don't need in-flight reference as we already have it rooted here
    const js_handle = corebindings._try_get_cs_owned_object_js_handle(
        root.value,
        0
    );
    if (js_handle) {
        if (js_handle === JSHandleDisposed) {
            throw new Error(
                "Cannot access a disposed JSObject at " + root.value
            );
        }
