...
the specified `@(WasmAssemblies)` are directly passed to
`WasmAppBuilder`.
  ...
  `@(WasmAssemblies)` and `$(WasmAOTDir)` are absolute paths.

- Assemblies to be bundled with the app are set via
`@(WasmAssemblies)` (which optionally will have dependencies
resolved)
