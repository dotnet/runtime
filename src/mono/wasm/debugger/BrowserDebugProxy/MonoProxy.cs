        ...
        internal async Task<JToken> RuntimeGetPropertiesInternal(SessionId id, DotnetObjectId objectId, JToken args, CancellationToken token)
        {
            ...
            try {
                switch (objectId.Scheme)
                {
                    ...
                    case "object":
                        return await SdbHelper.GetObjectValues(id, int.Parse(objectId.Value), true, false, accessorPropertiesOnly, ownProperties, false, false, token);
                    case "pointer":
            ...
            for (int i = 0 ; i < number_of_events; i++) {
                ...
                switch (event_kind)
                {
                    ...
                    case EventKind.Exception:
                    {
                        ...
                        var exceptionObject = await SdbHelper.GetObjectValues(sessionId, object_id, true, false, false, true, false, false, token);
