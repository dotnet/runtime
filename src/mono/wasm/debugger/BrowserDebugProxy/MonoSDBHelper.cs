...
namespace Microsoft.WebAssembly.Diagnostics
{
    ...
    internal sealed partial class MonoSDBHelper
    {
        ...
        public async Task<string> InvokeToStringAsync(IEnumerable<int> typeIds, bool isValueType, bool isEnum, int objectId, BindingFlags extraFlags, bool invokeObjectToString, CancellationToken token)
        {
            try
            {
                ...
                {
                    var typeInfo = await GetTypeInfo(typeId, token);
                    if (typeInfo == null || (typeInfo.Name == "object" && !invokeObjectToString))
                        continue;
