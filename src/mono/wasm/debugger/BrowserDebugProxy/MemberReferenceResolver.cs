    ...
    internal class MemberReferenceResolver
    {
        ...
        public async Task<JObject> GetValueFromObject(JToken objRet, CancellationToken token)
        {
            if (objRet["value"]?["className"]?.Value<string>() == "System.Exception")
            {
                ...
                {
                    var exceptionObject = await proxy.SdbHelper.GetObjectValues(sessionId, int.Parse(objectId.Value), withProperties: true, withSetter: false, accessorPropertiesOnly: false, ownProperties: true, fromDebuggerProxyAttribute: false, fromDebuggerDisplayAttribute: false, token);
