            ...
            bool includeStatic = false)
        {
            ...
            for (int i = 0; i < typeIdsIncludingParents.Count; i++)
            {
                ...
                allMembers = await ExpandPropertyValues(
                    ...
                    isAutoExpandable: getCommandType.HasFlag(GetObjectCommandOptions.ForDebuggerProxyAttribute) || getCommandType.HasFlag(GetObjectCommandOptions.AutoEvaluateProperties),
                    id,
                    isValueType: false,
                    isOwn,
                    token,
