...
try {
    const { setModuleImports, runMain, getAssemblyExports, getConfig } = await dotnet
        ...
        // dotnet-counters
        .withEnvironmentVariable("DOTNET_EventPipeThreadSamplingRate", "0")
    ...
    /* Call those from dev tools console to test diagnostics features:
    globalThis.getDotnetRuntime(0).collectGcDump();
    globalThis.getDotnetRuntime(0).collectCpuSamples({durationSeconds:10})
    globalThis.getDotnetRuntime(0).collectMetrics({
        durationSeconds: 10,
        extraProviders: [
            {
                providerName: "WasmHello",
                keywords: [0xFFFFFFFF, 0xFFFFFFFF], // all keywords
