...
public class Program
{
    ...
    [Fact]
    public static void TestGenericImplementation()
    {
        Verify(new GenericImplementation());
    }

    private static void Verify(IDerived instance)
    {
        for (var i = 0; i < 50; i++)
            Invoke(instance, i);
...
interface IGenericReabstracted<T> : IBase
{
    abstract int IBase.R00 { get; }
    abstract int IBase.R01 { get; }
    abstract int IBase.R02 { get; }
...
class GenericImplementation : Implementation, IGenericReabstracted<int>
{
}
