...
void oldBehaviour()
{
    ...
    NSString *DocumentDir = [InternetCache stringByAppendingPathComponent:@"Documents"];
    NSFileManager *fileManager = [NSFileManager defaultManager];
    if (![fileManager fileExistsAtPath:DocumentDir]) {
        NSError *error;
        [fileManager createDirectoryAtPath:DocumentDir withIntermediateDirectories:YES attributes:nil error:&error];
        if (error) {
