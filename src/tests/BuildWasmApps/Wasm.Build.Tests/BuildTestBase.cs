...
{
    public abstract class BuildTestBase : IClassFixture<SharedBuildClassFixture>, IDisposable
    {
        ...
        protected bool _cleanup = true;
        protected SharedBuildClassFixture _buildContext;
        ...
        public BuildTestBase(ITestOutputHelper output, SharedBuildClassFixture buildContext)
        {
        ...
        */
        ...
        public static void dump<T>(IEnumerable<T> enumerable, string label) where T: IEnumerable<object?>
        {
            Console.WriteLine ($"--------- {label} ----------");
            foreach (var e in enumerable)
            {
                var ae = e.ToArray();
                Console.WriteLine ($"e: {e}, ae: {ae}, ae.Count: {ae.Length}");
                if (ae?.GetType().IsArray == true)
                {
                    foreach (var ee in ae)
                    {
                        Console.WriteLine ($"\tee: {ee}");
                        if (ee?.GetType().IsArray == true)
                        {
                            foreach (var eee in (IEnumerable<object?>)ee)
        ...
        public void Dispose()
        {
            if (s_skipProjectCleanup || !_cleanup)
                return;
