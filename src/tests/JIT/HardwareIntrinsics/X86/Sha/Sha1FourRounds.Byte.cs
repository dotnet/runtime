...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleBinaryOpTest__Sha1FourRoundsByte
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(Sha).GetMethod(nameof(Sha.Sha1FourRounds), new Type[] { typeof(Vector128<Byte>), typeof(Vector128<Byte>) })
        ...
        private void ValidateResult(Byte[] left, Byte[] right, Byte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Sha)}.{nameof(Sha.Sha1FourRounds)}<Byte>(Vector128<Byte>, Vector128<Byte>): {method} failed:");
