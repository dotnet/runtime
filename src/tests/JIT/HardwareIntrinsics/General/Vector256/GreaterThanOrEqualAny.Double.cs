...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanOrEqualAnyDouble
    {
        ...
        private void ValidateResult(Double[] left, Double[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.GreaterThanOrEqualAny)}<Double>(Vector256<Double>, Vector256<Double>): {method} failed:");
