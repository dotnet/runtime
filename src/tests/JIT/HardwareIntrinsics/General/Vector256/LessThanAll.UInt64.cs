...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void LessThanAllUInt64()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__LessThanAllUInt64
    {
        ...
        private void ValidateResult(UInt64[] left, UInt64[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.LessThanAll)}<UInt64>(Vector256<UInt64>, Vector256<UInt64>): {method} failed:");
