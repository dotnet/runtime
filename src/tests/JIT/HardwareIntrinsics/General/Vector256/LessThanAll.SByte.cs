...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void LessThanAllSByte()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__LessThanAllSByte
    {
        ...
        private void ValidateResult(SByte[] left, SByte[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.LessThanAll)}<SByte>(Vector256<SByte>, Vector256<SByte>): {method} failed:");
