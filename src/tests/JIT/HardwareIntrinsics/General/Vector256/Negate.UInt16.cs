...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void NegateUInt16()
        {
    ...
    public sealed unsafe class VectorUnaryOpTest__NegateUInt16
    {
        ...
        private void ValidateResult(Vector256<UInt16> op1, void* result, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateResult(UInt16[] firstOp, UInt16[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.Negate)}<UInt16>(Vector256<UInt16>): {method} failed:");
