...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    public sealed unsafe class VectorBinaryOpTest__EqualsByte
    {
        ...
        private void ValidateResult(Byte[] left, Byte[] right, Byte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.Equals)}<Byte>(Vector256<Byte>, Vector256<Byte>): {method} failed:");
