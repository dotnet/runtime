...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void LessThanAnySingle()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__LessThanAnySingle
    {
        ...
        private void ValidateResult(Single[] left, Single[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.LessThanAny)}<Single>(Vector256<Single>, Vector256<Single>): {method} failed:");
