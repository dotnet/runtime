...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void LessThanOrEqualAnyDouble()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__LessThanOrEqualAnyDouble
    {
        ...
        private void ValidateResult(Double[] left, Double[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.LessThanOrEqualAny)}<Double>(Vector256<Double>, Vector256<Double>): {method} failed:");
