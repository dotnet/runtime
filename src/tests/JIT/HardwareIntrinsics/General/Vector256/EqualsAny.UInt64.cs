...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void EqualsAnyUInt64()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__EqualsAnyUInt64
    {
        ...
        private void ValidateResult(UInt64[] left, UInt64[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.EqualsAny)}<UInt64>(Vector256<UInt64>, Vector256<UInt64>): {method} failed:");
