...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void LessThanAnyUInt32()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__LessThanAnyUInt32
    {
        ...
        private void ValidateResult(UInt32[] left, UInt32[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.LessThanAny)}<UInt32>(Vector256<UInt32>, Vector256<UInt32>): {method} failed:");
