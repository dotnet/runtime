...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void BitwiseAndByte()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__BitwiseAndByte
    {
        ...
        private void ValidateResult(Byte[] left, Byte[] right, Byte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.BitwiseAnd)}<Byte>(Vector256<Byte>, Vector256<Byte>): {method} failed:");
