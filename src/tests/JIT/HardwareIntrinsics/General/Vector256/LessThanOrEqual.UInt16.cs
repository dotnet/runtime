...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void LessThanOrEqualUInt16()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__LessThanOrEqualUInt16
    {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, UInt16[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.LessThanOrEqual)}<UInt16>(Vector256<UInt16>, Vector256<UInt16>): {method} failed:");
