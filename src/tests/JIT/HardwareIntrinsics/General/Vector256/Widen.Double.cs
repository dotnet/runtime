...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    public sealed unsafe class VectorWidenTest__WidenDouble
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            Unsafe.Write(_dataTable.outLowerArrayPtr, (((Vector256<Double> Lower, Vector256<Double> Upper))(result)).Lower);
            Unsafe.Write(_dataTable.outUpperArrayPtr, (((Vector256<Double> Lower, Vector256<Double> Upper))(result)).Upper);
            ValidateResult(_dataTable.inArray1Ptr, _dataTable.outLowerArrayPtr, _dataTable.outUpperArrayPtr);
        }
        ...
        private void ValidateResult(Vector256<Single> op1, void* lowerResult, void* upperResult, [CallerMemberName] string method = "")
        {
            ...
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<Double, byte>(ref outLowerArray[0]), ref Unsafe.AsRef<byte>(lowerResult), (uint)Unsafe.SizeOf<Vector256<Double>>());
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<Double, byte>(ref outUpperArray[0]), ref Unsafe.AsRef<byte>(upperResult), (uint)Unsafe.SizeOf<Vector256<Double>>());
        ...
        private void ValidateResult(Single[] firstOp, Double[] lowerResult, Double[] upperResult, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.Widen)}<Double>(Vector256<Single>): {method} failed:");
