...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanOrEqualAnySingle
    {
        ...
        private void ValidateResult(Single[] left, Single[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.GreaterThanOrEqualAny)}<Single>(Vector256<Single>, Vector256<Single>): {method} failed:");
