...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorUnaryOpTest__ConvertToUInt64Double
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var method = typeof(Vector256).GetMethod(nameof(Vector256.ConvertToUInt64), new Type[] {
                typeof(Vector256<Double>)
            });

            if (method is null)
            {
                method = typeof(Vector256).GetMethod(nameof(Vector256.ConvertToUInt64), 1, new Type[] {
        ...
        private void ValidateResult(Double[] firstOp, UInt64[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.ConvertToUInt64)}<UInt64>(Vector256<Double>): {method} failed:");
