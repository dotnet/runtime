...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    public sealed unsafe class VectorTernaryOpTest__ConditionalSelectSingle
    {
        ...
        private void ValidateResult(Single[] firstOp, Single[] secondOp, Single[] thirdOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.ConditionalSelect)}<Single>(Vector256<Single>, Vector256<Single>, Vector256<Single>): {method} failed:");
