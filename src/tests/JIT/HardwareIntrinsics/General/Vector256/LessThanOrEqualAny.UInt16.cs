...
namespace JIT.HardwareIntrinsics.General._Vector256
{
    ...
    {
        [Fact]
        public static void LessThanOrEqualAnyUInt16()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__LessThanOrEqualAnyUInt16
    {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector256)}.{nameof(Vector256.LessThanOrEqualAny)}<UInt16>(Vector256<UInt16>, Vector256<UInt16>): {method} failed:");
