...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    {
        [Fact]
        public static void GreaterThanOrEqualAnySByte()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanOrEqualAnySByte
    {
        ...
        private void ValidateResult(SByte[] left, SByte[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.GreaterThanOrEqualAny)}<SByte>(Vector128<SByte>, Vector128<SByte>): {method} failed:");
