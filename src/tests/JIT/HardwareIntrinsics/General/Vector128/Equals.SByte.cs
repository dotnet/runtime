...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    {
        [Fact]
        public static void EqualsSByte()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__EqualsSByte
    {
        ...
        private void ValidateResult(SByte[] left, SByte[] right, SByte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.Equals)}<SByte>(Vector128<SByte>, Vector128<SByte>): {method} failed:");
