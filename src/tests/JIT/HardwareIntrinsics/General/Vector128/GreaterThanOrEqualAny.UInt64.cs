...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanOrEqualAnyUInt64
    {
        ...
        private void ValidateResult(UInt64[] left, UInt64[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.GreaterThanOrEqualAny)}<UInt64>(Vector128<UInt64>, Vector128<UInt64>): {method} failed:");
