...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    {
        [Fact]
        public static void BitwiseAndSingle()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__BitwiseAndSingle
    {
        ...
        private void ValidateResult(Single[] left, Single[] right, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.BitwiseAnd)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:");
