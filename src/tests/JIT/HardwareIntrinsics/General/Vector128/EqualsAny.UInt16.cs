...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    {
        [Fact]
        public static void EqualsAnyUInt16()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__EqualsAnyUInt16
    {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.EqualsAny)}<UInt16>(Vector128<UInt16>, Vector128<UInt16>): {method} failed:");
