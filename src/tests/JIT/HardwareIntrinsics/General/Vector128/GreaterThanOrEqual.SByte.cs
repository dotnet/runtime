...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    {
        [Fact]
        public static void GreaterThanOrEqualSByte()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__GreaterThanOrEqualSByte
    {
        ...
        private void ValidateResult(SByte[] left, SByte[] right, SByte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.GreaterThanOrEqual)}<SByte>(Vector128<SByte>, Vector128<SByte>): {method} failed:");
