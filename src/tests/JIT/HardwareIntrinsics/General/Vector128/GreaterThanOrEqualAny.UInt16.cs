...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanOrEqualAnyUInt16
    {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.GreaterThanOrEqualAny)}<UInt16>(Vector128<UInt16>, Vector128<UInt16>): {method} failed:");
