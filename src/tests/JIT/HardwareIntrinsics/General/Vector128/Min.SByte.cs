...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    public sealed unsafe class VectorBinaryOpTest__MinSByte
    {
        ...
        private void ValidateResult(SByte[] left, SByte[] right, SByte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.Min)}<SByte>(Vector128<SByte>, Vector128<SByte>): {method} failed:");
