...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    {
        [Fact]
        public static void BitwiseAndUInt16()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__BitwiseAndUInt16
    {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, UInt16[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.BitwiseAnd)}<UInt16>(Vector128<UInt16>, Vector128<UInt16>): {method} failed:");
