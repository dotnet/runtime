...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    public sealed unsafe class VectorUnaryOpTest__CeilingSingle
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var method = typeof(Vector128).GetMethod(nameof(Vector128.Ceiling), new Type[] {
                typeof(Vector128<Single>)
            });

            if (method is null)
            {
                method = typeof(Vector128).GetMethod(nameof(Vector128.Ceiling), 1, new Type[] {
        ...
        private void ValidateResult(Single[] firstOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.Ceiling)}<Single>(Vector128<Single>): {method} failed:");
