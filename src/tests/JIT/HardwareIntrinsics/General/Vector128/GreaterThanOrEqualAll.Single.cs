...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    {
        [Fact]
        public static void GreaterThanOrEqualAllSingle()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanOrEqualAllSingle
    {
        ...
        private void ValidateResult(Single[] left, Single[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.GreaterThanOrEqualAll)}<Single>(Vector128<Single>, Vector128<Single>): {method} failed:");
