...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    {
        [Fact]
        public static void GreaterThanOrEqualAnyDouble()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanOrEqualAnyDouble
    {
        ...
        private void ValidateResult(Double[] left, Double[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.GreaterThanOrEqualAny)}<Double>(Vector128<Double>, Vector128<Double>): {method} failed:");
