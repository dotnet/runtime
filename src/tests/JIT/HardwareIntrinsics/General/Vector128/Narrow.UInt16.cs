...
namespace JIT.HardwareIntrinsics.General._Vector128
{
    ...
    public sealed unsafe class VectorBinaryOpTest__NarrowUInt16
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var method = typeof(Vector128).GetMethod(nameof(Vector128.Narrow), new Type[] {
            ...
            if (method is null)
            {
                method = typeof(Vector128).GetMethod(nameof(Vector128.Narrow), 1, new Type[] {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, Byte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.Narrow)}<Byte>(Vector128<UInt16>, Vector128<UInt16>): {method} failed:");
