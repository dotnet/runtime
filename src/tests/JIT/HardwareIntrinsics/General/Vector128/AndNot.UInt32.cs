...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorBinaryOpTest__AndNotUInt32
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var method = typeof(Vector128).GetMethod(nameof(Vector128.AndNot), new Type[] {
            ...
            if (method is null)
            {
                method = typeof(Vector128).GetMethod(nameof(Vector128.AndNot), 1, new Type[] {
        ...
        private void ValidateResult(UInt32[] left, UInt32[] right, UInt32[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector128)}.{nameof(Vector128.AndNot)}<UInt32>(Vector128<UInt32>, Vector128<UInt32>): {method} failed:");
