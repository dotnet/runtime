...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void NarrowUInt32()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__NarrowUInt32
    {
        ...
        private void ValidateResult(UInt32[] left, UInt32[] right, UInt16[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.Narrow)}<UInt16>(Vector64<UInt32>, Vector64<UInt32>): {method} failed:");
