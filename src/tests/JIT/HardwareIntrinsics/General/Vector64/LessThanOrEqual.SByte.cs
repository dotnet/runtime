...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void LessThanOrEqualSByte()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__LessThanOrEqualSByte
    {
        ...
        private void ValidateResult(SByte[] left, SByte[] right, SByte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.LessThanOrEqual)}<SByte>(Vector64<SByte>, Vector64<SByte>): {method} failed:");
