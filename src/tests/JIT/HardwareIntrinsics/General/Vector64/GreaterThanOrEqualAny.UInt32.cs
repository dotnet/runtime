...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void GreaterThanOrEqualAnyUInt32()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanOrEqualAnyUInt32
    {
        ...
        private void ValidateResult(UInt32[] left, UInt32[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.GreaterThanOrEqualAny)}<UInt32>(Vector64<UInt32>, Vector64<UInt32>): {method} failed:");
