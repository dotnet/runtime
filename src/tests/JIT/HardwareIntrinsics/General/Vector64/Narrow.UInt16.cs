...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void NarrowUInt16()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__NarrowUInt16
    {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, Byte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.Narrow)}<Byte>(Vector64<UInt16>, Vector64<UInt16>): {method} failed:");
