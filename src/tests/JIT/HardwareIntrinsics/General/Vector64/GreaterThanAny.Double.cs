...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void GreaterThanAnyDouble()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanAnyDouble
    {
        ...
        private void ValidateResult(Vector64<Double> op1, Vector64<Double> op2, bool result, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateResult(Double[] left, Double[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.GreaterThanAny)}<Double>(Vector64<Double>, Vector64<Double>): {method} failed:");
