...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorUnaryOpTest__ConvertToInt64Int64
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var method = typeof(Vector64).GetMethod(nameof(Vector64.ConvertToInt64), new Type[] {
                typeof(Vector64<Double>)
            });

            if (method is null)
            {
                method = typeof(Vector64).GetMethod(nameof(Vector64.ConvertToInt64), 1, new Type[] {
        ...
        private void ValidateResult(Double[] firstOp, Int64[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.ConvertToInt64)}<Int64>(Vector64<Double>): {method} failed:");
