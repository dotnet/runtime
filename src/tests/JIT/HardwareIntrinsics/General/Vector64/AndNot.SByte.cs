...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void AndNotSByte()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__AndNotSByte
    {
        ...
        private void ValidateResult(SByte[] left, SByte[] right, SByte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.AndNot)}<SByte>(Vector64<SByte>, Vector64<SByte>): {method} failed:");
