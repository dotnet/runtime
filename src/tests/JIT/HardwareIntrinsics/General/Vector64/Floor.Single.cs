...
namespace JIT.HardwareIntrinsics.General
{
    ...
    public sealed unsafe class VectorUnaryOpTest__FloorSingle
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var method = typeof(Vector64).GetMethod(nameof(Vector64.Floor), new Type[] {
                typeof(Vector64<Single>)
            });

            if (method is null)
            {
                method = typeof(Vector64).GetMethod(nameof(Vector64.Floor), 1, new Type[] {
        ...
        private void ValidateResult(Single[] firstOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.Floor)}<Single>(Vector64<Single>): {method} failed:");
