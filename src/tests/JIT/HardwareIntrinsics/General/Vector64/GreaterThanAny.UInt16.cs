...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void GreaterThanAnyUInt16()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanAnyUInt16
    {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.GreaterThanAny)}<UInt16>(Vector64<UInt16>, Vector64<UInt16>): {method} failed:");
