...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void ConvertToSingleInt32()
        {
    ...
    public sealed unsafe class VectorUnaryOpTest__ConvertToSingleInt32
    {
        ...
        private void ValidateResult(Int32[] firstOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.ConvertToSingle)}<Single>(Vector64<Int32>): {method} failed:");
