...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void GreaterThanOrEqualAllSByte()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__GreaterThanOrEqualAllSByte
    {
        ...
        private void ValidateResult(SByte[] left, SByte[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.GreaterThanOrEqualAll)}<SByte>(Vector64<SByte>, Vector64<SByte>): {method} failed:");
