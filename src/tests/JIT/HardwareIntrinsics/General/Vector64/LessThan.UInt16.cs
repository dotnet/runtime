...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void LessThanUInt16()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__LessThanUInt16
    {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, UInt16[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.LessThan)}<UInt16>(Vector64<UInt16>, Vector64<UInt16>): {method} failed:");
