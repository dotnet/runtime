...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void EqualsUInt16()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__EqualsUInt16
    {
        ...
        private void ValidateResult(UInt16[] left, UInt16[] right, UInt16[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.Equals)}<UInt16>(Vector64<UInt16>, Vector64<UInt16>): {method} failed:");
