...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void LessThanAnyUInt64()
        {
    ...
    public sealed unsafe class VectorBooleanBinaryOpTest__LessThanAnyUInt64
    {
        ...
        private void ValidateResult(UInt64[] left, UInt64[] right, bool result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.LessThanAny)}<UInt64>(Vector64<UInt64>, Vector64<UInt64>): {method} failed:");
