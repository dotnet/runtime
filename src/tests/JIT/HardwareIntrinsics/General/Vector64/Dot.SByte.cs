...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    public sealed unsafe class VectorBinaryOpTest__DotSByte
    {
        ...
        private void ValidateResult(Vector64<SByte> op1, Vector64<SByte> op2, SByte result, [CallerMemberName] string method = "")
        {
        ...
        private void ValidateResult(SByte[] left, SByte[] right, SByte result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.Dot)}<SByte>(Vector64<SByte>, Vector64<SByte>): {method} failed:");
