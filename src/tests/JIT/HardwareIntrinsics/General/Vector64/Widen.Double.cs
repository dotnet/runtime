...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    public sealed unsafe class VectorWidenTest__WidenDouble
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            Unsafe.Write(_dataTable.outLowerArrayPtr, (((Vector64<Double> Lower, Vector64<Double> Upper))(result)).Lower);
            Unsafe.Write(_dataTable.outUpperArrayPtr, (((Vector64<Double> Lower, Vector64<Double> Upper))(result)).Upper);
            ValidateResult(_dataTable.inArray1Ptr, _dataTable.outLowerArrayPtr, _dataTable.outUpperArrayPtr);
        }
        ...
        private void ValidateResult(Vector64<Single> op1, void* lowerResult, void* upperResult, [CallerMemberName] string method = "")
        {
            ...
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<Double, byte>(ref outLowerArray[0]), ref Unsafe.AsRef<byte>(lowerResult), (uint)Unsafe.SizeOf<Vector64<Double>>());
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<Double, byte>(ref outUpperArray[0]), ref Unsafe.AsRef<byte>(upperResult), (uint)Unsafe.SizeOf<Vector64<Double>>());
        ...
        private void ValidateResult(Single[] firstOp, Double[] lowerResult, Double[] upperResult, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.Widen)}<Double>(Vector64<Single>): {method} failed:");
