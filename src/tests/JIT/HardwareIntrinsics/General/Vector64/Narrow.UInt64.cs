...
namespace JIT.HardwareIntrinsics.General._Vector64
{
    ...
    {
        [Fact]
        public static void NarrowUInt64()
        {
    ...
    public sealed unsafe class VectorBinaryOpTest__NarrowUInt64
    {
        ...
        private void ValidateResult(UInt64[] left, UInt64[] right, UInt32[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(Vector64)}.{nameof(Vector64.Narrow)}<UInt32>(Vector64<UInt64>, Vector64<UInt64>): {method} failed:");
