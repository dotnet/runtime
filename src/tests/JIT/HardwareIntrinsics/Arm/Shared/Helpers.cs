...
namespace JIT.HardwareIntrinsics.Arm
{
    static class Helpers
    {
        ...
        // AES encoding/decoding functions
        public static class Aes
        {
            // Fixed, public AES substitution box used by the forward SubBytes transform.
            private static readonly byte[] SBox =
            {
            ...
            // Fixed, public AES inverse substitution box used by the inverse SubBytes transform.
            ...
            private static byte Multiply(byte left, byte right)
            {
                ...
                // Multiply in GF(2^8) using the AES reduction polynomial x^8 + x^4 + x^3 + x + 1.
                while (right != 0)
                {
                    if ((right & 1) != 0)
                    {
            ...
            private static void LoadBlock(byte[] source, int start, byte[] block)
            {
                // Copy the 16-byte AES state containing the requested element.
                for (int i = 0; i < 16; i++)
                {
                    block[i] = source[start + i];
                }
            }

            private static void AddRoundKey(byte[] block, byte[] roundKeys, int start)
            {
                // XOR the state with the corresponding 16-byte round key.
                for (int i = 0; i < 16; i++)
                {
                    block[i] ^= roundKeys[start + i];
                }
            }

            private static void SubBytes(byte[] block, byte[] box)
            {
                // Substitute each state byte through the selected AES S-box.
                for (int i = 0; i < 16; i++)
                {
                    block[i] = box[block[i]];
                }
            ...
            {
                // Rotate AES state rows left by 0, 1, 2, and 3 bytes.
                byte b1 = block[1];
                block[1]  = block[5];
                block[5]  = block[9];
                block[9]  = block[13];
            ...
            {
                // Rotate AES state rows right by 0, 1, 2, and 3 bytes.
                byte b13 = block[13];
                block[13] = block[9];
                block[9]  = block[5];
                block[5]  = block[1];
            ...
            private static void MixColumn(byte[] block, int start)
            {
                byte b0 = block[start + 0];
                byte b1 = block[start + 1];
                byte b2 = block[start + 2];
                byte b3 = block[start + 3];

                // Forward MixColumns matrix multiplication for a single AES state column.
                block[start + 0] = (byte)(Multiply(0x02, b0) ^ Multiply(0x03, b1) ^ b2 ^ b3);
                block[start + 1] = (byte)(b0 ^ Multiply(0x02, b1) ^ Multiply(0x03, b2) ^ b3);
                block[start + 2] = (byte)(b0 ^ b1 ^ Multiply(0x02, b2) ^ Multiply(0x03, b3));
                block[start + 3] = (byte)(Multiply(0x03, b0) ^ b1 ^ b2 ^ Multiply(0x02, b3));
            }

            private static void InvMixColumn(byte[] block, int start)
            {
                byte b0 = block[start + 0];
                byte b1 = block[start + 1];
                byte b2 = block[start + 2];
                ...
                // Inverse MixColumns matrix multiplication for a single AES state column.
                block[start + 0] = (byte)(Multiply(0x0e, b0) ^ Multiply(0x0b, b1) ^ Multiply(0x0d, b2) ^ Multiply(0x09, b3));
                block[start + 1] = (byte)(Multiply(0x09, b0) ^ Multiply(0x0e, b1) ^ Multiply(0x0b, b2) ^ Multiply(0x0d, b3));
                block[start + 2] = (byte)(Multiply(0x0d, b0) ^ Multiply(0x09, b1) ^ Multiply(0x0e, b2) ^ Multiply(0x0b, b3));
                block[start + 3] = (byte)(Multiply(0x0b, b0) ^ Multiply(0x0d, b1) ^ Multiply(0x09, b2) ^ Multiply(0x0e, b3));
            }

            public static byte Decrypt(int index, byte[] value, byte[] roundKeys)
            {
                int start = index & ~0xF;
                byte[] block = new byte[16];

                // AESD applies AddRoundKey, InvSubBytes, and InvShiftRows to one 128-bit block.
                LoadBlock(value, start, block);
                AddRoundKey(block, roundKeys, start);
            ...
            public static byte Encrypt(int index, byte[] value, byte[] roundKeys)
            {
                int start = index & ~0xF;
                byte[] block = new byte[16];

                // AESE applies AddRoundKey, SubBytes, and ShiftRows to one 128-bit block.
                LoadBlock(value, start, block);
                AddRoundKey(block, roundKeys, start);
                SubBytes(block, SBox);
                ShiftRows(block);
            ...
            public static byte InverseMixColumns(int index, byte[] value)
            {
                int start = index & ~0xF;
                byte[] block = new byte[16];
                ...
                // AESIMC transforms each of the four AES state columns independently.
                for (int i = 0; i < 16; i += 4)
                {
                    InvMixColumn(block, i);
                }

                return block[index - start];
            }

            public static byte MixColumns(int index, byte[] value)
            {
                int start = index & ~0xF;
                byte[] block = new byte[16];
                ...
                // AESMC transforms each of the four AES state columns independently.
                for (int i = 0; i < 16; i += 4)
                {
                    MixColumn(block, i);
                }
