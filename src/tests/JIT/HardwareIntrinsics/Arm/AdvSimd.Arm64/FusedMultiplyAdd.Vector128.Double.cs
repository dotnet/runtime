...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void FusedMultiplyAdd_Vector128_Double()
        {
    ...
    public sealed unsafe class SimpleTernaryOpTest__FusedMultiplyAdd_Vector128_Double
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleTernaryOpTest__FusedMultiplyAdd_Vector128_Double testClass)
            {
                var result = AdvSimd.Arm64.FusedMultiplyAdd(_fld1, _fld2, _fld3);

                Unsafe.Write(testClass._dataTable.outArrayPtr, result);
                testClass.ValidateResult(_fld1, _fld2, _fld3, testClass._dataTable.outArrayPtr);
            }

            public void RunStructFldScenario_Load(SimpleTernaryOpTest__FusedMultiplyAdd_Vector128_Double testClass)
            {
                fixed (Vector128<Double>* pFld1 = &_fld1)
                fixed (Vector128<Double>* pFld2 = &_fld2)
                fixed (Vector128<Double>* pFld3 = &_fld3)
                {
                    var result = AdvSimd.Arm64.FusedMultiplyAdd(
        ...
        static SimpleTernaryOpTest__FusedMultiplyAdd_Vector128_Double()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetDouble(); }
        ...
        public SimpleTernaryOpTest__FusedMultiplyAdd_Vector128_Double()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd.Arm64).GetMethod(nameof(AdvSimd.Arm64.FusedMultiplyAdd), new Type[] { typeof(Vector128<Double>), typeof(Vector128<Double>), typeof(Vector128<Double>) })
        ...
        public void RunLclVarScenario_UnsafeRead()
        {
            ...
            var result = AdvSimd.Arm64.FusedMultiplyAdd(op1, op2, op3);
        ...
        private void ValidateResult(Double[] firstOp, Double[] secondOp, Double[] thirdOp, Double[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (BitConverter.DoubleToInt64Bits(Helpers.FusedMultiplyAdd(firstOp[i], secondOp[i], thirdOp[i])) != BitConverter.DoubleToInt64Bits(result[i]))
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd.Arm64)}.{nameof(AdvSimd.Arm64.FusedMultiplyAdd)}<Double>(Vector128<Double>, Vector128<Double>, Vector128<Double>): {method} failed:");
