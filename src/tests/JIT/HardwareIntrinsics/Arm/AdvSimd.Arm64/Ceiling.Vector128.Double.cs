...
namespace JIT.HardwareIntrinsics.Arm._AdvSimd.Arm64
{
    ...
    {
        [Fact]
        public static void Ceiling_Vector128_Double()
        {
    ...
    public sealed unsafe class SimpleUnaryOpTest__Ceiling_Vector128_Double
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario_Load(SimpleUnaryOpTest__Ceiling_Vector128_Double testClass)
            {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd.Arm64).GetMethod(nameof(AdvSimd.Arm64.Ceiling), new Type[] { typeof(Vector128<Double>) })
        ...
        private void ValidateResult(Double[] firstOp, Double[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (BitConverter.DoubleToInt64Bits(Helpers.Ceiling(firstOp[i])) != BitConverter.DoubleToInt64Bits(result[i]))
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd.Arm64)}.{nameof(AdvSimd.Arm64.Ceiling)}<Double>(Vector128<Double>): {method} failed:");
