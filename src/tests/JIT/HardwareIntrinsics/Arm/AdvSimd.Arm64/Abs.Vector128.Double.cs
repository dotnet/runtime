...
namespace JIT.HardwareIntrinsics.Arm._AdvSimd.Arm64
{
    ...
    public sealed unsafe class SimpleUnaryOpTest__Abs_Vector128_Double
    {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd.Arm64).GetMethod(nameof(AdvSimd.Arm64.Abs), new Type[] { typeof(Vector128<Double>) })
        ...
        private void ValidateResult(Double[] firstOp, Double[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (BitConverter.DoubleToInt64Bits(Helpers.Abs(firstOp[i])) != BitConverter.DoubleToInt64Bits(result[i]))
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd.Arm64)}.{nameof(AdvSimd.Arm64.Abs)}<Double>(Vector128<Double>): {method} failed:");
