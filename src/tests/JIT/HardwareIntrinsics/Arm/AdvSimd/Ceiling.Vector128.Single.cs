...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void Ceiling_Vector128_Single()
        {
    ...
    public sealed unsafe class SimpleUnaryOpTest__Ceiling_Vector128_Single
    {
        ...
        static SimpleUnaryOpTest__Ceiling_Vector128_Single()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetSingle(); }
            Unsafe.CopyBlockUnaligned(ref Unsafe.As<Vector128<Single>, byte>(ref _clsVar1), ref Unsafe.As<Single, byte>(ref _data1[0]), (uint)Unsafe.SizeOf<Vector128<Single>>());
        }

        public SimpleUnaryOpTest__Ceiling_Vector128_Single()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.Ceiling), new Type[] { typeof(Vector128<Single>) })
        ...
        private void ValidateResult(Single[] firstOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.Ceiling)}<Single>(Vector128<Single>): {method} failed:");
