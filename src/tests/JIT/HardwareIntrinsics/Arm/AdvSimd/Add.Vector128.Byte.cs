...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void Add_Vector128_Byte()
        {
            var test = new SimpleBinaryOpTest__Add_Vector128_Byte();
    ...
    public sealed unsafe class SimpleBinaryOpTest__Add_Vector128_Byte
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleBinaryOpTest__Add_Vector128_Byte testClass)
            {
            ...
            public void RunStructFldScenario_Load(SimpleBinaryOpTest__Add_Vector128_Byte testClass)
            {
        ...
        static SimpleBinaryOpTest__Add_Vector128_Byte()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetByte(); }
        ...
        public SimpleBinaryOpTest__Add_Vector128_Byte()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.Add), new Type[] { typeof(Vector128<Byte>), typeof(Vector128<Byte>) })
        ...
        private void ValidateResult(Byte[] left, Byte[] right, Byte[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (Helpers.Add(left[i], right[i]) != result[i])
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.Add)}<Byte>(Vector128<Byte>, Vector128<Byte>): {method} failed:");
