...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void MultiplyWideningLower_Vector64_UInt32()
        {
            var test = new SimpleBinaryOpTest__MultiplyWideningLower_Vector64_UInt32();
    ...
    public sealed unsafe class SimpleBinaryOpTest__MultiplyWideningLower_Vector64_UInt32
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleBinaryOpTest__MultiplyWideningLower_Vector64_UInt32 testClass)
            {
                var result = AdvSimd.MultiplyWideningLower(_fld1, _fld2);

                Unsafe.Write(testClass._dataTable.outArrayPtr, result);
                testClass.ValidateResult(_fld1, _fld2, testClass._dataTable.outArrayPtr);
            }

            public void RunStructFldScenario_Load(SimpleBinaryOpTest__MultiplyWideningLower_Vector64_UInt32 testClass)
            {
        ...
        static SimpleBinaryOpTest__MultiplyWideningLower_Vector64_UInt32()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetUInt32(); }
        ...
        public SimpleBinaryOpTest__MultiplyWideningLower_Vector64_UInt32()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.MultiplyWideningLower), new Type[] { typeof(Vector64<UInt32>), typeof(Vector64<UInt32>) })
        ...
        public void RunLclVarScenario_UnsafeRead()
        {
            ...
            var result = AdvSimd.MultiplyWideningLower(op1, op2);
        ...
        public void RunClassLclFldScenario()
        {
            ...
            var test = new SimpleBinaryOpTest__MultiplyWideningLower_Vector64_UInt32();
            var result = AdvSimd.MultiplyWideningLower(test._fld1, test._fld2);
        ...
        private void ValidateResult(UInt32[] left, UInt32[] right, UInt64[] result, [CallerMemberName] string method = "")
        {
            bool succeeded = true;

            for (var i = 0; i < RetElementCount; i++)
            {
                if (Helpers.MultiplyWidening(left[i], right[i]) != result[i])
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.MultiplyWideningLower)}<UInt64>(Vector64<UInt32>, Vector64<UInt32>): {method} failed:");
