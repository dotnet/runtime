...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void AbsoluteDifferenceWideningUpper_Vector128_SByte()
        {
            var test = new SimpleBinaryOpTest__AbsoluteDifferenceWideningUpper_Vector128_SByte();
    ...
    public sealed unsafe class SimpleBinaryOpTest__AbsoluteDifferenceWideningUpper_Vector128_SByte
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleBinaryOpTest__AbsoluteDifferenceWideningUpper_Vector128_SByte testClass)
            {
                var result = AdvSimd.AbsoluteDifferenceWideningUpper(_fld1, _fld2);

                Unsafe.Write(testClass._dataTable.outArrayPtr, result);
                testClass.ValidateResult(_fld1, _fld2, testClass._dataTable.outArrayPtr);
            }

            public void RunStructFldScenario_Load(SimpleBinaryOpTest__AbsoluteDifferenceWideningUpper_Vector128_SByte testClass)
            {
                fixed (Vector128<SByte>* pFld1 = &_fld1)
                fixed (Vector128<SByte>* pFld2 = &_fld2)
                {
                    var result = AdvSimd.AbsoluteDifferenceWideningUpper(
        ...
        static SimpleBinaryOpTest__AbsoluteDifferenceWideningUpper_Vector128_SByte()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetSByte(); }
        ...
        public SimpleBinaryOpTest__AbsoluteDifferenceWideningUpper_Vector128_SByte()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.AbsoluteDifferenceWideningUpper), new Type[] { typeof(Vector128<SByte>), typeof(Vector128<SByte>) })
        ...
        public void RunLclVarScenario_UnsafeRead()
        {
            ...
            var result = AdvSimd.AbsoluteDifferenceWideningUpper(op1, op2);
        ...
        public void RunClassLclFldScenario()
        {
            ...
            var test = new SimpleBinaryOpTest__AbsoluteDifferenceWideningUpper_Vector128_SByte();
            var result = AdvSimd.AbsoluteDifferenceWideningUpper(test._fld1, test._fld2);
        ...
        private void ValidateResult(SByte[] left, SByte[] right, UInt16[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (Helpers.AbsoluteDifferenceWideningUpper(left, right, i) != result[i])
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.AbsoluteDifferenceWideningUpper)}<UInt16>(Vector128<SByte>, Vector128<SByte>): {method} failed:");
