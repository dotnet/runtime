...
namespace JIT.HardwareIntrinsics.Arm._AdvSimd
{
    ...
    {
        [Fact]
        public static void FusedMultiplyAddNegatedScalar_Vector64_Single()
        {
    ...
    public sealed unsafe class SimpleTernaryOpTest__FusedMultiplyAddNegatedScalar_Vector64_Single
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleTernaryOpTest__FusedMultiplyAddNegatedScalar_Vector64_Single testClass)
            {
            ...
            public void RunStructFldScenario_Load(SimpleTernaryOpTest__FusedMultiplyAddNegatedScalar_Vector64_Single testClass)
            {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.FusedMultiplyAddNegatedScalar), new Type[] { typeof(Vector64<Single>), typeof(Vector64<Single>), typeof(Vector64<Single>) })
        ...
        private void ValidateResult(Single[] firstOp, Single[] secondOp, Single[] thirdOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (BitConverter.SingleToInt32Bits(Helpers.FusedMultiplyAddNegated(firstOp[0], secondOp[0], thirdOp[0])) != BitConverter.SingleToInt32Bits(result[0]))
            {
                succeeded = false;
            }
            else
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.FusedMultiplyAddNegatedScalar)}<Single>(Vector64<Single>, Vector64<Single>, Vector64<Single>): {method} failed:");
