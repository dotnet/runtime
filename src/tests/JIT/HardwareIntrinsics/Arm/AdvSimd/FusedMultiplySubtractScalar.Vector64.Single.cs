...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void FusedMultiplySubtractScalar_Vector64_Single()
        {
    ...
    public sealed unsafe class SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Single
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Single testClass)
            {
            ...
            public void RunStructFldScenario_Load(SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Single testClass)
            {
        ...
        static SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Single()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetSingle(); }
        ...
        public SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Single()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.FusedMultiplySubtractScalar), new Type[] { typeof(Vector64<Single>), typeof(Vector64<Single>), typeof(Vector64<Single>) })
        ...
        private void ValidateResult(Single[] firstOp, Single[] secondOp, Single[] thirdOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (BitConverter.SingleToInt32Bits(Helpers.FusedMultiplySubtract(firstOp[0], secondOp[0], thirdOp[0])) != BitConverter.SingleToInt32Bits(result[0]))
            {
                succeeded = false;
            }
            else
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.FusedMultiplySubtractScalar)}<Single>(Vector64<Single>, Vector64<Single>, Vector64<Single>): {method} failed:");
