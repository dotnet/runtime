...
namespace System.Data.OleDb
{
    ...
    public sealed class OleDbCommand : DbCommand, ICloneable, IDbCommand
    {
        ...
        private int ExecuteCommandTextForMultpleResults(tagDBPARAMS dbParams, out object executeResult)
        {
            ...
            OleDbHResult hr;
            hr = _icommandText!.Execute(0, ref ODB.IID_IMultipleResults, dbParams, out _recordsAffected, out executeResult);
        ...
        private int ExecuteCommandTextForSingleResult(tagDBPARAMS dbParams, out object executeResult)
        {
            ...
            if (_executeQuery)
            {
                hr = _icommandText!.Execute(0, ref ODB.IID_IRowset, dbParams, out _recordsAffected, out executeResult);
            }
            else
            {
                hr = _icommandText!.Execute(0, ref ODB.IID_NULL, dbParams, out _recordsAffected, out executeResult);
            }
            ExecuteCommandTextErrorHandling(hr);
        ...
        private int ExecuteCommandTextForSingleRow(tagDBPARAMS dbParams, out object executeResult)
        {
            ...
            {
                OleDbHResult hr;
                hr = _icommandText!.Execute(0, ref ODB.IID_IRow, dbParams, out _recordsAffected, out executeResult);
                        ...
                        using (DBPropSet? propSet = CommandPropertySets())
                        {
                            if (null != propSet)
                            {
                                ...
                                try
                                {
                                    ...
                                    hr = iopenRowset.Value.OpenRowset(0, tableID, 0, ref ODB.IID_IRowset, propSet.PropertySetCount, propSet.DangerousGetHandle(), out executeResult);
                                }
                                finally
                                {
                                    if (mustRelease)
                                ...
                                {
                                    hr = iopenRowset.Value.OpenRowset(0, tableID, 0, ref ODB.IID_IRowset, 0, 0, out executeResult);
                                }
                            }
                            else
                            {
