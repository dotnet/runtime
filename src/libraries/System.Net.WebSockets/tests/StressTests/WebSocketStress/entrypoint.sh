...
/live-runtime-artifacts/testhost/net$VERSION-linux-$CONFIGURATION-x64/dotnet exec --roll-forward Major ./bin/$CONFIGURATION/net$VERSION/WebSocketStress.dll $STRESS_ARGS

exit_code=$?

if [ $exit_code -ne 0 ]; then
    echo "WebSocketStress failed, copying artifacts for investigation"
