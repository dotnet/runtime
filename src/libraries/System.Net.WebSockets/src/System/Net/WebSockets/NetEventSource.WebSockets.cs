...
namespace System.Net
{
    ...
    internal sealed partial class NetEventSource
    {
        ...
        [Event(WsAsyncDbgId, Keywords = Keywords.Debug, Level = EventLevel.Verbose)]
        private void WsAsyncDbg(string objName, string memberName, string message) =>
            WriteEvent(WsAsyncDbgId, objName, memberName, message);
        ...
        [NonEvent]
        public static void DbgLockTaken(object? obj, [CallerMemberName] string? memberName = null)
            => AsyncDbgLog(obj, $"State update lock taken", memberName);

        [NonEvent]
        public static void DbgLockReleased(object? obj, [CallerMemberName] string? memberName = null)
            => AsyncDbgLog(obj, $"State update lock released", memberName);

        [NonEvent]
        public static void AsyncDbgLogSuccess(object? obj, string? schedulingMemberName, [CallerMemberName] string? memberName = null)
            => AsyncDbgLog(obj, $"Task issued by {schedulingMemberName ?? MissingMember} has completed successfully", memberName);

        [NonEvent]
        public static void AsyncDbgLogFailure(object? obj, string? schedulingMemberName, Exception e, [CallerMemberName] string? memberName = null)
            => AsyncDbgLog(obj, $"Task issued by {schedulingMemberName ?? MissingMember} has completed with an error: {e}", memberName);

        [NonEvent]
        public static void AsyncDbgLog(object? obj, string message, [CallerMemberName] string? memberName = null)
        {
            Debug.Assert(Log.IsEnabled());
