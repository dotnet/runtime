...
namespace System.Net.WebSockets
{
    ...
    internal sealed partial class ManagedWebSocket : WebSocket, IValueTaskSource<WebSocketReceiveResult>, IValueTaskSource<ValueWebSocketReceiveResult>
    {
        ...
        {
            _sender = new Sender(stream, isServer);
            _receiver = new Receiver(stream, isServer);
        ...
        private void OnAborted()
        {
            lock (StateUpdateLock)
            {
                ...
                {
                    _state = _state is not (WebSocketState.None or WebSocketState.Connecting) ?
                        WebSocketState.Aborted : WebSocketState.Closed;
                }
            }
        }
...
#pragma warning restore CA1416
        ...
        {
            Debug.Assert(_sendFrameAsyncLock.CurrentCount == 1, "Caller should hold the _sendFrameAsyncLock");
        ...
        private async ValueTask WaitForSendTaskAsync(ValueTask sendTask)
        {
            try
            {
        ...
        private async ValueTask HandleReceivedCloseAsync(ReadOnlyMemory<byte> payload)
        {
            ...
            else if (payload.Length >= 2)
            {
                ...
                if (payload.Length > 2)
                {
                    try
                    {
                        closeStatusDescription = s_textEncoding.GetString(payload.Span[2..]);
                    }
