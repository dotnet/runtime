...
namespace System.Security.Cryptography.SLHDsa.Tests
{
    ...
    internal sealed class SlhDsaMockImplementation : SlhDsa
    {
        ...
        public delegate void SignPreHashCoreAction(ReadOnlySpan<byte> hash, ReadOnlySpan<byte> context, HashAlgorithmName preHashAlgorithm, Span<byte> destination);
        public delegate bool VerifyPreHashCoreFunc(ReadOnlySpan<byte> hash, ReadOnlySpan<byte> context, HashAlgorithmName preHashAlgorithm, ReadOnlySpan<byte> signature);
        ...
        protected override void SignPreHashCore(ReadOnlySpan<byte> hash, ReadOnlySpan<byte> context, HashAlgorithmName preHashAlgorithm, Span<byte> destination)
        {
            ...
            SignPreHashCoreHook(hash, context, preHashAlgorithm, destination);
        }
        ...
        protected override bool VerifyPreHashCore(ReadOnlySpan<byte> hash, ReadOnlySpan<byte> context, HashAlgorithmName preHashAlgorithm, ReadOnlySpan<byte> signature)
        {
            ...
            return VerifyPreHashCoreHook(hash, context, preHashAlgorithm, signature);
        }
        ...
        public void AddLengthAssertion()
        {
            ...
            SignPreHashCoreHook = (ReadOnlySpan<byte> hash, ReadOnlySpan<byte> context, HashAlgorithmName preHashAlgorithm, Span<byte> destination) =>
            {
            ...
            VerifyPreHashCoreHook = (ReadOnlySpan<byte> hash, ReadOnlySpan<byte> context, HashAlgorithmName preHashAlgorithm, ReadOnlySpan<byte> signature) =>
            {
                bool ret = oldVerifyPreHashCoreHook(hash, context, preHashAlgorithm, signature);
                Assert.Equal(Algorithm.SignatureSizeInBytes, signature.Length);
                return ret;
            };
        }
        ...
        public void AddDestinationBufferIsSameAssertion(ReadOnlyMemory<byte> buffer)
        {
            ...
            SignPreHashCoreHook = (ReadOnlySpan<byte> hash, ReadOnlySpan<byte> context, HashAlgorithmName preHashAlgorithm, Span<byte> destination) =>
            {
                oldSignPreHashCoreHook(hash, context, preHashAlgorithm, destination);
                AssertExtensions.Same(buffer.Span, destination);
            };
