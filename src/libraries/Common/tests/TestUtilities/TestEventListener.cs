...
public sealed class TestEventListener : EventListener
{
    ...
    public TestEventListener(TextWriter output, bool enableActivityId, params string[] sourceNames)
        : this(str => output.WriteLine(str), enableActivityId, sourceNames)
    { }

    public TestEventListener(TextWriter output, params string[] sourceNames)
        : this(output, enableActivityId: false, sourceNames)
    { }
    ...
    internal static class ActivityHelpers
    {
        ...
        {
            return guid == Guid.Empty ? string.Empty : IsActivityPath(guid, 0) ? CreateActivityPathString(guid) : guid.ToString();
        }

        internal static unsafe string CreateActivityPathString(Guid guid, bool includePID = false)
        {
