...
namespace System.Net
{
    internal static partial class IPv6AddressHelper
    {
        ...
        internal static unsafe bool IsValidStrict(char* name, int start, int end)
        {
                    ...
                    switch (currentCh)
                    {
                        ...
                        case ']':
                            ...
                            else
                            {
                                ...
                                for (; i < end; i++)
                                {
                                    if (!char.IsAsciiDigit((char)(ushort)(name[i])))
                                    {
                                        return false;
                                    }
                                }
        ...
        internal static void Parse(ReadOnlySpan<char> address, scoped Span<ushort> numbers, out ReadOnlySpan<char> scopeId)
        {
            int number = 0;
            int currentCh;
            int index = 0;
            ...
            scopeId = ReadOnlySpan<char>.Empty;
        ...
        //  Remarks: MUST NOT be used unless all input indexes are verified and trusted.
        //           start must be next to '[' position, or error is reported
        internal static unsafe bool IsValidStrict(byte* name, int start, int end)
        {
            // Number of components in this IPv6 address
            int sequenceCount = 0;
            // Length of the component currently being constructed
        ...
        internal static void Parse(ReadOnlySpan<byte> address, scoped Span<ushort> numbers, out ReadOnlySpan<byte> scopeId)
        {
            int number = 0;
            int currentCh;
            int index = 0;
            ...
            scopeId = ReadOnlySpan<byte>.Empty;

            // Skip the start '[' character, if present. Stop parsing at the end IPv6 address terminator (']').
            for (int i = (address[0] == (byte)('[') ? 1 : 0); i < address.Length && address[i] != (byte)(']');)
            {
                currentCh = (ushort)(address[i]);
                ...
                switch (currentCh)
                {
                    case '%':
                        ...
                        for (++i; i < address.Length && address[i] != (byte)(']') && address[i] != (byte)('/'); ++i)
                        {
                        }
                        scopeId = address.Slice(scopeStart, i - scopeStart);

                        // Ignore the prefix (if any.)
                        for (; i < address.Length && address[i] != (byte)(']'); ++i)
                        {
                        }
                        break;
                    ...
                    case ':':
                                        ...
                                        (j < i + 4); ++j)
                        {
                            ...
                            if (address[j] == (byte)('.'))
                            {
                                ...
                                // or the end-of-string (which we conveniently
                                // delimited with ']').
                                while (j < address.Length && (address[j] != (byte)(']')) && (address[j] != (byte)('/')) && (address[j] != (byte)('%')))
                                {
                                    ++j;
                                }
                                int ipv4Address = IPv4AddressHelper.ParseHostNumber(address, i, j);
                    ...
                    case '/':
                        ...
                        for (++i; i < address.Length && address[i] != (byte)(']'); i++)
                        {
                        }
