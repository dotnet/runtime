...
namespace System.Data.ProviderBase
{
    internal abstract partial class DbConnectionFactory
    {
        ...
        private DbConnectionPool? GetConnectionPool(DbConnection owningObject, DbConnectionPoolGroup connectionPoolGroup)
        {
            ...
            if (connectionPoolGroup.IsDisabled && (connectionPoolGroup.PoolGroupOptions is not null))
            {
                ...
                Debug.Assert(connectionOptions is not null, "prevent expansion of connectionString");
        ...
        internal DbConnectionPoolGroup? GetConnectionPoolGroup(DbConnectionPoolKey key, DbConnectionPoolGroupOptions? poolOptions, ref DbConnectionOptions? userConnectionOptions)
        {
            ...
            DbConnectionPoolGroup? connectionPoolGroup;
            Dictionary<DbConnectionPoolKey, DbConnectionPoolGroup> connectionPoolGroups = _connectionPoolGroups;
            if (!connectionPoolGroups.TryGetValue(key, out connectionPoolGroup) || (connectionPoolGroup.IsDisabled && (connectionPoolGroup.PoolGroupOptions is not null)))
            {
                ...
                Debug.Assert(connectionPoolGroup is not null, "how did we not create a pool entry?");
                Debug.Assert(userConnectionOptions is not null, "how did we not have user connection options?");
            }
