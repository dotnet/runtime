...
namespace System.Reflection
{
    ...
    internal ref partial struct TypeNameParser
    {
        ...
        //
        // The typename parser has a strange attitude towards whitespace. It throws away whitespace between punctuation tokens and whitespace
        ...
        private sealed partial class NamespaceTypeName : TypeName
        {
            ...
            public override Type? ResolveType(ref TypeNameParser parser, string? containingAssemblyIfAny)
            {
                return parser.GetType(_fullName, containingAssemblyIfAny);
            }
        }
        ...
        //
        private sealed class NestedTypeName : TypeName
        {
            private readonly TypeName _declaringType;
            private readonly string _name;

            public NestedTypeName(string name, TypeName declaringType)
            {
                _name = name;
            ...
            public override Type? ResolveType(ref TypeNameParser parser, string? containingAssemblyIfAny)
            {
                Type? declaringType = _declaringType.ResolveType(ref parser, containingAssemblyIfAny);
                if (declaringType is null)
                    return null;
                return parser.GetNestedType(declaringType, _name);
            }
        }
