...
namespace System.Security.Cryptography.Asn1.Pkcs7
{
    ...
    [StructLayout(LayoutKind.Sequential)]
    internal ref partial struct ValueIssuerAndSerialNumberAsn
    {
        internal ReadOnlySpan<byte> Issuer;
        internal ReadOnlySpan<byte> SerialNumber;

        internal static void Decode(ReadOnlySpan<byte> encoded, AsnEncodingRules ruleSet, out ValueIssuerAndSerialNumberAsn decoded)
        {
            Decode(Asn1Tag.Sequence, encoded, ruleSet, out decoded);
        }

        internal static void Decode(Asn1Tag expectedTag, ReadOnlySpan<byte> encoded, AsnEncodingRules ruleSet, out ValueIssuerAndSerialNumberAsn decoded)
        {
            try
            {
        ...
        internal static void Decode(scoped ref ValueAsnReader reader, out ValueIssuerAndSerialNumberAsn decoded)
        {
            Decode(ref reader, Asn1Tag.Sequence, out decoded);
        }

        internal static void Decode(scoped ref ValueAsnReader reader, Asn1Tag expectedTag, out ValueIssuerAndSerialNumberAsn decoded)
        {
            try
            {
                DecodeCore(ref reader, expectedTag, out decoded);
        ...
        private static void DecodeCore(scoped ref ValueAsnReader reader, Asn1Tag expectedTag, out ValueIssuerAndSerialNumberAsn decoded)
        {
            decoded = default;
            ValueAsnReader sequenceReader = reader.ReadSequence(expectedTag);
