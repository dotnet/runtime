...
namespace System.Security.Cryptography.Asn1.Pkcs7
{
    ...
    [StructLayout(LayoutKind.Sequential)]
    internal ref partial struct ValueCertificateChoiceAsn
    {
        internal ReadOnlySpan<byte> Certificate;
        internal bool HasCertificate;
        internal ReadOnlySpan<byte> ExtendedCertificate;
        internal bool HasExtendedCertificate;
        internal ReadOnlySpan<byte> AttributeCertificateV1;
        internal bool HasAttributeCertificateV1;
        internal ReadOnlySpan<byte> AttributeCertificateV2;
        internal bool HasAttributeCertificateV2;
        internal ReadOnlySpan<byte> OtherCertificateFormat;
        internal bool HasOtherCertificateFormat;

        internal static void Decode(ReadOnlySpan<byte> encoded, AsnEncodingRules ruleSet, out ValueCertificateChoiceAsn decoded)
        {
            try
            {
        ...
        internal static void Decode(scoped ref ValueAsnReader reader, out ValueCertificateChoiceAsn decoded)
        {
            try
            {
                DecodeCore(ref reader, out decoded);
        ...
        private static void DecodeCore(scoped ref ValueAsnReader reader, out ValueCertificateChoiceAsn decoded)
        {
            decoded = default;
            Asn1Tag tag = reader.PeekTag();
            ...
            if (tag.HasSameClassAndValue(new Asn1Tag((UniversalTagNumber)16)))
            {
                if (!reader.PeekTag().HasSameClassAndValue(new Asn1Tag((UniversalTagNumber)16)))
                {
                    throw new CryptographicException();
                }

                decoded.Certificate = reader.ReadEncodedValue();
                decoded.HasCertificate = true;
            }
            else if (tag.HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 0)))
            {
                ...
                decoded.HasExtendedCertificate = true;
            }
            else if (tag.HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 1)))
            {
                if (!reader.PeekTag().HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 1)))
                {
                    throw new CryptographicException();
                }

                decoded.AttributeCertificateV1 = reader.ReadEncodedValue();
                decoded.HasAttributeCertificateV1 = true;
            }
            else if (tag.HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 2)))
            {
                if (!reader.PeekTag().HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 2)))
                {
                    throw new CryptographicException();
                }

                decoded.AttributeCertificateV2 = reader.ReadEncodedValue();
                decoded.HasAttributeCertificateV2 = true;
            }
            else if (tag.HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 3)))
            {
                decoded.OtherCertificateFormat = reader.ReadEncodedValue();
                decoded.HasOtherCertificateFormat = true;
            }
            else
            {
                throw new CryptographicException();
