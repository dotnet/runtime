...
namespace System.Security.Cryptography.Asn1.Pkcs12
{
    ...
    {
        internal int Version;
        internal ReadOnlySpan<byte> AuthSafe;
        internal ReadOnlySpan<byte> MacData;
        internal bool HasMacData;
        ...
        private static void DecodeCore(scoped ref ValueAsnReader reader, Asn1Tag expectedTag, out ValuePfxAsn decoded)
        {
            ...
            decoded.AuthSafe = sequenceReader.ReadEncodedValue();

            if (sequenceReader.HasData && sequenceReader.PeekTag().HasSameClassAndValue(Asn1Tag.Sequence))
            {
                decoded.MacData = sequenceReader.ReadEncodedValue();
