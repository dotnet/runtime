...
internal static partial class Interop
{
    internal static partial class Sys
    {
        ...
        // Pass singleIssuer: 1 to request IORING_SETUP_SINGLE_ISSUER: every subsequent
        // IoRingSubmit/IoRingKick/IoRingWaitForCompletions call for the returned ring must then come from
        // the exact same OS thread that called this method (not merely the first thread to call one of
        // those - confirmed empirically) for the ring's whole lifetime, including
        ...
        //
        // Pass deferTaskRun: 1 (only meaningful/allowed together with singleIssuer: 1 - silently ignored
        // otherwise) to additionally request IORING_SETUP_DEFER_TASKRUN. This is a caller-configurable
        // performance trade-off (see PortableThreadPool.IoUring.Unix.cs's DOTNET_IORING_SETUP_DEFER_TASKRUN
        // handling), not something with a single universally-best setting.
        [LibraryImport(Libraries.SystemNative, EntryPoint = "SystemNative_IoRingCreate", SetLastError = true)]
        internal static partial int IoRingCreate(int submissionQueueDepth, int completionQueueDepth, int singleIssuer, int deferTaskRun, out IntPtr ringHandle);
