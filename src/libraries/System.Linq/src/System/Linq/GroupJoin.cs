...
namespace System.Linq
{
    ...
    public static partial class Enumerable
    {
        ...
        /// <see cref="O:Enumerable.GroupJoin" /> produces hierarchical results, which means that elements from <paramref name="outer" /> are paired with collections of matching elements from <paramref name="inner" />. `GroupJoin` enables you to base your results on a whole set of matches for each element of <paramref name="outer" />.
        ...
        /// ]]></format>
        /// The <paramref name="resultSelector" /> function is called only one time for each <paramref name="outer" /> element together with a collection of all the <paramref name="inner" /> elements that match the <paramref name="outer" /> element. This differs from the <see cref="O:Enumerable.Join" /> method, in which the result selector function is invoked on pairs that contain one element from <paramref name="outer" /> and one element from <paramref name="inner" />.
        ...
        /// <see cref="O:Enumerable.GroupJoin" /> has no direct equivalent in traditional relational database terms. However, this method does implement a superset of inner joins and left outer joins. Both of these operations can be written in terms of a grouped join. See <a href="https://msdn.microsoft.com/library/442d176d-028c-4beb-8d22-407d4ef89107">Join Operations</a>.
        /// In query expression syntax, a `join … into` (Visual C#) or `Group Join` (Visual Basic) clause translates to an invocation of <see cref="O:Enumerable.GroupJoin" />.</remarks>
        /// <example>The following code example demonstrates how to use <see cref="GroupJoin{T1,T2,T3,T4}(IEnumerable{T1},IEnumerable{T2},Func{T1,T3},Func{T2,T3},Func{T1,IEnumerable{T2},T4})" /> to perform a grouped join on two sequences.
        ...
        /// <summary>Correlates the elements of two sequences based on key equality and groups the results. A specified <see cref="IEqualityComparer{T}" /> is used to compare keys.</summary>
        /// <typeparam name="TOuter">The type of the elements of the first sequence.</typeparam>
        /// <typeparam name="TInner">The type of the elements of the second sequence.</typeparam>
        /// <typeparam name="TKey">The type of the keys returned by the key selector functions.</typeparam>
        /// <typeparam name="TResult">The type of the result elements.</typeparam>
