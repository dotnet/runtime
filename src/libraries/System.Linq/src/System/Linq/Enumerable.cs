...
namespace System.Linq
{
    ...
    public static partial class Enumerable
    {
        ...
        /// <remarks>
        /// <para>The <see cref="AsEnumerable{T}(IEnumerable{T})" /> method has no effect other than to change the compile-time type of <paramref name="source" /> from a type that implements <see cref="IEnumerable{T}" /> to <see cref="IEnumerable{T}" /> itself.</para>
        /// <para><see cref="AsEnumerable{T}(IEnumerable{T})" /> can be used to choose between query implementations when a sequence implements <see cref="IEnumerable{T}" /> but also has a different set of public query methods available. For example, given a generic class `Table` that implements <see cref="IEnumerable{T}" /> and has its own methods such as `Where`, `Select`, and `SelectMany`, a call to `Where` would invoke the public `Where` method of `Table`. A `Table` type that represents a database table could have a `Where` method that takes the predicate argument as an expression tree and converts the tree to SQL for remote execution. If remote execution is not desired, for example because the predicate invokes a local method, the <see cref="O:Enumerable.AsEnumerable" /> method can be used to hide the custom methods and instead make the standard query operators available.</para>
        /// </remarks>
