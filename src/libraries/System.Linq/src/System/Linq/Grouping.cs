...
namespace System.Linq
{
    ...
    public static partial class Enumerable
    {
        ...
        /// <remarks>This method is implemented by using deferred execution. The immediate return value is an object that stores all the information that is required to perform the action. The query represented by this method is not executed until the object is enumerated either by calling its `GetEnumerator` method directly or by using `foreach` in Visual C# or `For Each` in Visual Basic.
        /// The <see cref="GroupBy{T1,T2,T3}(IEnumerable{T1},Func{T1,T2},Func{T1,T3})" /> method returns a collection of <see cref="IGrouping{T1,T2}" /> objects, one for each distinct key that was encountered. An <see cref="IGrouping{T1,T2}" /> is an <see cref="IEnumerable{T}" /> that also has a key associated with its elements.
    ...
    /// <remarks>An <see cref="IGrouping{T1,T2}" /> is an <see cref="IEnumerable{T}" /> that additionally has a key. The key represents the attribute that is common to each value in the <see cref="IGrouping{T1,T2}" />.
    /// The values of an <see cref="IGrouping{T1,T2}" /> are accessed much as the elements of an <see cref="IEnumerable{T}" /> are accessed. For example, you can access the values by using a `foreach` in Visual C# or `For Each` in Visual Basic loop to iterate through the <see cref="IGrouping{T1,T2}" /> object. The Example section contains a code example that shows you how to access both the key and the values of an <see cref="IGrouping{T1,T2}" /> object.
    /// The <see cref="IGrouping{T1,T2}" /> type is used by the <see cref="O:Enumerable.GroupBy" /> standard query operator methods, which return a sequence of elements of type <see cref="IGrouping{T1,T2}" />.</remarks>
