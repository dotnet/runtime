...
namespace System.Linq
{
    ...
    public static partial class Enumerable
    {
        ...
        /// <remarks>This method is implemented by using deferred execution. The immediate return value is an object that stores all the information that is required to perform the action. The query represented by this method is not executed until the object is enumerated either by calling its `GetEnumerator` method directly or by using `foreach` in Visual C# or `For Each` in Visual Basic.
        /// The <see cref="SelectMany{T1,T2,T3}(IEnumerable{T1},Func{T1,int,IEnumerable{T2}},Func{T1,T2,T3})" /> method is useful when you have to keep the elements of <paramref name="source" /> in scope for query logic that occurs after the call to <see cref="SelectMany{T1,T2,T3}(IEnumerable{T1},Func{T1,int,IEnumerable{T2}},Func{T1,T2,T3})" />. See the Example section for a code example. If there is a bidirectional relationship between objects of type <typeparamref name="TSource" /> and objects of type <typeparamref name="TCollection" />, that is, if an object of type <typeparamref name="TCollection" /> provides a property to retrieve the <typeparamref name="TSource" /> object that produced it, you do not need this overload of <see cref="SelectMany{T1,T2,T3}(IEnumerable{T1},Func{T1,int,IEnumerable{T2}},Func{T1,T2,T3})" />. Instead, you can use <see cref="SelectMany{T1,T2}(IEnumerable{T1},Func{T1,int,IEnumerable{T2}})" /> and navigate back to the <typeparamref name="TSource" /> object through the <typeparamref name="TCollection" /> object.</remarks>
        ...
        /// <exception cref="System.ArgumentNullException"><paramref name="source" /> or <paramref name="collectionSelector" /> or <paramref name="resultSelector" /> is <see langword="null" />.</exception>
        /// <remarks>This method is implemented by using deferred execution. The immediate return value is an object that stores all the information that is required to perform the action. The query represented by this method is not executed until the object is enumerated either by calling its `GetEnumerator` method directly or by using `foreach` in Visual C# or `For Each` in Visual Basic.
        /// The <see cref="SelectMany{T1,T2,T3}(IEnumerable{T1},Func{T1,IEnumerable{T2}},Func{T1,T2,T3})" /> method is useful when you have to keep the elements of <paramref name="source" /> in scope for query logic that occurs after the call to <see cref="SelectMany{T1,T2,T3}(IEnumerable{T1},Func{T1,IEnumerable{T2}},Func{T1,T2,T3})" />. See the Example section for a code example. If there is a bidirectional relationship between objects of type <typeparamref name="TSource" /> and objects of type <typeparamref name="TCollection" />, that is, if an object of type <typeparamref name="TCollection" /> provides a property to retrieve the <typeparamref name="TSource" /> object that produced it, you do not need this overload of <see cref="SelectMany{T1,T2,T3}(IEnumerable{T1},Func{T1,IEnumerable{T2}},Func{T1,T2,T3})" />. Instead, you can use <see cref="SelectMany{T1,T2}(IEnumerable{T1},Func{T1,IEnumerable{T2}})" /> and navigate back to the <typeparamref name="TSource" /> object through the <typeparamref name="TCollection" /> object.
        /// In query expression syntax, each `from` clause (Visual C#) or `From` clause (Visual Basic) after the initial one translates to an invocation of <see cref="O:Enumerable.SelectMany" />.</remarks>
