...
namespace System.Linq
{
    public static partial class Enumerable
    {
        ...
        /// <example>
        /// <para>
        /// The following code example demonstrates how to use <see cref="FullJoin{TOuter, TInner, TKey, TResult}(IEnumerable{TOuter}, IEnumerable{TInner}, Func{TOuter, TKey}, Func{TInner, TKey}, Func{TOuter, TInner, TResult})" /> to perform a full outer join of two sequences based on a common key.
        /// </para>
        /// <code>
        /// class Person
        /// {
        ...
        ///     Pet stray = new Pet { Name = "Stray", Owner = null };
        ///
        ...
        ///     // All people and all pets are included, even if unmatched.
        ///     var query =
        ///         people.FullJoin(pets,
        ///             person => person,
        ///             pet => pet.Owner,
        ///             (person, pet) =>
        ...
        /// <see cref="FullJoin{TOuter, TInner, TKey, TResult}(IEnumerable{TOuter}, IEnumerable{TInner}, Func{TOuter, TKey}, Func{TInner, TKey}, Func{TOuter, TInner, TResult})" />
        ...
        /// 'Full outer' means that elements of both the first and second sequences are returned regardless of whether matching elements are found in the other sequence.
        ...
        /// <example>
        /// <para>
        /// The following code example demonstrates how to use <see cref="FullJoin{TOuter, TInner, TKey, TResult}(IEnumerable{TOuter}, IEnumerable{TInner}, Func{TOuter, TKey}, Func{TInner, TKey}, Func{TOuter, TInner, TResult}, IEqualityComparer{TKey})" /> to perform a full outer join of two sequences based on a common key.
        /// </para>
        /// <code>
        /// class Person
        /// {
        ...
        /// <para>
        /// A join refers to the operation of correlating the elements of two sources of information based on a common key.
        /// <see cref="FullJoin{TOuter, TInner, TKey, TResult}(IEnumerable{TOuter}, IEnumerable{TInner}, Func{TOuter, TKey}, Func{TInner, TKey}, Func{TOuter, TInner, TResult}, IEqualityComparer{TKey})" />
        /// brings the two information sources and the keys by which they are matched together in one method call.
        /// </para>
        /// <para>
        ...
        private static IEnumerable<TResult> FullJoinIterator<TOuter, TInner, TKey, TResult>(IEnumerable<TOuter> outer, IEnumerable<TInner> inner, Func<TOuter, TKey> outerKeySelector, Func<TInner, TKey> innerKeySelector, Func<TOuter?, TInner?, TResult> resultSelector, IEqualityComparer<TKey>? comparer)
        {
            ...
            HashSet<Grouping<TKey, TInner>>? matchedGroupings = innerLookup.Count > 0 ? new HashSet<Grouping<TKey, TInner>>() : null;

            foreach (TOuter outerItem in outer)
            {
                Grouping<TKey, TInner>? g = innerLookup.GetGrouping(outerKeySelector(outerItem), create: false);
                if (g is null)
                {
                ...
                else
                {
                    ...
                    {
                        yield return resultSelector(outerItem, elements[i]);
                    }
                }
            }

            // Yield inner elements that had no matching outer element.
            Grouping<TKey, TInner>? lastGrouping = innerLookup._lastGrouping;
            if (lastGrouping is not null)
            {
                Grouping<TKey, TInner>? g = lastGrouping;
                do
                {
                    g = g._next!;
                    if (!matchedGroupings!.Contains(g))
                    {
                        int count = g._count;
                        TInner[] elements = g._elements;
                        for (int i = 0; i != count; ++i)
