...
namespace System.Linq
{
    public static partial class Enumerable
    {
        ...
        /// <para>The default equality comparer, <see cref="O:EqualityComparer{T}.Default" />, is used to hash and compare keys.</para>
        /// <para>A join refers to the operation of correlating the elements of two sources of information based on a common key. <see cref="O:Enumerable.Join" /> brings the two information sources and the keys by which they are matched together in one method call. This differs from the use of `SelectMany`, which requires more than one method call to perform the same operation.</para>
        /// <para><see cref="O:Enumerable.Join" /> preserves the order of the elements of <paramref name="outer" />, and for each of these elements, the order of the matching elements of <paramref name="inner" />.</para>
        /// <para>In query expression syntax, a `join` (Visual C#) or `Join` (Visual Basic) clause translates to an invocation of <see cref="O:Enumerable.Join" />.</para>
        /// <para>In relational database terms, the <see cref="O:Enumerable.Join" /> method implements an inner equijoin. 'Inner' means that only elements that have a match in the other sequence are included in the results. An 'equijoin' is a join in which the keys are compared for equality. A left outer join operation has no dedicated standard query operator, but can be performed by using the <see cref="O:Enumerable.GroupJoin" /> method. See <a href="https://msdn.microsoft.com/library/442d176d-028c-4beb-8d22-407d4ef89107">Join Operations</a>.</para>
        /// </remarks>
        ...
        /// <remarks>
        /// <para>This method is implemented by using deferred execution. The immediate return value is an object that stores all the information that is required to perform the action. The query represented by this method is not executed until the object is enumerated either by calling its `GetEnumerator` method directly or by using `foreach` in Visual C# or `For Each` in Visual Basic.</para>
        /// <para>If <paramref name="comparer" /> is <see langword="null" />, the default equality comparer, <see cref="O:EqualityComparer{T}.Default" />, is used to hash and compare keys.</para>
        /// <para>A join refers to the operation of correlating the elements of two sources of information based on a common key. <see cref="O:Enumerable.Join" /> brings the two information sources and the keys by which they are matched together in one method call. This differs from the use of `SelectMany`, which requires more than one method call to perform the same operation.</para>
        /// <para><see cref="O:Enumerable.Join" /> preserves the order of the elements of <paramref name="outer" />, and for each of these elements, the order of the matching elements of <paramref name="inner" />.</para>
        /// <para>In relational database terms, the <see cref="O:Enumerable.Join" /> method implements an inner equijoin. 'Inner' means that only elements that have a match in the other sequence are included in the results. An 'equijoin' is a join in which the keys are compared for equality. A left outer join operation has no dedicated standard query operator, but can be performed by using the <see cref="O:Enumerable.GroupJoin" /> method. See <a href="https://msdn.microsoft.com/library/442d176d-028c-4beb-8d22-407d4ef89107">Join Operations</a>.</para>
        /// </remarks>
