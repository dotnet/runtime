...
namespace System.Linq
{
    ...
    public static partial class Enumerable
    {
        ...
        /// The <see cref="OfType{T}(System.Collections.IEnumerable)" /> method returns only those elements in <paramref name="source" /> that can be cast to type <typeparamref name="TResult" />. To instead receive an exception if an element cannot be cast to type <typeparamref name="TResult" />, use <see cref="Cast{T}(System.Collections.IEnumerable)" />.
        /// This method is one of the few standard query operator methods that can be applied to a collection that has a non-parameterized type, such as an <see cref="System.Collections.ArrayList" />. This is because <see cref="OfType" /> extends the type <see cref="System.Collections.IEnumerable" />. <see cref="OfType" /> cannot only be applied to collections that are based on the parameterized <see cref="IEnumerable{T}" /> type, but collections that are based on the non-parameterized <see cref="System.Collections.IEnumerable" /> type also.
        /// By applying <see cref="OfType" /> to a collection that implements <see cref="System.Collections.IEnumerable" />, you gain the ability to query the collection by using the standard query operators. For example, specifying a type argument of <see cref="object" /> to <see cref="OfType" /> would return an object of type `IEnumerable&lt;Object&gt;` in C# or `IEnumerable(Of Object)` in Visual Basic, to which the standard query operators can be applied.</remarks>
        /// <example>The following code example demonstrates how to use <see cref="OfType" /> to filter the elements of an <see cref="System.Collections.IEnumerable" />.
        ...
        /// <remarks>This method is implemented by using deferred execution. The immediate return value is an object that stores all the information that is required to perform the action. The query represented by this method is not executed until the object is enumerated either by calling its `GetEnumerator` method directly or by using `foreach` in Visual C# or `For Each` in Visual Basic.
        /// The <see cref="Cast{T}(System.Collections.IEnumerable)" /> method enables the standard query operators to be invoked on non-generic collections by supplying the necessary type information. For example, <see cref="System.Collections.ArrayList" /> does not implement <see cref="IEnumerable{T}" />, but by calling <see cref="Cast{T}(System.Collections.IEnumerable)" /> on the <see cref="System.Collections.ArrayList" /> object, the standard query operators can then be used to query the sequence.
        ...
        /// To obtain only those elements that can be converted to type <typeparamref name="TResult" />, use the <see cref="OfType" /> method instead of <see cref="Cast{T}(System.Collections.IEnumerable)" />.
        /// In a query expression, an explicitly typed iteration variable translates to an invocation of <see cref="Cast{T}(System.Collections.IEnumerable)" />. This example shows the syntax for an explicitly typed range variable.
        ...
        /// </code>
        /// </remarks>
        /// <example>The following code example demonstrates how to use <see cref="Cast{T}(System.Collections.IEnumerable)" /> to enable the use of the standard query operators on an <see cref="System.Collections.ArrayList" />.
