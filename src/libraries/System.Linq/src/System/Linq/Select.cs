...
namespace System.Linq
{
    ...
    public static partial class Enumerable
    {
        ...
        /// <returns>An <see cref="System.Collections.Generic.IEnumerable{T}" /> whose elements are the result of invoking the transform function on each element of <paramref name="source" />.</returns>
        ...
        /// This projection method requires the transform function, <paramref name="selector" />, to produce one value for each value in the source sequence, <paramref name="source" />. If <paramref name="selector" /> returns a value that is itself a collection, it is up to the consumer to traverse the subsequences manually. In such a situation, it might be better for your query to return a single coalesced sequence of values. To achieve this, use the <see cref="O:System.Linq.Enumerable.SelectMany" /> method instead of <see cref="O:System.Linq.Enumerable.Select" />. Although `SelectMany` works similarly to `Select`, it differs in that the transform function returns a collection that is then expanded by `SelectMany` before it is returned.
        /// In query expression syntax, a `select` (Visual C#) or `Select` (Visual Basic) clause translates to an invocation of <see cref="O:System.Linq.Enumerable.Select" />.</remarks>
        /// <example>The following code example demonstrates how to use <see cref="System.Linq.Enumerable.Select{T1,T2}(System.Collections.Generic.IEnumerable{T1},System.Func{T1,T2})" /> to project over a sequence of values.
        ...
        /// This projection method requires the transform function, <paramref name="selector" />, to produce one value for each value in the source sequence, <paramref name="source" />. If <paramref name="selector" /> returns a value that is itself a collection, it is up to the consumer to traverse the subsequences manually. In such a situation, it might be better for your query to return a single coalesced sequence of values. To achieve this, use the <see cref="O:System.Linq.Enumerable.SelectMany" /> method instead of <see cref="O:System.Linq.Enumerable.Select" />. Although `SelectMany` works similarly to `Select`, it differs in that the transform function returns a collection that is then expanded by `SelectMany` before it is returned.</remarks>
        /// <example>The following code example demonstrates how to use <see cref="System.Linq.Enumerable.Select{T1,T2}(System.Collections.Generic.IEnumerable{T1},System.Func{T1,int,T2})" /> to project over a sequence of values and use the index of each element.
