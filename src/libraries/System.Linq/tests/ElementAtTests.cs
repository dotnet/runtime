    ...
    public class ElementAtTests : EnumerableTests
    {
        ...
        public void SameResultsRepeatCallsIntQuery()
        {
            var q1 = from x in new[] { 9999, 0, 888, -1, 66, -777, 1, 2, -12345 }
                    ...
                    select x;
            Assert.Equal(q1.ElementAt(3), q1.ElementAt(3));

            var q2 = from x in new[] { 9999, 0, 888, -1, 66, -777, 1, 2, -12345 }
                where x > int.MinValue
                select x;
            Assert.Equal(q2.ElementAt(new Index(3)), q2.ElementAt(new Index(3)));

            var q3 = from x in new[] { 9999, 0, 888, -1, 66, -777, 1, 2, -12345 }
                where x > int.MinValue
                select x;
            Assert.Equal(q3.ElementAt(^6), q3.ElementAt(^6));
        }
        ...
        public void SameResultsRepeatCallsStringQuery()
        {
            var q1 = from x in new[] { "!@#$%^", "C", "AAA", "", "Calling Twice", "SoS", string.Empty }
                    ...
                    select x;
            Assert.Equal(q1.ElementAt(4), q1.ElementAt(4));

            var q2 = from x in new[] { "!@#$%^", "C", "AAA", "", "Calling Twice", "SoS", string.Empty }
                where !string.IsNullOrEmpty(x)
                select x;
            Assert.Equal(q2.ElementAt(new Index(4)), q2.ElementAt(new Index(4)));

            var q3 = from x in new[] { "!@#$%^", "C", "AAA", "", "Calling Twice", "SoS", string.Empty }
                where !string.IsNullOrEmpty(x)
                select x;
            Assert.Equal(q3.ElementAt(^2), q3.ElementAt(^2));
        }
