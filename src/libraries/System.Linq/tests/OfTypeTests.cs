...
namespace System.Linq.Tests
{
    public class OfTypeTests : EnumerableTests
    {
        ...
        {
            IEnumerable<object> objects = ["1", null, "22", null, 3, 4, "55555"];
            Assert.Equal([1, 2, 5], objects.OfType<string>().Select(s => s.Length));

            Assert.Equal([1, 2, 3, 4, 5], new int[] { 1, 2, 3, 4, 5 }.OfType<object>().Select(o => (int)o));
        }
