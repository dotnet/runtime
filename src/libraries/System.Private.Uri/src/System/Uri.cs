...
namespace System
{
    ...
    public partial class Uri : ISerializable
    {
        ...
        {
            if (name is null || name.Length == 0 || name.Length > short.MaxValue)
            {
                return UriHostNameType.Unknown;
            }
            int end = name.Length;
        ...
        private unsafe void GetHostViaCustomSyntax()
        {
            ...
            int port = 0;
            if (portStr is null || portStr.Length == 0)
            {
                // It's like no port
            ...
            ref ParsingError err, ref Flags flags, UriParser syntax, ref string? newHost)
        {
           ...
           DomainNameHelper.IsValid(pString, start, ref end, ref dnsNotCanonical, StaticNotAny(flags, Flags.ImplicitFile)))
            ...
            {
                CheckAuthorityHelperHandleDnsIri(pString, start, end, hasUnicode, ref flags, ref justNormalized, ref newHost, ref err);
            }
