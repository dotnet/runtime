...
namespace DotnetFuzzing
{
    internal sealed class PooledBoundedMemory<T> : IDisposable where T : unmanaged
    {
        ...
        {
            if (_pool is null ||
                Interlocked.CompareExchange(ref _pool[_memory.Length], this, null) != null)
            {
                _memory.Dispose();
            }
        }
        ...
        public static PooledBoundedMemory<T> Rent(int elementCount, PoisonPagePlacement placement)
        {
            ...
            PooledBoundedMemory<T>[] pool = null;
            switch (placement)
            {
                case PoisonPagePlacement.Before:
                    pool = s_memoryWithPoisonBefore;
                    break;
                case PoisonPagePlacement.After:
                    pool = s_memoryWithPoisonAfter;
                    break;
                default:
