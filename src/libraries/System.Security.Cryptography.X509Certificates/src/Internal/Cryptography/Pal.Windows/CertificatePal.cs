...
namespace Internal.Cryptography.Pal
{
    internal sealed partial class CertificatePal : IDisposable, ICertificatePal
    {
        ...
        public static ICertificatePal FromHandle(IntPtr handle)
        {
            ...
            SafeCertContextHandle safeCertContextHandle = CertDuplicateCertificateContext(handle);
            if (safeCertContextHandle.IsInvalid)
                throw ErrorCode.HRESULT_INVALID_HANDLE.ToCryptographicException();

            DATA_BLOB dataBlob;
            int cbData = 0;
            bool deleteKeyContainer = CertGetCertificateContextProperty(safeCertContextHandle, CertContextPropId.CERT_CLR_DELETE_KEY_PROP_ID, out dataBlob, ref cbData);
        ...
        public byte[] Thumbprint
        {
            ...
            {
                int cbData = 0;
                if (!CertGetCertificateContextProperty(_certContext, CertContextPropId.CERT_SHA1_HASH_PROP_ID, null, ref cbData))
                    throw Marshal.GetHRForLastWin32Error().ToCryptographicException();

                byte[] thumbprint = new byte[cbData];
                if (!CertGetCertificateContextProperty(_certContext, CertContextPropId.CERT_SHA1_HASH_PROP_ID, thumbprint, ref cbData))
                    throw Marshal.GetHRForLastWin32Error().ToCryptographicException();
                return thumbprint;
            }
        }
            ...
            get
            {
                unsafe
                {
                    ...
                    if (keyAlgorithmOid == Oids.Rsa)
                        ...
                        algId = FindOidInfo(CryptOidInfoKeyType.CRYPT_OID_INFO_OID_KEY, keyAlgorithmOid, OidGroup.PublicKeyAlgorithm, fallBackToAllGroups: true).AlgId;
        ...
        private byte[] PropagateKeyAlgorithmParametersFromChain()
        {
            unsafe
            {
                ...
                {
                    int cbData = 0;
                    if (!CertGetCertificateContextProperty(_certContext, CertContextPropId.CERT_PUBKEY_ALG_PARA_PROP_ID, null, ref cbData))
                    {
                    ...
                    if (!CertGetCertificateContextProperty(_certContext, CertContextPropId.CERT_PUBKEY_ALG_PARA_PROP_ID, keyAlgorithmParameters, ref cbData))
                        throw Marshal.GetHRForLastWin32Error().ToCryptographicException();
        ...
        public bool Archived
        {
            ...
            {
                int uninteresting = 0;
                bool archivePropertyExists = CertGetCertificateContextProperty(_certContext, CertContextPropId.CERT_ARCHIVED_PROP_ID, null!, ref uninteresting);
            ...
            set
            {
                unsafe
                {
                    DATA_BLOB blob = new DATA_BLOB(IntPtr.Zero, 0);
                    DATA_BLOB* pValue = value ? &blob : (DATA_BLOB*)null;
                    if (!CertSetCertificateContextProperty(_certContext, CertContextPropId.CERT_ARCHIVED_PROP_ID, CertSetPropertyFlags.None, pValue))
        ...
        public string FriendlyName
        {
            get
            {
                ...
                {
                    int cbData = 0;
                    if (!CertGetCertificateContextPropertyString(_certContext, CertContextPropId.CERT_FRIENDLY_NAME_PROP_ID, null, ref cbData))
                        return string.Empty;
                    ...
                    {
                        if (!CertGetCertificateContextPropertyString(_certContext, CertContextPropId.CERT_FRIENDLY_NAME_PROP_ID, (byte*)ptr, ref cbData))
                            return string.Empty;
                    }
            ...
            set
            {
                ...
                unsafe
                {
                    ...
                    try
                    {
                        DATA_BLOB blob = new DATA_BLOB(pFriendlyName, checked(2 * ((uint)friendlyName.Length + 1)));
                        if (!CertSetCertificateContextProperty(_certContext, CertContextPropId.CERT_FRIENDLY_NAME_PROP_ID, CertSetPropertyFlags.None, &blob))
                            throw Marshal.GetLastWin32Error().ToCryptographicException();
                    }
                    finally
                    {
        ...
        internal SafeCertContextHandle CertContext
        {
            get
            {
                SafeCertContextHandle certContext = CertDuplicateCertificateContext(_certContext.DangerousGetHandle());
        ...
        private CertificatePal(SafeCertContextHandle certContext, bool deleteKeyContainer)
        {
            if (deleteKeyContainer)
            {
                ...
                certContext = CertDuplicateCertificateContextWithKeyContainerDeletion(oldCertContext.DangerousGetHandle());
