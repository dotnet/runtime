...
namespace System.Linq.Tests
{
    public class RightJoinTests : AsyncEnumerableTests
    {
        ...
        [Fact]
        public void TupleRightJoin_InvalidInputs_Throws()
        {
        ...
        [Fact]
        public void TupleRightJoin_Empty_ProducesEmpty()
        {
            IAsyncEnumerable<string> empty = AsyncEnumerable.Empty<string>();
            IAsyncEnumerable<string> nonEmpty = CreateSource("1", "2", "3");

            Assert.Same(AsyncEnumerable.Empty<(string, string)>(), empty.RightJoin(empty, s => s, s => s));
            Assert.Same(AsyncEnumerable.Empty<(string, string)>(), empty.RightJoin(empty, async (s, ct) => s, async (s, ct) => s));

            Assert.Same(AsyncEnumerable.Empty<(string, string)>(), nonEmpty.RightJoin(empty, s => s, s => s));
            Assert.Same(AsyncEnumerable.Empty<(string, string)>(), nonEmpty.RightJoin(empty, async (s, ct) => s, async (s, ct) => s));

            Assert.NotSame(AsyncEnumerable.Empty<(string, string)>(), empty.RightJoin(nonEmpty, s => s, s => s));
            Assert.NotSame(AsyncEnumerable.Empty<(string, string)>(), empty.RightJoin(nonEmpty, async (s, ct) => s, async (s, ct) => s));
        }

#if NET
        [Fact]
        public async Task TupleRightJoin_VariousValues_MatchesEnumerable_String()
        {
            Random rand = new(42);
            foreach (int length in new[] { 0, 1, 2, 1000 })
            {
                ...
                {
                    await AssertEqual(
                        values.RightJoin(values, s => s.Length > 0 ? s[0] : ' ', s => s.Length > 1 ? s[1] : ' ').Select(t => t.Outer + t.Inner),
                        source.RightJoin(source, s => s.Length > 0 ? s[0] : ' ', s => s.Length > 1 ? s[1] : ' ').Select(t => t.Outer + t.Inner));

                    await AssertEqual(
                        values.RightJoin(values, s => s.Length > 0 ? s[0] : ' ', s => s.Length > 1 ? s[1] : ' ').Select(t => t.Outer + t.Inner),
                        source.RightJoin(source, async (s, ct) => s.Length > 0 ? s[0] : ' ', async (s, ct) => s.Length > 1 ? s[1] : ' ').Select(t => t.Outer + t.Inner));
                }
            }
        }
#endif
        ...
        [Fact]
        public async Task TupleRightJoin_InterfaceCalls_ExpectedCounts()
        {
            TrackingAsyncEnumerable<int> outer, inner;
