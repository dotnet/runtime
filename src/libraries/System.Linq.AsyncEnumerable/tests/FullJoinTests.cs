...
{
    public class FullJoinTests : AsyncEnumerableTests
    {
        [Fact]
        ...
        public async Task VariousValues_MatchesEnumerable_String()
        {
            ...
            foreach (int length in new[] { 0, 1, 2, 1000 })
            {
                ...
                {
                    await AssertEqual(
                        values.FullJoin(values, s => s.Length > 0 ? s[0] : ' ', s => s.Length > 1 ? s[1] : ' ', (outer, inner) => outer + inner),
                        source.FullJoin(source, s => s.Length > 0 ? s[0] : ' ', s => s.Length > 1 ? s[1] : ' ', (outer, inner) => outer + inner));

                    await AssertEqual(
                        values.FullJoin(values, s => s.Length > 0 ? s[0] : ' ', s => s.Length > 1 ? s[1] : ' ', (outer, inner) => outer + inner),
                        source.FullJoin(source, async (s, ct) => s.Length > 0 ? s[0] : ' ', async (s, ct) => s.Length > 1 ? s[1] : ' ', async (outer, inner, ct) => outer + inner));
                }
            }
        }
#endif
        ...
        public async Task Cancellation_Cancels()
        {
            ...
            await Assert.ThrowsAsync<OperationCanceledException>(async () =>
            {
                ...
                await ConsumeAsync(source.FullJoin(source, outer =>
                {
                    cts.Cancel();
                    return outer;
                },
            ...
            {
                CancellationTokenSource cts = new();
                await ConsumeAsync(source.FullJoin(source,
                async (outer, ct) =>
                {
        ...
        public async Task InterfaceCalls_ExpectedCounts()
        {
            ...
            await ConsumeAsync(outer.FullJoin(inner, outer => outer, inner => inner, (outer, inner) => outer + inner));
            ...
            outer = CreateSource(2, 4, 8, 16).Track();
            inner = CreateSource(1, 2, 3, 4).Track();
            await ConsumeAsync(outer.FullJoin(inner, async (outer, ct) => outer, async (inner, ct) => inner, async (outer, inner, ct) => outer + inner));
            Assert.Equal(5, outer.MoveNextAsyncCount);
            Assert.Equal(4, outer.CurrentCount);
            Assert.Equal(1, outer.DisposeAsyncCount);
            Assert.Equal(5, inner.MoveNextAsyncCount);
