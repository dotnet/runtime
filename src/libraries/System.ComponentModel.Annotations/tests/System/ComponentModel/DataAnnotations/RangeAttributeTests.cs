...
namespace System.ComponentModel.DataAnnotations.Tests
{
    public class RangeAttributeTests : ValidationAttributeTestBase
    {
        ...
        [Fact]
        public static void FormatMessage_UsesSuppliedTemplateAndConvertedBounds()
        {
            const string ExternalMessageString = "external {0}:{1:D2}:{2:D2}";
            const string ErrorMessageString = "internal {0}:{1:D2}:{2:D2}";
            var attribute = new RangeAttribute(typeof(int), "1", "3") { ErrorMessage = ErrorMessageString };

            Assert.Equal("external name:01:03", attribute.FormatMessage(ExternalMessageString, "name"));
