...
namespace Microsoft.Extensions.Configuration
{
    ...
    public static class ConfigurationBinder
    {
            ...
            if (!TryBindAsConfigurationSection(type, bindingPoint, config))
            {
                if (!TryBindAsScalarValue(type, bindingPoint, config))
                {
                    if (config != null)
                    {
                        ...
                        {
                            if (!TryBindAsArray(type, bindingPoint, config, options))
                            {
        ...
        [RequiresDynamicCode(DynamicCodeWarningMessage)]
        [RequiresUnreferencedCode(TrimmingWarningMessage)]
        private static bool TryBindAsArray(
            Type type,
            BindingPoint bindingPoint,
            IConfiguration config,
            BinderOptions options)
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static bool TryBindAsConfigurationSection(
            Type type,
            BindingPoint bindingPoint,
            IConfiguration config)
        {
        ...
        [RequiresDynamicCode(DynamicCodeWarningMessage)]
        [RequiresUnreferencedCode(TrimmingWarningMessage)]
        private static bool TryBindAsSet(
            Type type,
            BindingPoint bindingPoint,
            IConfiguration config,
            BinderOptions options)
