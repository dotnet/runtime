    ...
    public abstract class Http1ResponseStreamZeroByteReadTestBase
    {
        ...
        public async Task ZeroByteRead_IssuesZeroByteReadOnUnderlyingStream(StreamConformanceTests.ReadWriteMode readMode, bool useSsl)
        {
            ...
            try
            {
                ...
                if (useSsl)
                {
                    ...
                    using (X509Certificate2 cert = Test.Common.Configuration.Certificates.GetServerCertificate())
                    {
                        await ((SslStream)server).AuthenticateAsServerAsync(
                            ...
                            checkCertificateRevocation: false).WaitAsync(TimeSpan.FromSeconds(TestHelper.PassingTestTimeoutMilliseconds));
                    }
                }

                await ResponseConnectedStreamConformanceTests.ReadHeadersAsync(server).WaitAsync(TimeSpan.FromSeconds(TestHelper.PassingTestTimeoutMilliseconds));
                await server.WriteAsync(Encoding.ASCII.GetBytes(GetResponseHeaders()));

                using HttpResponseMessage response = await clientTask.WaitAsync(TimeSpan.FromSeconds(TestHelper.PassingTestTimeoutMilliseconds));
                ...
                await sawZeroByteRead.Task.WaitAsync(TimeSpan.FromSeconds(TestHelper.PassingTestTimeoutMilliseconds));
                Assert.False(zeroByteReadTask.IsCompleted);
                ...
                Assert.Equal(0, await zeroByteReadTask.WaitAsync(TimeSpan.FromSeconds(TestHelper.PassingTestTimeoutMilliseconds)));
                ...
                {
                    read += await StreamConformanceTests.ReadAsync(readMode, clientStream, readBuffer, read, readBuffer.Length - read, CancellationToken.None).WaitAsync(TimeSpan.FromSeconds(TestHelper.PassingTestTimeoutMilliseconds));
                }
