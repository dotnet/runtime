...
namespace System.Net.Http.Functional.Tests
{
    ...
    public abstract class SocketsHttpHandler_ConnectionPoolPartitioning_Test : HttpClientHandlerTestBase
    {
        ...
        public async Task DoNotPartitionConnectionPoolBySni_SharesConnectionAcrossSslHostNames(bool doNotPartitionBySni)
        {
            ...
            await LoopbackServerFactory.CreateClientAndServerAsync(
                async uri =>
                {
                    ...
                    {
                        SetDoNotPartitionConnectionPoolBySni(request1);
                        SetDoNotPartitionConnectionPoolBySni(request2);
                    }
        ...
        private static void SetDoNotPartitionConnectionPoolBySni(HttpRequestMessage request)
        {
            // Mirrors HttpRequestMessage.ExperimentalDoNotPartitionConnectionPoolBySni (value 16).
            GetSendStatus(request) |= 16;

            [UnsafeAccessor(UnsafeAccessorKind.Field, Name = "_sendStatus")]
            static extern ref int GetSendStatus(HttpRequestMessage request);
        }
    }
