    ...
    public sealed class SocketsHttpHandlerTest_Http2 : HttpClientHandlerTest_Http2
    {
        ...
        public async Task Http2_MultipleConnectionsEnabled_MaxConcurrentStreamsIncreasedAfterLimitIsReached_WaitingRequestsUnblocked()
        {
            ...
            using (HttpClient client = CreateHttpClient(handler))
            {
                ...
                List<int> remainingStreams = await AcceptRequests(connections[0], TotalRequestCount - acceptedStreamCount).ConfigureAwait(false);

                Assert.Equal(TotalRequestCount, acceptedStreamCount + remainingStreams.Count);

                await Task.WhenAll(
                    SendResponses(connections[0], acceptedStreams[0].Concat(remainingStreams)),
                    SendResponses(connections[1], acceptedStreams[1]))
        ...
        public async Task Http2_MultipleConnectionsEnabled_InfiniteRequestsCompletelyBlockOneConnection_AllRemaningRequestsHandledBySecondConnection()
        {
            ...
            using (HttpClient client = CreateHttpClient(handler))
            {
                ...
                Assert.Equal(TotalRequestCount - MaxConcurrentStreams, handledRequestCount);
        ...
        private static async Task<Http2LoopbackConnection> GetConnection(Http2LoopbackServer server, uint maxConcurrentStreams) =>
            await server.EstablishConnectionAsync(TimeSpan.FromSeconds(5), new SettingsEntry { SettingId = SettingId.MaxConcurrentStreams, Value = maxConcurrentStreams }).ConfigureAwait(false);
