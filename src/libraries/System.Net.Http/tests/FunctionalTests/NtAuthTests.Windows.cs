...
namespace System.Net.Http.Functional.Tests
{
    ...
    public class InpersonificatedNtAuthTests : IClassFixture<WindowsIdentityFixture>
    {
        public static bool CanRunInpersonificatedTests = PlatformDetection.IsWindows && PlatformDetection.IsNotWindowsNanoServer;
        private readonly WindowsIdentityFixture _fixture;
        private readonly ITestOutputHelper _output;

        public InpersonificatedNtAuthTests(WindowsIdentityFixture windowsIdentityFixture, ITestOutputHelper output)
        {
            _output = output;
        ...
        [OuterLoop]
        [ConditionalTheory(nameof(CanRunInpersonificatedTests))]
        [InlineData(true)]
        [InlineData(false)]
        [PlatformSpecific(TestPlatforms.Windows)]
        public async Task DefaultHandler_InpersonificatedUser_Success(bool useNtlm)
        {
            await LoopbackServer.CreateClientAndServerAsync(
                async uri =>
                {
