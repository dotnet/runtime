...
{
    static class CborReaderHelpers
    {
        public static void VerifyValue(CborReader reader, object expectedValue)
        {
            switch (expectedValue)
            {
                ...
                case byte[] expected:
                    ...
                    break;
                case object[] nested when CborWriterHelpers.IsCborMapRepresentation(nested):
        ...
        {
            if(!CborWriterHelpers.IsCborMapRepresentation(expectedValues))
            {
                throw new ArgumentException($"cbor map expected values missing '{CborWriterHelpers.MapPrefixIdentifier}' prefix.");
            }
