                ...
                foreach (string includeURI in new string[] { "", $@" URI=""""", $@" URI="""" URI=""""" })
                    foreach (string includeType in new string[] { "", $@" Type=""""", $@" Type="""" Type=""""" })
                    {
                        ...
                        if (throwsXmlException)
                            Assert.Throws<XmlException>(() => Helpers.VerifyCryptoExceptionOnLoad(xml, loadXmlThrows: !SignatureSupport.SupportsRsaSha1Signatures));
                        else
                        {
                            bool validSignature = includeID == "" && includeURI != "" && includeType == "";
                            Helpers.VerifyCryptoExceptionOnLoad(xml, loadXmlThrows: !SignatureSupport.SupportsRsaSha1Signatures, validSignature);
                        }
                    }
        }
