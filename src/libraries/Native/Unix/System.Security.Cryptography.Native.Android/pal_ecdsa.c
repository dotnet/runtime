...
{
    // The maximum size of a signature for the provided key is 2* bitlength of the order + extra bytes for the DER encoding.
    // The DER encoding is as follows (with R and S being the components of the signature and all lengths being one byte width):
    ...
    // As the ECDSA algorithm is defined, the maximum length of R and S each is the bitlength of the order, so as a result
    // we get the maximum size as 2 * bitlength of the order + 6.
