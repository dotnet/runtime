...
namespace System.Net.Sockets.Tests
{
    ...
    public partial class IoUring
    {
        ...
        private static async Task<bool> WaitForIoUringTelemetryCounterAtLeastAsync(string counterFieldName, ulong targetValue, int timeoutMilliseconds = 30000)
        {
        ...
        private static async Task WithIoUringNativeDiagnosticsSnapshotDeltaAsync(
            Func<Task> scenario,
            Action<IoUringNativeDiagnosticsSnapshot, IoUringNativeDiagnosticsSnapshot> validateDelta,
        ...
        private static async Task RunSlotCapacityStressScenarioAsync(int connectionCount)
        {
            ...
            ulong highWaterBefore = GetIoUringTelemetryCounterValue("_ioUringCompletionSlotHighWaterMark");
            await RunMultishotAcceptHighConnectionRateScenarioAsync(connectionCount);
            ulong highWaterAfter = GetIoUringTelemetryCounterValue("_ioUringCompletionSlotHighWaterMark");

            Assert.True(highWaterAfter >= highWaterBefore, "Completion-slot high-water mark should be monotonic.");
            Assert.True(highWaterAfter >= 4000, $"Expected completion-slot high-water mark to reach >= 4000; observed {highWaterAfter}.");
        }
        ...
        private static async Task RunBoundedWaitBufferPressureScenarioAsync(int connectionCount)
        {
            await WithIoUringNativeDiagnosticsSnapshotDeltaAsync(
                async () =>
                {
                    ...
                    var clients = new List<Socket>(connectionCount);
                    var servers = new List<Socket>(connectionCount);
                    var receiveBuffers = new List<byte[]>(connectionCount);
                    var receiveTasks = new List<Task<int>>(connectionCount);
                    var sendTasks = new List<Task<int>>(connectionCount);
        ...
        private static async Task RunCqOverflowRecoveryScenarioAsync()
        {
            ...
            int baselineCompletionSlotsInUse = GetIoUringCompletionSlotsInUseForTest();
            int baselineTrackedOperations = GetIoUringTrackedOperationCountForTest();
            ulong overflowBefore = GetIoUringTelemetryCounterValue("_ioUringCqOverflow");
            ulong recoveryBefore = GetIoUringTelemetryCounterValue("_ioUringCqOverflowRecoveries");
            ...
            Assert.True(
                await WaitForIoUringTelemetryCounterAtLeastAsync("_ioUringCqOverflowRecoveries", recoveryBefore + 1),
                "Expected io_uring CQ overflow recovery counter to increment after injected overflow.");
            Assert.True(
                GetIoUringTelemetryCounterValue("_ioUringCqOverflow") > overflowBefore,
                "Expected io_uring CQ overflow counter to increment after injected overflow.");
            Assert.True(
        ...
        private static async Task RunCqOverflowRecoveryWithZeroTrackedOperationsScenarioAsync()
        {
            ...
            Assert.True(
                await WaitForIoUringTelemetryCounterAtLeastAsync("_ioUringCqOverflowRecoveries", recoveryBefore + 1),
                "Expected io_uring CQ overflow recovery counter to increment for zero-tracked-operations recovery.");
            Assert.True(
                await WaitForIoUringTrackedOperationsAtMostAsync(0),
        ...
        private static async Task RunCqOverflowRecoveryWithSmallRingScenarioAsync()
        {
            ...
            for (int round = 0; round < 6; round++)
            {
                ...
                ulong overflowAfterRound = GetIoUringTelemetryCounterValue("_ioUringCqOverflow");
                if (overflowAfterRound > overflowBefore)
                {
                    Assert.True(
                        await WaitForIoUringTelemetryCounterAtLeastAsync("_ioUringCqOverflowRecoveries", recoveryBefore + 1),
                        "Expected CQ overflow recovery counter to increment after real CQ overflow.");
                    Assert.True(
                        await WaitForIoUringCompletionSlotsInUseAtMostAsync(baselineCompletionSlotsInUse + 2, timeoutMilliseconds: 15000),
                        "Completion-slot usage did not settle after real CQ overflow recovery.");
                    Assert.True(
                        await WaitForIoUringTrackedOperationsAtMostAsync(baselineTrackedOperations + 2, timeoutMilliseconds: 15000),
                        "Tracked-operation count did not settle after real CQ overflow recovery.");
                    return;
                }
            }

            // Non-deterministic fallback: on very fast machines the ring may drain without overflow.
            Assert.True(
                GetIoUringTelemetryCounterValue("_ioUringCqOverflow") >= overflowBefore,
                "Real CQ overflow was not observed; counter should still be nondecreasing.");
        }
        ...
        private static async Task RunMultishotAcceptOverflowArmingScenarioAsync()
        {
            ...
            Assert.True(
                await WaitForIoUringTelemetryCounterAtLeastAsync("_ioUringCqOverflowRecoveries", recoveryBefore + 1),
                "Expected CQ overflow recovery counter to increment after multishot-accept overflow scenario.");
            Assert.True(
        ...
        private static async Task RunSustainedOverflowReentrancyScenarioAsync()
        {
            ...
            Assert.True(
                await WaitForIoUringTelemetryCounterAtLeastAsync("_ioUringCqOverflowRecoveries", recoveryBefore + 1),
                "Expected CQ overflow recovery counter to increment during sustained overflow scenario.");
            Assert.True(
                await WaitForIoUringCompletionSlotsInUseAtMostAsync(baselineCompletionSlotsInUse + 2, timeoutMilliseconds: 15000),
        ...
        [ConditionalFact(typeof(RemoteExecutor), nameof(RemoteExecutor.IsSupported))]
        [PlatformSpecific(TestPlatforms.Linux)] // io_uring is Linux-specific.
        public static async Task IoUringCompletionMode_BoundedWaitBufferPressure_NoLossAndCountersIncrease()
        {
            await RemoteExecutor.Invoke(
                static () => RunBoundedWaitBufferPressureScenarioAsync(connectionCount: 32),
                CreateSocketEngineOptions(testEventBufferCount: 1)).DisposeAsync();
        }
        ...
        [ConditionalFact(typeof(RemoteExecutor), nameof(RemoteExecutor.IsSupported))]
        [PlatformSpecific(TestPlatforms.Linux)] // io_uring is Linux-specific.
        public static async Task IoUringCompletionMode_NonPinnableFallbackTelemetryCounter_Increments()
        {
            await RemoteExecutor.Invoke(
                static () => RunNonPinnableFallbackTelemetryScenarioAsync(),
                CreateSocketEngineOptions()).DisposeAsync();
        }
        ...
        [ConditionalFact(typeof(RemoteExecutor), nameof(RemoteExecutor.IsSupported))]
        [PlatformSpecific(TestPlatforms.Linux)] // SQPOLL telemetry counters are Linux io_uring-specific.
        public static async Task IoUringCompletionMode_SqPoll_TelemetryCounters_Emitted()
        {
            await RemoteExecutor.Invoke(
                static () => RunSqPollTelemetryCountersScenarioAsync(),
                CreateSocketEngineOptions(sqPollEnabled: true)).DisposeAsync();
        }
        ...
        public static async Task IoUringCompletionMode_ForcedEagain_Recv_RequeuesViaCompletionPath()
        {
            await RemoteExecutor.Invoke(
                static () =>
                {
                    long requeueFailureBefore = GetIoUringCompletionRequeueFailureCounterValue();
                    ...
                    return Task.Run(async () =>
                    {
                        ...
                        Assert.Equal(requeueFailureBefore, GetIoUringCompletionRequeueFailureCounterValue());
