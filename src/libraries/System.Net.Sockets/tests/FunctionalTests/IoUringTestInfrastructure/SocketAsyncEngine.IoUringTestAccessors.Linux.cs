    ...
    internal sealed unsafe partial class SocketAsyncEngine
    {
        ...
        internal static IoUringNativeDiagnosticsSnapshotForTest GetIoUringNativeDiagnosticsSnapshotForTest()
        {
            ...
            if (!hasIoUringPort)
            {
                return new IoUringNativeDiagnosticsSnapshotForTest(
                    hasIoUringPort: false,
                    asyncCancelRequestCqeCount: 0,
                    asyncCancelRequestCqeEnoentCount: 0,
                    asyncCancelRequestCqeEalreadyCount: 0,
                    asyncCancelRequestCqeOtherCount: 0,
                    socketEventBufferFullCount: 0,
                    unsupportedOpcodePrepareCount: 0,
                    cqOverflowCount: 0);
            }

            // Native io_uring diagnostics are not exposed through managed interop in this test build.
            // Use telemetry-backed counters for the managed subset and zero the native-only breakdowns.
            return new IoUringNativeDiagnosticsSnapshotForTest(
                hasIoUringPort: true,
                asyncCancelRequestCqeCount: SocketsTelemetry.GetIoUringCounterValueForTest(SocketsTelemetry.IoUringCounterFieldForTest.AsyncCancelRequestCqes),
                asyncCancelRequestCqeEnoentCount: 0,
                asyncCancelRequestCqeEalreadyCount: 0,
                asyncCancelRequestCqeOtherCount: 0,
                socketEventBufferFullCount: SocketsTelemetry.GetIoUringCounterValueForTest(SocketsTelemetry.IoUringCounterFieldForTest.SocketEventBufferFull),
                unsupportedOpcodePrepareCount: 0,
                cqOverflowCount: SocketsTelemetry.GetIoUringCounterValueForTest(SocketsTelemetry.IoUringCounterFieldForTest.CqOverflow));
        }
        ...
        internal static IoUringZeroCopyPinHoldSnapshotForTest GetIoUringZeroCopyPinHoldSnapshotForTest()
        {
            ...
            foreach (SocketAsyncEngine engine in s_engines)
            {
                ...
                pendingNotificationCount += engine.CountZeroCopyNotificationPendingSlots();
            }
