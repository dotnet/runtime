...
namespace System.Net.Sockets
{
    internal sealed unsafe partial class SocketAsyncEngine
    {
        ...
        {
            return ResolveIoUringDirectSqeDisabled(s_cachedConfigInputs);
        }
        ...
        {
            // Override order: environment variable wins over AppContext switch.
            if (string.Equals(inputs.SqPollEnvironmentValue, "1", StringComparison.Ordinal))
            {
                return true;
            }

            if (string.Equals(inputs.SqPollEnvironmentValue, "0", StringComparison.Ordinal))
            {
                return false;
            }

            return inputs.SqPollFeatureSwitchEnabled;
        }
