...
namespace System.Net.Sockets
{
    internal sealed unsafe class SocketAsyncEngine : IThreadPoolWorkItem
    {
        ...
        //
        // Maps handle values to SocketAsyncContext GCHandle instances.
        //
        private readonly ConcurrentDictionary<IntPtr, GCHandle> _handleToContextMap = new ConcurrentDictionary<IntPtr, GCHandle>();
        ...
        private void RegisterCore(IntPtr socketHandle, SocketAsyncContext context)
        {
            ...
            try
            {
                ...
                bool added = _handleToContextMap.TryAdd(socketHandle, handle);
                ...
                Interop.Error error = Interop.Sys.TryChangeSocketEventRegistration(_port, socketHandle, Interop.Sys.SocketEvents.None,
                    Interop.Sys.SocketEvents.Read | Interop.Sys.SocketEvents.Write, GCHandle.ToIntPtr(handle));
        ...
        {
            if (_handleToContextMap.TryRemove(socketHandle, out GCHandle handle))
            {
                handle.Free();
            }
            else
            {
                Debug.Assert(false, "Double socket free");
            }
        }
        ...
        private void EventLoop()
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    {
                        IntPtr data = socketEvent.Data;
                        GCHandle handle = GCHandle.FromIntPtr(data);

                        if (handle.Target is SocketAsyncContext context)
                        {
