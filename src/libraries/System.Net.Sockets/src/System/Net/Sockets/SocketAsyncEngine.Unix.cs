    ...
    internal sealed unsafe class SocketAsyncEngine : IThreadPoolWorkItem
    {
        ...
        private void ExecuteAio()
        {
            ...
            while (true)
            {
                ...
                if (result != currentBatchSize)
                {
                    Interop.Error lastError = Interop.Sys.GetLastError();
                    Environment.FailFast($"IoSubmit has failed with {lastError}, returned {result} instead {currentBatchSize}"); // todo: implement a while loop
                }

                // todo: perf: avoid the syscall by using a well known pattern that reads from the ring
                result = Interop.Sys.IoGetEvents(aioContext.Ring, currentBatchSize, currentBatchSize, aioEventsSegment);
                if (result != currentBatchSize)
                {
                    Interop.Error lastError = Interop.Sys.GetLastError();
                    Environment.FailFast($"IoGetEvents has failed with {lastError}, returned {result} instead {currentBatchSize}");
                }
