    ...
    internal sealed class SystemIPGlobalProperties : IPGlobalProperties
    {
        ...
        private unsafe List<SystemTcpConnectionInformation> GetAllTcpConnections()
        {
            ...
            if (Socket.OSSupportsIPv4)
            {
                ...
                result = Interop.IpHlpApi.GetTcpTable(IntPtr.Zero, &size, order: 1);
                ...
                while (result == Interop.IpHlpApi.ERROR_INSUFFICIENT_BUFFER)
                {
                    ...
                    try
                    {
                        result = Interop.IpHlpApi.GetTcpTable(buffer, &size, order: 1);
            ...
            if (Socket.OSSupportsIPv6)
            {
                // Get the buffer size needed.
                size = 0;
                result = Interop.IpHlpApi.GetExtendedTcpTable(IntPtr.Zero, &size, order: 1,
                ...
                while (result == Interop.IpHlpApi.ERROR_INSUFFICIENT_BUFFER)
                {
                    ...
                    try
                    {
                        result = Interop.IpHlpApi.GetExtendedTcpTable(buffer, &size, order: 1,
                                                                                (uint)AddressFamily.InterNetworkV6,
                                                                                Interop.IpHlpApi.TcpTableClass.TcpTableOwnerPidAll, 0);
                        if (result == Interop.IpHlpApi.ERROR_SUCCESS)
                        {
        ...
        public unsafe override IPEndPoint[] GetActiveUdpListeners()
        {
            ...
            {
                // Get the buffer size needed.
                result = Interop.IpHlpApi.GetUdpTable(IntPtr.Zero, &size, order: 1);
                while (result == Interop.IpHlpApi.ERROR_INSUFFICIENT_BUFFER)
                {
                    ...
                    try
                    {
                        result = Interop.IpHlpApi.GetUdpTable(buffer, &size, order: 1);
            ...
            if (Socket.OSSupportsIPv6)
            {
                // Get the buffer size needed.
                size = 0;
                result = Interop.IpHlpApi.GetExtendedUdpTable(IntPtr.Zero, &size, order: 1,
                                                                        (uint)AddressFamily.InterNetworkV6,
                ...
                while (result == Interop.IpHlpApi.ERROR_INSUFFICIENT_BUFFER)
                {
                    ...
                    try
                    {
                        result = Interop.IpHlpApi.GetExtendedUdpTable(buffer, &size, order: 1,
                                                                                (uint)AddressFamily.InterNetworkV6,
                                                                                Interop.IpHlpApi.UdpTableClass.UdpTableOwnerPid, 0);
