...
namespace System.Net.NetworkInformation
{
    ...
    internal sealed class AndroidNetworkInterface : UnixNetworkInterface
    {
            ...
            : base(name)
        {
            ...
            {
                _physicalAddress = new PhysicalAddress(((ReadOnlySpan<byte>)networkInterfaceInfo->AddressBytes).Slice(0, networkInterfaceInfo->NumAddressBytes).ToArray());
            }
        ...
        {
            var address = new IPAddress(((ReadOnlySpan<byte>)addressInfo->AddressBytes).Slice(0, addressInfo->NumAddressBytes));
