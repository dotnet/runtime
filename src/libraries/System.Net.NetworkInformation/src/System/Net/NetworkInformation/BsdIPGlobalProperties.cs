    ...
    internal sealed class BsdIPGlobalProperties : UnixIPGlobalProperties
    {
        private unsafe TcpConnectionInformation[] GetTcpConnections(bool listeners)
        {
            ...
            for (int i = 0; i < infoCount; i++)
            {
                ...
                IPAddress localIPAddress = new IPAddress(((ReadOnlySpan<byte>)nativeInfo.LocalEndPoint.AddressBytes).Slice(0, checked((int)nativeInfo.LocalEndPoint.NumAddressBytes)));
                ...
                IPAddress remoteIPAddress = nativeInfo.RemoteEndPoint.NumAddressBytes == 0 ?
                    IPAddress.Any :
                    new IPAddress(((ReadOnlySpan<byte>)nativeInfo.RemoteEndPoint.AddressBytes).Slice(0, checked((int)nativeInfo.RemoteEndPoint.NumAddressBytes)));
        ...
        public override unsafe IPEndPoint[] GetActiveUdpListeners()
        {
            ...
            for (int i = 0; i < infoCount; i++)
            {
                ...
                IPAddress ipAddress = endPointInfo.NumAddressBytes == 0 ?
                    IPAddress.Any :
                    new IPAddress(((ReadOnlySpan<byte>)endPointInfo.AddressBytes).Slice(0, checked((int)endPointInfo.NumAddressBytes)));
