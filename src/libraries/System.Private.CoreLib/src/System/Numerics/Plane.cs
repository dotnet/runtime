...
namespace System.Numerics
{
    ...
    public struct Plane : IEquatable<Plane>
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Plane CreateFromVertices(Vector3 point1, Vector3 point2, Vector3 point3) => CreateFromVertices(point1.AsVector128Unsafe(), point2.AsVector128Unsafe(), point3.AsVector128Unsafe()).AsPlane();

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static Vector128<float> CreateFromVertices(Vector128<float> point1, Vector128<float> point2, Vector128<float> point3)
        {
            ...
            Vector128<float> normal = Vector128.Normalize(Vector3.Cross(point2 - point1, point3 - point1).WithElement(3, 0.0f));
            return normal.WithElement(3, -Vector128.Dot(normal, point1.WithElement(3, 0.0f)));
        }
        ...
        public static float DotCoordinate(Plane plane, Vector3 value)
        {
            ...
            return Vector128.Dot(plane.AsVector128(), value.AsVector128().WithElement(3, 1.0f));
        }
        ...
        public static float DotNormal(Plane plane, Vector3 value)
        {
            ...
            return Vector128.Dot(plane.AsVector128().WithElement(3, 0.0f), value.AsVector128());
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Plane Normalize(Plane value) => Normalize(value.AsVector128()).AsPlane();

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static Vector128<float> Normalize(Vector128<float> value)
        {
            // This implementation is based on the DirectX Math Library XMPlaneNormalize method
            // https://github.com/microsoft/DirectXMath/blob/master/Inc/DirectXMathMisc.inl

            Vector128<float> lengthSquared = Vector128.Create(Vector128.LengthSquared(value.WithElement(3, 0.0f)));

            return Vector128.AndNot(
                (value / Vector128.Sqrt(lengthSquared)),
                Vector128.IsPositiveInfinity(lengthSquared)
            );
        }
        ...
        {
            Matrix4x4.Invert(matrix.AsROImpl(), out Matrix4x4.Impl inverseMatrix);
            return Vector4.Transform(plane.AsVector128(), Matrix4x4.Transpose(inverseMatrix)).AsPlane();
        }
        ...
        /// <remarks><paramref name="plane" /> must already be normalized so that its <see cref="Normal" /> vector is of unit length before this method is called.</remarks>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static Plane Transform(Plane plane, Quaternion rotation) => Vector4.Transform(plane.AsVector128(), rotation.AsVector128()).AsPlane();
