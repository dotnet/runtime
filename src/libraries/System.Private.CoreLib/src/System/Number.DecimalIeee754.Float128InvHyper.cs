...
internal static partial class Number
{
    ...
    // reduced ratio directly and evaluates it with `Float128LogPoly`; otherwise the big path forms the
    // full argument and calls `Float128Ln`. The evaluation runs entirely in the software binary128
    ...
    private static Float128 Float128Asinh(Float128 x)
    {
        uint sign = x._sign;
        x._sign = 0; // |x|
        ...
        Float128 square = x;
        Float128Multiply(ref square, ref square, out Float128 tmp); // x^2

        Span<Float128> one = stackalloc Float128[1];
        Float128AddSub(tmp, Float128One, UxAdd, one); // x^2 + 1
        tmp = one[0];
        Float128Normalize(ref tmp);
        tmp = Float128Sqrt(tmp); // sqrt(x^2 + 1)

        Float128 result;

        if ((exponent < -1) || ((exponent == -1) && (fHi <= InvHyperSqrt2Over4)))
        {
            Float128AddSub(tmp, Float128One, UxAdd, one); // sqrt(x^2 + 1) + 1
            Float128Divide(x, one[0], Float128FullPrecision, out tmp); // x / (sqrt(x^2 + 1) + 1)
            result = Float128LogPoly(tmp);
        }
        else
        {
            Float128AddSub(tmp, x, UxAdd, one); // sqrt(x^2 + 1) + x
            tmp = one[0];
            Float128Normalize(ref tmp);
            result = Float128Ln(tmp);
        }
    ...
    private static Float128 Float128Acosh(Float128 x)
    {
        int exponent = x._exponent;
        ulong fHi = x._hi;

        Span<Float128> parts = stackalloc Float128[2];
        Float128AddSub(x, Float128One, UxAddSub, parts); // parts[0] = x + 1, parts[1] = x - 1

        if ((exponent == 1) && (fHi <= InvHyperThreeSqrt2Over4))
        {
            Float128Divide(parts[1], parts[0], Float128FullPrecision, out Float128 ratio); // (x - 1) / (x + 1)
            return Float128LogPoly(Float128Sqrt(ratio));
        }

        Float128Multiply(ref parts[1], ref parts[0], out Float128 product); // x^2 - 1
        Float128Normalize(ref product);
        Float128 root = Float128Sqrt(product); // sqrt(x^2 - 1)

        Span<Float128> sum = stackalloc Float128[1];
        Float128AddSub(root, x, UxAdd, sum); // sqrt(x^2 - 1) + x
        return Float128Ln(sum[0]);
    }

    /// <summary>Computes <c>atanh(x)</c> for a finite <paramref name="x"/> with <c>|x| &lt; 1</c> (Intel's <c>F_ATANH</c>).</summary>
    private static Float128 Float128Atanh(Float128 x)
    {
        uint sign = x._sign;
        x._sign = 0; // |x|
        ...
        {
            result = Float128LogPoly(x); // log((1 + |x|) / (1 - |x|))
        }
        else
        {
            Span<Float128> parts = stackalloc Float128[2];
            Float128AddSub(x, Float128One, UxAddSub, parts); // parts[0] = |x| + 1, parts[1] = |x| - 1
            Float128Divide(parts[1], parts[0], Float128FullPrecision, out Float128 ratio); // (|x| - 1) / (|x| + 1)
            Float128Normalize(ref ratio);
            result = Float128Ln(ratio); // magnitude only: log((1 - |x|) / (1 + |x|))
        }
