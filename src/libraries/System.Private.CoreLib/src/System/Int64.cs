...
namespace System
{
          ...
          IBinaryIntegerParseAndFormatInfo<long>
    {
        ...
        public static Int128 BigMul(long left, long right) => Math.BigMul(left, right);
        ...
        static bool IBinaryInteger<long>.TryReadLittleEndian(ReadOnlySpan<byte> source, bool isUnsigned, out long value)
        {
            ...
            if (source.Length != 0)
            {
                ...
                {
                    // We have at least 8 bytes, so just read the ones we need directly
                    result = BinaryPrimitives.ReadInt64LittleEndian(source);
                }
                else
                {
                    // We have between 1 and 7 bytes, so construct the relevant value directly
        ...
        {
            if (BinaryPrimitives.TryWriteInt64BigEndian(destination, m_value))
            {
        ...
        {
            if (BinaryPrimitives.TryWriteInt64LittleEndian(destination, m_value))
            {
                bytesWritten = sizeof(long);
                return true;
            }
        ...
        int IBinaryInteger<long>.WriteBigEndian(byte[] destination) => BinaryIntegerWriter.WriteBigEndian(this, destination);

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteBigEndian(byte[], int)" />
        int IBinaryInteger<long>.WriteBigEndian(byte[] destination, int startIndex) => BinaryIntegerWriter.WriteBigEndian(this, destination.AsSpan(startIndex));

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteBigEndian(Span{byte})" />
        int IBinaryInteger<long>.WriteBigEndian(Span<byte> destination) => BinaryIntegerWriter.WriteBigEndian(this, destination);

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteLittleEndian(byte[])" />
        int IBinaryInteger<long>.WriteLittleEndian(byte[] destination) => BinaryIntegerWriter.WriteLittleEndian(this, destination);

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteLittleEndian(byte[], int)" />
        int IBinaryInteger<long>.WriteLittleEndian(byte[] destination, int startIndex) => BinaryIntegerWriter.WriteLittleEndian(this, destination.AsSpan(startIndex));

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteLittleEndian(Span{byte})" />
        int IBinaryInteger<long>.WriteLittleEndian(Span<byte> destination) => BinaryIntegerWriter.WriteLittleEndian(this, destination);
        ...
        static long INumberBase<long>.MultiplyAddEstimate(long left, long right, long addend) => (long)((left * right) + addend);
        ...
        public static bool TryParsePartial([NotNullWhen(true)] string? s, NumberStyles style, IFormatProvider? provider, out long result, out int charsConsumed)
        {
            NumberFormatInfo.ValidateParseStyleInteger(style);
        ...
        public static bool TryParsePartial(ReadOnlySpan<char> s, NumberStyles style, IFormatProvider? provider, out long result, out int charsConsumed)
        {
            NumberFormatInfo.ValidateParseStyleInteger(style);
        ...
        public static bool TryParsePartial(ReadOnlySpan<byte> utf8Text, NumberStyles style, IFormatProvider? provider, out long result, out int bytesConsumed)
        {
            NumberFormatInfo.ValidateParseStyleInteger(style);
