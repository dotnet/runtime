    ...
    public sealed partial class TimeZoneInfo
    {
                ...
                for (int i = transitionInfo.index; i < boundary; i++)
                {
                    ...
                    if (utcNowTicks >= transition.DateStart.Ticks && utcNowTicks <= transition.DateEnd.Ticks)
                    {
                        ...
                        if (transition.DaylightSavingOn)
                        {
                            ...
                            DateTime localTransitionStart = new DateTime(GetValidTicks(transition.DateStart.Ticks + transition.Offset.Ticks), DateTimeKind.Unspecified);
                            DateTime localTransitionEnd = new DateTime(GetValidTicks(transition.DateEnd.Ticks + transition.Offset.Ticks), DateTimeKind.Unspecified);
                            ...
                            if (i < boundary - 1)
                            {
                                ...
                                DateTime nextLocalTransitionStart = new DateTime(GetValidTicks(nextTransition.DateStart.Ticks + nextTransition.Offset.Ticks), DateTimeKind.Unspecified);
                                DateTime nextLocalTransitionEnd = new DateTime(GetValidTicks(nextTransition.DateEnd.Ticks + nextTransition.Offset.Ticks), DateTimeKind.Unspecified);
                            ...
                            if (i > 0)
                            {
                                ...
                                DateTime prevLocalTransitionStart = new DateTime(GetValidTicks(prevTransition.DateStart.Ticks + prevTransition.Offset.Ticks), DateTimeKind.Unspecified);
                                DateTime prevLocalTransitionEnd = new DateTime(GetValidTicks(prevTransition.DateEnd.Ticks + prevTransition.Offset.Ticks), DateTimeKind.Unspecified);
        ...
        private bool TryLocalToUtc(DateTime localDateTime, out DateTime utcDateTime)
        {
            if (TryGetUtcOffset(localDateTime, out TimeSpan offset))
            {
                ...
                utcDateTime = new DateTime(GetValidTicks(ticks), DateTimeKind.Utc);
                return true;
            }
        ...
        private static long GetValidTicks(long ticks)
            => (ulong)ticks <= DateTime.MaxTicks ? ticks : (ticks < 0 ? 0 : DateTime.MaxTicks);
        ...
        private TimeSpan GetOffsetForUtcDate(DateTime utcDateTime, out bool isDaylightSavingTime, out bool isAmbiguous)
        {
            ...
            DateTime localTime = new DateTime(GetValidTicks(utcDateTime.Ticks + offset.Ticks), DateTimeKind.Unspecified);
        ...
        private DateTime UtcToLocal(DateTime utcDateTime, out bool isDaylightSavingTime)
        {
            ...
            return new DateTime(GetValidTicks(ticks), DateTimeKind.Unspecified);
        }
        ...
        private bool TryGetUtcOffset(DateTime localDateTime, out TimeSpan offset)
        {
            ...
            for (int i = transitionInfo.index; i < boundary; i++)
            {
                ...
                long localTicks = GetValidTicks(localDateTime.Ticks - offset.Ticks);
                if (localTicks >= transition.DateStart.Ticks && localTicks <= transition.DateEnd.Ticks) // Start and End dates in the transitions are inclusive
                {
                    ...
                    if (i + 1 < boundary && transition.DaylightSavingOn && (localDateTime.Kind != DateTimeKind.Local || !localDateTime.IsAmbiguousDaylightSavingTime()))
                    {
                        ...
                        localTicks = GetValidTicks(localDateTime.Ticks - nextOffset.Ticks);
        ...
        private (int index, int count) CacheTransitionsForYear(int year)
        {
            ...
            if (rule.NoDaylightTransitions)
            {
                ...
                if (rule.DateStart > startOfYear || localTicks > startOfYear.Ticks) // Ensure local start of the year is covered
                {
                    ...
                    if (previousRule is not null && previousRule.DateEnd.Year >= year - 1)
                    {
                        if (previousRule.NoDaylightTransitions)
                        {
                            ...
                            DateTime previousYearTransitionEnd = new DateTime(GetValidTicks(previousRule.DateEnd.Ticks + 1)); // UTC coordinate
                            if (previousYearTransitionEnd.Ticks < rule.DateStart.Ticks - 1)
                            {
                                ...
                                AddTransition(ref allTransitions, ref transitionCount,
                                    new TimeTransition(previousYearTransitionEnd, new DateTime(GetValidTicks(rule.DateStart.Ticks - 1)), previousRule.BaseUtcOffsetDelta, false));
                            }
                            else
                            {
                                // previous year transition end at the current year transition start, then include last year transition
                                AddTransition(ref allTransitions, ref transitionCount,
                                    new TimeTransition(previousRule.DateStart, previousRule.DateEnd, previousRule.BaseUtcOffsetDelta + previousRule.DaylightDelta, previousRule.HasDaylightSaving));
                            }
                        }
                        else
                        {
                            ...
                            if (previousYearStart < previousYearEnd)
                            {
                                ...
                                DateTime previousYearEndUtc = previousYearEnd.Add(-(_baseUtcOffset + previousRule.BaseUtcOffsetDelta + previousRule.DaylightDelta)).AddTicks(-1);

                                if (previousYearEndUtc.Ticks < rule.DateStart.Ticks - 1)
                                {
                                    // Gap between the last year transition end and current year transition start. no daylight
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(previousYearEndUtc, new DateTime(GetValidTicks(rule.DateStart.Ticks - 1)), previousRule.BaseUtcOffsetDelta, false));
                                }
                                else
                                {
                                    // previous year transition end at the current year transition start, then include last year transition
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(
                                                previousYearStart.Add(-(_baseUtcOffset + previousRule.BaseUtcOffsetDelta)),
                                                new DateTime(GetValidTicks(rule.DateStart.Ticks - 1)),
                                                previousRule.BaseUtcOffsetDelta + previousRule.DaylightDelta,
                            ...
                            else // previousYearStart > previousYearEnd
                            {
                                ...
                                DateTime previousEndOfYearUtc = new DateTime(GetValidTicks(previousEndOfYearUtcTicks), DateTimeKind.Utc);
                                ...
                                if (previousEndOfYearUtc.Ticks < rule.DateStart.Ticks - 1)
                                {
                                    ...
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(new DateTime(GetValidTicks(previousEndOfYearUtc.Ticks + 1)), new DateTime(GetValidTicks(rule.DateStart.Ticks - 1)), rule.BaseUtcOffsetDelta, false));
                                }

                                // The daylight start should be around the end of the year and go through the end
                                DateTime previousYearStartUtc = previousYearStart.Add(-(_baseUtcOffset + previousRule.BaseUtcOffsetDelta)); // daylight offset is not counted in this start

                                AddTransition(ref allTransitions, ref transitionCount,
                                    new TimeTransition(previousYearStartUtc, previousEndOfYearUtc, previousRule.BaseUtcOffsetDelta + previousRule.DaylightDelta, previousRule.HasDaylightSaving));
                            }
                        }
                    }
                    else
                    {
                        ...
                        {
                            transitionStart = new DateTime(GetValidTicks(transitionStart.Ticks - _baseUtcOffset.Ticks));

                            AddTransition(ref allTransitions, ref transitionCount,
                                new TimeTransition(transitionStart, new DateTime(GetValidTicks(rule.DateStart.Ticks - 1)), TimeSpan.Zero, false));
                        }
                    }
                }
                ...
                AddTransition(ref allTransitions, ref transitionCount,
                    new TimeTransition(rule.DateStart, rule.DateEnd, rule.BaseUtcOffsetDelta + rule.DaylightDelta, rule.HasDaylightSaving));
                        ...
                        allTransitions[transitionCount - 1].DateEnd.Ticks + allTransitions[transitionCount - 1].Offset.Ticks + _baseUtcOffset.Ticks < endOfCurrentYear.Ticks) // local end date still not reached the end of the year
                {
                    ...
                    if (ruleIndex < _adjustmentRules.Length && _adjustmentRules[ruleIndex].DateStart.Year <= allTransitions[transitionCount - 1].DateEnd.Year) // ensure the year is included in the rule
                    {
                        ...
                        if (nextYearRule.NoDaylightTransitions)
                        {
                            ...
                            AddTransition(ref allTransitions, ref transitionCount,
                                new TimeTransition(nextYearRule.DateStart, nextYearRule.DateEnd, nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta, nextYearRule.HasDaylightSaving));
                        }
                        else
                        {
                            //
                            ...
                            if (nextYearStart < nextYearEnd)
                            {
                                ...
                                DateTime nextYearStartUtc = nextYearStart.Add(-(_baseUtcOffset + nextYearRule.BaseUtcOffsetDelta));
                                ...
                                AddTransition(ref allTransitions, ref transitionCount,
                                    new TimeTransition(
                                            nextYearStartUtc,
                                            nextYearEnd.Add(-(_baseUtcOffset + nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta)).AddTicks(-1),
                                            nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta,
                            ...
                            else // nextYearStart > nextYearEnd
                            {
                                ...
                                DateTime nextYearEndUtc = nextYearEnd.Add(-(_baseUtcOffset + nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta)).AddTicks(-1);
                                DateTime nextBeginningOfYearUtc = new DateTime(year + 1, 1, 1).Add(-(_baseUtcOffset + nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta));
                                ...
                                if (allTransitions[transitionCount - 1].DateEnd.Ticks < nextBeginningOfYearUtc.Ticks - 1)
                                {
                                    ...
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        ...
                                        new TimeTransition(allTransitions[transitionCount - 1].DateEnd.AddTicks(1), nextBeginningOfYearUtc.AddTicks(-1), rule.BaseUtcOffsetDelta + rule.DaylightDelta, rule.HasDaylightSaving));
                                }

                                AddTransition(ref allTransitions, ref transitionCount,
                                    new TimeTransition(allTransitions[transitionCount - 1].DateEnd.AddTicks(1), nextYearEndUtc, nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta, nextYearRule.HasDaylightSaving));
                            }
            ...
            else
            {
                ...
                long ticks = GetValidTicks(localStart.Ticks - (_baseUtcOffset.Ticks + rule.BaseUtcOffsetDelta.Ticks + (startWithDaylightOn ? rule.DaylightDelta.Ticks : 0)));
                DateTime utcStart = new DateTime(ticks, DateTimeKind.Utc);

                ticks = GetValidTicks(localEnd.Ticks - (_baseUtcOffset.Ticks + rule.BaseUtcOffsetDelta.Ticks + rule.DaylightDelta.Ticks) - 1);
                DateTime utcEnd = new DateTime(ticks, DateTimeKind.Utc);
                ...
                DateTime startOfCurrentYearUtc = new DateTime(GetValidTicks(startOfCurrentYearUtcTicks), DateTimeKind.Utc);
                ...
                if (localStart < localEnd)
                {
                    ...
                    if (utcStart > currentYearStart || localStart > currentYearStart) // check either the year start in Utc or local are covering the beginning of the year
                    {
                        ...
                        if (previousYearRule is not null)
                        {
                            if (previousYearRule.NoDaylightTransitions)
                            {
                                ...
                                DateTime previousYearTransitionEnd = new DateTime(GetValidTicks(previousYearRule.DateEnd.Ticks + 1)); // UTC coordinate
                                if (previousYearTransitionEnd.Ticks < startOfCurrentYearUtc.Ticks - 1)
                                {
                                    // Gap between the last year transition end and current year transition start. no daylight
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(previousYearTransitionEnd, new DateTime(GetValidTicks(startOfCurrentYearUtc.Ticks - 1)), previousYearRule.BaseUtcOffsetDelta, false));
                                }
                                else
                                {
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(previousYearRule.DateStart, previousYearRule.DateEnd, previousYearRule.BaseUtcOffsetDelta + previousYearRule.DaylightDelta, previousYearRule.HasDaylightSaving));
                                }
                            }
                            else
                            {
                                ...
                                if (previousYearStart < previousYearEnd)
                                {
                                    ...
                                    // Get previous rule end in UTC coordinates
                                    DateTime previousYearEndUtc = previousYearEnd.Add(-(_baseUtcOffset + previousYearRule.BaseUtcOffsetDelta + previousYearRule.DaylightDelta));

                                    // previous year transition end at the current year transition start, then include last year transition
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(
                                                previousYearStart.Add(-(_baseUtcOffset + previousYearRule.BaseUtcOffsetDelta)),
                                                previousYearEndUtc.AddTicks(-1),
                                                previousYearRule.BaseUtcOffsetDelta + previousYearRule.DaylightDelta,
                                ...
                                else // previousYearStart > previousYearEnd
                                {
                                    ...
                                    // The daylight start should be around the end of the year and go through the end
                                    DateTime previousYearStartUtc = previousYearStart.Add(-(_baseUtcOffset + previousYearRule.BaseUtcOffsetDelta)); // daylight offset is not counted in this start

                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(previousYearStartUtc, startOfCurrentYearUtc.AddTicks(-1), previousYearRule.BaseUtcOffsetDelta + previousYearRule.DaylightDelta, previousYearRule.HasDaylightSaving));
                                }
                            }
                        }
                        else
                        {
                            ...
                            AddTransition(ref allTransitions, ref transitionCount,
                                new TimeTransition(previousYearStart, startOfCurrentYearUtc.Ticks > 0 ? startOfCurrentYearUtc.AddTicks(-1) : startOfCurrentYearUtc, TimeSpan.Zero, false));
                        }
                    ...
                    AddTransition(ref allTransitions, ref transitionCount,
                        new TimeTransition(utcStart, utcEnd, rule.BaseUtcOffsetDelta + rule.DaylightDelta, rule.HasDaylightSaving));
                    ...
                    DateTime endOfCurrentYearUtc = new DateTime(GetValidTicks(endOfCurrentYearUtcTicks), DateTimeKind.Utc);
                    ...
                    if (endOfCurrentYearUtc < endOfCurrentYear)
                    {
                        ...
                        if (nextYearRule is not null)
                        {
                            ...
                            else
                            {
                                ...
                                DateTime nextYearStartUtc = nextYearStart.Add(-(_baseUtcOffset + nextYearRule.BaseUtcOffsetDelta +
                                                                                (nextYearRule.IsStartDateMarkerForBeginningOfYear() ? nextYearRule.DaylightDelta : TimeSpan.Zero)));
                                DateTime nextYearEndUtc = nextYearEnd.Add(-(_baseUtcOffset + nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta)).AddTicks(-1);
                                ...
                                if (nextYearStart < nextYearEnd)
                                {
                                    ...
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(nextYearStartUtc, nextYearEndUtc, nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta, nextYearRule.HasDaylightSaving));
                                }
                                else // nextYearStart > nextYearEnd
                                {
                                    //
                                    ...
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(endOfCurrentYearUtc.AddTicks(1), nextYearEndUtc, nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta, nextYearRule.HasDaylightSaving));
                                }
                            }
                        }
                        else
                ...
                else // localStart > localEnd
                {
                    ...
                    if (startOfCurrentYearUtc > currentYearStart)
                    {
                        ...
                        if (previousYearRule is not null)
                        {
                            ...
                            else
                            {
                                ...
                                DateTime previousYearStartUtc = previousYearStart.Add(-(_baseUtcOffset +
                                                    previousYearRule.BaseUtcOffsetDelta +
                                                    (previousYearRule.IsStartDateMarkerForBeginningOfYear() ? previousYearRule.DaylightDelta : TimeSpan.Zero)));
                                ...
                                if (previousYearStart < previousYearEnd)
                                {
                                    ...
                                    DateTime previousYearEndUtc = previousYearEnd.Add(-(_baseUtcOffset + previousYearRule.BaseUtcOffsetDelta + previousYearRule.DaylightDelta)).AddTicks(-1);
                                    if (previousYearEndUtc.Ticks < startOfCurrentYearUtc.Ticks - 1)
                                    {
                                        AddTransition(ref allTransitions, ref transitionCount,
                                    ...
                                    else
                                    {
                                        ...
                                        AddTransition(ref allTransitions, ref transitionCount,
                                            new TimeTransition(previousYearStartUtc, previousYearEndUtc, previousYearRule.BaseUtcOffsetDelta + previousYearRule.DaylightDelta, previousYearRule.HasDaylightSaving));
                                    }
                                }
                                else // previousYearStart > previousYearEnd
                                {
                    ...
                    AddTransition(ref allTransitions, ref transitionCount,
                        new TimeTransition(startOfCurrentYearUtc, utcEnd, rule.BaseUtcOffsetDelta + rule.DaylightDelta, rule.HasDaylightSaving));

                    AddTransition(ref allTransitions, ref transitionCount,
                        new TimeTransition(utcEnd.AddTicks(1), utcStart.AddTicks(-1), rule.BaseUtcOffsetDelta, false));

                    AddTransition(ref allTransitions, ref transitionCount,
                        new TimeTransition(utcStart, endOfCurrentYearUtc, rule.BaseUtcOffsetDelta + rule.DaylightDelta, rule.HasDaylightSaving));
                    ...
                    if (endOfCurrentYearUtc < endOfCurrentYear)
                    {
                        ...
                        if (nextYearRule is not null)
                        {
                            if (nextYearRule.NoDaylightTransitions)
                            {
                                ...
                                if (nextYearRule.DateStart.Ticks > endOfCurrentYearUtc.Ticks - 1)
                                {
                                    AddTransition(ref allTransitions, ref transitionCount,
                                ...
                                {
                                    AddTransition(ref allTransitions, ref transitionCount,
                                        new TimeTransition(endOfCurrentYearUtc.AddTicks(1), nextYearRule.DateEnd, nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta, nextYearRule.HasDaylightSaving));
                                }
                            }
                            else
                            {
                                ...
                                DateTime nextYearStart = StartTransitionTimeToDateTime(year + 1, nextYearRule); // in local time
                                DateTime nextYearEnd = EndTransitionTimeToDateTime(year + 1, nextYearRule);     // in local time
                                DateTime nextYearStartUtc = nextYearStart.Add(-(
                                                                _baseUtcOffset +
                                                                nextYearRule.BaseUtcOffsetDelta + (nextYearRule.IsStartDateMarkerForBeginningOfYear() ? nextYearRule.DaylightDelta : TimeSpan.Zero)));
                                DateTime nextYearEndUtc = nextYearEnd.Add(-(_baseUtcOffset + nextYearRule.BaseUtcOffsetDelta + nextYearRule.DaylightDelta)).AddTicks(-1);
