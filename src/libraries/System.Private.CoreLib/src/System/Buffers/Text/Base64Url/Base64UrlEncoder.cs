...
namespace System.Buffers.Text
{
    public static partial class Base64Url
    {
        ...
        public static OperationStatus EncodeToChars(ReadOnlySpan<byte> source, Span<char> destination,
            ...
            EncodeToChars<Base64UrlEncoder>(source, destination, out bytesConsumed, out charsWritten, isFinalBlock);

        internal static unsafe OperationStatus EncodeToChars<TBase64Encoder>(ReadOnlySpan<byte> bytes, Span<char> destination, out int bytesConsumed, out int bytesWritten, bool isFinalBlock = true)
            where TBase64Encoder : IBase64Encoder
        {
            if (bytes.IsEmpty)
            {
                bytesConsumed = 0;
            ...
            fixed (char* destBytes = &MemoryMarshal.GetReference(destination))
            {
                ...
                if (maxSrcLength >= 16)
                {
                    ...
                    {
                        Avx512Encode<TBase64Encoder>(ref src, ref dest, end, maxSrcLength, destLength, srcBytes, (ushort*)destBytes);
                    ...
                    {
                        Avx2Encode<TBase64Encoder>(ref src, ref dest, end, maxSrcLength, destLength, srcBytes, (ushort*)destBytes);
                    ...
                    {
                        AdvSimdEncode<TBase64Encoder>(ref src, ref dest, end, maxSrcLength, destLength, srcBytes, (ushort*)destBytes);
                    ...
                    {
                        Vector128Encode<TBase64Encoder>(ref src, ref dest, end, maxSrcLength, destLength, srcBytes, (ushort*)destBytes);
                ...
                while (src < srcMax)
                {
                    EncodeAndWrite(src, dest, ref encodingMap);
                    src += 3;
                    dest += 4;
                }
                ...
                if (src + 1 == srcEnd)
                {
                    result = TBase64Encoder.EncodeOneOptionallyPadTwo(src, ref encodingMap);
                    if (BitConverter.IsLittleEndian)
                    {
                        dest[0] = (char)(result & 0x7F);
                        dest[1] = (char)(result >> 8);
                    }
                    else
                    {
                        dest[0] = (char)(result >> 8);
                        dest[1] = (char)(result & 0x7F);
                    }
                ...
                {
                    result = TBase64Encoder.EncodeTwoOptionallyPadOne(src, ref encodingMap);
                    if (BitConverter.IsLittleEndian)
                    {
                        dest[0] = (char)(result & 0x7F);
                        dest[1] = (char)((result >> 8) & 0x7F);
                        dest[2] = (char)(result >> 16);
                    }
                    else
                    {
                        dest[0] = (char)(result >> 16);
                        dest[1] = (char)((result >> 8) & 0x7F);
                        dest[2] = (char)(result & 0x7F);
                    }
        ...
        private static unsafe void Avx512Encode<TBase64Encoder>(ref byte* srcBytes, ref char* destBytes,
            byte* srcEnd, int sourceLength, int destLength, byte* srcStart, ushort* destStart)
            where TBase64Encoder : IBase64Encoder
        {
            ...
            while (true)
            {
                ...
                (Vector512<ushort> utf16LowVector, Vector512<ushort> utf16HighVector) = Vector512.Widen(str.AsByte());
        ...
        private static unsafe void Avx2Encode<TBase64Encoder>(ref byte* srcBytes, ref char* destBytes,
            byte* srcEnd, int sourceLength, int destLength, byte* srcStart, ushort* destStart)
            where TBase64Encoder : IBase64Encoder
        {
            ...
            while (true)
            {
                ...
                (Vector256<ushort> utf16LowVector, Vector256<ushort> utf16HighVector) = Vector256.Widen(str.AsByte());
                utf16LowVector.Store(dest);
        ...
        private static unsafe void AdvSimdEncode<TBase64Encoder>(ref byte* srcBytes, ref char* destBytes,
            byte* srcEnd, int sourceLength, int destLength, byte* srcStart, ushort* destStart)
            where TBase64Encoder : IBase64Encoder
        {
            ...
            do
            {
                ...
                Base64.AssertRead<Vector128<byte>>(src, srcStart, sourceLength);
        ...
        [CompExactlyDependsOn(typeof(AdvSimd.Arm64))]
        private static unsafe void Vector128Encode<TBase64Encoder>(ref byte* srcBytes, ref char* destBytes,
            byte* srcEnd, int sourceLength, int destLength, byte* srcStart, ushort* destStart)
            where TBase64Encoder : IBase64Encoder
        {
        ...
        private static unsafe void AssertWrite<TVector>(ushort* dest, ushort* destStart, int destLength)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe void EncodeAndWrite(byte* threeBytes, char* destination, ref byte encodingMap)
        {
