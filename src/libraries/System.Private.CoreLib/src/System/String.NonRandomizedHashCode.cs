...
namespace System
{
    public partial class String
    {
        ...
        private static int GetNonRandomizedHashCodeLarge<TCasing>(ReadOnlySpan<byte> span) where TCasing : struct, IHashCasing
        {
            ...
            if (length > 64)
            {
                // Eight lanes absorb twice the bytes per dependent step, which is what the
                // block loop is limited by, but seeding and folding them needs enough
                // blocks to pay for itself.
                return length >= WideHashThreshold
                    ? GetNonRandomizedHashCodeWide<TCasing>(span, length)
                    : GetNonRandomizedHashCodeLong<TCasing>(span, length);
            }
            ulong a = BitConverter.ToUInt64(span);
            ulong b = BitConverter.ToUInt64(span.Slice(length - 8));
            if (((a | b) & TCasing.NonAsciiMask) != 0)
        ...
        // four and hands the remainder to the four-lane loop. Lane i always takes the ulong
        // at byte offset 8 * i, so every width here and the scalar loop produce the same
        // hash. Its own method so that shorter inputs, which never reach it, keep the
        // register allocation they had before it existed.
        [MethodImpl(MethodImplOptions.NoInlining)]
        private static int GetNonRandomizedHashCodeWide<TCasing>(ReadOnlySpan<byte> span, int byteLength)
            where TCasing : struct, IHashCasing
        {
            ulong length = (uint)byteLength;
            ulong h0 = length + HashSeed1, h1 = length + HashSeed2;
            ulong h2 = length + ~HashSeed1, h3 = length + ~HashSeed2;
            ...
            return ascii
                ? GetNonRandomizedHashCodeLong<TCasing>(span, byteLength, h0, h1, h2, h3, hasState: true)
                : GetNonRandomizedHashCodeOrdinalIgnoreCaseSlow(span, byteLength, h0, h1, h2, h3);
        }
        ...
        private static int GetNonRandomizedHashCodeLong<TCasing>(ReadOnlySpan<byte> span, int byteLength,
            ...
            bool hasState = false) where TCasing : struct, IHashCasing
        {
            ulong length = (uint)byteLength;
            int initialLength = span.Length;
            bool seeded = initialLength != byteLength;
            if (Vector128.IsHardwareAccelerated)
            {
                Vector128<ulong> hash0 = seeded
                    ? Vector128.Create(h0, h1)
                    : Vector128.Create(length) + Vector128.Create(HashSeed1, HashSeed2);
                Vector128<ulong> hash1 = seeded
            ...
            uint result = hasState || initialLength >= 32 ? (uint)MixNonRandomizedHash(
            ...
            ulong h0 = 0, ulong h1 = 0, ulong h2 = 0, ulong h3 = 0, uint tailHash = 0)
        {
            ...
            try
            {
                ...
                if (byteLength <= 64)
                    return GetNonRandomizedHashCodeCore<UpperNormalizedHashing>(normalized);

                // Nothing had been absorbed yet if the very first block needed folding, so
                // the whole normalized input is still ahead and can take the wide loop.
                return normalized.Length == byteLength && byteLength >= WideHashThreshold
                    ? GetNonRandomizedHashCodeWide<UpperNormalizedHashing>(normalized, byteLength)
