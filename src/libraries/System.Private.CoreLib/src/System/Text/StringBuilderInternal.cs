...
{
    internal interface IStringBuilderInternal
    {
        void Append(char item);
        void Append(char item, int count);
        void Append(ReadOnlySpan<char> value);
        void UnsafeGrow(int size);
        Span<char> RemainingCurrentChunk { get; }
    }

    internal static class StringBuilderInternal
    {
        /// <inheritdoc cref="StringBuilder.AppendFormat(IFormatProvider?, string, ReadOnlySpan{object?})" />
        internal static void AppendFormatHelper<TBuilder>(ref TBuilder builder, IFormatProvider? provider, string format, ReadOnlySpan<object?> args)
            where TBuilder : IStringBuilderInternal, allows ref struct
        {
            ...
            while (true)
            {
                ...
                if (s == null)
                {
                    ...
                    if ((leftJustify || width == 0) &&
                        ...
                        spanFormattableArg.TryFormat(builder.RemainingCurrentChunk, out int charsWritten, itemFormatSpan, provider))
                    {
                        if ((uint)charsWritten > (uint)builder.RemainingCurrentChunk.Length)
                        {
                        ...
                        builder.UnsafeGrow(charsWritten);

                        // Pad the end, if needed.
                        if (leftJustify && width > charsWritten)
                        {
                            builder.Append(' ', width - charsWritten);
                        }
