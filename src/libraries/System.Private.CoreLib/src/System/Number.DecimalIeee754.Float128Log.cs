...
internal static partial class Number
{
    ...
    private static readonly Float128FixedCoefficient[] Log2Coefficients =
    [
    ...
    private static Float128 LogLn2 => new Float128(0, 0, 0xb17217f7d1cf79ab, 0xc9e3b39803f2f6af);
    private static Float128 LogLog10Of2 => new Float128(0, -1, 0x9a209a84fbcff798, 0x8f8959ac0b7c9178);
    private static Float128 LogTwo => new Float128(0, 2, 0x8000000000000000, 0);

    /// <summary>Converts a signed integer to an unpacked binary128 value (Intel's <c>WORD_TO_UX</c>).</summary>
    private static Float128 Float128FromWord(long n)
    {
        if (n == 0)
        {
            return new Float128(0, UxZeroExponent, 0, 0);
        }
        ...
        return new Float128(sign, 64 - shift, magnitude << shift, 0);
    }
    ...
    /// </summary>
    private static void Float128EvaluateLogPolynomial(scoped in Float128 arg, ReadOnlySpan<Float128FixedCoefficient> coefficients, int degree, int trailingExponent, out Float128 result)
    {
        Float128 a = arg;
        Float128Multiply(ref a, ref a, out Float128 argumentSquared);
        Float128Normalize(ref argumentSquared);

        long shift = -(long)degree * argumentSquared._exponent;
        Float128EvaluatePositivePolynomial(argumentSquared, shift, coefficients, 0, degree, out result);

        Float128 original = arg;
        Float128Multiply(ref original, ref result, out result);
    ...
    /// </summary>
    private static Float128 Float128Log(Float128 arg, bool scaleValid, scoped in Float128 scale)
    {
        ...
        Float128AddSub(arg, Float128One, UxAddSub | UxMagnitudeOnly, tmp); // tmp[0] = g + 1, tmp[1] = g - 1

        Float128Divide(tmp[1], tmp[0], Float128FullPrecision, out Float128 z);
        Float128EvaluateLogPolynomial(z, Log2Coefficients, Log2Degree, Log2TrailingExponent, out Float128 poly);

        Float128 result = Float128FromWord(m);
        Span<Float128> sum = stackalloc Float128[1];
        Float128AddSub(result, poly, UxAdd | UxNoNormalization, sum);
        result = sum[0];

        if (scaleValid)
        {
            Float128 s = scale;
            Float128Multiply(ref result, ref s, out result);
        }

        return result;
    }

    private static Float128 Float128Ln(scoped in Float128 arg) => Float128Log(arg, scaleValid: true, LogLn2);
    private static Float128 Float128Log2(scoped in Float128 arg) => Float128Log(arg, scaleValid: false, default);
    ...
    /// <paramref name="scaleValid"/> selects the base exactly as in <see cref="Float128Log"/>.
    /// </summary>
    private static Float128 Float128Log1p(Float128 arg, bool scaleValid, scoped in Float128 scale)
    {
        ...
        if (small)
        {
            ...
            Float128AddSub(LogTwo, arg, UxAdd, t);
            Float128Divide(arg, t[0], Float128FullPrecision, out Float128 reduced);
            Float128EvaluateLogPolynomial(reduced, Log2Coefficients, Log2Degree, Log2TrailingExponent, out Float128 result);
        ...
        {
            Span<Float128> t = stackalloc Float128[1];
            Float128AddSub(Float128One, arg, UxAdd, t);
            return Float128Log(t[0], scaleValid, scale);
        }
    }

    private static Float128 Float128Ln1p(scoped in Float128 arg) => Float128Log1p(arg, scaleValid: true, LogLn2);
    private static Float128 Float128Log2P1(scoped in Float128 arg) => Float128Log1p(arg, scaleValid: false, default);
    private static Float128 Float128Log10P1(scoped in Float128 arg) => Float128Log1p(arg, scaleValid: true, LogLog10Of2);
}
