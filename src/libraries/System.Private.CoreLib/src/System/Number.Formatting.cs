...
namespace System
{
    ...
    internal static partial class Number
    {
        ...
        public static string FormatDecimal(decimal value, ReadOnlySpan<char> format, NumberFormatInfo info)
        {
            ...
            Span<byte> digitsBuffer = stackalloc byte[DecimalNumberBufferLength];
            NumberBuffer number = new NumberBuffer(NumberBufferKind.Decimal, digitsBuffer);
        ...
        public static string FormatInt32(int value, int hexMask, string? format, IFormatProvider? provider)
        {
            ...
            static string FormatInt32Slow(int value, int hexMask, string? format, IFormatProvider? provider)
            {
                ...
                else
                {
                    ...
                    Span<byte> digitsBuffer = stackalloc byte[Int32NumberBufferLength];
                    NumberBuffer number = new NumberBuffer(NumberBufferKind.Integer, digitsBuffer);
        ...
        public static string FormatUInt32(uint value, string? format, IFormatProvider? provider)
        {
            ...
            static string FormatUInt32Slow(uint value, string? format, IFormatProvider? provider)
            {
                ...
                else
                {
                    ...
                    Span<byte> digitsBuffer = stackalloc byte[UInt32NumberBufferLength];
                    NumberBuffer number = new NumberBuffer(NumberBufferKind.Integer, digitsBuffer);
        ...
        public static string FormatInt64(long value, string? format, IFormatProvider? provider)
        {
            ...
            static string FormatInt64Slow(long value, string? format, IFormatProvider? provider)
            {
                ...
                else
                {
                    ...
                    Span<byte> digitsBuffer = stackalloc byte[Int64NumberBufferLength];
                    NumberBuffer number = new NumberBuffer(NumberBufferKind.Integer, digitsBuffer);
        ...
        public static string FormatUInt64(ulong value, string? format, IFormatProvider? provider)
        {
            ...
            static string FormatUInt64Slow(ulong value, string? format, IFormatProvider? provider)
            {
                ...
                else
                {
                    ...
                    Span<byte> digitsBuffer = stackalloc byte[UInt64NumberBufferLength];
                    NumberBuffer number = new NumberBuffer(NumberBufferKind.Integer, digitsBuffer);
        ...
        private static bool TryUInt32ToDecStr(uint value, int digits, Span<char> destination, out int charsWritten)
        {
            ...
            if (digits <= 1)
            {
                do
                {
                    uint remainder;
                    (value, remainder) = Math.DivRem(value, 10);
                    destination[--p] = (char)(remainder + '0');
                }
                while (value != 0);
            }
            else
                    ...
                    switch (ch)
                    {
                        ...
                        case '.':
                            while (adjust > 0)
                            {
                                ...
                                sb.Append(dig[cur] != 0 ? (char)(dig[cur++]) : '0');
        ...
        private static void FormatFixed(ref ValueStringBuilder sb, ref NumberBuffer number, int nMaxDigits, int[]? groupDigits, string? sDecimal, string? sGroup)
        {
            ...
            if (digPos > 0)
            {
                if (groupDigits != null)
                {
                    ...
                    Span<char> spanPtr = sb.AppendSpan(bufferSize);
                    int p = bufferSize - 1;
                    for (int i = digPos - 1; i >= 0; i--)
                    {
                        spanPtr[p--] = (i < digStart) ? (char)(dig[i]) : '0';
                    ...
                    dig = dig.Slice(digStart);
                }
                else
                {
                    do
                    {
                        sb.Append(dig[digIndex] != 0 ? (char)(dig[digIndex++]) : '0');
                    }
                    while (--digPos > 0);
                }
            }
        ...
        internal static void RoundNumber(ref NumberBuffer number, int pos, bool isCorrectlyRounded)
        {
            ...
            static bool ShouldRoundUp(Span<byte> dig, int i, NumberBufferKind numberKind, bool isCorrectlyRounded)
            {
