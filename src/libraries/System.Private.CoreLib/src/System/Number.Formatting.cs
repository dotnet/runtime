...
namespace System
{
    ...
    internal static partial class Number
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static unsafe bool FormatNumber<TChar>(ref NumberBuffer number, char fmt, int digits, ReadOnlySpan<char> format, NumberFormatInfo info, Span<TChar> destination, out int charsWritten)
            where TChar : unmanaged, IUtfChar<TChar>
        {
            TChar* stackPtr = stackalloc TChar[CharStackBufferSize];
            var vlb = new ValueListBuilder<TChar>(new Span<TChar>(stackPtr, CharStackBufferSize));
