...
namespace System.Threading
{
    public abstract partial class WaitHandle
    {
        ...
        private static unsafe int WaitOneCoreCore(IntPtr handle, int millisecondsTimeout)
        {
            return WaitForMultipleObjectsIgnoringSyncContext(&handle, 1, false, millisecondsTimeout);
        }

        private static int SignalAndWaitCoreCore(IntPtr handleToSignal, IntPtr handleToWaitOn, int millisecondsTimeout)
        {
            Debug.Assert(millisecondsTimeout >= -1);
        ...
        private static Exception ExceptionFromCreationErrorCore(int errorCode, string path)
        {
            switch (errorCode)
            {
