...
public partial interface IFloatingPoint<TSelf, That> : IScalarNumber<TSelf, That>
    where TSelf : IFloatingPoint<TSelf, That>
{
    #region Constants
    ...
    /// <summary> Represents <c>NaN</c>, an unusable Number.</summary>
    /// <remarks>
    /// NaN are produced by Operations like 0/0 or Infinity/Infinity
    /// that can only be well-defined with a converging Limit.
    ///
    /// NaN propagate through all Operations, instead of throwing Exceptions.
    /// NaN also go largely unnoticed, unlike e.g. alternative <see cref="Nullable{T}"/>.
    /// </remarks>
    ...
    /// <summary>Gets the mathematical constant <c>tau</c> = 2*<see cref="Pi"/>.</summary>
    /// <remarks>
    /// Introduced in python 3.6 and .NET Core 3.
    /// </remarks>
    static abstract TSelf Tau { get; }

    /// <summary> 1/0 the Reciprocal of 0. A single value that represents (positive) <c>infinity</c>.</summary>
    static abstract TSelf PositiveInfinity { get; }

    /// <summary> -1/0 = Reciprocal of -0 representing negative <c>infinity</c>.</summary>
    /// <remarks>
    /// This is needed when trying to carry over the Direction in arithmetic Operations.
    ///
    /// The Sign of Infinity gets lost in Transformations,
    /// so often a projective Representation is sufficient.
    /// </remarks>
    static abstract TSelf NegativeInfinity { get; }

    /// <summary> -0 represents a numeric Underflow on the negative side of 0.</summary>
    /// <remarks> -0 is the Reciprocal of -<see cref="NegativeInfinity"/>. </remarks>
