          ...
          IBinaryIntegerParseAndFormatInfo<int>
    {
        ...
        static bool IBinaryInteger<int>.TryReadLittleEndian(ReadOnlySpan<byte> source, bool isUnsigned, out int value)
        {
            ...
            if (source.Length != 0)
            {
                ...
                {
                    // We have at least 4 bytes, so just read the ones we need directly
                    result = BinaryPrimitives.ReadInt32LittleEndian(source);
                }
                else
                {
                    // We have between 1 and 3 bytes, so construct the relevant value directly
        ...
        int IBinaryInteger<int>.WriteBigEndian(byte[] destination) => BinaryIntegerWriter.WriteBigEndian(this, destination);

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteBigEndian(byte[], int)" />
        int IBinaryInteger<int>.WriteBigEndian(byte[] destination, int startIndex) => BinaryIntegerWriter.WriteBigEndian(this, destination.AsSpan(startIndex));

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteBigEndian(Span{byte})" />
        int IBinaryInteger<int>.WriteBigEndian(Span<byte> destination) => BinaryIntegerWriter.WriteBigEndian(this, destination);

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteLittleEndian(byte[])" />
        int IBinaryInteger<int>.WriteLittleEndian(byte[] destination) => BinaryIntegerWriter.WriteLittleEndian(this, destination);

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteLittleEndian(byte[], int)" />
        int IBinaryInteger<int>.WriteLittleEndian(byte[] destination, int startIndex) => BinaryIntegerWriter.WriteLittleEndian(this, destination.AsSpan(startIndex));

        /// <inheritdoc cref="IBinaryInteger{TSelf}.WriteLittleEndian(Span{byte})" />
        int IBinaryInteger<int>.WriteLittleEndian(Span<byte> destination) => BinaryIntegerWriter.WriteLittleEndian(this, destination);
        ...
        public static bool TryParsePartial([NotNullWhen(true)] string? s, NumberStyles style, IFormatProvider? provider, out int result, out int charsConsumed)
        {
            NumberFormatInfo.ValidateParseStyleInteger(style);
        ...
        public static bool TryParsePartial(ReadOnlySpan<char> s, NumberStyles style, IFormatProvider? provider, out int result, out int charsConsumed)
        {
            NumberFormatInfo.ValidateParseStyleInteger(style);
        ...
        public static bool TryParsePartial(ReadOnlySpan<byte> utf8Text, NumberStyles style, IFormatProvider? provider, out int result, out int bytesConsumed)
        {
            NumberFormatInfo.ValidateParseStyleInteger(style);
