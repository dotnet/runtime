    ...
    public class ToStringTest
    {
        ...
        static void VerifyTryFormatUtf8(string test, string format, IFormatProvider provider, bool expectError, string expectedResult)
        {
            try
            {
                ...
                byte[] utf8Destination = expectedResult != null ? new byte[expectedResult.Length] : Array.Empty<byte>();
                ...
                if (expectedResult.Length > 0)
                {
                    Assert.False(bi.TryFormat(new byte[expectedResult.Length - 1], out bytesWritten, format, provider));
