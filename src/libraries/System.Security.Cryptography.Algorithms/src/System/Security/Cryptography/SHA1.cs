...
namespace System.Security.Cryptography
{
    ...
    public abstract class SHA1 : HashAlgorithm
    {
        ...
        /// <summary>
        /// Async Computes the hash of data using the SHA1 algorithm.
        /// </summary>
        /// <param name="source">The data to hash.</param>
        ...
        public static async Task<(bool IsSuccess, int BytesWritten)> TryHashDataAsync(byte[] source, byte[] destination, CancellationToken cancellationToken = default)
        {
            ...
            int bytesWritten = await HashProviderDispenser.OneShotHashProvider.HashDataAsync(HashAlgorithmNames.SHA1, source, destination, cancellationToken).ConfigureAwait(false);
            Debug.Assert(bytesWritten == HashSizeBytes);
