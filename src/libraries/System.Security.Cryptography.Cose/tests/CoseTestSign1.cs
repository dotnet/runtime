    ...
    public sealed class CoseTestSign1
    {
        ...
        public static IEnumerable<CoseTestSign1> GetImplementations()
        {
            ...
            yield return new(true, "Sign/VerifyEmbedded(ROS<byte>)",
                ...
                (key, _, message) => message.VerifyEmbedded(key.KeyAsymmetricAlgorithm, ReadOnlySpan<byte>.Empty));
            ...
            yield return new(false, "Sign/VerifyDetached(byte[])",
                ...
                (key, payload, message) => message.VerifyDetached(key.KeyAsymmetricAlgorithm, payload, Array.Empty<byte>()));
            ...
            yield return new(false, "Sign/VerifyDetached(ROS<byte>)",
                ...
                (key, payload, message) => message.VerifyDetached(key.KeyAsymmetricAlgorithm, payload.AsSpan(), ReadOnlySpan<byte>.Empty));
            ...
            yield return new(false, "Sign/VerifyDetached(Stream)",
                ...
                {
                    using MemoryStream stream = new MemoryStream(payload);
                    return message.VerifyDetached(key.KeyAsymmetricAlgorithm, stream, ReadOnlySpan<byte>.Empty);
                });
            ...
            yield return new(false, "Sign/VerifyDetachedAsync(Stream)",
                ...
                {
                    using MemoryStream stream = new MemoryStream(payload);
                    return message.VerifyDetachedAsync(key.KeyAsymmetricAlgorithm, stream, ReadOnlyMemory<byte>.Empty).GetAwaiter().GetResult();
                });
        }
