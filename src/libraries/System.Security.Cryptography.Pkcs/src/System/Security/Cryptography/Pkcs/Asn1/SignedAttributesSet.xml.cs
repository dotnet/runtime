...
namespace System.Security.Cryptography.Pkcs
{
    ...
    [StructLayout(LayoutKind.Sequential)]
    internal ref partial struct ValueSignedAttributesSet
    {
        internal ReadOnlySpan<byte> SignedAttributes;
        internal bool HasSignedAttributes;

        internal static void Decode(ReadOnlySpan<byte> encoded, AsnEncodingRules ruleSet, out ValueSignedAttributesSet decoded)
        {
            try
            {
        ...
        internal static void Decode(scoped ref ValueAsnReader reader, out ValueSignedAttributesSet decoded)
        {
            try
            {
                DecodeCore(ref reader, out decoded);
        ...
        private static void DecodeCore(scoped ref ValueAsnReader reader, out ValueSignedAttributesSet decoded)
        {
            decoded = default;
            Asn1Tag tag = reader.PeekTag();
            ...
            if (tag.HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 0)))
            {
                decoded.SignedAttributes = reader.ReadEncodedValue();
