...
namespace System.Security.Cryptography.Pkcs.Asn1
{
    ...
    internal partial struct EnvelopedDataAsn
    {
        ...
        internal static void Decode(ref ValueAsnReader reader, ReadOnlyMemory<byte> rebind, out EnvelopedDataAsn decoded)
        {
            Decode(ref reader, Asn1Tag.Sequence, rebind, out decoded);
        }

        internal static void Decode(ref ValueAsnReader reader, Asn1Tag expectedTag, ReadOnlyMemory<byte> rebind, out EnvelopedDataAsn decoded)
        {
            try
            {
        ...
        private static void DecodeCore(ref ValueAsnReader reader, Asn1Tag expectedTag, ReadOnlyMemory<byte> rebind, out EnvelopedDataAsn decoded)
        {
            decoded = default;
    ...
    [StructLayout(LayoutKind.Sequential)]
    internal ref partial struct ValueEnvelopedDataAsn
    {
        internal int Version;
        internal ReadOnlySpan<byte> OriginatorInfo;
        internal bool HasOriginatorInfo;
        internal ReadOnlySpan<byte> RecipientInfos;
        ...
        internal static void Decode(ReadOnlySpan<byte> encoded, AsnEncodingRules ruleSet, out ValueEnvelopedDataAsn decoded)
        {
            Decode(Asn1Tag.Sequence, encoded, ruleSet, out decoded);
        }

        internal static void Decode(Asn1Tag expectedTag, ReadOnlySpan<byte> encoded, AsnEncodingRules ruleSet, out ValueEnvelopedDataAsn decoded)
        {
            try
            {
        ...
        internal static void Decode(scoped ref ValueAsnReader reader, out ValueEnvelopedDataAsn decoded)
        {
            Decode(ref reader, Asn1Tag.Sequence, out decoded);
        }

        internal static void Decode(scoped ref ValueAsnReader reader, Asn1Tag expectedTag, out ValueEnvelopedDataAsn decoded)
        {
            try
            {
                DecodeCore(ref reader, expectedTag, out decoded);
        ...
        private static void DecodeCore(scoped ref ValueAsnReader reader, Asn1Tag expectedTag, out ValueEnvelopedDataAsn decoded)
        {
            decoded = default;
            ValueAsnReader sequenceReader = reader.ReadSequence(expectedTag);
            ...
            if (sequenceReader.HasData && sequenceReader.PeekTag().HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 0)))
            {
                decoded.OriginatorInfo = sequenceReader.ReadEncodedValue();
                decoded.HasOriginatorInfo = true;
            }

            decoded.RecipientInfos = sequenceReader.ReadEncodedValue();
