...
namespace System.Security.Cryptography.Pkcs.Asn1
{
    ...
    [StructLayout(LayoutKind.Sequential)]
    internal ref partial struct ValueOriginatorIdentifierOrKeyAsn
    {
        internal ReadOnlySpan<byte> IssuerAndSerialNumber;
        internal bool HasIssuerAndSerialNumber;
        internal ReadOnlySpan<byte> SubjectKeyIdentifier;
        internal bool HasSubjectKeyIdentifier;
        internal ReadOnlySpan<byte> OriginatorKey;
        internal bool HasOriginatorKey;

        internal static void Decode(ReadOnlySpan<byte> encoded, AsnEncodingRules ruleSet, out ValueOriginatorIdentifierOrKeyAsn decoded)
        {
            try
            {
        ...
        internal static void Decode(scoped ref ValueAsnReader reader, out ValueOriginatorIdentifierOrKeyAsn decoded)
        {
            try
            {
                DecodeCore(ref reader, out decoded);
        ...
        private static void DecodeCore(scoped ref ValueAsnReader reader, out ValueOriginatorIdentifierOrKeyAsn decoded)
        {
            decoded = default;
            Asn1Tag tag = reader.PeekTag();
            ReadOnlySpan<byte> tmpSpan;
            ...
            if (tag.HasSameClassAndValue(Asn1Tag.Sequence))
            {
                decoded.IssuerAndSerialNumber = reader.ReadEncodedValue();
                decoded.HasIssuerAndSerialNumber = true;
            }
            else if (tag.HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 0)))
            {
                ...
                decoded.HasSubjectKeyIdentifier = true;
            }
            else if (tag.HasSameClassAndValue(new Asn1Tag(TagClass.ContextSpecific, 1)))
            {
                decoded.OriginatorKey = reader.ReadEncodedValue();
                decoded.HasOriginatorKey = true;
            }
            else
            {
                throw new CryptographicException();
