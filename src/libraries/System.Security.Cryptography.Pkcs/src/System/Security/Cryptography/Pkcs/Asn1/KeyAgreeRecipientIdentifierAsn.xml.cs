...
namespace System.Security.Cryptography.Pkcs.Asn1
{
    ...
    [StructLayout(LayoutKind.Sequential)]
    internal ref partial struct ValueKeyAgreeRecipientIdentifierAsn
    {
        internal ReadOnlySpan<byte> IssuerAndSerialNumber;
        internal bool HasIssuerAndSerialNumber;
        internal ReadOnlySpan<byte> RKeyId;
        internal bool HasRKeyId;

        internal static void Decode(ReadOnlySpan<byte> encoded, AsnEncodingRules ruleSet, out ValueKeyAgreeRecipientIdentifierAsn decoded)
        {
            try
            {
        ...
        internal static void Decode(scoped ref ValueAsnReader reader, out ValueKeyAgreeRecipientIdentifierAsn decoded)
        {
            try
            {
                DecodeCore(ref reader, out decoded);
        ...
        private static void DecodeCore(scoped ref ValueAsnReader reader, out ValueKeyAgreeRecipientIdentifierAsn decoded)
        {
            decoded = default;
            Asn1Tag tag = reader.PeekTag();
            ...
            {
                decoded.RKeyId = reader.ReadEncodedValue();
                decoded.HasRKeyId = true;
            }
            else
            {
                throw new CryptographicException();
