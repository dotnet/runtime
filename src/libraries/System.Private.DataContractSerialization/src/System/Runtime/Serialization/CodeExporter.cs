                ...
                foreach (KeyValuePair<string, string> pair in _options.Namespaces)
                {
                    ...
                    if (_clrNamespaces.TryGetValue(clrNamespace, out currentDataContractNamespace))
                    {
                        if (dataContractNamespace != currentDataContractNamespace)
                            throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.Format(SR.CLRNamespaceMappedMultipleTimes, currentDataContractNamespace, dataContractNamespace, clrNamespace)));
                    }
                    else
...
#pragma warning restore IL3000 // Avoid accessing Assembly file path when publishing as a single file
                ...
                foreach (KeyValuePair<XmlQualifiedName, DataContract> pair in _dataContractSet)
                {
                    ...
                    if (!contractCodeDomInfo.IsProcessed)
                    {
                        if (dataContract is ClassDataContract)
                        {
                            ...
                            throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SR.Format(SR.UnexpectedContractType, DataContract.GetClrTypeFullName(dataContract.GetType()), DataContract.GetClrTypeFullName(dataContract.UnderlyingType))));
        ...
        private void InvokeProcessImportedType(CollectionBase collection)
        {
            ...
            Debug.Assert(_dataContractSet.SerializationExtendedSurrogateProvider != null);   // The only caller into this method is inside a null check for the surrogate provider

            foreach (object obj in objects)
            {
                if (obj is CodeTypeDeclaration codeTypeDeclaration)
                {
                    CodeTypeDeclaration? newCodeTypeDeclaration = DataContractSurrogateCaller.ProcessImportedType(_dataContractSet.SerializationExtendedSurrogateProvider, codeTypeDeclaration, _codeCompileUnit);
...
#else
        private static CollectionDataContract GenericListContract
        {
            [RequiresUnreferencedCode(DataContract.SerializerTrimmerWarning)]
...
#else
        private static CollectionDataContract GenericDictionaryContract
        {
            [RequiresUnreferencedCode(DataContract.SerializerTrimmerWarning)]
        ...
        private CodeTypeReference? GetReferencedType(DataContract dataContract)
        {
...
#endif
                                throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.Format(SR.ReferencedTypeDoesNotMatch, type.AssemblyQualifiedName, dataContract.StableName.Name, dataContract.StableName.Namespace)));
                        }
                        else
                        {
            ...
            else if (dataContract.GenericInfo != null)
            {
                ...
                if (referencedContract != null && !referencedContract.Equals(dataContract))
                {
                    ...
                    throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.Format(SR.ReferencedTypeDoesNotMatch,
        ...
        private static void ThrowIfReferencedBaseTypeSealed(Type baseType, DataContract dataContract)
        {
            if (baseType.IsSealed)
                throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.Format(SR.CannotDeriveFromSealedReferenceType, dataContract.StableName.Name, dataContract.StableName.Namespace, DataContract.GetClrTypeFullName(baseType))));
        }
            ...
            if (!foundDictionaryBase)
            {
                ...
                if (!TryGetReferencedListType(itemContract, isItemTypeNullable, out baseTypeReference))
                {
                    ...
                    else
                    {
                        ...
                        throw System.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SR.Format(SR.ReferencedBaseTypeDoesNotExist,
