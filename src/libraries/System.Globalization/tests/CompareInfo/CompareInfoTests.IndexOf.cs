    ...
    public class CompareInfoIndexOfTests
    {
        ...
        public void IndexOf_String(CompareInfo compareInfo, string source, string value, int startIndex, int count, CompareOptions options, int expected)
        {
            ...
            static void RunSpanIndexOfTest(CompareInfo compareInfo, ReadOnlySpan<char> source, ReadOnlySpan<char> value, CompareOptions options, int expected)
            {
                ...
                Assert.Equal(expected, compareInfo.IndexOfNew(sourceBoundedMemory.Span, valueBoundedMemory.Span, options));

                if (TryCreateRuneFrom(value, out Rune rune))
                {
                    Assert.Equal(expected, compareInfo.IndexOfNew(sourceBoundedMemory.Span, rune, options)); // try the Rune-based version
                }
            }
        }
