...
public sealed partial class QuicListener : IAsyncDisposable
{
    ...
    private unsafe int HandleEventNewConnection(ref NEW_CONNECTION_DATA data)
    {
        ...
        SslClientHelloInfo clientHello = new SslClientHelloInfo(ata.Info->ServerNameLength > 0 ? Encoding.UTF8.GetString((byte*)data.Info->ServerName, data.Info->ServerNameLength) : "", SslProtocols.Tls13);
