...
namespace Microsoft.Quic
{
    public class QuicException : Exception
    {
        ...
        public QuicException(int status, string? message = null, Exception? innerException = null)
            : base($"{(message ?? nameof(QuicException))}: {GetErrorCodeForStatus(status)}", innerException)
        {
            Status = status;
        }
