    ...
    private unsafe int HandleEventShutdownComplete(ref SHUTDOWN_COMPLETE_DATA data)
    {
        if (data.ConnectionShutdown != 0)
        {
            ...
            Exception exception = (shutdownByApp, closedRemotely) switch
            {
                ...
                (shutdownByApp: false, closedRemotely: true) => ThrowHelper.GetExceptionForMsQuicStatus(data.ConnectionCloseStatus, data.ConnectionErrorCode, $"Shutdown by transport {data.ConnectionErrorCode}"),
                // It's local shutdown by transport, most likely due to a timeout, throw error based on the status.
                (shutdownByApp: false, closedRemotely: false) => ThrowHelper.GetExceptionForMsQuicStatus(data.ConnectionCloseStatus, data.ConnectionErrorCode),
            };
            _startedTcs.TrySetException(exception);
