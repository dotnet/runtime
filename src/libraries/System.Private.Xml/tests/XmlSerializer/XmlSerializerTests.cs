...
public static partial class XmlSerializerTests
{
    ...
    public static void Xml_TypeWithDateTimePropertyAsXmlTime()
    {
        ...
        Assert.Equal(localTimeObject.Value, localTimeObjectRoundTrip.Value);
        ...
        Assert.Equal(utcTimeObject.Value, utcTimeRoundTrip.Value);
    }
...
<ArrayOfAnyType xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  ...
  <anyType xsi:type=""xsd:string"">a2</anyType>
...
#if !XMLSERIALIZERGENERATORTESTS
    [Theory]
    [MemberData(nameof(Xml_ReadOnlyCollectionAsRoot_MemberData))]
    public static void Xml_ReadOnlyCollectionAsRoot(Type type, object collection, string expectedXml, object[] expectedItems)
    {
        var serializer = new XmlSerializer(type);
        string actualXml = Serialize(collection, expectedXml, () => serializer, skipStringCompare: expectedXml == null);
        object roundTripped = Deserialize(serializer, actualXml);

        Assert.NotNull(roundTripped);
        Assert.IsAssignableFrom(type, roundTripped);
        // Not every read-only collection overrides Equals, so compare contents rather than instances.
        Assert.Equal(expectedItems, ((IEnumerable)roundTripped).Cast<object>());
    }
    public static IEnumerable<object[]> Xml_ReadOnlyCollectionAsRoot_MemberData()
    {
        static string ArrayOfInt(params int[] values) => WithXmlHeader(
            $"<ArrayOfInt xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\">{string.Concat(values.Select(v => $"<int>{v}</int>"))}</ArrayOfInt>");

        object[] oneTwoThree = new object[] { 1, 2, 3 };

        yield return new object[] { typeof(ImmutableArray<int>), ImmutableArray.Create(1, 2, 3), ArrayOfInt(1, 2, 3), oneTwoThree };
        yield return new object[] { typeof(ImmutableList<int>), ImmutableList.Create(1, 2, 3), ArrayOfInt(1, 2, 3), oneTwoThree };
        yield return new object[] { typeof(ImmutableSortedSet<int>), ImmutableSortedSet.Create(1, 2, 3), ArrayOfInt(1, 2, 3), oneTwoThree };
        yield return new object[] { typeof(ImmutableQueue<int>), ImmutableQueue.Create(1, 2, 3), ArrayOfInt(1, 2, 3), oneTwoThree };
        yield return new object[] { typeof(ReadOnlyCollection<int>), new ReadOnlyCollection<int>(new int[] { 1, 2, 3 }), ArrayOfInt(1, 2, 3), oneTwoThree };

        // A stack enumerates newest-first, so writing one out and reading it back reverses it. This matches how a
        // C# collection expression builds a stack, and is the same round-trip a caller would get from
        // ImmutableStack.CreateRange(stack).
        yield return new object[] { typeof(ImmutableStack<int>), ImmutableStack.Create(1, 2, 3), ArrayOfInt(3, 2, 1), oneTwoThree };

        // Sets have no ordering guarantee, so their serialized form isn't compared against a baseline.
        yield return new object[] { typeof(ImmutableHashSet<int>), ImmutableHashSet.Create(1), ArrayOfInt(1), new object[] { 1 } };
        yield return new object[] { typeof(FrozenSet<int>), FrozenSet.Create(1), ArrayOfInt(1), new object[] { 1 } };
        yield return new object[] { typeof(ReadOnlySet<int>), new ReadOnlySet<int>(new HashSet<int>() { 1 }), ArrayOfInt(1), new object[] { 1 } };

        yield return new object[] { typeof(ReadOnlyCollection<string>),
            new ReadOnlyCollection<string>(new string[] { "one", "two" }),
            WithXmlHeader("<ArrayOfString xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\"><string>one</string><string>two</string></ArrayOfString>"),
            new object[] { "one", "two" } };

        // Nullable enum elements used to fail outright. (dotnet/runtime#66264)
        yield return new object[] { typeof(ImmutableArray<ConsoleColor?>),
            ImmutableArray.Create<ConsoleColor?>(ConsoleColor.Blue, null, ConsoleColor.Red),
            null,
            new object[] { ConsoleColor.Blue, null, ConsoleColor.Red } };

        yield return new object[] { typeof(ImmutableArray<object>), ImmutableArray.Create<object>(), null, Array.Empty<object>() };
    }

    [Theory]
    [InlineData(true)]
    [InlineData(false)]
    public static void Xml_ReadOnlyCollectionAsRoot_EmptyAndNil(bool nil)
    {
        var serializer = new XmlSerializer(typeof(ImmutableArray<int>));
        string xml = WithXmlHeader(nil
            ? "<ArrayOfInt xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" xsi:nil=\"true\" />"
            : "<ArrayOfInt xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xmlns:xsd=\"http://www.w3.org/2001/XMLSchema\" />");

        var roundTripped = (ImmutableArray<int>)Deserialize(serializer, xml);

        Assert.False(roundTripped.IsDefault);
        Assert.Empty(roundTripped);
    }

    // A nil collection deserializes to an empty collection rather than to null, which is how XmlSerializer has
    // always treated the collections it populates in place. List<int> is here as the control.
    [Theory]
#if !ReflectionOnly
    // The reflection based reader leaves a nil root level mutable collection null. That is a pre-existing
    // difference from the IL generating reader which this change deliberately leaves alone.
    ...
    public static void Xml_NilCollectionAsRoot_BecomesEmpty(Type type)
    {
        var serializer = new XmlSerializer(type);
        object roundTripped = Deserialize(serializer, WithXmlHeader(
            "<ArrayOfInt xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:nil=\"true\" />"));

        Assert.NotNull(roundTripped);
        Assert.Empty((IEnumerable)roundTripped);
    }
#endif  // !XMLSERIALIZERGENERATORTESTS

    [Fact]
    public static void Xml_ReadOnlyCollectionAsMember()
    {
        var value = new TypeWithReadOnlyCollections()
        {
            ImmutableArray = ImmutableArray.Create(1, 2, 3),
            ImmutableList = ImmutableList.Create("one", "two"),
        ...
        Assert.Equal(value.ImmutableArray, roundTripped.ImmutableArray);
        Assert.Equal(value.ImmutableList, roundTripped.ImmutableList);
    ...
    [Fact]
    public static void Xml_GetOnlyReadOnlyCollectionAsMember_KeepsGetterValue()
    {
        // A collection created from its complete contents cannot be added to, and a get-only member has nowhere
        // to store a new instance, so the elements read are dropped and the getter's value survives. This matches
        // how a get-only collection that cannot be added to has always behaved.
        var serializer = new XmlSerializer(typeof(TypeWithGetOnlyReadOnlyCollections));
...
#if !XMLSERIALIZERGENERATORTESTS
    [Theory]
    [MemberData(nameof(Xml_UnsupportedReadOnlyCollections_MemberData))]
    public static void Xml_UnsupportedReadOnlyCollections(Type type, Type expectedException, string expectedMessage)
    {
#if ReflectionOnly
        // The reflection based serializer defers the failure until the serializer is first used. Deserializing
        // avoids needing an instance of a type that cannot be constructed in the first place.
        var serializer = new XmlSerializer(type);
        var ex = Assert.Throws(expectedException, () => Deserialize(serializer, WithXmlHeader("<Root />")));
#else
        var ex = Assert.Throws(expectedException, () => new XmlSerializer(type));
#endif
        Assert.Contains(expectedMessage, $"{ex.Message} : {ex.InnerException?.Message}");
    }
    public static IEnumerable<object[]> Xml_UnsupportedReadOnlyCollections_MemberData()
    {
        // Dictionaries stay unsupported whether or not they carry a collection builder.
#if ReflectionOnly
        ...
        yield return new object[] { typeof(FrozenDictionary<string, int>), typeof(InvalidOperationException), "is not supported because it implements IDictionary." };
#else
        ...
        yield return new object[] { typeof(FrozenDictionary<string, int>), typeof(NotSupportedException), "is not supported because it implements IDictionary." };
#endif
        // ImmutableSortedDictionary is doubly unsupported: it is a dictionary, and it has no collection builder.
        yield return new object[] { typeof(ImmutableSortedDictionary<string, int>),
#if ReflectionOnly
...
@"<?xml version=""1.0""?>
<MyEnum>Two</MyEnum>"));
        Assert.Equal(ByteEnum.Option1, SerializeAndDeserialize<ByteEnum>(ByteEnum.Option1,
@"<?xml version=""1.0""?>
<ByteEnum>Option1</ByteEnum>"));
        Assert.Equal(SByteEnum.Option1, SerializeAndDeserialize<SByteEnum>(SByteEnum.Option1,
@"<?xml version=""1.0""?>
<SByteEnum>Option1</SByteEnum>"));
        Assert.Equal(ShortEnum.Option1, SerializeAndDeserialize<ShortEnum>(ShortEnum.Option1,
@"<?xml version=""1.0""?>
<ShortEnum>Option1</ShortEnum>"));
        Assert.Equal(IntEnum.Option1, SerializeAndDeserialize<IntEnum>(IntEnum.Option1,
@"<?xml version=""1.0""?>
<IntEnum>Option1</IntEnum>"));
        Assert.Equal(UIntEnum.Option1, SerializeAndDeserialize<UIntEnum>(UIntEnum.Option1,
@"<?xml version=""1.0""?>
<UIntEnum>Option1</UIntEnum>"));
        Assert.Equal(LongEnum.Option1, SerializeAndDeserialize<LongEnum>(LongEnum.Option1,
@"<?xml version=""1.0""?>
<LongEnum>Option1</LongEnum>"));
        Assert.Equal(ULongEnum.Option1, SerializeAndDeserialize<ULongEnum>(ULongEnum.Option1,
@"<?xml version=""1.0""?>
...
<DCClassWithEnumAndStruct xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  ...
  <MyEnum1>One</MyEnum1>
        ...
        Assert.Equal(value.MyEnum1, actual.MyEnum1);
...
<ClassImplementsInterface xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  ...
  <IsLoaded>true</IsLoaded>
        ...
        Assert.Equal(value.IsLoaded, actual.IsLoaded);
    }
...
<AttributeTesting xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"" XmlAttributeName=""2"">
  ...
  <XmlEnumProperty>
    ...
    <ItemChoiceType>None</ItemChoiceType>
        ...
        Assert.Equal(actual.EnumType, value.EnumType);
        Assert.Equal(actual.MyChoice, value.MyChoice);
        object[] stringArray = actual.XmlArrayProperty.Where(x => x != null)
            .Select(x => x.ToString())
            .ToArray();
        Assert.Equal(stringArray, value.XmlArrayProperty);
        Assert.Equal(actual.XmlAttributeProperty, value.XmlAttributeProperty);
        Assert.Equal(actual.XmlElementProperty, value.XmlElementProperty);
        Assert.Equal(actual.XmlEnumProperty, value.XmlEnumProperty);
        Assert.Equal(actual.XmlIncludeProperty, value.XmlIncludeProperty);
    ...
    public static void Xml_XmlAnyAttributeTest()
    {
        ...
        using (var stream = new MemoryStream())
        {
            ...
            Assert.Equal(2, obj.Attributes.Length);
...
<WithStruct xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  <Some>
    ...
    <B>2</B>
        ...
        // Assert
        Assert.Equal(result.Some.A, value.Some.A);
        Assert.Equal(result.Some.B, value.Some.B);
    }
...
<ClassImplementingIXmlSerializable StringValue=""Hello world"" BoolValue=""True"" />");
        ...
        Assert.Equal(value.GetPrivateMember(), actual.GetPrivateMember());
...
@"<?xml version=""1.0""?><TypeWithPropertyNameSpecified xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema""><MyField>MyField</MyField></TypeWithPropertyNameSpecified>");
        ...
        Assert.Equal(0, actual.MyFieldIgnored);
    }
...
@"<?xml version=""1.0""?><TypeWithXmlSchemaFormAttribute xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema""><UnqualifiedSchemaFormListProperty><int>1</int></UnqualifiedSchemaFormListProperty><NoneSchemaFormListProperty><NoneParameter>abc</NoneParameter></NoneSchemaFormListProperty><QualifiedSchemaFormListProperty><QualifiedParameter>true</QualifiedParameter></QualifiedSchemaFormListProperty></TypeWithXmlSchemaFormAttribute>");
        ...
        Assert.Equal(value.NoneSchemaFormListProperty.Count, actual.NoneSchemaFormListProperty.Count);
        Assert.Equal(value.NoneSchemaFormListProperty[0], actual.NoneSchemaFormListProperty[0]);
        Assert.Equal(value.UnqualifiedSchemaFormListProperty.Count, actual.UnqualifiedSchemaFormListProperty.Count);
        Assert.Equal(value.UnqualifiedSchemaFormListProperty[0], actual.UnqualifiedSchemaFormListProperty[0]);
        Assert.Equal(value.QualifiedSchemaFormListProperty.Count, actual.QualifiedSchemaFormListProperty.Count);
        Assert.Equal(value.QualifiedSchemaFormListProperty[0], actual.QualifiedSchemaFormListProperty[0]);
    }
...
<TypeWithDateTimeOffsetProperties xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  ...
  <NullableDefaultDTO xsi:nil=""true"" />
        ...
        Assert.Equal(obj.DTO, deserializedObj.DTO);
        Assert.Equal(obj.DTO2, deserializedObj.DTO2);
        Assert.Equal(defDTO, deserializedObj.DTO2);
        Assert.Equal(obj.DTOWithDefault, deserializedObj.DTOWithDefault);
        Assert.Equal(defDTO, deserializedObj.DTOWithDefault);
        Assert.Equal(obj.NullableDTO, deserializedObj.NullableDTO);
        Assert.True(deserializedObj.NullableDTO == null);
        Assert.Equal(obj.NullableDTOWithDefault, deserializedObj.NullableDTOWithDefault);
    ...
    public static void Xml_TypeWithDateOnlyAndTimeOnly()
    {
        ...
        // Verify round trip
        Assert.Equal(doObj.Today, doResult.Today);
        Assert.Equal(doObj.CustomDate, doResult.CustomDate);
        Assert.Equal(defaultDateOnly, doResult.DefaultDate);
        Assert.Null(doResult.NullableDate);
        Assert.Equal(doObj.NullableDateWithValue, doResult.NullableDateWithValue);
        Assert.Null(doResult.NullableDefaultDate);
        Assert.Equal(doObj.Now, doResult.Now);
        Assert.Equal(doObj.CustomTime, doResult.CustomTime);
        Assert.Equal(defaultTimeOnly, doResult.DefaultTime);
    ...
    public static void Xml_XsdDate_With_DateOnly_And_DateTime()
    {
        ...
        Assert.Equal(now.Date, dtdObj.TestValue);
        ...
        Assert.Equal(DateOnly.FromDateTime(dtdObj.TestValue /* now */), doObj.TestValue);
        ...
        xml = Serialize(dtdObj, WithXmlHeader($"""
            ...
            """), () => dtdSerializer);
        doObj = (DateOnlyWrapper)Deserialize(doSerializer, xml);
        Assert.Equal(DateOnly.FromDateTime(dtdObj.TestValue /* localNow */), doObj.TestValue);
        ...
        xml = Serialize(dtdObj, WithXmlHeader($"""
            ...
            """), () => dtdSerializer);
        doObj = (DateOnlyWrapper)Deserialize(doSerializer, xml);
        Assert.Equal(DateOnly.FromDateTime(dtdObj.TestValue /* utcNow */), doObj.TestValue);
    }
    ...
    public static void Xml_XsdTime_With_TimeOnly_And_DateTime(string dateTimeString, DateTimeKind kind)
    {
        ...
        Assert.Equal(testTime.TimeOfDay, dttObj.TestValue.TimeOfDay);
        ...
        Assert.Equal(TimeOnly.FromDateTime(dttObj.TestValue /* now */), toaxtObj.TestValue);
    }
...
WithXmlHeader(@"<TypeWithByteProperty xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  <ByteProperty>123</ByteProperty>
        ...
        Assert.Equal(obj.ByteProperty, deserializedObj.ByteProperty);
    }
...
WithXmlHeader(@"<SimpleType xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  ...
  <P2>1</P2>
        ...
        Assert.Equal(obj.P2, deserializedObj.P2);
    }
...
<DerivedClassWithSameProperty2 xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  ...
  <ListProperty>
    ...
    <string>three</string>
        ...
        BaseClassWithSamePropertyName castAsBase = (BaseClassWithSamePropertyName)actual;
        Assert.Equal(default(DateTime), castAsBase.DateTimeProperty);
        Assert.Equal(default(int), castAsBase.IntProperty);
        Assert.Null(castAsBase.StringProperty);
        Assert.Null(castAsBase.ListProperty);
        ...
        Assert.Equal(value.IntProperty, castAsMiddle.IntProperty);
        Assert.Equal(value.StringProperty, castAsMiddle.StringProperty);
        // The other properties should be default
        Assert.Equal(default(DateTime), castAsMiddle.DateTimeProperty);
...
<DerivedClassWithSameProperty2 xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  ...
  <DateTimeProperty>0001-01-01T00:00:00.00001Z</DateTimeProperty>
    ...
    public static void Xml_TypeWithPropertiesHavingDefaultValue_DefaultValue()
    {
        ...
        Assert.Equal(value.IntProperty, actual.IntProperty);
        Assert.Equal(value.CharProperty, actual.CharProperty);
    }
    ...
    public static void Xml_TypeWithEnumPropertyHavingDefaultValue()
    {
        ...
        Assert.NotNull(actual);
        Assert.Equal(value.EnumProperty, actual.EnumProperty);
...
<TypeA xmlns:xsi=""http://www.w3.org/2001/XMLSchema-instance"" xmlns:xsd=""http://www.w3.org/2001/XMLSchema"">
  <Name>Lily&amp;Lucy</Name>
    ...
    public static void ValidateXElement()
    {
        ...
        Assert.Equal(wrapper.Value.Name, retWrapper.Value.Name);
        Assert.Equal(wrapper.Value.ToString(), retWrapper.Value.ToString());
    }
