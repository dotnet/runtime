...
namespace System.Numerics.Tests
{
    ...
    public unsafe class GenericVectorTests
    {
        ...
        private void TestAddSaturate<T>(T left, T right, T expected) where T : struct, INumber<T>
        {
            var actual = Vector.AddSaturate(Vector.Create(left), Vector.Create(right));
            ValidateVector(actual,
                (index, val) =>
                {
        ...
        private void TestSubtractSaturate<T>(T left, T right, T expected) where T : struct, INumber<T>
        {
            var actual = Vector.SubtractSaturate(Vector.Create(left), Vector.Create(right));
            ValidateVector(actual,
                (index, val) =>
                {
                    Assert.Equal(expected, val);
