    ...
    public sealed class ImageAttributes : ICloneable, IDisposable
    {
        ...
        public void SetRemapTable(ColorMap[] map, ColorAdjustType type)
        {
            ...
            try
            {
                ...
                {
                    Marshal.StructureToPtr(map[index].OldColor.ToArgb(), (nint)((long)memory + index * size * 2), false);
                    Marshal.StructureToPtr(map[index].NewColor.ToArgb(), (nint)((long)memory + index * size * 2 + size), false);
                }
