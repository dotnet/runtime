    ...
    public abstract class CalendarTestBase
    {
        ...
        public void GetEra_Invalid_ThrowsArgumentOutOfRangeException()
        {
            ...
            Assert.All(DateTime_TestData(calendar), dt =>
            {
                ...
                if ((calendar is JapaneseCalendar && (PlatformDetection.IsNlsGlobalization || PlatformDetection.IsHybridGlobalizationOnOSX)) || calendar is HebrewCalendar || calendar is TaiwanLunisolarCalendar || calendar is JapaneseLunisolarCalendar)
                {
                    calendar.GetEra(dt);
                }
                else
