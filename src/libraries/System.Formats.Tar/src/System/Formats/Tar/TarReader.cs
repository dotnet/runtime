    ...
    public sealed class TarReader : IDisposable
    {
        ...
        public TarEntry? GetNextEntry(bool copyData = false)
        {
            ...
            if (TryGetNextEntryHeader(out TarHeader header, copyData))
            {
                ...
                {
                    TarFormat.Pax => header._typeFlag is TarEntryType.GlobalExtendedAttributes ?
        ...
        private bool TryProcessExtendedAttributesHeader(TarHeader firstHeader, bool copyData, out TarHeader secondHeader)
        {
            ...
            // We're currently processing an extended attributes header, so we
            // can never have two extended entries in a row
            if (secondHeader._typeFlag is TarEntryType.GlobalExtendedAttributes or TarEntryType.ExtendedAttributes or TarEntryType.LongLink or TarEntryType.LongPath)
            {
                throw new FormatException(string.Format(SR.TarUnexpectedMetadataEntry, secondHeader._typeFlag, TarEntryType.ExtendedAttributes));
            }

            secondHeader._extendedAttributes = firstHeader._extendedAttributes ?? new Dictionary<string, string>();
            // Replace all the attributes representing standard fields with the extended ones, if any
            secondHeader.ReplaceNormalAttributesWithExtended();
