...
namespace System.Formats.Tar.Tests
{
    public class TarWriter_Async_Tests : TarTestsBase
    {
        ...
        public async void Write_To_UnseekableStream_Async()
        {
            ...
            TarWriter writer = new TarWriter(wrapped, TarEntryFormat.Pax, leaveOpen: true);
            await using (writer)
            {
            ...
            TarReader reader = new TarReader(wrapped);
            await using (reader)
            {
        ...
        public async Task VerifyChecksumV7_Async()
        {
            ...
            TarWriter writer = new TarWriter(archive, TarEntryFormat.V7, leaveOpen: true);
            await using (writer)
            {
