...
namespace System.Diagnostics
{
    ...
    /// To asynchronously collect the redirected <xref:System.Diagnostics.Process.StandardOutput> or <xref:System.Diagnostics.Process.StandardError> stream output of a process, add your event handler to the <xref:System.Diagnostics.Process.OutputDataReceived> or <xref:System.Diagnostics.Process.ErrorDataReceived> event. These events are raised each time the process writes a line to the corresponding redirected stream. When the redirected stream is closed, a null line is sent to the event handler. Ensure that your event handler checks for this condition before accessing the <xref:System.Diagnostics.DataReceivedEventArgs.Data> property. For example, you can use the `static` method <xref:string.IsNullOrEmpty> to validate the <xref:System.Diagnostics.DataReceivedEventArgs.Data> property in your event handler.
    /// ## Examples
    /// The following code example illustrates how to perform asynchronous read operations on the redirected <xref:System.Diagnostics.Process.StandardOutput> stream of the **sort** command. The **sort** command is a console application that reads and sorts text input.
    /// The example creates a <xref:System.Diagnostics.DataReceivedEventHandler> delegate for the `SortOutputHandler` event handler and associates the delegate with the <xref:System.Diagnostics.Process.OutputDataReceived> event. The event handler receives text lines from the redirected <xref:System.Diagnostics.Process.StandardOutput> stream, formats the text, and writes the text to the screen.
    ...
    /// <remarks><format type="text/markdown"><![CDATA[
    /// To asynchronously collect the redirected <xref:System.Diagnostics.Process.StandardOutput> or <xref:System.Diagnostics.Process.StandardError> stream output of a process, you must create a method that handles the redirected stream output events. The event-handler method is called when the process writes to the redirected stream. The event delegate calls your event handler with an instance of <xref:System.Diagnostics.DataReceivedEventArgs>. The <xref:System.Diagnostics.DataReceivedEventArgs.Data> property contains the text line that the process wrote to the redirected stream.
    /// ## Examples
    /// The following code example illustrates how to perform asynchronous read operations on the redirected <xref:System.Diagnostics.Process.StandardOutput> stream of the `sort` command. The `sort` command is a console application that reads and sorts text input.
    ...
    public class DataReceivedEventArgs : EventArgs
    {
        ...
        /// <remarks><format type="text/markdown"><![CDATA[
        /// When you redirect the <xref:System.Diagnostics.Process.StandardOutput> or <xref:System.Diagnostics.Process.StandardError> stream of a <xref:System.Diagnostics.Process> to your event handler, an event is raised each time the process writes a line to the redirected stream. The <xref:System.Diagnostics.DataReceivedEventArgs.Data> property is the line that the <xref:System.Diagnostics.Process> wrote to the redirected output stream. Your event handler can use the <xref:System.Diagnostics.DataReceivedEventArgs.Data> property to filter process output or write output to an alternate location. For example, you might create an event handler that stores all error output lines into a designated error log file.
        /// A line is defined as a sequence of characters followed by a line feed ("\n") or a carriage return immediately followed by a line feed ("\r\n"). The line characters are encoded using the default system ANSI code page. The <xref:System.Diagnostics.DataReceivedEventArgs.Data> property does not include the terminating carriage return or line feed.
        /// When the redirected stream is closed, a null line is sent to the event handler. Ensure your event handler checks the <xref:System.Diagnostics.DataReceivedEventArgs.Data> property appropriately before accessing it. For example, you can use the static method <xref:string.IsNullOrEmpty> to validate the <xref:System.Diagnostics.DataReceivedEventArgs.Data> property in your event handler.
        /// ## Examples
