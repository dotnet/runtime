...
public static partial class ZipFileExtensions
{
    ...
    public static Task<ZipArchiveEntry> CreateEntryFromFileAsync(this ZipArchive destination,
        string sourceFileName, string entryName, string password, ZipArchiveEntry.EncryptionMethod encryption, CancellationToken cancellationToken = default) =>
    ...
    /// <returns>A task that represents the asynchronous operation. The value of the task is the newly created entry.</returns>
    public static Task<ZipArchiveEntry> CreateEntryFromFileAsync(this ZipArchive destination,
        string sourceFileName, string entryName, CompressionLevel compressionLevel, string password, ZipArchiveEntry.EncryptionMethod encryption, CancellationToken cancellationToken = default) =>
    ...
    internal static async Task<ZipArchiveEntry> DoCreateEntryFromFileAsync(this ZipArchive destination, string sourceFileName, string entryName,
        CompressionLevel? compressionLevel, string password, ZipArchiveEntry.EncryptionMethod encryption, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        await using (fs)
        {
            Stream es = encryption != ZipArchiveEntry.EncryptionMethod.None
                ? await entry.OpenAsync(password, encryption, cancellationToken).ConfigureAwait(false)
                : await entry.OpenAsync(cancellationToken).ConfigureAwait(false);
