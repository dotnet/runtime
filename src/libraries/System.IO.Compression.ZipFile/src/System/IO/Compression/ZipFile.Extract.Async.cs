...
public static partial class ZipFile
{
    ...
    /// <summary>
    /// Asynchronously extracts all of the files in the specified password-protected archive to a directory on the file system.
    /// The specified directory must not exist. This method will create all subdirectories and the specified directory.
    /// If there is an error while extracting the archive, the archive will remain partially extracted. Each entry will
    /// be extracted such that the extracted file has the same relative path to the destinationDirectoryName as the entry
    /// has to the archive. The path is permitted to specify relative or absolute path information. Relative path information
    ...
    /// <param name="cancellationToken">The cancellation token to monitor for cancellation requests.</param>
    /// <returns>A task that represents the asynchronous extract operation. The task completes when all entries have been extracted or an error occurs.</returns>
    private static async Task ExtractToDirectoryAsync(string sourceArchiveFileName, string destinationDirectoryName, Encoding? entryNameEncoding, bool overwriteFiles, ReadOnlyMemory<char> password, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    /// <summary>
    /// Asynchronously extracts all the files from the password-protected zip archive stored in the specified stream and places them in the specified destination directory on the file system, uses the specified character encoding for entry names, and optionally allows choosing if the files in the destination directory should be overwritten.
    /// </summary>
    /// <param name="source">The stream from which the zip archive is to be extracted.</param>
    /// <param name="destinationDirectoryName">The path to the directory in which to place the extracted files, specified as a relative or absolute path. A relative path is interpreted as relative to the current working directory.</param>
    /// <param name="entryNameEncoding">The encoding to use when reading or writing entry names and comments in this archive. Specify a value for this parameter only when an encoding is required for interoperability with zip archive tools and libraries that do not support UTF-8 encoding for entry names or comments.</param>
    ...
    /// <exception cref="OperationCanceledException">An asynchronous operation is cancelled.</exception>
    /// <returns>A task that represents the asynchronous extract operation. The task completes when all entries have been extracted or an error occurs.</returns>
    private static async Task ExtractToDirectoryAsync(Stream source, string destinationDirectoryName, Encoding? entryNameEncoding, bool overwriteFiles, ReadOnlyMemory<char> password, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    /// <summary>
    /// Asynchronously extracts all of the files in the specified archive to a directory on the file system using the specified options.
    /// </summary>
    /// <param name="sourceArchiveFileName">The path to the archive on the file system that is to be extracted.</param>
    ...
    public static Task ExtractToDirectoryAsync(string sourceArchiveFileName, string destinationDirectoryName, ZipExtractionOptions options, CancellationToken cancellationToken = default)
    {
        ...
        return ExtractToDirectoryAsync(sourceArchiveFileName, destinationDirectoryName, options.EntryNameEncoding, options.OverwriteFiles, options.Password, cancellationToken);
    }

    /// <summary>
    /// Asynchronously extracts all of the files in the specified stream to a directory on the file system using the specified options.
    /// </summary>
    ...
    public static Task ExtractToDirectoryAsync(Stream source, string destinationDirectoryName, ZipExtractionOptions options, CancellationToken cancellationToken = default)
    {
        ...
        return ExtractToDirectoryAsync(source, destinationDirectoryName, options.EntryNameEncoding, options.OverwriteFiles, options.Password, cancellationToken);
    }
}
