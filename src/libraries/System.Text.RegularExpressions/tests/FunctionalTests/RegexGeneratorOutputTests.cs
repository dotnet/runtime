    ...
    public partial class RegexGeneratorOutputTests
    {
        ...
        public static IEnumerable<object[]> ValidateExpectedOutput_MemberData()
        {
            yield return new object[]
            {
                ...
                partial class C
                {
                    [GeneratedRegex("[x\t] # match an x or a tab\ny     # then match a y", RegexOptions.IgnorePatternWhitespace)]
                ...
                partial class C
                {
                    ...
                    /// Explanation:<br/>
                    /// <code>
                    /// ○ Match a character in the set [\tx].<br/>
                    /// ○ Match 'y'.<br/>
                    /// </code>
                    /// </remarks>
                ...
                namespace System.Text.RegularExpressions.Generated
                {
                    ...
                    file sealed class Valid_0 : Regex
                    {
                        ...
                        private Valid_0()
                        {
                            base.pattern = "[x\t] # match an x or a tab\ny     # then match a y";
                        ...
                        private sealed class RunnerFactory : RegexRunnerFactory
                        {
                            ...
                            private sealed class Runner : RegexRunner
                            {
                                ...
                                private bool TryFindNextPossibleStartingPosition(ReadOnlySpan<char> inputSpan)
                                {
                                    ...
                                    {
                                        // The pattern begins with a character in the set [\tx].
                                        // Find the next occurrence. If it can't be found, there's no match.
                                        ReadOnlySpan<char> span = inputSpan.Slice(pos);
                                        for (int i = 0; i < span.Length - 1; i++)
                                        {
                                            int indexOfPos = span.Slice(i).IndexOfAny('\t', 'x');
                                            if (indexOfPos < 0)
                                            {
                                                goto NoMatchFound;
                                            }
                                ...
                                private bool TryMatchAtCurrentPosition(ReadOnlySpan<char> inputSpan)
                                {
                                    ...
                                    if ((uint)slice.Length < 2 ||
                                        (((ch = slice[0]) != '\t') & (ch != 'x')) || // Match a character in the set [\tx].
                                        slice[1] != 'y') // Match 'y'.
                                    {
            ...
            yield return new object[]
            {
                ...
                partial class C
                {
                    [GeneratedRegex("[x ] # match an x or a space\r\ny    # then match a y", RegexOptions.IgnorePatternWhitespace)]
                    public static partial Regex Valid();
                }
                """,
                ...
                namespace System.Text.RegularExpressions.Generated
                {
                    ...
                    file sealed class Valid_0 : Regex
                    {
                        ...
                        private Valid_0()
                        {
                            base.pattern = "[x ] # match an x or a space\r\ny    # then match a y";
                            base.roptions = RegexOptions.IgnorePatternWhitespace;
                            ValidateMatchTimeout(Utilities.s_defaultTimeout);
                            base.internalMatchTimeout = Utilities.s_defaultTimeout;
                            base.factory = new RunnerFactory();
                        ...
                        private sealed class RunnerFactory : RegexRunnerFactory
                        {
                            ...
                            private sealed class Runner : RegexRunner
                            {
                                ...
                                private bool TryFindNextPossibleStartingPosition(ReadOnlySpan<char> inputSpan)
                                {
                                    ...
                                    if (pos <= inputSpan.Length - 2)
                                    {
                                        // The pattern matches a character in the set y at index 1.
                                        // Find the next occurrence. If it can't be found, there's no match.
                                        ReadOnlySpan<char> span = inputSpan.Slice(pos);
                                        for (int i = 0; i < span.Length - 1; i++)
                                        {
                                            int indexOfPos = span.Slice(i + 1).IndexOf('y');
                                            if (indexOfPos < 0)
                                            {
                                                goto NoMatchFound;
                                            }
            ...
            yield return new object[]
            {
                ...
                namespace System.Text.RegularExpressions.Generated
                {
                        ...
                        private sealed class RunnerFactory : RegexRunnerFactory
                        {
                            ...
                            private sealed class Runner : RegexRunner
                            {
                                ...
                                private bool TryFindNextPossibleStartingPosition(ReadOnlySpan<char> inputSpan)
                                {
                                    ...
                                    if (pos <= inputSpan.Length - 2)
                                    {
                                        // The pattern has the literal "xy" at the beginning of the pattern. Find the next occurrence.
                                        // If it can't be found, there's no match.
                                        int i = inputSpan.Slice(pos).IndexOfAny(Utilities.s_indexOfString_xy_Ordinal);
                                        if (i >= 0)
                                        {
                                            base.runtextpos = pos + i;
                                            return true;
                    ...
                    file static class Utilities
                    {
                        ...
                        /// <summary>Supports searching for the string "xy".</summary>
                        internal static readonly SearchValues<string> s_indexOfString_xy_Ordinal = SearchValues.Create(["xy"], StringComparison.Ordinal);
                    }
                }
                """
            };
                        ...
                        private sealed class RunnerFactory : RegexRunnerFactory
                        {
                            ...
                            private sealed class Runner : RegexRunner
                            {
                                ...
                                private bool TryFindNextPossibleStartingPosition(ReadOnlySpan<char> inputSpan)
                                {
                                    ...
                                    if (pos <= inputSpan.Length - 2)
                                    {
                                        // The pattern matches a character in the set \0 at index 1.
                                        // Find the next occurrence. If it can't be found, there's no match.
                                        ReadOnlySpan<char> span = inputSpan.Slice(pos);
                                        for (int i = 0; i < span.Length - 1; i++)
                                        {
                                            int indexOfPos = span.Slice(i + 1).IndexOf('\0');
                                            if (indexOfPos < 0)
                                            {
                                                goto NoMatchFound;
                                            }
