...
namespace System.Text.RegularExpressions.Generator
{
    public partial class RegexGenerator
    {
        ...
        /// <summary>Emits the code for the specified regular expression classes to a string.</summary>
        ...
        private static string Emit(IReadOnlyList<RegexType> regexClasses, CancellationToken cancellationToken)
        {
            ...
            writer.WriteLine("#pragma warning disable CS0168 // Variable declared but never used");
            ...
            foreach (RegexType rc in regexClasses)
            {
                ...
                writer.WriteLine();
                GenerateRegexClass(writer, rc, ref counter);
            }
        ...
        private static void GenerateRegexClass(IndentedTextWriter writer, RegexType regexClass, ref int counter)
        {
            ...
            counter++;
            string generatedName = $"GeneratedRegex_{regexClass.Method.MethodName}_{counter:X}_";
            generatedName += ComputeStringHash(generatedName).ToString("X");

            // Generate the regex type
            GenerateRegex(writer, regexClass.Method, generatedName);
        ...
        private static void GenerateRegex(IndentedTextWriter writer, RegexMethod rm, string id)
        {
            string patternExpression = Literal(rm.Pattern);
            string optionsExpression = $"(RegexOptions)({rm.Options})";
            ...
            writer.WriteLine();
            writer.WriteLine(s_generatedCodeAttribute);
            writer.WriteLine($"{(writer.Indent != 0 ? "private" : "internal")} sealed class {id} : Regex");
            writer.WriteLine("{");
            writer.Write($"    public static Regex Instance {{ get; }} = ");

            // If we can't support custom generation for this regex, spit out a Regex constructor call.
            if (!SupportsCustomCodeGeneration(rm))
            {
                writer.WriteLine($"new Regex({patternExpression}, {optionsExpression}, {timeoutExpression});");
                writer.WriteLine("}");
                return;
            }
            ...
            writer.WriteLine($"        InitializeReferences();");
            writer.WriteLine($"    }}");
            writer.WriteLine("    ");
            ...
            writer.WriteLine($"            {{");
            writer.Indent += 4;
            GenerateFindFirstChar(writer, rm, id);
            writer.Indent -= 4;
            writer.WriteLine($"            }}");
            writer.WriteLine();
        ...
        private static void GenerateFindFirstChar(IndentedTextWriter writer, RegexMethod rm, string id)
        {
            ...
            {
                GenerateAnchorAndLeadingChecks();
            }
            writer.WriteLine();
            ...
            void GenerateAnchorAndLeadingChecks()
            {
                ...
                else
                {
                    ...
                    writer.WriteLine("ReadOnlySpan<char> span = runtext.AsSpan(runtextpos, runtextend - runtextpos);");
                    ...
                    if (canUseIndexOf)
                    {
                        ...
                        {
                            1 => $"{span}.IndexOf({Literal(setChars[0])})",
                            2 => $"{span}.IndexOfAny({Literal(setChars[0])}, {Literal(setChars[1])})",
                            _ => $"{span}.IndexOfAny({Literal(setChars[0])}, {Literal(setChars[1])}, {Literal(setChars[2])})",
                        };
        ...
        private static void GenerateGo(IndentedTextWriter writer, RegexMethod rm, string id)
        {
            ...
            {
                GenerateSimplifiedGo(writer, rm, id);
            }
            else
            {
                GenerateCompleteGo(writer, rm, id);
            }
        }
        ...
        private static void GenerateSimplifiedGo(IndentedTextWriter writer, RegexMethod rm, string id)
        {
            RegexOptions options = (RegexOptions)rm.Options;
            var code = rm.Code;
            var lcc = code.LeadingCharClasses;
            ...
            writer.WriteLine("Capture(0, originalruntextpos, runtextpos);");
            ...
            {
                writer.WriteLine("while (Crawlpos() != 0) Uncapture();");
            }
            else
            {
            ...
            void LoadTextSpanLocal(IndentedTextWriter writer, bool defineLocal = false)
            {
                ...
                writer.WriteLine($"{textSpanLocal} = runtext.AsSpan(runtextpos, runtextend - runtextpos);");
            }
            ...
            void EmitAtomicAlternate(RegexNode node)
            {
                ...
                {
                    writer.WriteLine("int startingCrawlpos = Crawlpos();");
                }
                writer.WriteLine();
            ...
            void EmitCapture(RegexNode node)
            {
                ...
                TransferTextSpanPosToRunTextPos();
                writer.WriteLine($"Capture({capnum}, {startingRunTextPosName}, runtextpos);");
            }

            // Emits code to unwind the capture stack until the crawl position specified in the provided local.
            void EmitUncaptureUntil()
            {
                writer.WriteLine("while (Crawlpos() != startingCrawlpos) Uncapture();");
            }
            ...
            void EmitMultiChar(RegexNode node)
            {
                ...
                {
                    using (EmitBlock(writer, $"if (!{textSpanLocal}.Slice({textSpanPos}).StartsWith({Literal(node.Str)}))"))
                    {
                        writer.WriteLine($"goto {doneLabel};");
                    }
            ...
            void EmitSingleCharRepeater(RegexNode node)
            {
                ...
                else
                {
                    ...
                    writer.WriteLine($"ReadOnlySpan<char> {spanLocal} = {textSpanLocal}.Slice({textSpanPos}, {iterations});");
                    string i = GetNextLocalId();
            ...
            void EmitSingleCharAtomicLoop(RegexNode node)
            {
                    ...
                    (!IsCaseInsensitive(node) || !RegexCharClass.ParticipatesInCaseConversion(node.Ch)))
                {
                    ...
                    writer.Write($"int {iterationLocal} = {textSpanLocal}");
                    if (textSpanPos > 0)
                    {
        ...
        private static void GenerateCompleteGo(IndentedTextWriter writer, RegexMethod rm, string id)
        {
            ...
            for (int codepos = 0; codepos < codes.Length; codepos += RegexCode.OpcodeSize(codes[codepos]))
            {
                ...
                GenerateOneCode(labels[codepos]);
                writer.WriteLine();
            }
            ...
            writer.WriteLine("int limit = runtrackcount * 4;");
            using (EmitBlock(writer, "if (runstackpos < limit)"))
            {
                writer.WriteLine("base.runstackpos = runstackpos;");
                writer.WriteLine("DoubleStack(); // might change runstackpos and runstack");
            ...
            using (EmitBlock(writer, "if (runtrackpos < limit)"))
            {
                ...
                writer.WriteLine("DoubleTrack(); // might change runtrackpos and runtrack");
            ...
            using (EmitBlock(writer, $"switch (runtrack[runtrackpos++])"))
            {
                for (int i = 0; i < noteCount; i++)
                {
                    using (EmitBlock(writer, $"case {i}:"))
                    {
                        ...
                        if (n.flags != 0)
                        {
                            ...
                            GenerateOneCode(null); // should always end in a goto
                        }
                        else
                        {
                ...
                {
                    writer.WriteLine("Debug.Fail($\"Unexpected backtracking state {runtrack[runtrackpos - 1]}\");");
            ...
            /// </remarks>
            void GenerateOneCode(string? label)
            {
                ...
                switch (currentOpcode)
                {
                    ...
                    case RegexCode.Testref:
                        using (EmitBlock(writer, $"if (!IsMatched({Operand(0)}))"))
                        {
                            writer.WriteLine($"goto {Backtrack};");
                        }
                        break;
                    ...
                    case RegexCode.Capturemark:
                        {
                            if (Operand(1) != -1)
                            {
                                using (EmitBlock(writer, $"if (!IsMatched({Operand(1)}))"))
                                {
                                    writer.WriteLine($"goto {Backtrack};");
                                }
                            }
                            ...
                            writer.WriteLine(Operand(1) != -1 ?
                                $"TransferCapture({Operand(0)}, {Operand(1)}, {Stacked}, runtextpos);" :
                                $"Capture({Operand(0)}, {Stacked}, runtextpos);");
                            PushTrack(Stacked);
                    ...
                    case RegexCode.Backjump:
                        {
                            ...
                            writer.WriteLine($"while (Crawlpos() != {Stacked}) Uncapture();");
                            writer.WriteLine($"goto {Backtrack};");
                        }
                        break;
                    ...
                    case RegexCode.Forejump | RegexCode.Back:
                        {
                            ...
                            writer.WriteLine($"while (Crawlpos() != {TrackedCrawlpos}) Uncapture();");
                            writer.WriteLine($"goto {Backtrack};");
                        }
                        break;
                    ...
                    case RegexCode.NonBoundary:
                        using (EmitBlock(writer, $"if ({(Code() == RegexCode.Boundary ? "!" : "")}IsBoundary(runtextpos, runtextbeg, runtextend))"))
                        {
                            writer.WriteLine($"goto {Backtrack};");
                        }
                        break;
                    ...
                    case RegexCode.NonECMABoundary:
                        using (EmitBlock(writer, $"if ({(Code() == RegexCode.ECMABoundary ? "!" : "")}IsECMABoundary(runtextpos, runtextbeg, runtextend))"))
                        {
                            writer.WriteLine($"goto {Backtrack};");
                        }
                        break;
                    ...
                    case RegexCode.Ref | RegexCode.Ci | RegexCode.Rtl:
                        {
                            ...
                            writer.WriteLine($"{Length} = MatchLength({Operand(0)}); // length");
                            ...
                            if (!IsRightToLeft())
                            {
                                writer.WriteLine($"{Index} = MatchIndex({Operand(0)}) + {Length}; // index");
                                writer.WriteLine($"runtextpos += {Length};");
                            }
                            else
                            {
                                writer.WriteLine($"{Index} = MatchIndex({Operand(0)}); // index");
                                writer.WriteLine($"runtextpos -= {Length};");
                            }
                    ...
                    case RegexCode.Setloopatomic | RegexCode.Ci | RegexCode.Rtl:
                        {
                            ...
                            if (c != 0)
                            {
                                ...
                                {
                                    writer.WriteLine(!IsRightToLeft() ?
                                        $"{Len} = Math.Min(runtextend - runtextpos, {c}); // length" :
                                        $"{Len} = Math.Min(runtextpos - runtextbeg, {c}); // length");
                                }
                                ...
                                {
                                    writer.WriteLine($"{I} = runtext.AsSpan(runtextpos, {Len}).IndexOf({Literal((char)Operand(0))}); // i");
                                    ...
                                    RegexCharClass.IsNegated(set!))
                                {
                                    ...
                                    writer.Write($"{I} = runtext.AsSpan(runtextpos, {Len}).IndexOfAny({Literal(setChars[0])}, {Literal(setChars[1])}");
                                    if (numSetChars == 3)
                                    {
                    ...
                    case RegexCode.Setlazy | RegexCode.Ci | RegexCode.Rtl:
                        {
                            ...
                            if (count != 0)
                            {
                                ...
                                {
                                    writer.WriteLine(!IsRightToLeft() ?
                                        $"{C} = Math.Min(runtextend - runtextpos, {count}); // count" :
                                        $"{C} = Math.Min(runtextpos - runtextbeg, {count}); // count");
                                }
        ...
        private static string MatchCharacterClass(bool hasTextInfo, RegexOptions options, string chExpr, string charClass, bool caseInsensitive)
        {
            ...
            {
                return $"(char.GetUnicodeCategory({chExpr}) {(negated ? "!=" : "==")} UnicodeCategory.{category})";
            }
            ...
            if (!invariant)
            {
                ...
                if (!RegexCharClass.IsNegated(charClass))
                {
                    ...
                    {
                        case 2:
                            return $"({chExpr} is {Literal(setChars[0])} or {Literal(setChars[1])})";

                        case 3:
                            return $"({chExpr} is {Literal(setChars[0])} or {Literal(setChars[1])} or {Literal(setChars[2])})";
                    }
                }
            }
            ...
            if (!invariant) // if we're being asked to do a case insensitive, invariant comparison, use the lookup table
            {
                if (analysis.ContainsNoAscii)
                {
                    ...
                    return invariant ?
                        $"((ch = {chExpr}) >= 128 && CharInClass(char.ToLowerInvariant((char)ch), {Literal(charClass)}))" :
                        $"((ch = {chExpr}) >= 128 && CharInClass((char)ch, {Literal(charClass)}))";
                }
