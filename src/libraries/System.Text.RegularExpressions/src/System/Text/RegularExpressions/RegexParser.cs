...
namespace System.Text.RegularExpressions
{
    ...
    internal ref struct RegexParser
    {
            ...
            while (CharsRight() > 0)
            {
                ...
                switch (ch)
                {
                    ...
                    case ')':
                        if (EmptyStack())
                        {
                            throw MakeException(RegexParseError.InsufficientOpeningParentheses, SR.TooManyParens);
                        }
                    ...
                    case '\\':
                        if (CharsRight() == 0)
                        {
                            throw MakeException(RegexParseError.UnescapedEndingBackslash, SR.IllegalEndEscape);
                        }
                    ...
                    case '?':
                        if (Unit() == null)
                        {
                            throw wasPrevQuantifier ?
                                MakeException(RegexParseError.NestedQuantifiersNotParenthesized, SR.Format(SR.NestedQuantify, ch)) :
                                MakeException(RegexParseError.QuantifierAfterNothing, SR.QuantifyAfterNothing);
                        }
                        MoveLeft();
                        break;
                ...
                while (Unit() != null)
                {
                    ...
                    if (min > max)
                    {
                        throw MakeException(RegexParseError.ReversedQuantifierRange, SR.IllegalRange);
                    }
        ...
        BreakOuterScan:
            ...
            if (!EmptyStack())
            {
                throw MakeException(RegexParseError.InsufficientClosingParentheses, SR.NotEnoughParens);
            }
                    ...
                    switch (ch = RightCharMoveRight())
                    {
                        ...
                        case 'd':
                            if (!scanOnly)
                            {
                                if (inRange)
                                {
                                    throw MakeException(RegexParseError.ShorthandClassInCharacterRange, SR.Format(SR.BadClassInCharRange, ch));
                                }
                        ...
                        case '-':
                            if (!scanOnly)
                            {
                                if (inRange)
                                {
                                    if (chPrev > ch)
                                    {
                                        throw MakeException(RegexParseError.ReversedCharacterRange, SR.ReversedCharRange);
                                    }
                ...
                if (inRange)
                {
                    ...
                    if (!scanOnly)
                    {
                        if (ch == '[' && !translatedChar && !firstChar)
                        {
                            ...
                            if (CharsRight() > 0 && RightChar() != ']')
                            {
                                throw MakeException(RegexParseError.ExclusionGroupNotLast, SR.SubtractionMustBeLast);
                            }
                        }
                        else
                        {
                ...
                switch (ch)
                {
                        ...
                        switch (ch = RightCharMoveRight())
                        {
                            ...
                            default:
                                ...
                                if ((uint)(ch - '0') <= 9)
                                {
                                    ...
                                    {
                                        throw MakeException(RegexParseError.CaptureGroupNameInvalid, SR.InvalidGroupName);
                                    }

                                    if (capnum == 0)
                                    {
                                        throw MakeException(RegexParseError.CaptureGroupOfZero, SR.CapnumNotZero);
                                    }
                                }
                                ...
                                if ((capnum != -1 || proceed == true) && CharsRight() > 1 && RightChar() == '-')
                                {
                                    ...
                                    if ((uint)(ch - '0') <= 9)
                                    {
                                        ...
                                        {
                                            throw MakeException(RegexParseError.UndefinedNumberedReference, SR.Format(SR.UndefinedBackref, uncapnum));
                                        }
                                    ...
                                    else if (RegexCharClass.IsWordChar(ch))
                                    {
                                        ...
                                        {
                                            throw MakeException(RegexParseError.UndefinedNamedReference, SR.Format(SR.UndefinedNameRef, uncapname));
                                        }
                    ...
                    case '(':
                        ...
                        if (CharsRight() > 0)
                        {
                            ...
                            if (ch >= '0' && ch <= '9')
                            {
                                ...
                                if (CharsRight() > 0 && RightCharMoveRight() == ')')
                                {
                                    ...
                                    throw MakeException(RegexParseError.AlternationHasUndefinedReference, SR.Format(SR.UndefinedReference, capnum.ToString()));
                                }

                                throw MakeException(RegexParseError.AlternationHasMalformedReference, SR.Format(SR.MalformedReference, capnum.ToString()));
                            }
                            else if (RegexCharClass.IsWordChar(ch))
                            {
                                string capname = ScanCapname();
                        ...
                        if (charsRight >= 3 && RightChar(1) == '?')
                        {
                            ...
                            if (rightchar2 == '#')
                            {
                                throw MakeException(RegexParseError.AlternationHasComment, SR.AlternationCantHaveComment);
                            }

                            // disallow named capture group (?<..>..) in the condition
                            if (rightchar2 == '\'')
                            {
                                throw MakeException(RegexParseError.AlternationHasNamedCapture, SR.AlternationCantCapture);
                            }
        ...
        BreakRecognize:
            ...
            throw MakeException(RegexParseError.InvalidGroupingConstruct, SR.UnrecognizedGrouping);
        }
        ...
        private RegexNode? ScanBasicBackslash(bool scanOnly)
        {
            ...
            if (ch == 'k')
            {
                ...
                {
                    throw MakeException(RegexParseError.MalformedNamedReference, SR.MalformedNameRef);
                }
            ...
            if (angled && ch >= '0' && ch <= '9')
            {
                ...
                if (CharsRight() > 0 && RightCharMoveRight() == close)
                {
                    return
                        ...
                        throw MakeException(RegexParseError.UndefinedNumberedReference, SR.Format(SR.UndefinedBackref, capnum.ToString()));
                }
            }
            ...
            else if (angled && RegexCharClass.IsWordChar(ch))
            {
                ...
                if (CharsRight() > 0 && RightCharMoveRight() == close)
                {
                    return
                        ...
                        throw MakeException(RegexParseError.UndefinedNamedReference, SR.Format(SR.UndefinedNameRef, capname));
                }
            }
            ...
            if (ch >= '0' && ch <= '9')
            {
                if (!angled && UseOptionE())
                {
                    ...
                    while (CharsRight() > 0 && (ch = RightChar()) >= '0' && ch <= '9')
                    {
                        ...
                        {
                            throw MakeException(RegexParseError.QuantifierOrCaptureGroupOutOfRange, SR.CaptureGroupOutOfRange);
                        }
        ...
        private char ScanHex(int c)
        {
            ...
            if (c > 0)
            {
                throw MakeException(RegexParseError.InsufficientOrInvalidHexDigits, SR.TooFewHex);
            }
        ...
        private char ScanControl()
        {
            if (CharsRight() == 0)
            {
                throw MakeException(RegexParseError.MissingControlCharacter, SR.MissingControl);
            }
            ...
            throw MakeException(RegexParseError.UnrecognizedControlCharacter, SR.UnrecognizedControl);
        }
        ...
        private string ParseProperty()
        {
            if (CharsRight() < 3)
            {
                throw MakeException(RegexParseError.InvalidUnicodePropertyEscape, SR.IncompleteSlashP);
            }

            char ch = RightCharMoveRight();
            if (ch != '{')
            {
                throw MakeException(RegexParseError.MalformedUnicodePropertyEscape, SR.MalformedSlashP);
            }
        ...
        private void PopGroup()
        {
            ...
            if (_group.Type == RegexNode.Testgroup && _group.ChildCount() == 0)
            {
                if (_unit == null)
                {
                    throw MakeException(RegexParseError.AlternationHasMalformedCondition, SR.IllegalCondition);
                }
        ...
        private void AddGroup()
        {
            if (_group!.Type == RegexNode.Testgroup || _group.Type == RegexNode.Testref)
            {
                ...
                {
                    throw MakeException(RegexParseError.AlternationHasTooManyConditions, SR.TooManyAlternates);
                }
            }
            else
            {
