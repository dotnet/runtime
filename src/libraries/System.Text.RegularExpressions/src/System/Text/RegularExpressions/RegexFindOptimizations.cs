...
namespace System.Text.RegularExpressions
{
    ...
    internal sealed class RegexFindOptimizations
    {
        ...
        /// </summary>
        /// <remarks>
        /// Frequency ranks are based on empirical byte frequency analysis of typical text corpora
        /// (similar to the approach used in Rust's aho-corasick crate). Higher rank means more frequent.
        /// </remarks>
        private static bool HasHighFrequencyStartingChars(string[] prefixes)
        {
            ReadOnlySpan<byte> ranks = AsciiCharFrequencyRank;
            int totalRank = 0;
            int count = 0;
            ...
            // If the average frequency rank of starting chars exceeds this threshold, the characters
            // are common enough that IndexOfAny would match many positions, making SearchValues<string>
            // the better choice. Common characters like lowercase letters rank above the threshold,
            // while less frequent characters like uppercase letters rank below it, so IndexOfAny
            // with rare starting chars is already an effective filter and is preferred.
            // See remarks on AsciiCharFrequencyRank for coupling details.
            const int HighFrequencyThreshold = 200;
            return count > 0 && totalRank >= HighFrequencyThreshold * count;
        }

        /// <summary>
        /// Empirical frequency ranks for ASCII characters (0-127), derived from analysis of large text
        /// corpora. Higher values indicate more frequently occurring characters.
        /// </summary>
        /// <remarks>
        /// These values are the first 128 entries of the BYTE_FREQUENCIES table from @BurntSushi's
        /// aho-corasick crate (MIT-licensed):
        /// https://github.com/BurntSushi/aho-corasick/blob/e88e1fce8f7c/src/util/byte_frequencies.rs
        /// That table was generated by measuring byte distributions across a large, varied text corpus.
        /// Ranks range from 0 (rarest) to 255 (most common, e.g. space).
        ///
        /// The values in this table are coupled to <see cref="HasHighFrequencyStartingChars"/>'s
        /// HighFrequencyThreshold constant (currently 200). Uppercase letters generally rank below
        /// this threshold while most lowercase letters rank above it.
        /// If this table is recalibrated, the threshold must be re-evaluated.
        /// </remarks>
        private static ReadOnlySpan<byte> AsciiCharFrequencyRank =>
        [
            55, 52, 51, 50, 49, 48, 47, 46, 45, 103, 242, 66, 67, 229, 44, 43,  // 0x00-0x0F
            42, 41, 40, 39, 38, 37, 36, 35, 34, 33, 56, 32, 31, 30, 29, 28,     // 0x10-0x1F
            255, 148, 164, 149, 136, 160, 155, 173, 221, 222, 134, 122, 232, 202, 215, 224, // ' ' to '/'
            208, 220, 204, 187, 183, 179, 177, 168, 178, 200, 226, 195, 154, 184, 174, 126, // '0' to '?'
            120, 191, 157, 194, 170, 189, 162, 161, 150, 193, 142, 137, 171, 176, 185, 167, // '@' to 'O'
            186, 112, 175, 192, 188, 156, 140, 143, 123, 133, 128, 147, 138, 146, 114, 223, // 'P' to '_'
            151, 249, 216, 238, 236, 253, 227, 218, 230, 247, 135, 180, 241, 233, 246, 244, // '`' to 'o'
            231, 139, 245, 243, 251, 235, 201, 196, 240, 214, 152, 182, 205, 181, 127, 27,  // 'p' to DEL
        ];
    }
