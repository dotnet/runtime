    ...
    public class File_ReadWriteAllBytesAsync : FileSystemTest
    {
        ...
        public async Task WriteToReadOnlyFileAsync()
        {
            ...
            try
            {
                ...
                {
                    await File.WriteAllBytesAsync(path, "text"u8);
                    Assert.Equal("text"u8, await File.ReadAllBytesAsync(path));
                }
                else
                    await Assert.ThrowsAsync<UnauthorizedAccessException>(async () => await File.WriteAllBytesAsync(path, "text"u8));
            }
            finally
            {
