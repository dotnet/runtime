...
namespace System.Data.Common
{
    ...
    public sealed class DataColumnMapping : MarshalByRefObject, IColumnMapping, ICloneable
    {
        ...
        public string SourceColumn
        {
            ...
            set
            {
                if ((Parent is not null) && (0 != ADP.SrcCompare(_sourceColumnName, value)))
                {
        ...
        public static DataColumn? GetDataColumnBySchemaAction(string? sourceColumn, string? dataSetColumn, DataTable dataTable, Type? dataType, MissingSchemaAction schemaAction)
        {
            ...
            Debug.Assert(columns is not null, "GetDataColumnBySchemaAction: unexpected null DataColumnCollection");

            int index = columns.IndexOf(dataSetColumn);
            if ((0 <= index) && (index < columns.Count))
            {
                DataColumn dataColumn = columns[index];
                Debug.Assert(dataColumn is not null, "GetDataColumnBySchemaAction: unexpected null dataColumn");
                ...
                if ((dataType is null) || (dataType.IsArray == dataColumn.DataType.IsArray))
                {
                    return dataColumn;
                }
