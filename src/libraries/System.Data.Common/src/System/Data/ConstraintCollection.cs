    ...
    public sealed class ConstraintCollection : InternalDataCollectionBase
    {
        ...
        internal UniqueConstraint? FindKeyConstraint(DataColumn[] columns)
        {
            ...
            for (int i = 0; i < constraintCount; i++)
            {
                ...
                if ((constraint is not null) && CompareArrays(constraint.Key.ColumnsReference, columns))
                {
                    return constraint;
                }
            }
        ...
        internal UniqueConstraint? FindKeyConstraint(DataColumn column)
        {
            ...
            {
                UniqueConstraint? constraint = (List[i] as UniqueConstraint);
                if ((constraint is not null) && (constraint.Key.ColumnsReference.Length == 1) && (constraint.Key.ColumnsReference[0] == column))
                    return constraint;
            }
            return null;
        }
        ...
        {
            int cachedI = -1;
            if ((constraintName is not null) && (0 < constraintName.Length))
            {
                int constraintCount = List.Count;
                int result = 0;
                for (int i = 0; i < constraintCount; i++)
