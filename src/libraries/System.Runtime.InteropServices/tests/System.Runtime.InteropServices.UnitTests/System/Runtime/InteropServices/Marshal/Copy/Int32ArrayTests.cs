    ...
    public class Int32ArrayTests
    {
        ...
        public void CopyTo_Roundtrip_MatchesOriginalInput(int[] values)
        {
            ...
            try
            {
                ...
                Assert.Equal<int>(values.AsSpan(2, values.Length - 4).ToArray(), array2.AsSpan(2, values.Length - 4).ToArray());
            }
            finally
            {
        ...
        {
            AssertExtensions.Throws<ArgumentNullException>("destination", () => Marshal.Copy(new int[10], 0, IntPtr.Zero, 0));
            AssertExtensions.Throws<ArgumentNullException>("destination", () => Marshal.Copy(new IntPtr(1), (int[])null, 0, 0));
        }

        [Fact]
        public void CopyTo_NullSource_ThrowsArgumentNullException()
        {
            AssertExtensions.Throws<ArgumentNullException>("source", () => Marshal.Copy((int[])null, 0, new IntPtr(1), 0));
            AssertExtensions.Throws<ArgumentNullException>("source", () => Marshal.Copy(IntPtr.Zero, new int[10], 0, 0));
        }
