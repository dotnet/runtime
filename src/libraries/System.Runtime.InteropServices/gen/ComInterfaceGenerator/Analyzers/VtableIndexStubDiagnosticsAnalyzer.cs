...
namespace Microsoft.Interop.Analyzers
{
    ...
    public class VtableIndexStubDiagnosticsAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static void AnalyzeMethod(SymbolAnalysisContext context, MethodDeclarationSyntax methodSyntax, IMethodSymbol method, StubEnvironment env)
        {
            ...
            {
                var (_, diagnostics) = VirtualMethodPointerStubGenerator.GenerateManagedToNativeStub(stubContext, (env, dir) => new ByValueContentsMarshalKindValidator(VtableIndexStubGeneratorHelpers.GetGeneratorResolver(env, dir)));
            ...
            {
                var (_, diagnostics) = VirtualMethodPointerStubGenerator.GenerateNativeToManagedStub(stubContext, (env, dir) => new ByValueContentsMarshalKindValidator(VtableIndexStubGeneratorHelpers.GetGeneratorResolver(env, dir)));
                foreach (DiagnosticInfo diag in diagnostics)
                    context.ReportDiagnostic(diag.ToDiagnostic());
            }
        }
