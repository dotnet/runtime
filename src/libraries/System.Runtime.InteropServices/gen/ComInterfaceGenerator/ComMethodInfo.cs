...
namespace Microsoft.Interop
{
        ...
        SequenceEqualImmutableArray<AttributeInfo> Attributes)
    {
        ...
        /// </summary>
        public static SequenceEqualImmutableArray<DiagnosticsOr<(ComMethodInfo ComMethod, IMethodSymbol Symbol)>> GetMethodsFromInterface((ComInterfaceInfo ifaceContext, INamedTypeSymbol ifaceSymbol) data, CancellationToken ct)
        {
            var methods = ImmutableArray.CreateBuilder<DiagnosticsOr<(ComMethodInfo, IMethodSymbol)>>();
        ...
        private static DiagnosticsOr<(ComMethodInfo, IMethodSymbol)> CalculateMethodInfo(ComInterfaceInfo ifaceContext, IMethodSymbol method, CancellationToken ct)
        {
            ...
            {
                return DiagnosticsOr<(ComMethodInfo, IMethodSymbol)>.From(Diagnostic.Create(GeneratorDiagnostics.MethodNotDeclaredInAttributedInterface, method.Locations.FirstOrDefault(), method.ToDisplayString()));
            }
            ...
            {
                return DiagnosticsOr<(ComMethodInfo, IMethodSymbol)>.From(Diagnostic.Create(GeneratorDiagnostics.CannotAnalyzeMethodPattern, method.Locations.FirstOrDefault(), method.ToDisplayString()));
            }

            var diag = GetDiagnosticIfInvalidMethodForGeneration(comMethodDeclaringSyntax, method);
            if (diag is not null)
            {
                return DiagnosticsOr<(ComMethodInfo, IMethodSymbol)>.From(diag);
            }
            ...
            return DiagnosticsOr<(ComMethodInfo, IMethodSymbol)>.From((comMethodInfo, method));
        }
    }
}
