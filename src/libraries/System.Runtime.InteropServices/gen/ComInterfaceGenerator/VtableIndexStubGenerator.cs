...
namespace Microsoft.Interop
{
    ...
    public sealed class VtableIndexStubGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            var managedToNativeStubs = managedToNativeStubContexts.Collect().Select(static (data, _) =>
            {
                if (data.IsEmpty)
                    return "";
                ...
                foreach (SourceAvailableIncrementalMethodStubGenerationContext stub in data)
                {
                    ...
                    stub.ContainingSyntaxContext.WriteToWithUnsafeModifier(writer, (member, stub.TypeKeyOwner.FullTypeName), static (writer, data) =>
                    {
                        ...
                        {
                            data.member.WriteTo(writer, data.FullTypeName);
                        }
                    });
                }

                return writer.ToString();
            }).WithTrackingName("GeneratedManagedToNativeVtableStubs");
            context.RegisterSourceOutput(managedToNativeStubs, static (context, source) =>
            {
                if (source.Length != 0)
                    context.AddSource("ManagedToNativeStubs.g.cs", source);
            });
            ...
            var nativeToManagedStubs = nativeToManagedStubContexts.Collect().Select(static (data, _) =>
            {
                if (data.IsEmpty)
                    return "";
                ...
                return writer.ToString();
            }).WithTrackingName("GeneratedNativeToManagedVtableStubs");
            context.RegisterSourceOutput(nativeToManagedStubs, static (context, source) =>
            {
                if (source.Length != 0)
                    context.AddSource("NativeToManagedStubs.g.cs", source);
            });
            ...
            var nativeInterfaces = syntaxContexts.Select(static (data, _) =>
            {
                if (data.IsEmpty)
                    return "";
                ...
                return writer.ToString();
            }).WithTrackingName("GeneratedVtableNativeInterfaces");
            context.RegisterSourceOutput(nativeInterfaces, static (context, source) =>
            {
                if (source.Length != 0)
                    context.AddSource("NativeInterfaces.g.cs", source);
            });

            var vtablePopulations = nativeToManagedStubContexts.Collect().Select(static (data, _) =>
            {
                if (data.IsEmpty)
                    return "";
                ...
                foreach (var group in data.GroupBy(s => s.ContainingSyntaxContext))
                {
                    ...
                    group.Key.WriteToWithUnsafeModifier(writer, group, static (writer, data) =>
                    {
                        ...
                        {
                            string functionPointerType = VirtualMethodPointerStubGenerator.GenerateUnmanagedFunctionPointerTypeForMethod(method, VtableIndexStubGeneratorHelpers.GetGeneratorResolver);
                ...
                return writer.ToString();
            }).WithTrackingName("GeneratedVtablePopulation");
            context.RegisterSourceOutput(vtablePopulations, static (context, source) =>
            {
                if (source.Length != 0)
                    context.AddSource("PopulateVTable.g.cs", source);
            });
        }
        ...
        internal static SourceAvailableIncrementalMethodStubGenerationContext CalculateStubInformation(MethodDeclarationSyntax syntax, IMethodSymbol symbol, StubEnvironment environment, CancellationToken ct)
        {
            ...
            string unwrapperType = unmanagedObjectUnwrapper.AttributeClass.TypeArguments[0].ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
            ...
            return new SourceAvailableIncrementalMethodStubGenerationContext(
                ...
                new ObjectUnwrapperInfo(unwrapperType),
