        ...
        private byte[]? ChangeKeyImpl(ReadOnlySpan<byte> key)
        {
            ...
            if (key.Length > _blockSize && _blockSize > 0)
            {
                ...
                modifiedKey = _hashAlgorithmId switch
                {
                    ...
                    HashAlgorithmNames.SHA3_256 when SHA3_256.IsSupported => SHA3_256.HashData(key),
                    HashAlgorithmNames.SHA3_384 when SHA3_384.IsSupported => SHA3_384.HashData(key),
                    HashAlgorithmNames.SHA3_512 when SHA3_512.IsSupported => SHA3_512.HashData(key),
