...
namespace System.IO.Compression
{
    public class ZLibStreamUnitTests : CompressionStreamUnitTestBase
    {
        ...
        public void AutomaticStreamRewinds_WhenDecompressionFinishes()
        {
            ...
            {
                using (var zlibStream = new ZLibStream(ms, CompressionMode.Compress))
                {
                    zlibStream.Write(originalData);
                }
            ...
            using (var stream = new MemoryStream(combinedData))
            {
                ...
                using (var zlibStream = new ZLibStream(stream, CompressionMode.Decompress, leaveOpen: true))
                {
                ...
                // ZLibStream should have automatically rewound the stream to the end of compressed data
        ...
        public async Task AutomaticStreamRewinds_WhenDecompressionFinishes_Async()
        {
            ...
            using (var ms = new MemoryStream())
            {
                using (var zlibStream = new ZLibStream(ms, CompressionMode.Compress))
                {
                    await zlibStream.WriteAsync(originalData);
                }
                compressedData = ms.ToArray();
            }
