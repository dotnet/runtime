...
namespace System.IO.Tests
{
    public class FileSystemNameTests
    {
        ...
        public static TheoryData<string, string, bool, bool> SimpleMatchData => new TheoryData<string, string, bool, bool>
        {
            ...
            // prefix???suffix patterns (StartsWith + EndsWith with exact wildcard count)
            { "pre???fix", "pre123fix", false, true },
            { "pre???fix", "pre123fix", true, true },
            { "pre???fix", "PRE123FIX", false, false },
            { "pre???fix", "PRE123FIX", true, true },
            { "pre???fix", "preabcfix", false, true },
            { "pre???fix", "prefix", false, false },
            { "pre???fix", "pre12fix", false, false },
            { "pre???fix", "pre1234fix", false, false },
            { "pre?fix", "pre1fix", false, true },
            { "pre?fix", "prefix", false, false },
            { "pre?fix", "pre12fix", false, false },
            { "file??txt", "file12txt", false, true },
            { "file??txt", "file1txt", false, false },
            { "file??txt", "file123txt", false, false },
            { @"*", @"foo.txt", true, true },
            { @".", @"foo.txt", true, false },
            { @".", @"footxt", true, false },
            { @"*.*", @"foo.txt", true, true },
