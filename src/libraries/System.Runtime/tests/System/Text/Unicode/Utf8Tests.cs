...
namespace System.Text.Unicode.Tests
{
    public partial class Utf8Tests
    {
        ...
        [Theory]
        [InlineData("80")] // sequence cannot begin with continuation character
        [InlineData("8182")] // sequence cannot begin with continuation character
        [InlineData("838485")] // sequence cannot begin with continuation character
        [InlineData(X_UTF8 + "80")] // sequence cannot begin with continuation character
        [InlineData(X_UTF8 + "8182")] // sequence cannot begin with continuation character
        [InlineData("C0")] // [ C0 ] is always invalid
        [InlineData("C080")] // [ C0 ] is always invalid
        [InlineData("C08081")] // [ C0 ] is always invalid
        [InlineData(X_UTF8 + "C1")] // [ C1 ] is always invalid
        [InlineData(X_UTF8 + "C180")] // [ C1 ] is always invalid
        [InlineData(X_UTF8 + "C27F")] // [ C2 ] is improperly terminated
        [InlineData("E2827F")] // [ E2 82 ] is improperly terminated
        [InlineData("E09F80")] // [ E0 9F ... ] is overlong
        [InlineData("E0C080")] // [ E0 ] is improperly terminated
        [InlineData("ED7F80")] // [ ED ] is improperly terminated
        [InlineData("EDA080")] // [ ED A0 ... ] is surrogate
        public void IsValid_WithSmallInvalidBuffers(string utf8HexInput)
        {
            Assert.InRange(utf8HexInput.Length, 0, 6);
            Assert.False(Utf8.IsValid(DecodeHex(utf8HexInput)));
        }

        [Theory]
        [InlineData("C2")] // [ C2 ] is an incomplete sequence
        [InlineData("E282")] // [ E2 82 ] is an incomplete sequence
        [InlineData(X_UTF8 + "C2")] // [ C2 ] is an incomplete sequence
        [InlineData(X_UTF8 + "E0")] // [ E0 ] is an incomplete sequence
        [InlineData(X_UTF8 + "E0BF")] // [ E0 BF ] is an incomplete sequence
        [InlineData(X_UTF8 + "F0")] // [ F0 ] is an incomplete sequence
        [InlineData(X_UTF8 + "F0BF")] // [ F0 BF ] is an incomplete sequence
        [InlineData(X_UTF8 + "F0BFA0")] // [ F0 BF A0 ] is an incomplete sequence
        [InlineData(E_ACUTE_UTF8 + "C2")] // [ C2 ] is an incomplete sequence
        [InlineData(E_ACUTE_UTF8 + "E0")] // [ E0 ] is an incomplete sequence
        [InlineData(E_ACUTE_UTF8 + "F0")] // [ F0 ] is an incomplete sequence
        [InlineData(E_ACUTE_UTF8 + "E0BF")] // [ E0 BF ] is an incomplete sequence
        [InlineData(E_ACUTE_UTF8 + "F0BF")] // [ F0 BF ] is an incomplete sequence
        [InlineData(EURO_SYMBOL_UTF8 + "C2")] // [ C2 ] is an incomplete sequence
        [InlineData(EURO_SYMBOL_UTF8 + "E0")] // [ E0 ] is an incomplete sequence
        [InlineData(EURO_SYMBOL_UTF8 + "F0")] // [ F0 ] is an incomplete sequence
        public void IsValid_WithVariousIncompleteBuffers(string utf8HexInput)
        {
            Assert.False(Utf8.IsValid(DecodeHex(utf8HexInput)));
        }
        ...
        [InlineData(GRINNING_FACE_UTF16 + "@AB")] // single 4-byte sequence + 3 ASCII bytes, exercises 4-byte sequence processing and draining logic
        [InlineData(WOMAN_CARTWHEELING_MEDSKIN_UTF16)] // exercises switching between multiple sequence lengths
        public void IsValid_ValidBuffers(string utf16Input)
        {
            Rune[] enumeratedScalars = utf16Input.EnumerateRunes().ToArray();
            ...
            Assert.True(Utf8.IsValid(utf8Input));
        }

        [Theory]
        [InlineData("3031" + "80" + "202122232425")] // Continuation character at start of sequence should match no bitmask
        [InlineData("3031" + "C080" + "2021222324")] // Overlong 2-byte sequence at start of DWORD
        [InlineData("3031" + "C180" + "2021222324")] // Overlong 2-byte sequence at start of DWORD
        [InlineData("C280" + "C180")] // Overlong 2-byte sequence at end of DWORD
        [InlineData("C27F" + "C280")] // Improperly terminated 2-byte sequence at start of DWORD
        [InlineData("C2C0" + "C280")] // Improperly terminated 2-byte sequence at start of DWORD
        [InlineData("C280" + "C27F")] // Improperly terminated 2-byte sequence at end of DWORD
        [InlineData("C280" + "C2C0")] // Improperly terminated 2-byte sequence at end of DWORD
        [InlineData("C280" + "C280" + "80203040")] // Continuation character at start of sequence, within "stay in 2-byte processing" optimization
        [InlineData("C280" + "C280" + "C180" + "C280")] // Overlong 2-byte sequence at start of DWORD, within "stay in 2-byte processing" optimization
        [InlineData("C280" + "C280" + "C280" + "C180")] // Overlong 2-byte sequence at end of DWORD, within "stay in 2-byte processing" optimization
        [InlineData("3031" + "E09F80" + EURO_SYMBOL_UTF8 + EURO_SYMBOL_UTF8)] // Overlong 3-byte sequence at start of DWORD
        [InlineData("3031" + "E07F80" + EURO_SYMBOL_UTF8 + EURO_SYMBOL_UTF8)] // Improperly terminated 3-byte sequence at start of DWORD
        [InlineData("3031" + "E0C080" + EURO_SYMBOL_UTF8 + EURO_SYMBOL_UTF8)] // Improperly terminated 3-byte sequence at start of DWORD
        [InlineData("3031" + "E17F80" + EURO_SYMBOL_UTF8 + EURO_SYMBOL_UTF8)] // Improperly terminated 3-byte sequence at start of DWORD
        [InlineData("3031" + "E1C080" + EURO_SYMBOL_UTF8 + EURO_SYMBOL_UTF8)] // Improperly terminated 3-byte sequence at start of DWORD
        [InlineData("3031" + "EDA080" + EURO_SYMBOL_UTF8 + EURO_SYMBOL_UTF8)] // Surrogate 3-byte sequence at start of DWORD
        [InlineData("3031" + "E69C88" + "E59B" + "E69C88")] // Incomplete 3-byte sequence surrounded by valid 3-byte sequences
        [InlineData("3031" + "F5808080")] // [ F5 ] is always invalid
        [InlineData("3031" + "F6808080")] // [ F6 ] is always invalid
        [InlineData("3031" + "F7808080")] // [ F7 ] is always invalid
        [InlineData("3031" + "F8808080")] // [ F8 ] is always invalid
        [InlineData("3031" + "F9808080")] // [ F9 ] is always invalid
        [InlineData("3031" + "FA808080")] // [ FA ] is always invalid
        [InlineData("3031" + "FB808080")] // [ FB ] is always invalid
        [InlineData("3031" + "FC808080")] // [ FC ] is always invalid
        [InlineData("3031" + "FD808080")] // [ FD ] is always invalid
        [InlineData("3031" + "FE808080")] // [ FE ] is always invalid
        [InlineData("3031" + "FF808080")] // [ FF ] is always invalid
        public void IsValid_WithLargeInvalidBuffers(string utf8HexInput)
        {
            Assert.True(utf8HexInput.Length >= 8);
            Assert.False(Utf8.IsValid(DecodeHex(utf8HexInput)));
        }
        ...
        [Theory]
        [InlineData(X_UTF8 + "80" + X_UTF8)] // stray continuation byte [ 80 ]
        [InlineData(X_UTF8 + "FF" + X_UTF8)] // invalid UTF-8 byte [ FF ]
        [InlineData(X_UTF8 + "C2" + X_UTF8)] // 2-byte sequence starter [ C2 ] not followed by continuation byte
        [InlineData(X_UTF8 + "C1C180" + X_UTF8)] // [ C1 80 ] is overlong but consists of two maximal invalid subsequences, each of length 1 byte
        [InlineData(X_UTF8 + E_ACUTE_UTF8 + "E08080")] // [ E0 80 ] is overlong 2-byte sequence (1 byte maximal invalid subsequence), and following [ 80 ] is stray continuation byte
        [InlineData(GRINNING_FACE_UTF8 + "F08F8080" + GRINNING_FACE_UTF8)] // [ F0 8F ] is overlong 4-byte sequence (1 byte maximal invalid subsequence), and following [ 80 ] instances are stray continuation bytes
        [InlineData(GRINNING_FACE_UTF8 + "F4908080" + GRINNING_FACE_UTF8)] // [ F4 90 ] is out-of-range 4-byte sequence (1 byte maximal invalid subsequence), and following [ 80 ] instances are stray continuation bytes
        [InlineData(E_ACUTE_UTF8 + "EDA0" + X_UTF8)] // [ ED A0 ] is encoding of UTF-16 surrogate code point, so consists of two maximal invalid subsequences, each of length 1 byte
        [InlineData(E_ACUTE_UTF8 + "ED80" + X_UTF8)] // [ ED 80 ] is incomplete 3-byte sequence, so is 2-byte maximal invalid subsequence
        [InlineData(E_ACUTE_UTF8 + "F380" + X_UTF8)] // [ F3 80 ] is incomplete 4-byte sequence, so is 2-byte maximal invalid subsequence
        [InlineData(E_ACUTE_UTF8 + "F38080" + X_UTF8)] // [ F3 80 80 ] is incomplete 4-byte sequence, so is 3-byte maximal invalid subsequence
        public void IsValid_WithReplacement(string utf8HexInput)
        {
            Assert.False(Utf8.IsValid(DecodeHex(utf8HexInput)));
        }

        [Fact]
        public void IsValid_AllPossibleScalarValues()
        {
            Assert.True(Utf8.IsValid(s_allScalarsAsUtf8.Span));
        }
    }
}
