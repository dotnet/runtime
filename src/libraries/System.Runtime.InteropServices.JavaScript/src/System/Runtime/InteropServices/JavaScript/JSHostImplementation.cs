...
namespace System.Runtime.InteropServices.JavaScript
{
    internal static partial class JSHostImplementation
    {
        ...
        public static unsafe JSFunctionBinding BindManagedFunction(Action<IntPtr> wrapper, string methodName, int signatureHash, ReadOnlySpan<JSMarshalerType> signatures)
        {
            var ctx = JSProxyContext.CurrentThreadContext;
            var clazz = wrapper.Method.DeclaringType!;
            var assemblyName = clazz.Assembly.GetName().Name ?? string.Empty;
            var shortClassName = clazz.Name;
            var nameSpace = clazz.Namespace ?? string.Empty;
            var containing = string.Empty;
            while (clazz.DeclaringType != null)
            {
                clazz = clazz.DeclaringType;
                containing = string.IsNullOrEmpty(containing)
                                ? clazz.Name
                                : clazz.Name + "." + containing;
            }
