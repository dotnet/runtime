...
public sealed partial class SseTests
{
    ...
    public void ConvertToNIntWithTruncation_nint_64Bit(long lower, long upper, long expectedResult)
    {
        nint expectedNativeResult = (nint)expectedResult;

        Vector128<float> vector = Vector128.Create(lower, upper).AsSingle();
        nint actualResult = Sse.ConvertToNIntWithTruncation(vector);
    ...
    [InlineData(0b0000_0000, 0b0000_0000, 0)]
    public void ConvertToNIntWithTruncation_nint_32Bit(long lower, long upper, int expectedResult)
    {
