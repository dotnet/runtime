...
{
    private static bool RunTests => Sse3.IsSupported;
    ...
    public void TestReflectionCalling()
    {
        if (RunTests)
        {
            ReflectionTester.Test(typeof(Sse3));
        }
        else
        {
            Assert.Throws<PlatformNotSupportedException>(() => ReflectionTester.Test(typeof(Sse3)));
        }
    }
}
