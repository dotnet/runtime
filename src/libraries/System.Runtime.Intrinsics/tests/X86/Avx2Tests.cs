...
{
    private static bool RunTests => Avx2.IsSupported;
    ...
    public void TestReflectionCalling()
    {
        if (RunTests)
        {
            ReflectionTester.Test(typeof(Avx2));
        }
        else
        {
            Assert.Throws<PlatformNotSupportedException>(() => ReflectionTester.Test(typeof(Avx2)));
        }
    }
}
