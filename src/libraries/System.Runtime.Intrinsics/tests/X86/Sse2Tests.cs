...
{
    private static bool RunTests => Sse2.IsSupported;
    ...
    public void TestReflectionCalling()
    {
        if (RunTests)
        {
            ReflectionTester.Test(typeof(Sse2));
        }
        else
        {
            Assert.Throws<PlatformNotSupportedException>(() => ReflectionTester.Test(typeof(Sse2)));
        }
    }
}
