...
namespace System.Text.Json
{
    ...
    internal struct WriteStack
    {
        ...
        /// </summary>
        public JsonConverter Initialize<TValue>(in TValue value, JsonSerializerOptions options, bool supportContinuation)
        {
            JsonTypeInfo jsonTypeInfo = options.GetOrAddClassForRootType(typeof(TValue));
            Debug.Assert(options == jsonTypeInfo.Options);
            return Initialize(in value, jsonTypeInfo, supportContinuation);
        }

        internal JsonConverter Initialize<TValue>(in TValue value, JsonTypeInfo jsonTypeInfo, bool supportContinuation)
        {
            // For polymorphic root types that are not System.Object, ensure that we have dispatched to the right converter ahead of serialization.
            // This is a temporary measure to minimize infrastructural interventions in WriteStack/JsonConverter.TryWrite implementations.
            // TODO should be removed once https://github.com/dotnet/runtime/issues/30083 is implemented.
            if (!typeof(TValue).IsValueType && value is not null && jsonTypeInfo.CanBeWritePolymorphic)
            {
                Type runtimeType = value.GetType();
                if (runtimeType != jsonTypeInfo.Type)
                {
                    jsonTypeInfo = jsonTypeInfo.Options.GetOrAddClass(runtimeType);
                }
            }
        ...
        /// <summary>
        /// Determines whether the next value to be written should be polymorphic.
        /// Assumes that polymorphic dispatch for root values has been handled ahead of serialization.
        /// </summary>
        public bool NextValueCanBePolymorphic =>
            _count > 0 && // polymorphism for root-level values is handled extraneously
            !Current.IsPolymorphicReEntryStarted && // already dispatched from another polymorphic converter
            Current.DeclaredJsonPropertyInfo!.RuntimeTypeInfo.CanBeWritePolymorphic;
        ...
        public void Pop(bool success)
        {
            ...
            if (!success)
            {
                ...
                {
                    // reached the root frame, no need to copy.
                    return;
                }
