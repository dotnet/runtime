    ...
    public abstract class OpenGenericTests
    {
        ...
        public void SerializeOpenGeneric()
        {
            ...
            foreach (object[] types in TypesWithOpenGenerics_ToSerialize())
            {
                ...
                ArgumentException ae = Assert.Throws<ArgumentException>(() => Serializer.Serialize(obj, type));
            }

            Type openNullableType = typeof(Nullable<>);

            obj = Activator.CreateInstance(openNullableType.MakeGenericType(typeof(int)));
            InvalidOperationException nse = Assert.Throws<InvalidOperationException>(() => Serializer.Serialize(obj, openNullableType));
            Assert.Contains(openNullableType.ToString(), nse.ToString());
        }
