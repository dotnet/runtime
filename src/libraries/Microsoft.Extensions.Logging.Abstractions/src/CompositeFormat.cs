...
namespace Microsoft.Extensions.Logging
{
    ...
    internal sealed class InternalCompositeFormat
    {
        ...
        private InternalCompositeFormat(string format, (string? Literal, int ArgIndex, int Alignment, string? Format)[] segments)
        {
            // Store the format.
        ...
        /// <returns>The parsed <see cref="InternalCompositeFormat"/>.</returns>
        /// <exception cref="ArgumentNullException"><paramref name="format"/> is null.</exception>
        /// <exception cref="FormatException">A format item in <paramref name="format"/> is invalid.</exception>
        public static InternalCompositeFormat Parse(string format)
        {
        ...
        /// <param name="format">The string to parse.</param>
        /// <param name="compositeFormat">The parsed <see cref="InternalCompositeFormat"/> if parsing was successful; otherwise, null.</param>
        /// <returns><see langword="true"/> the <paramref name="format"/> can be parsed successfully; otherwise, <see langword="false"/>.</returns>
        public static bool TryParse(string? format, [NotNullWhen(true)] out InternalCompositeFormat? compositeFormat)
        {
            if (format is not null)
            {
                ...
                {
                    compositeFormat = new InternalCompositeFormat(format, segments.ToArray());
                    return true;
                }
            }
        ...
        /// <summary>Gets the original composite format string used to create this <see cref="InternalCompositeFormat"/> instance.</summary>
        ...
        private static bool TryParseLiterals(ReadOnlySpan<char> format, List<(string? Literal, int ArgIndex, int Alignment, string? Format)> segments)
        {
            ...
            var vsb = new ValueStringBuilder(stackalloc char[StackallocCharBufferSizeLimit]);
