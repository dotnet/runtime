    ...
    public sealed class ZipPackage : Package
    {
        ...
        protected override PackagePart[] GetPartsCore()
        {
            ...
            foreach (ZipArchiveEntry zipArchiveEntry in zipArchiveEntries)
            {
                ...
                if (IsZipItemValidOpcPartOrPiece(zipArchiveEntry.FullName))
                {
                    ...
                    if (PieceNameHelper.TryCreatePieceInfo(zipArchiveEntry, out PieceInfo? pieceInfo))
                    {
            ...
            List<PackagePart> parts)
        {
            ...
            parts.Add(new ZipPackagePart(this, _zipArchive, _zipStreamManager, pieces.GetRange(startIndex, numItems), partUri, contentType.ToString(),
                GetCompressionOptionFromZipFileInfo(pieces[startIndex].ZipArchiveEntry)));
        }
        ...
        private sealed class ContentTypeHelper
        {
            ...
            internal ContentTypeHelper(ZipArchive zipArchive, FileMode packageFileMode, FileAccess packageFileAccess, ZipStreamManager zipStreamManager, IgnoredItemHelper ignoredItemHelper)
            {
                ...
                //__contentTypeZipArchiveEntry - We will either find an atomin part, or
            ...
            private Stream? OpenContentTypeStream(ReadOnlyCollection<ZipArchiveEntry> zipFiles)
            {
                ...
                SortedDictionary<PieceInfo, ZipArchiveEntry>? contentTypeStreamPieces = null;

                foreach (ZipArchiveEntry zipFileInfo in zipFiles)
                {
                    if (zipFileInfo.FullName.ToUpperInvariant().StartsWith(ContentTypesFileUpperInvariant, StringComparison.Ordinal))
                    {
                        // Atomic name.
                        ...
                        // Piece name.
                        else if (PieceNameHelper.TryCreatePieceInfo(zipFileInfo, out PieceInfo? pieceInfo))
                        {
                            // Lazy init.
                            if (contentTypeStreamPieces == null)
                                contentTypeStreamPieces = new SortedDictionary<PieceInfo, ZipArchiveEntry>(PieceNameHelper.PieceNameComparer);
                ...
                if (contentTypeStreamPieces != null)
                {
                    ...
                    if (pieces[0].PieceNumber != 0)
                    {
                        ...
                        _ignoredItemHelper.AddItemsForInvalidSequence(ContentTypesFileUpperInvariant, pieces, 0, pieces.Count);
                        ...
                        if (pieces != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (lastPieceNumber < pieces.Count - 1)
                                {
                                    ...
                                    _ignoredItemHelper.AddItemsForInvalidSequence(ContentTypesFileUpperInvariant, pieces, lastPieceNumber + 1, pieces.Count - lastPieceNumber - 1);
                ...
                if (_contentTypeZipArchiveEntry != null && pieces != null)
                {
                ...
                if (pieces != null)
                {
                    ...
                    return new InterleavedZipPartStream(
                        pieces[0].PrefixName, _zipStreamManager,
