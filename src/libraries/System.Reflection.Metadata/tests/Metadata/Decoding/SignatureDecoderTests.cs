...
namespace System.Reflection.Metadata.Decoding.Tests
{
    public partial class SignatureDecoderTests
    {
        ...
        {
            using (FileStream stream = File.OpenRead(PathHelper.GetAssemblyLocation(typeof(VarArgsToDecode).GetTypeInfo().Assembly)))
        ...
        {
            using (FileStream stream = File.OpenRead(PathHelper.GetAssemblyLocation(typeof(SignaturesToDecode<>).GetTypeInfo().Assembly)))
            using (var peReader = new PEReader(stream))
            {
        ...
        {
            using (FileStream stream = File.OpenRead(PathHelper.GetAssemblyLocation(typeof(PinnedAndUnpinnedLocalsToDecode).GetTypeInfo().Assembly)))
            using (var peReader = new PEReader(stream))
            {
                MetadataReader reader = peReader.GetMetadataReader();
                var provider = new DisassemblingTypeProvider();
