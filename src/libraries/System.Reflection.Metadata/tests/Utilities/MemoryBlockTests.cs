...
namespace System.Reflection.Metadata.Tests
{
    public class MemoryBlockTests
    {
        ...
        public unsafe void Utf8NullTerminatedStringStartsWithAsciiPrefix()
        {
            ...
            fixed (byte* heapPtr = (heap = "Hello World!\0"u8))
            {
            ...
            fixed (byte* heapPtr = (heap = "x\0"u8))
            {
            ...
            fixed (byte* heapPtr = (heap = "abcx"u8))
            {
        ...
        public unsafe void DefaultDecodingFallbackMatchesBcl()
        {
            ...
            fixed (byte* ptr = (buffer = new byte[] { 0xC0 }))
            {
                ...
                s = new MemoryBlock(ptr, buffer.Length).PeekUtf8NullTerminated(0, "Hello"u8, decoder, out bytesRead);
        ...
        public unsafe void DecoderIsUsedCorrectly()
        {
            ...
            fixed (byte* fixedPtr = (buffer = "Test"u8))
            {
                ptr = fixedPtr;
                ...
                prefixed = true;
                Assert.Equal("Intercepted", new MemoryBlock(ptr, buffer.Length).PeekUtf8NullTerminated(0, "Prefix"u8, decoder, out bytesRead));
                Assert.Equal(buffer.Length, bytesRead);
            }
