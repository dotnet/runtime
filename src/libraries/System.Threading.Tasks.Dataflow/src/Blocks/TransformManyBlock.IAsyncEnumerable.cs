...
namespace System.Threading.Tasks.Dataflow
{
    public partial class TransformManyBlock<TInput, TOutput>
    {
        ...
        public TransformManyBlock(Func<TInput, IAsyncEnumerable<TOutput>> transform, ExecutionDataflowBlockOptions dataflowBlockOptions)
        {
            ...
            Initialize(messageId => ProcessMessage(transform, messageId), dataflowBlockOptions, ref _source!, ref _target!, ref _reorderingBuffer, async: true);
        }
        ...
        private async void ProcessMessage(Func<TInput, IAsyncEnumerable<TOutput>> transformFunction, KeyValuePair<TInput, long> messageWithId)
        {
            ...
            finally
            {
                ...
                {
                    await StoreOutputItemsAsync(messageWithId, null).ConfigureAwait(false);
                }
        ...
        private async ValueTask StoreOutputItemsAsync(
        ...
        private async ValueTask StoreOutputItemsReorderedAsync(long id, IAsyncEnumerable<TOutput>? item)
        {
        ...
        private async ValueTask StoreOutputItemsNonReorderedWithIterationAsync(IAsyncEnumerable<TOutput> outputItems)
        {
