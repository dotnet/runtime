...
namespace System.Threading.Tasks.Dataflow.Tests
{
    public class ConcurrentTests
    {
        ...
        private static BufferBlock<int> ConstructBufferNewWithNMessages(int messagesCount)
        {
            ...
            Assert.True(SpinWait.SpinUntil(() => block.Count == messagesCount, 30_000)); // spin until messages available
            return block;
        }
        ...
        private static TransformBlock<int, string> ConstructTransformWithNMessages(int messagesCount)
        {
            ...
            block.PostRange(0, messagesCount);
            Assert.True(SpinWait.SpinUntil(() => block.OutputCount == messagesCount, 30_000));
            return block;
        }
        ...
        private static TransformManyBlock<int, int> ConstructTransformManyWithNMessages(int messagesCount)
        {
            ...
            block.PostRange(0, messagesCount);
            Assert.True(SpinWait.SpinUntil(() => block.OutputCount == messagesCount, 30_000)); // spin until messages available
            return block;
        }
