...
public:
    ...
    //static int emitIns_O_R_R_R(int op, int rd, int rj, int rk) {//emitIns_R_R_R_O
    //    _ASSERTE(!(rd >> 5));
    //    _ASSERTE(!(rj >> 5));
    //    _ASSERTE(!(rk >> 5));
    //    _ASSERTE(!(op >> 6));
    //    return (rd & 0x1f) | ((rj & 0x1f)<<5) | ((rk & 0x1f)<<10) | ((op & 0x3f)<<26);
    //}
...
struct StubPrecode {
    ...
    //pcaddi $r21,4       //4, for Type encoding.
    //ld.d  $t2,$r21,8
    //ld.d  $r21,$r21,0
    //jirl  $r0,$r21,0
    // dcd pTarget
...
struct NDirectImportPrecode {
    ...
    //pcaddi $r21,1        //1, for Type encoding.
    //ld.d  $t2,$r21,20   //=m_pMethodDesc
    //ld.d  $r21,$r21,12   //=m_pTarget
    //jirl  $r0,$r21,0
    DWORD   m_rgCode[4];
    TADDR   m_pTarget;
    TADDR   m_pMethodDesc;
...
struct FixupPrecode {
    ...
    //pcaddi  $r21,3           //3, for Type encoding.
    //addi.d  $t2,$r21,-12
    //ld.d  $r21,$r21,4
    //jirl  $r0,$r21,0
    // dcd m_pTarget
    ...
    void InitCommon()
    {
        ...
        m_rgCode[0] = 0x18000075; //pcaddi  $r21,3         //3, for Type encoding.
        m_rgCode[1] = 0x02ffd2ae; //addi.d  $t2,$r21,-12
        m_rgCode[2] = 0x28c012b5; //ld.d  $r21,$r21,4
        m_rgCode[3] = 0x4c0002a0; //jirl  $r0,$r21,0

        _ASSERTE((UINT32*)&m_pTarget == &m_rgCode[4]);

        _ASSERTE(4 == ARRAY_SIZE(m_rgCode));
    }
    ...
    static BOOL IsFixupPrecodeByASM(PCODE addr)
    {
        ...
        return
            ((pInstr[0] == 0x18000075) && //pcaddi  $r21,3         //3, for Type encoding.
             (pInstr[1] == 0x02ffd2ae) && //addi.d  $t2,$r21,-12
             (pInstr[2] == 0x28c012b5) && //ld.d  $r21,$r21,4
             (pInstr[3] == 0x4c0002a0));  //jirl  $r0,$r21,0
    }
...
public:
    CallCountingStubShort(CallCountingStubShort* stubRX, CallCount *remainingCallCountCell, PCODE targetForMethod)
        : m_part0{
            0x18000015, //pcaddi  $r21,0
            0x28c0c2b5, //ld.d  $r21, $r21, #(m_remainingCallCountCell) =48
            0x284002ad, //ld.h  $t1, $r21, 0
            0x02bffdad, //addi.w  $t1, $t1, -1
            0x294002ad, //st.h  $t1, $r21, 0
            0x580011a0, //beq   $t1, $zero, //L0 =16
            0x18000015, //pcaddi  $r21,0
            0x28C082b5, //ld.d  $r21, $r21, #(m_targetForMethod) =32
            0x4c0002a0, //jirl  $r0,$r21,0
//L0:
            0x19fffeed, //pcaddi  $t1,-9
            0x28c101b5, //ld.d  $r21, $t1, #(m_targetForThresholdReached) =64
            0x4c0002a0}, //jirl  $r0,$r21,0
