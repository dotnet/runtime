...
class AlternateBlockHelper
{
//    COR_ILMETHOD* pHeader;
...
public:
    ...
    AlternateBlockHelper(MethodDesc *pMD, SArray<uint32_t> *pTypeRefTokenStreamInput) :
//        pHeader(pMD->GetILHeader(TRUE)),
    ...
    // Convert a token which may be a memberref parent into a member ref coded index
    uint32_t DecodeMemberRefCodedIndexToken(uint32_t codedIndex)
    {
        static const mdToken s_tableTokenTypes[] = {mdtTypeDef, mdtTypeRef, mdtModuleRef, mdtMethodDef, mdtTypeSpec, 0, 0, 0};
        return TokenFromRid((codedIndex & 0xFFFFFF8) >> 3, s_tableTokenTypes[codedIndex & 0x7]);
    }
    ...
    uint32_t MemberRefParentCodedIndex(mdToken tk)
    {
        ...
        for (uint32_t codedIndexType = 0; codedIndexType <= 7; codedIndexType++)
        {
            if (DecodeMemberRefCodedIndexToken(rid + codedIndexType) == tk)
            {
    ...
    class StandaloneSigTranslator
    {
        ...
        public:
        StandaloneSigTranslator(SigParser *sigInput, SigBuilder* sigOutput, AlternateBlockHelper *helper) :
            pSigInput(sigInput),
...
static MapSHash<MethodDesc*, MethodBlock*> s_methodBlocks;

void GenerateMethodBlock(MethodDesc *pMD, MethodBlock *pBlock)
{
    SArray<uint32_t> tokenStream;
    SArray<uint8_t> byteData;
    AlternateBlockHelper helper(pMD, &tokenStream);
...
{
    s_csMethodBlocks.Init(CrstLeafLock);
}
...
MethodBlock* GetMethodBlock(MethodDesc *pMD)
{
    ...
    {
        CrstHolder lock(&s_csMethodBlocks);
        if (s_methodBlocks.Lookup(pMD, &retVal))
        {
            return retVal;
        }
    }

    NewHolder<MethodBlock> newMethodBlock = new MethodBlock();
...

    {
        ...
        s_methodBlocks.Add(pMD, newMethodBlock);
