...
#ifndef DACCESS_COMPILE
CRLToCOMMethodFrame::CRLToCOMMethodFrame(TransitionBlock * pTransitionBlock, MethodDesc * pMD)
    : FramedMethodFrame(pTransitionBlock, pMD)
{
    LIMITED_METHOD_CONTRACT;
}
...
//virtual
void CRLToCOMMethodFrame::GcScanRoots(promote_func* fn, ScanContext* sc)
{
    WRAPPER_NO_CONTRACT;

    // CRLToCOMMethodFrame is only used in the event call / late bound call code path where we do not have IL stub
...
// by a CRLToCOMMethodFrame or a ComMethodFrame. Since GCScanRoots for those frames are
