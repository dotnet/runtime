...
BOOL GetDefaultDllImportSearchPathsAttributeValue(Module *pModule, mdToken token, DWORD * pDllImportSearchPathFlags)
{
    ...
    ParseKnownCaArgs(ca, args, MINIPAL_LENGTHOF(args));
...
int GetLCIDParameterIndex(MethodDesc *pMD)
{
    ...
    if (hr == S_OK)
    {
        ...
        IfFailGo(ParseKnownCaArgs(caLCID, args, MINIPAL_LENGTHOF(args)));
...
#ifdef _DEBUG
        WCHAR      logStr[200];
        swprintf_s(logStr, MINIPAL_LENGTHOF(logStr), W("Object gone: CoReleaseMarshalData returned %x, file %S, line %d\n"), hr, __FILE__, __LINE__);
        LogInterop(logStr);
        if (hr != S_OK)
        {
