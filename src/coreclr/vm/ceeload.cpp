...
void Append_Next_Item(LPWSTR* ppCursor, SIZE_T* pRemainingLen, LPCWSTR pItem, bool addSpace)
{
    ...
    SIZE_T itemLen = minipal_u16_strlen((const CHAR16_T*)pItem);
...
void SaveManagedCommandLine(LPCWSTR pwzAssemblyPath, int argc, LPCWSTR *argv)
{
    ...
    SIZE_T  commandLineLen = (minipal_u16_strlen((const CHAR16_T*)osCommandLine) + 1);

    // We will append pwzAssemblyPath to the 'corerun' osCommandLine
    commandLineLen += (minipal_u16_strlen((const CHAR16_T*)pwzAssemblyPath) + 1);

    for (int i = 0; i < argc; i++)
    {
        commandLineLen += (minipal_u16_strlen((const CHAR16_T*)argv[i]) + 1);
    }
