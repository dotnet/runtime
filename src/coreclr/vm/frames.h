...
Calli:
 The same as P/Invoke. The IL stub is created when the caller is JIT-compiled
 (see code:CEEInfo::convertPInvokeCalliToCall) and receives the unmanaged target
...
#ifdef HOST_64BIT
        // m_Datum is a MethodDesc* when it is set. It is NULL for indirect calls that do not
        // have a stub secret argument.
