...
struct HostStruct
{
    int32_t (*add_system_assembly_search_path)(const char* raw_path);
    ...
    MonoDomain* (*create_assembly_load_context)(const char* raw_name);
    ...
    MonoDomain* (*get_system_assembly_load_context)();
    ...
    intptr_t (*load_assembly_from_data)(MonoDomain* rawAlc, const char* data, int64_t size);
    intptr_t (*load_assembly_from_path)(MonoDomain* rawAlc, const char* path, int32_t length);
    ...
    MonoObject* (*unload_assembly_load_context)(MonoDomain* raw_alc);
...
static SString* s_AssemblyDir = NULL;
static SString* s_EtcDir = NULL;
static SString* s_AssemblyPaths = NULL;
...
{
    return mono_unity_class_get(image, type_token);
}
...
{
    GCX_PREEMP(); // temporary until we sort out our GC thread model
    return (MonoAssembly*)g_HostStruct->load_assembly_from_path(domain, name, (int32_t)strlen(name));;
}

extern "C" EXPORT_API MonoDomain* EXPORT_CC mono_domain_create_appdomain(const char* domainname, const char* configfile)
{
    return g_HostStruct->create_assembly_load_context(domainname);
}
...
{
    TRACE_API("%p", domain);
    g_HostStruct->unload_assembly_load_context(domain);
}
...
// Generated by UnityEmbedHost.Generator - Commit these changes
extern "C" EXPORT_API MonoObject* EXPORT_CC mono_gchandle_get_target_v2(uintptr_t handleIn)
{
    GCX_PREEMP(); // temporary until we sort out our GC thread model
    return (MonoObject*)g_HostStruct->gchandle_get_target_v2(handleIn);
}
...
extern "C" EXPORT_API MonoClass* EXPORT_CC mono_unity_class_get(MonoImage* assemblyIntPtrHandle, guint32 type_token)
{
    CONTRACTL
    {
        THROWS;
    ...
    ASSEMBLYREF assemblyRef = ASSEMBLYREF_From_AssemblyIntPtrHandle(assemblyIntPtrHandle);
    TypeHandle th = ClassLoader::LoadTypeDefThrowing(assemblyRef->GetAssembly()->GetModule(),
        type_token,
...
extern "C" EXPORT_API MonoMethod* EXPORT_CC mono_get_method (MonoImage* assemblyIntPtrHandle, guint32 token, MonoClass * klass)
{
    CONTRACTL
    {
        THROWS;
    ...
    EX_TRY
    {
        methodDesc = MemberLoader::GetMethodDescFromMemberDefOrRefOrSpec(assemblyRef->GetAssembly()->GetModule(),
            token,
            nullptr,
            false,
            false);
    ...
    EX_CATCH
    {
        ...
        unity_log("Exception calling %s: %d\n", mono_get_method, token);
    }
    EX_END_CATCH(SwallowAllExceptions)

    return (MonoMethod*)methodDesc;
}
...
extern "C" EXPORT_API MonoImage* EXPORT_CC mono_image_open_from_data_with_name(char *data, guint32 data_len, gboolean need_copy, int *status, gboolean refonly, const char *name)
{
    GCX_PREEMP(); // temporary until we sort out our GC thread model
    gint64 len = data_len;
    return (MonoImage*)g_HostStruct->load_assembly_from_data(gCurrentDomain, data, len);
}
...
extern "C" EXPORT_API void EXPORT_CC mono_unity_domain_unload(MonoDomain * domain, MonoUnityExceptionFunc callback)
{
    ...
    MonoObject *exc = g_HostStruct->unload_assembly_load_context(domain);
    if (exc)
        callback(exc);
}
...
// we don't actually bother with detaching and attaching the thread at all.
// TODO: Investigate whether or not we need to alter preemptive GC mode here.
