...
struct HostStruct
{
    ...
    int32_t (*image_get_table_rows)(MonoImage* image, int32_t table_id);
...
// Generated by UnityEmbedHost.Generator - Commit these changes
extern "C" EXPORT_API int32_t EXPORT_CC mono_image_get_table_rows(MonoImage* image, int32_t table_id)
{
    GCX_PREEMP(); // temporary until we sort out our GC thread model
    return g_HostStruct->image_get_table_rows(image, table_id);
}
...
{
    GCX_PREEMP(); // temporary until we sort out our GC thread model
    return g_HostStruct->unity_class_get(image, type_token);
}
