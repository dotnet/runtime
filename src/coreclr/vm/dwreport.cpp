    ...
    )                     // Size of buf, wide chars.
{
    ...
    _snwprintf_s(buf, MINIPAL_LENGTHOF(buf), _TRUNCATE, W("\\StringFileInfo\\%04x%04x\\FileDescription"),
    ...
    int cchBuf)                     // Size of buf, wide chars.
{
    ...
    WCHAR buf[64];                                                 //----+----1----+----2----+----3----+----4----+
    _snwprintf_s(buf, MINIPAL_LENGTHOF(buf), _TRUNCATE, W("\\StringFileInfo\\%04x%04x\\Assembly Version"),
               translation->language, translation->codePage);
