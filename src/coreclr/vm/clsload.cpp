                                              ...
                                              ClassLoadLevel level/*=CLASS_LOADED*/)
{
    ...
    TypeKey key(callConv, ntypars, inst, numCustomMods, customMods);
    RETURN(LoadConstructedTypeThrowing(&key, fLoadTypes, level));
}
...
TypeHandle ClassLoader::CreateTypeHandleForTypeKey(TypeKey* pKey, AllocMemTracker* pamTracker)
{
    ...
    else if (pKey->GetKind() == ELEMENT_TYPE_FNPTR)
    {
        ...
        FnPtrTypeDescCustomMod* dstMods = NULL;
        if (numCustomMods)
        {
            FnPtrTypeDescCustomMod* srcMods = pKey->GetCustomMods();

            dstMods = (FnPtrTypeDescCustomMod*) pamTracker->Track(loaderHeap->AllocMem(
