...
ProcessFuncletsForGCReporting:
                ...
                if ((m_fProcessNonFilterFunclet == true) || (m_fProcessIntermediaryNonFilterFunclet == true) || (m_flags & (FUNCTIONSONLY | SKIPFUNCLETS)))
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (!m_sfParent.IsNull() || m_crawl.pFunc->IsDiagnosticsILStub())
                            {
                                STRESS_LOG4(LF_GCROOTS, LL_INFO100,
                                    "STACKWALK: %s: not making callback for this frame, SPOfParent = %p, \
                                    isDiagnosticsILStub = %d, m_crawl.pFunc = %pM\n",
                                    (!m_sfParent.IsNull() ? "SKIPPING_TO_FUNCLET_PARENT" : "IS_DIAGNOSTICS_IL_STUB"),
                                    m_sfParent.SP,
                                    (m_crawl.pFunc->IsDiagnosticsILStub() ? 1 : 0),
                                    m_crawl.pFunc);
                ...
                {
                    if (m_crawl.pFunc->IsDiagnosticsILStub())
                    {
                        LOG((LF_GCROOTS, LL_INFO100000,
                             "STACKWALK: IS_DIAGNOSTICS_IL_STUB: not making callback for this frame, m_crawl.pFunc = %s\n",
