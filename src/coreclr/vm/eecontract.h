...
// perfectly reasonable to find EE_THREAD_REQUIRED inside the scope that does not have that.
// (e.g., a scope can have two possible code paths--one with an EE Thread and one without).
// So we can't do any meaningful testing statically. It's all gotta be done at runtime.
#define EE_THREAD_REQUIRED   do { REQUEST_TEST(Contract::EE_THREAD_Required); } while(0)
