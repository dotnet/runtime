...
BOOL Thread::RedirectThreadAtHandledJITCase(PFN_REDIRECTTARGET pTgt)
{
    ...
    SetXStateFeaturesMask(pCtx, (XSTATE_MASK_AVX | XSTATE_MASK_AVX512 | /*XSATE_MASK_APX*/(0x80000)));
...
BOOL Thread::RedirectCurrentThreadAtHandledJITCase(PFN_REDIRECTTARGET pTgt, CONTEXT *pCurrentThreadCtx)
{
    ...
    if (srcFeatures != 0)
    {
        success = SetXStateFeaturesMask(pCurrentThreadCtx, srcFeatures & (XSTATE_MASK_AVX | XSTATE_MASK_AVX512 | /*XSATE_MASK_APX*/(0x80000)));
        _ASSERTE(success);
        if (!success)
            return FALSE;
    }
