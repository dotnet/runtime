...
// Apple thread local variables are described by a TLV descriptor that the linker emits into the
// `__thread_vars` section. Both descriptor layouts that have shipped so far are 24 bytes:
//
//     struct TLVDescriptor                     struct TLVDescriptor  // legacy
//     {                                        {
//         void*    (*thunk)(TLVDescriptor*);       void*    (*thunk)(TLVDescriptor*);
//         uint32_t key;                            uintptr_t key;
//         uint32_t offset;                         uintptr_t offset;
//         uint32_t initialContentDelta;        };
//         uint32_t initialContentSize;
//     };
//
// `thunk` is dyld's `tlv_get_addr`, which loads the thread local block of the image from the
// pthread TSD array of the current thread using `key` (allocating it on demand) and then adds
// `offset` to it. The JIT can inline that fast path, but it needs to know `key` and `offset`.
//
// `key` is read as the first 32-bit word following the thunk, which is correct for both layouts on
// little endian targets since pthread keys always fit into 32 bits. `offset` is not read from the
// descriptor; it is derived from the address of `t_ThreadStatics` on the current thread and then
// required to match the `offset` field of one of the two layouts. That way a descriptor layout we
// do not understand can never be mistaken for one we do - we simply keep calling the thunk.

// One past the largest pthread TSD key we can inline an access for. Darwin's TSD array has 768
// entries (the first 256 are reserved for the platform, PTHREAD_KEYS_MAX == 512 dynamic keys
// follow). Reading a key outside of that range means that we did not read a valid key, and it also
// keeps the TSD probe below from reading out of bounds.
static constexpr uint32_t TlvPthreadKeyLimit = 768;

// Reads the thread local block of the current thread from the pthread TSD array exactly the way
// the code generated by the JIT will read it.
static void* GetThreadLocalBlockFromTSD(uint32_t key)
{
    LIMITED_METHOD_CONTRACT;
    _ASSERTE(key < TlvPthreadKeyLimit);

    void* block;
#if defined(TARGET_ARM64)
    // The low 3 bits of TPIDRRO_EL0 hold the CPU number on Apple platforms and have to be masked off.
    uintptr_t tsd;
    __asm__ volatile ("mrs %0, TPIDRRO_EL0" : "=r" (tsd));
    block = ((void**)(tsd & ~(uintptr_t)7))[key];
#elif defined(TARGET_AMD64)
    __asm__ volatile ("movq %%gs:(,%1,8), %0" : "=r" (block) : "r" ((uint64_t)key));
#else
#error Unsupported architecture for Apple thread local storage
#endif
    return block;
}

static bool TryGetThreadStaticsTlvKeyAndOffset(void* threadVarsSection, uint32_t* pKey, size_t* pOffset)
{
    CONTRACTL
    {
        NOTHROW;
    ...
    *pOffset = 0;
    ...
    if (threadVarsSection == NULL)
    {
        return false;
    }

    uint8_t* descriptor = (uint8_t*)threadVarsSection;
    uint32_t key        = *(uint32_t*)(descriptor + sizeof(void*));
    if ((key == 0) || (key >= TlvPthreadKeyLimit))
    {
        // Key 0 is reserved by libpthread, so it can never be a thread local block key.
        return false;
    }

    // Invoke the thunk exactly the way the JIT does today. Besides guaranteeing that the thread
    // local block of coreclr is instantiated for the current thread, this validates that the
    // descriptor really is the one describing `t_ThreadStatics`.
    typedef void* (*TLVThunk)(void*);
    uint8_t* threadStatics = (uint8_t*)(*(TLVThunk*)descriptor)(descriptor);
    if (threadStatics != (uint8_t*)&t_ThreadStatics)
    {
        return false;
    }

    void* block = GetThreadLocalBlockFromTSD(key);
    if (block == NULL)
    {
        return false;
    }

    // The offset of `t_ThreadStatics` within the block is a property of the image layout, so it is
    // the same on every thread. Require it to match what the descriptor says, which validates the
    // key we read, the way we read the TSD array and our understanding of the descriptor layout all
    // at once. Anything else means our assumptions do not hold and we have to keep calling the thunk.
    ptrdiff_t offset = threadStatics - (uint8_t*)block;
    if (offset < 0)
    {
        return false;
    }

    uint32_t  offsetField       = *(uint32_t*)(descriptor + sizeof(void*) + sizeof(uint32_t));
    uintptr_t legacyOffsetField = *(uintptr_t*)(descriptor + 2 * sizeof(void*));
    if (((uintptr_t)offset != offsetField) && ((uintptr_t)offset != legacyOffsetField))
    {
        return false;
    }
...
void GetThreadLocalStaticBlocksInfo(CORINFO_THREAD_STATIC_BLOCKS_INFO* pInfo)
{
    ...
    uint32_t tlvKey = 0;
    size_t tlvOffset = 0;
    if (GetThreadStaticsTlvKeyAndOffset(pInfo->threadVarsSection, &tlvKey, &tlvOffset))
    {
        // The JIT can inline the fast path of `tlv_get_addr` instead of calling it. When this is
        // not possible, `tlsPthreadKey` stays 0 and the JIT falls back to calling the thunk stored
        // in `threadVarsSection`, which already returns the address of `t_ThreadStatics` (hence
        // `threadStaticBaseOffset` has to remain 0 in that case).
        pInfo->tlsPthreadKey = tlvKey;
        threadStaticBaseOffset = tlvOffset;
    }
