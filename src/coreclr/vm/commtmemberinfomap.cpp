...
void ComMTMemberInfoMap::SetupPropsForIClassX(size_t sizeOfPtr)
{
    ...
    for (i=0; i<nSlots; ++i)
    {
        if (pCMT->IsSlotAField(i))
        {
            ...
            ULONG cchpName = ((int)minipal_u16_strlen((const CHAR16_T*)rName.Ptr())) + 1;
...
UnLink:
            ...
            ULONG cchpNewName = ((int)minipal_u16_strlen((const CHAR16_T*)m_MethodProps[ixGet].pName)) + 4 + 1;
            ...
            pNewName = reinterpret_cast<WCHAR*>(m_sNames.Alloc((int)((4+minipal_u16_strlen((const CHAR16_T*)m_MethodProps[ixSet].pName))*sizeof(WCHAR)+2)));
    ...
    CDescPool   &sNames)                // Pool of possibly decorated names.
{
    ...
    else
    {
        ...
        ULONG len = ((int)minipal_u16_strlen((const CHAR16_T*)pName)) + 1;
    ...
    UINT        nSlots)                 // Count of entries
{
    ...
    else if (nSlots < 20)
    {
        ...
        for (iCur=0; iCur<nSlots; ++iCur)
        {
            ...
            {
                // Duplicate.
                DWORD cchName = (DWORD) minipal_u16_strlen((const CHAR16_T*)rProps[iCur].pName);
                ...
                ULONG len = ((int)minipal_u16_strlen((const CHAR16_T*)rcName)) + 1;
