...
public:
    ...
    // When the EE loads Vector<T> type, it asks the JIT what the maximum width, in bits, that
    // Vector<T> is allowed to be. Zero means the JIT doesn't support SIMD intrinsics, so the
    // EE should use the default size (i.e. the size of the IL implementation).
    virtual unsigned getMaxVectorTBitWidth(CORJIT_FLAGS cpuCompileFlags) { return 0; }
