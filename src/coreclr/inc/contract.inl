                                        ...
                                        int lineNum)
{
    ...
    if ((cCombinedLocks <= MINIPAL_LENGTHOF(m_rgTakenLockInfos)) &&
        (cCombinedLocks + cTakes > MINIPAL_LENGTHOF(m_rgTakenLockInfos)))
    {
    ...
    for (UINT i = cCombinedLocks;
         i < min (MINIPAL_LENGTHOF(m_rgTakenLockInfos), cCombinedLocks + cTakes);
         i++)
    {
...
inline void DbgStateLockData::LockReleased(DbgStateLockType dbgStateLockType, UINT cReleases, void * pvLock)
{
    ...
    for (UINT i = cCombinedLocks - cReleases;
         i < min (MINIPAL_LENGTHOF(m_rgTakenLockInfos), cCombinedLocks);
         i++)
    {
...
inline BOOL DbgStateLockState::IsLockRetaken(void * pvLock)
{
    ...
    for (UINT i = 0;
        i < min(MINIPAL_LENGTHOF(m_pLockData->m_rgTakenLockInfos), m_cLocksEnteringCannotRetakeLock);
        ++i)
    {
                     ...
                     int   lineNum)
{
    ...
    if (_check.EnterAssert())
    {
        ...
        sprintf_s(Buf,MINIPAL_LENGTHOF(Buf), "CONTRACT VIOLATION by %s at \"%s\" @ %d\n\n%s\n", szFunction, szFile, lineNum, szElaboration);
            ...
            while (pRec != NULL)
            {
                ...
                if (count != 0 || fshowconflict)
                {
                    ...
                    else
                    {
                        ...
                        strcat_s(Buf, MINIPAL_LENGTHOF(Buf), "\n                        ...");

                    }

                    sprintf_s(tmpbuf,MINIPAL_LENGTHOF(tmpbuf),
                            "\n%s  %s in %s at \"%s\" @ %d",
                    ...
                    strcat_s(Buf, MINIPAL_LENGTHOF(Buf), tmpbuf);
                }
        ...
        if (!foundconflict && count != 0)
        {
            ...
            {
                strcat_s(Buf,MINIPAL_LENGTHOF(Buf), "You can't throw here because there is no handler on the stack.\n");
            }
            else
            {
                strcat_s(Buf,MINIPAL_LENGTHOF(Buf), "We can't find the violated contract. Look for an old-style non-holder-based contract.\n");
            }
        }
