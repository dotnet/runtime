...
RETAIL_CONFIG_STRING_INFO(EXTERNAL_PerfMapIlSourcePath, W("PerfMapIlSourcePath"), "If set (and jitdump is enabled via PerfMapEnabled), emit JIT_CODE_DEBUG_INFO jitdump records that map native code to synthetic IL source files under this directory (contract: IL instruction at offset K is on line K + 2 of <dir>/<assembly>/m_<token>.il). Disabled when unset.")
#endif
