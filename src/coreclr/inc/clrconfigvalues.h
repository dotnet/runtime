...
RETAIL_CONFIG_DWORD_INFO(INTERNAL_CdacStress, W("CdacStress"), 0, "Enable cDAC stress verification. Split into byte regions: WHERE (byte 0) = trigger points {0x01=ALLOC}; WHAT (byte 1) = sub-checks {0x100=GCREFS, 0x200=ARGITER}; MODIFIERS (byte 2) {0x10000=VERBOSE}. Combine at least one WHERE and one WHAT (e.g. 0x101 = ALLOC + GCREFS).")
