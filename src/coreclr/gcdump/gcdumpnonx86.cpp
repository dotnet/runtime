...
PCSTR GetRegName (UINT32 regnum)
{
    ...
    if (regnum < 29)
    {
        _snprintf_s(szRegName, MINIPAL_LENGTHOF(szRegName), sizeof(szRegName), "X%u", regnum);
        return szRegName;
    }
    else if(regnum == 29)
    {
    ...
    static CHAR szRegName[16];
    _snprintf_s(szRegName, MINIPAL_LENGTHOF(szRegName), sizeof(szRegName), "r%u", regnum);
    return szRegName;
