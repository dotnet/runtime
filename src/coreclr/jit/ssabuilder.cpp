...
//------------------------------------------------------------------------
// RenameCall: Rename a local or memory definition generated by a GT_CALL node.
//
// Arguments:
//    asgNode - The GT_CALL node that generates the definition
//    block - The basic block that contains `callNode`
//
void SsaBuilder::RenameCall(GenTreeCall* callNode, BasicBlock* block)
{
    GenTreeLclVarCommon* retBufArg;
    bool                 isFullDef;
    bool                 isLocal = callNode->DefinesLocal(m_pCompiler, &retBufArg, &isFullDef);
    ...
    if (isLocal)
    {
        ...
        retBufArg->SetSsaNum(SsaConfig::RESERVED_SSA_NUM);
    }
}
