...
#ifdef TARGET_AMD64
//------------------------------------------------------------------------
// gtNewCastULongToDouble: Creates a GenTree for a Cast from ULONG to DOUBLE
// Arguments:
//    src           -
//
// Return Value:
...
//
//
GenTree* Compiler::gtNewCastULongToDouble(GenTree* origSrc)
{
    // X64/x86 only has an instruction that does a signed conversion
    // We have test if the upper bit is set
    //

    // We will need three copies of 'origSrc'
    //
    GenTree* src1 = nullptr;
    GenTree* src2 = nullptr;
    GenTree* src3 = nullptr;
    GenTree* src4 = nullptr;

    // Does 'origSrc' have no side-effects?
    //
    if ((origSrc->gtFlags & GTF_SIDE_EFFECT) == 0)
    {
        // Note that gtClone can return nullptr
        src2 = gtClone(origSrc, true);

        // If we are able to clone 'src' then
        // src1 is the orginal Src, which still needs to be used
        // src2 and src3 are clones of this expression
        //
        if (src2 != nullptr)
        {
            src1 = origSrc;
            src3 = gtClone(origSrc, true);
            src4 = gtClone(origSrc, true);
        }
    }

    // Store 'origSrc' in a new temp, when "src" has side effects or is too complex to clone.
    //
    if (src2 == nullptr)
    {
        // If src2 is nullptr then we are not able to clone 'src'
        //so we assign 'origSrc' to a new temp LclVar
        // and src1,src2 and src3 are new LclVar nodes of it.
        //
        unsigned srcLclNum = lvaGrabTemp(false DEBUGARG("src for cast of ulong to double"));
        GenTree* asg = gtNewTempAssign(srcLclNum, origSrc);
        src1 = gtNewLclvNode(srcLclNum, TYP_I_IMPL);
        src2 = gtNewLclvNode(srcLclNum, TYP_I_IMPL);
        src3 = gtNewLclvNode(srcLclNum, TYP_I_IMPL);
        src4 = gtNewLclvNode(srcLclNum, TYP_I_IMPL);
    }

    // ---------------------------------------------------------------------------------------------------------------------
    // the 'if' part

    // Compare src against zero
    //
    GenTree* relopNode  = gtNewOperNode(GT_LT, TYP_INT, src1, gtNewIconNode(0, TYP_I_IMPL));

    // ---------------------------------------------------------------------------------------------------------------------
    // the 'then' part

    // We perform a signed GT_CAST operation
    //
    GenTree* thenNode   = gtNewCastNode(TYP_DOUBLE, src2, false, TYP_DOUBLE);

    // ---------------------------------------------------------------------------------------------------------------------
    // the 'else' part

    // We divide 'src' by two using a right shift of one bit and we round in the IEEE required way by using an GT_OR
    //
    GenTree* rszNode = gtNewOperNode(GT_RSZ, TYP_I_IMPL, src4, gtNewIconNode(1, TYP_I_IMPL));
    GenTree* andNode = gtNewOperNode(GT_AND, TYP_I_IMPL, src3, gtNewIconNode(1, TYP_I_IMPL));
    GenTree* orNode = gtNewOperNode(GT_OR, TYP_I_IMPL, rszNode, andNode);

    // Next perform a signed GT_CAST operation on 'orNode' yielding a double that is exactly half of our final result
    //
    GenTree* cvtNode = gtNewCastNode(TYP_DOUBLE, orNode, false, TYP_DOUBLE);

    // Create a tmp for floating add in the 'elseNode'
    //
    unsigned tmpLclNum = lvaGrabTemp(true DEBUGARG("tmp for cast of ulong to double"));

    GenTree* tmpAsgNode = gtNewTempAssign(tmpLclNum, cvtNode);

    GenTree* faddNode = gtNewOperNode(GT_ADD, TYP_DOUBLE, gtNewLclvNode(tmpLclNum, TYP_DOUBLE), gtNewLclvNode(tmpLclNum, TYP_DOUBLE));

    GenTree* elseNode = gtNewOperNode(GT_COMMA, TYP_DOUBLE, tmpAsgNode, faddNode);

    // ---------------------------------------------------------------------------------------------------------------------
    // create the GT_QMARK and GT_COLON nodes
    //
    GenTree* colonNode  = new (this, GT_COLON) GenTreeColon(TYP_VOID, thenNode, elseNode);
    GenTree* resultNode = gtNewQmarkNode(TYP_VOID, relopNode, colonNode);

#ifdef DEBUG
    if (verbose)
    {
        printf("\ngtNewCastULongToDouble created tree is:\n");
        gtDispTree(resultNode);
        printf("\n");
    }
#endif
