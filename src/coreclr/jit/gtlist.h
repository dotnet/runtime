...
// Copies a variable from its current location to a register that satisfies
// code generation constraints. The operand is the actual lclVar node.
GTNODE(COPY             , GenTreeCopyOrReload,0,GTK_UNOP|DBK_NOTHIR)
GTNODE(RELOAD           , GenTreeCopyOrReload,0,GTK_UNOP|DBK_NOTHIR)
...
// Starts a new instruction group that will be non-gc interruptible.
GTNODE(START_NONGC      , GenTree            ,0,(GTK_LEAF|GTK_NOVALUE|DBK_NOTHIR))
// Starts a new instruction group where preemptive GC is enabled.
GTNODE(START_PREEMPTGC  , GenTree            ,0,(GTK_LEAF|GTK_NOVALUE|DBK_NOTHIR))
// Profiler Enter/Leave/TailCall hook.
GTNODE(PROF_HOOK        , GenTree            ,0,(GTK_LEAF|GTK_NOVALUE|DBK_NOTHIR))
...
#ifdef TARGET_ARM64
GTNODE(MADD             , GenTreeOp          ,0, GTK_BINOP|DBK_NOTHIR)    // Generates the Multiply-Add instruction (madd/msub)
...
#endif
GTNODE(PUTARG_TYPE      , GenTreeOp          ,0,GTK_UNOP|GTK_NOTLIR)              // operator that saves argument type between importation and morph
