...
GenTree* Compiler::impExpandHalfConstEqualsSIMD(GenTree* data, WCHAR* cns, int len, int dataOffset)
{
    ...
    GenTree* or   = gtNewSimdBinOpNode(GT_OR, simdType, xor1, xor2, type, simdSize, false);
    return gtNewSimdHWIntrinsicNode(TYP_BOOL, useSingleVector ? xor1 : or, zero, niEquals, type, simdSize);
#else
    return nullptr;
#endif
}
...
GenTreeStrCon* Compiler::impGetStrConFromSpan(GenTree* span)
{
    ...
    // TODO: Do the same for '"cns".AsSpan()' and 'new ReadOnlySpan("cns")'
    if ((argCall != nullptr) && (lookupNamedIntrinsic(argCall->gtCallMethHnd) == NI_System_String_op_Implicit))
    {
