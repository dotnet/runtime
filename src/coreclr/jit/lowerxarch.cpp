    ...
    switch (intrinsicId)
    {
        ...
        case NI_AVX2_BlendVariable:
        {
            ...
            if (op3->IsVectorPerElementMask(simdBaseType, simdSize) && (op1->IsVectorZero() || op2->IsVectorZero()))
            {
                ...
                {
                    binOp = comp->gtNewSimdBinOpNode(GT_AND_NOT, simdType, op1, op3, simdBaseType, simdSize);
                    BlockRange().Remove(op2);
                }
        ...
        case NI_AVX512_ConvertVectorToMask:
        {
            ...
            if (op1->IsCnsVec())
            {
                GenTree* mask = comp->gtFoldExprConvertVecCnsToMask(node, op1->AsVecCon());
...
GenTree* Lowering::LowerHWIntrinsicCndSel(GenTreeHWIntrinsic* node)
{
    ...
    if (op1->IsVectorPerElementMask(simdBaseType, simdSize))
    {
        ...
        if (blendVariableId != NI_Illegal)
        {
            ...
            resultNode = comp->gtNewSimdHWIntrinsicNode(simdType, op3, op2, op1, blendVariableId, simdBaseType, simdSize);
        }
    }
    ...
    if (resultNode == nullptr)
    {
        if (comp->compOpportunisticallyDependsOn(InstructionSet_AVX512))
        {
            ...
            resultNode = comp->gtNewSimdHWIntrinsicNode(simdType, op1, op2, op3, control, NI_AVX512_TernaryLogic,
        ...
        else
        {
            ...
            GenTree* tmp2 = comp->gtNewSimdBinOpNode(GT_AND, simdType, op2, op1, simdBaseType, simdSize);
            GenTree* tmp3 = comp->gtNewSimdBinOpNode(GT_AND_NOT, simdType, op3, tmp1, simdBaseType, simdSize);
            resultNode    = comp->gtNewSimdBinOpNode(GT_OR, simdType, tmp2, tmp3, simdBaseType, simdSize);
