                    ...
                    else
                    {
                        ...
                        if ((srcType == TYP_DOUBLE) && (dstType == TYP_UINT))
                        {
                            ...
                            if (comp->compOpportunisticallyDependsOn(InstructionSet_SSE42))
                            {
                                ...
                                floorVal = comp->gtNewSimdHWIntrinsicNode(TYP_SIMD16, floorVal,
                                                                          NI_SSE42_RoundToZeroScalar, srcBaseType, 16);
                                castRange.InsertAtEnd(floorVal);
                            }
                            else
                            {
                                // We don't have `roundsd` available, but we can truncate the value by simply zeroing
                                // out the low 21 bits of the double. This works because we know we will only use the
                                // negative value when the exponent is exactly 31, meaning 31 of the 52 bits in the
                                // significand are used for the whole portion of the number, and the remaining 21 bits
                                // are fractional.
                                //
                                // This creates the equivalent of the following C# code:
                        ...
                        if (dstType == TYP_UINT)
                        {
                            ...
                            else
                            {
                                ...
                                GenTree* mask      = comp->gtNewSimdBinOpNode(GT_RSH, TYP_SIMD16, result, thirtyOne,
                                                                              CORINFO_TYPE_INT, 16);
                                GenTree* andMask =
