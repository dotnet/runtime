...
public:
    ...
    const AllRegsMask AllRegsMask_CALLEE_TRASH =
        AllRegsMask(RBM_INT_CALLEE_TRASH, RBM_FLT_CALLEE_TRASH, RBM_MSK_CALLEE_TRASH);
#else
    const AllRegsMask AllRegsMask_CALLEE_SAVED = AllRegsMask(RBM_CALLEE_SAVED);
    const AllRegsMask AllRegsMask_CALLEE_TRASH = AllRegsMask(RBM_CALLEE_TRASH);
#endif

#if defined(TARGET_X86)

    const AllRegsMask AllRegsMask_CALLEE_TRASH_NOGC = GprRegsMask(RBM_EDX);
    // The registers trashed by profiler enter/leave/tailcall hook
    // See vm\i386\asmhelpers.asm for more details.
    const AllRegsMask AllRegsMask_PROFILER_ENTER_TRASH    = AllRegsMask_NONE;
    const AllRegsMask AllRegsMask_PROFILER_LEAVE_TRASH    = AllRegsMask();
    const AllRegsMask AllRegsMask_PROFILER_TAILCALL_TRASH = (AllRegsMask_CALLEE_TRASH & GprRegsMask(~RBM_ARG_REGS));

#ifdef FEATURE_USE_ASM_GC_WRITE_BARRIERS
    const AllRegsMask AllRegsMask_CALLEE_TRASH_WRITEBARRIER = GprRegsMask(RBM_EAX | RBM_EDX);
#else  // !FEATURE_USE_ASM_GC_WRITE_BARRIERS
    const AllRegsMask AllRegsMask_CALLEE_TRASH_WRITEBARRIER = AllRegsMask_CALLEE_TRASH;
#endif // !FEATURE_USE_ASM_GC_WRITE_BARRIERS

    // Registers no longer containing GC pointers after CORINFO_HELP_ASSIGN_REF and CORINFO_HELP_CHECKED_ASSIGN_REF.
    const AllRegsMask AllRegsMask_CALLEE_GCTRASH_WRITEBARRIER = GprRegsMask(RBM_EDX);

    // Registers killed by CORINFO_HELP_ASSIGN_BYREF.
    const AllRegsMask AllRegsMask_CALLEE_TRASH_WRITEBARRIER_BYREF = GprRegsMask(RBM_ESI | RBM_EDI | RBM_ECX);
    ...
    const AllRegsMask AllRegsMask_CALLEE_GCTRASH_WRITEBARRIER_BYREF = GprRegsMask(RBM_ECX);

    // The registers trashed by the CORINFO_HELP_STOP_FOR_GC helper
    const AllRegsMask AllRegsMask_STOP_FOR_GC_TRASH = AllRegsMask_CALLEE_TRASH;
    ...
    // Registers no longer containing GC pointers after CORINFO_HELP_ASSIGN_REF and CORINFO_HELP_CHECKED_ASSIGN_REF.
    const AllRegsMask AllRegsMask_CALLEE_GCTRASH_WRITEBARRIER = AllRegsMask_CALLEE_TRASH_NOGC;
    const AllRegsMask AllRegsMask_CALLEE_GCTRASH_WRITEBARRIER_BYREF =
        GprRegsMask(RBM_CALLEE_GCTRASH_WRITEBARRIER_BYREF);
    const AllRegsMask AllRegsMask_CALLEE_TRASH_WRITEBARRIER_BYREF = GprRegsMask(RBM_CALLEE_TRASH_WRITEBARRIER_BYREF);
    ...
    const AllRegsMask AllRegsMask_STOP_FOR_GC_TRASH =
        AllRegsMask(RBM_INT_CALLEE_TRASH & ~(RBM_INTRET | RBM_INTRET_1),
                    (RBM_FLT_CALLEE_TRASH & ~(RBM_FLOATRET | RBM_FLOATRET_1)),
                    RBM_MSK_CALLEE_TRASH);
    const AllRegsMask AllRegsMask_PROFILER_ENTER_TRASH = AllRegsMask((RBM_INT_CALLEE_TRASH & ~RBM_ARG_REGS),
                                                                     (RBM_FLT_CALLEE_TRASH & ~RBM_FLTARG_REGS),
                                                                     RBM_MSK_CALLEE_TRASH);
#else
    ...
    const AllRegsMask AllRegsMask_PROFILER_ENTER_TRASH = AllRegsMask_CALLEE_TRASH;
#endif // UNIX_AMD64_ABI

    const AllRegsMask AllRegsMask_PROFILER_LEAVE_TRASH    = AllRegsMask_STOP_FOR_GC_TRASH;
    const AllRegsMask AllRegsMask_PROFILER_TAILCALL_TRASH = AllRegsMask_PROFILER_LEAVE_TRASH;

    // The registers trashed by the CORINFO_HELP_INIT_PINVOKE_FRAME helper.
    const AllRegsMask AllRegsMask_INIT_PINVOKE_FRAME_TRASH     = AllRegsMask_CALLEE_TRASH;
    const AllRegsMask AllRegsMask_VALIDATE_INDIRECT_CALL_TRASH = GprRegsMask(RBM_VALIDATE_INDIRECT_CALL_TRASH);

#elif defined(TARGET_ARM)

    const AllRegsMask AllRegsMask_CALLEE_TRASH_NOGC    = GprRegsMask(_RBM_CALLEE_TRASH_NOGC);
    const AllRegsMask AllRegsMask_PROFILER_ENTER_TRASH = AllRegsMask_NONE;

    // Registers killed by CORINFO_HELP_ASSIGN_REF and CORINFO_HELP_CHECKED_ASSIGN_REF.
    const AllRegsMask AllRegsMask_CALLEE_TRASH_WRITEBARRIER =
        GprRegsMask(RBM_R0 | RBM_R3 | RBM_LR | RBM_DEFAULT_HELPER_CALL_TARGET);

    // Registers no longer containing GC pointers after CORINFO_HELP_ASSIGN_REF and CORINFO_HELP_CHECKED_ASSIGN_REF.
    const AllRegsMask AllRegsMask_CALLEE_GCTRASH_WRITEBARRIER = AllRegsMask_CALLEE_TRASH_WRITEBARRIER;

    // Registers killed by CORINFO_HELP_ASSIGN_BYREF.
    const AllRegsMask AllRegsMask_CALLEE_TRASH_WRITEBARRIER_BYREF =
    ...
    const AllRegsMask AllRegsMask_CALLEE_GCTRASH_WRITEBARRIER_BYREF = AllRegsMask_CALLEE_TRASH_NOGC;
    const AllRegsMask AllRegsMask_PROFILER_RET_SCRATCH              = GprRegsMask(RBM_R2);
    // While REG_PROFILER_RET_SCRATCH is not trashed by the method, the register allocator must
    // consider it killed by the return.
    const AllRegsMask AllRegsMask_PROFILER_LEAVE_TRASH    = AllRegsMask_PROFILER_RET_SCRATCH;
    const AllRegsMask AllRegsMask_PROFILER_TAILCALL_TRASH = AllRegsMask_NONE;
    ...
    // The registers trashed by the CORINFO_HELP_INIT_PINVOKE_FRAME helper.
    const AllRegsMask AllRegsMask_INIT_PINVOKE_FRAME_TRASH =
        (AllRegsMask_CALLEE_TRASH | GprRegsMask(RBM_PINVOKE_TCB | RBM_PINVOKE_SCRATCH));

    const AllRegsMask AllRegsMask_VALIDATE_INDIRECT_CALL_TRASH = GprRegsMask(RBM_INT_CALLEE_TRASH);

#elif defined(TARGET_ARM64)

    const AllRegsMask AllRegsMask_CALLEE_TRASH_NOGC = GprRegsMask(_RBM_CALLEE_TRASH_NOGC);
    const AllRegsMask AllRegsMask_PROFILER_ENTER_TRASH =
        AllRegsMask((RBM_INT_CALLEE_TRASH & ~(RBM_ARG_REGS | RBM_ARG_RET_BUFF | RBM_FP)),
                    (RBM_FLT_CALLEE_TRASH & ~RBM_FLTARG_REGS),
                    RBM_MSK_CALLEE_TRASH);
    // Registers killed by CORINFO_HELP_ASSIGN_REF and CORINFO_HELP_CHECKED_ASSIGN_REF.
    const AllRegsMask AllRegsMask_CALLEE_TRASH_WRITEBARRIER = GprRegsMask(RBM_R14 | _RBM_CALLEE_TRASH_NOGC);
    ...
    const AllRegsMask AllRegsMask_PROFILER_LEAVE_TRASH    = AllRegsMask_PROFILER_ENTER_TRASH;
    const AllRegsMask AllRegsMask_PROFILER_TAILCALL_TRASH = AllRegsMask_PROFILER_ENTER_TRASH;
