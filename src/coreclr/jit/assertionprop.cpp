                                            ...
                                            bool             helperCallArgs)
{
    ...
    if (op1->OperIs(GT_BOUNDS_CHECK))
    {
        if (assertionKind == OAK_NO_THROW)
        {
            ...
            assertion.op1.bnd.vnIdx      = optConservativeVN(arrBndsChk->GetIndex());
            assertion.op1.bnd.vnLen      = optConservativeVN(arrBndsChk->GetArrayLength());
            goto DONE_ASSERTION;
        }
    }
    ...
    else if (op1->gtOper == GT_LCL_VAR)
    {
        ...
        if (helperCallArgs)
        {
            ...
            assertion.op1.lcl.lclNum   = lclNum;
            assertion.op1.vn           = optConservativeVN(op1);
            assertion.op1.lcl.ssaNum   = op1->AsLclVarCommon()->GetSsaNum();
            assertion.op2.u1.iconVal   = op2->AsIntCon()->gtIconVal;
            assertion.op2.vn           = optConservativeVN(op2);
    ...
    else if (op1->gtOper == GT_IND)
    {
        ...
        if (op1->gtOper == GT_LCL_VAR)
        {
            ...
            if (op2->gtOper == GT_IND)
            {
                ...
                assertion.op2.vn         = optConservativeVN(op2->AsOp()->gtOp1);
