...
void Compiler::optAssertionInit(bool isLocalProp)
{
    ...
    if (isLocalProp)
    {
        ...
        if (optCrossBlockLocalAssertionProp)
        {
            ...
            // Allow for roughly 1.5 assertions per tracked local, up to the tracked limit.
            // (empirical studies show about 0.6 asserions/local)
            //
            optMaxAssertionCount = (AssertionIndex)min(maxTrackedLocals, ((lvaTrackedCount / 64) + 1) * 96);
        }
        else
        {
