...
/*****************************************************************************
 *
 *  Given a GT_NOT node, try to optimize away redundant NOT operations by
 *  following SSA def-use chains. This physically removes redundant NOT nodes
 *  from the IR before codegen.
 *
 *  For an even number of NOTs: replace with the base variable/expression.
 *  For an odd number: reduce to a single NOT of the base.
 *
 *  Returns the modified tree, or nullptr if no optimization took place.
 */

GenTree* Compiler::optAssertionProp_Not(ASSERT_VALARG_TP assertions, GenTreeOp* notNode, Statement* stmt)
{
    assert(notNode->OperIs(GT_NOT));

    // Follow the chain of NOT operations through SSA
    GenTree* currentNode = notNode->gtOp1;
    unsigned notCount    = 1; // Start with 1 for the current NOT

    // Unwind chains of NOT operations, including those through local variables
    while (currentNode != nullptr)
    {
        if (currentNode->OperIs(GT_NOT))
        {
            // Direct nested NOT
            notCount++;
            currentNode = currentNode->AsOp()->gtOp1;
        }
        else if (currentNode->OperIs(GT_LCL_VAR))
        {
            // Check if this local is defined by a NOT operation via SSA
            GenTreeLclVar* lclVar = currentNode->AsLclVar();
            unsigned       lclNum = lclVar->GetLclNum();
            unsigned       ssaNum = lclVar->GetSsaNum();

            // Skip if no SSA number (shouldn't happen in SSA form, but be safe)
            if (ssaNum == SsaConfig::RESERVED_SSA_NUM)
            {
                break;
            }

            // Get the SSA definition
            ...
            if (defNode == nullptr)
            {
                // No def node (uninitialized or param) - stop here
                break;
            }

            // Check if the definition is a store
            if (defNode->OperIs(GT_STORE_LCL_VAR))
            {
                GenTree* rhsNode = defNode->AsOp()->gtOp1;
                if (rhsNode->OperIs(GT_NOT))
                {
                    // The local is defined by a NOT - continue the chain
                    notCount++;
                    currentNode = rhsNode->AsOp()->gtOp1;
                    continue;
                }
            }

            // Not a NOT definition - stop here with currentNode as base
            break;
        }
        else
        {
            // Not a NOT or LCL_VAR - this is the base expression
            break;
        }
    }

    // If we found at least 2 NOT operations, we can optimize
    if (notCount < 2)
    {
        return nullptr;
    }

    // Get the base expression (currentNode)
    GenTree* baseExpr = currentNode;
    if (baseExpr == nullptr)
    {
    ...
    // Even number of NOTs: replace with base expression
    // Odd number of NOTs: replace with single NOT of base
    GenTree* replacement = nullptr;

    if ((notCount % 2) == 0)
    {
        // Even number: use base expression directly
        replacement = gtCloneExpr(baseExpr);
    }
    else
    {
        // Odd number: create single NOT of base
        GenTree* clonedBase = gtCloneExpr(baseExpr);
...
GenTree* Compiler::optAssertionProp(ASSERT_VALARG_TP assertions, GenTree* tree, Statement* stmt, BasicBlock* block)
{
    switch (tree->gtOper)
    {
        ...
        case GT_NOT:
            return optAssertionProp_Not(assertions, tree->AsOp(), stmt);
