...
//------------------------------------------------------------------------------
// fgSplitLdvirtftnIndirectCall : split ldvirtftn indirect call
//
PhaseStatus Compiler::fgSplitLdvirtftnIndirectCall()
{
    PhaseStatus result = PhaseStatus::MODIFIED_NOTHING;
    ...
    for (BasicBlock* const block : Blocks())
    {
        for (Statement* const stmt : block->Statements())
        {
            ...
            for (GenTree* const tree : stmt->TreeList())
            {
                if (!tree->IsCall() || tree->AsCall()->gtCallType != CT_INDIRECT ||
                    !tree->AsCall()->gtCallAddr->IsHelperCall(this, CORINFO_HELP_VIRTUAL_FUNC_PTR))
                {
                    continue;
                }

                modified |= fgSplitLdvirtftnIndirectCall(block, stmt, tree->AsCall());
            }
        }
    }
...
bool Compiler::fgSplitLdvirtftnIndirectCall(BasicBlock* block, Statement* stmt, GenTreeCall* call)
{
    assert(call->gtCallType == CT_INDIRECT && call->gtCallAddr->IsHelperCall(this, CORINFO_HELP_VIRTUAL_FUNC_PTR));
