...
class IndirectGenericVirtualWalker : public GenTreeVisitor<IndirectGenericVirtualWalker>
{
...
public:
    ...
    IndirectGenericVirtualWalker(Compiler* comp)
        : GenTreeVisitor(comp)
    {
    }
    ...
    fgWalkResult PreOrderVisit(GenTree** use, GenTree* user)
    {
        ...
        if (call->IsVirtualGeneric() && call->gtCallType == CT_INDIRECT)
        {
            CallArg* methodHnd = call->gtArgs.FindWellKnownArg(WellKnownArg::MethodInstHandle);
            if (methodHnd != nullptr)
            {
                call->gtArgs.Remove(methodHnd);
                JITDUMP("Removing MethodInstHandle argument from call [%06u].\n", m_compiler->dspTreeID(call));
                m_madeChanges = true;
            }
        }
...
//------------------------------------------------------------------------------
// fgExpandGenericVirtual : expand "new helpers" for generic virtual indirect calls
//
// Returns:
//    PhaseStatus indicating what, if anything, was changed.
//
PhaseStatus Compiler::fgExpandGenericVirtual()
{
    PhaseStatus result = PhaseStatus::MODIFIED_NOTHING;

    if (!doesMethodHaveGenericVirtual())
    {
        // The method being compiled doesn't have any generic virtual calls.
        return result;
    }

    IndirectGenericVirtualWalker walker(this);
    ...
    return walker.MadeChanges() ? PhaseStatus::MODIFIED_EVERYTHING : PhaseStatus::MODIFIED_NOTHING;
}
