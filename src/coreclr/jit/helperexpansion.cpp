...
bool Compiler::fgVNBasedIntrinsicExpansionForCall(BasicBlock** pBlock, Statement* stmt, GenTreeCall* call)
{
    ...
    // Expand String.StartsWith when one argument is a frozen string
    // from a static readonly field (detected via VN)
    if (ni == NI_System_String_StartsWith)
    {
        return fgVNBasedIntrinsicExpansionForCall_StringComparison(pBlock, stmt, call);
    }
...
//------------------------------------------------------------------------------
// fgVNBasedIntrinsicExpansionForCall_StringComparison:
//    Expand String.StartsWith when one argument is a frozen string
//    (e.g., from a static readonly field) that can be detected via VN.
//
//    This handles cases like:
//      private static readonly string s_prefix = "/api/data";
//      bool result = requestPath.StartsWith(s_prefix, StringComparison.Ordinal);
//
//    When s_prefix points to a frozen/immutable string object, we can read its
//    content at JIT time and unroll the comparison using SWAR/SIMD.
//
// Arguments:
//    pBlock - Block containing the call
//    stmt   - Statement containing the call
//    call   - The String.StartsWith call
//
// Returns:
//    True if expanded, false otherwise.
//
bool Compiler::fgVNBasedIntrinsicExpansionForCall_StringComparison(BasicBlock** pBlock,
                                                                   Statement*   stmt,
                                                                   GenTreeCall* call)
{
    BasicBlock* block = *pBlock;

    // This optimization creates several temps and additional code
    // Note: we check block->isRunRarely() here, not compCurBB, since we're in a late phase
    if (block->isRunRarely())
    {
        JITDUMP("StringComparison: block is cold - not profitable to expand.\n")
        return false;
    }

    // String.StartsWith has two overloads we care about:
    //   bool StartsWith(string value)  - 2 args (this + value)
    //   bool StartsWith(string value, StringComparison comparisonType) - 3 args
    //
    // We only support Ordinal and OrdinalIgnoreCase comparison types
    unsigned argCount = call->gtArgs.CountUserArgs();
    if (argCount < 2 || argCount > 3)
    {
        JITDUMP("StringComparison: unexpected arg count %u\n", argCount)
        return false;
    }
    ...
    GenTree* thisStr  = call->gtArgs.GetUserArgByIndex(0)->GetNode();
    GenTree* otherStr = call->gtArgs.GetUserArgByIndex(1)->GetNode();

    // Try to find a frozen string object from VN for one of the arguments
    ssize_t               strObjOffset = 0;
    CORINFO_OBJECT_HANDLE strObj       = nullptr;
    GenTree*              cnsArg       = nullptr;
    GenTree*              varArg       = nullptr;

    // Check if otherStr is a frozen object (the common case for StartsWith)
    // First verify tree has valid VN information
    if (!otherStr->gtVNPair.BothEqual() || otherStr->IsCall())
    {
        JITDUMP("StringComparison: otherStr doesn't have valid VN or is a call\n")
        return false;
    }

    if (GetObjectHandleAndOffset(otherStr, &strObjOffset, &strObj))
    {
        cnsArg = otherStr;
        varArg = thisStr;
    }
    // For Equals we could also check thisStr, but StartsWith is not commutative

    if (strObj == nullptr)
    {
        JITDUMP("StringComparison: neither argument is a frozen object\n")
        return false;
    }

    // Verify the object is immutable (frozen)
    if (!info.compCompHnd->isObjectImmutable(strObj))
    {
        JITDUMP("StringComparison: object is not immutable\n")
        return false;
    }

    // Get the string length
    int strLen = info.compCompHnd->getArrayOrStringLength(strObj);
    if (strLen < 0)
    {
        JITDUMP("StringComparison: could not get string length\n")
        return false;
    }

    // Check if the string is too long to unroll
    // Use the same threshold as import-time vectorization
    if (strLen > (int)(getUnrollThreshold(MemcmpU16) / 2))
    {
        JITDUMP("StringComparison: string too long to unroll (%d chars)\n", strLen)
        return false;
    }

    // Read the string content
    // Note: strings in .NET have their chars at offset OFFSETOF__CORINFO_String__chars
    const int             charsOffset = OFFSETOF__CORINFO_String__chars;
    constexpr int         maxChars    = 64; // Same as MaxPossibleUnrollSize in importervectorization.cpp
    uint16_t              strChars[maxChars];
    ...
    if (!info.compCompHnd->getObjectContent(strObj, (uint8_t*)strChars, strLen * sizeof(uint16_t), charsOffset))
    {
        JITDUMP("StringComparison: could not read string content\n")
        return false;
    }

#ifdef DEBUG
    JITDUMP("StringComparison: found frozen string of length %d: \"", strLen)
    for (int i = 0; i < min(strLen, 20); i++)
    {
        if (strChars[i] >= 32 && strChars[i] < 127)
        {
            JITDUMP("%c", (char)strChars[i]);
        }
        else
        {
            JITDUMP("\\x%04x", strChars[i]);
        }
    }
    if (strLen > 20)
    {
    ...
    // For OrdinalIgnoreCase, check that all chars are ASCII
    // (same restriction as import-time vectorization)
    if (cmpMode == OrdinalIgnoreCase)
    {
        for (int i = 0; i < strLen; i++)
        {
            if (strChars[i] > 127)
            {
                JITDUMP("StringComparison: non-ASCII char in OrdinalIgnoreCase mode\n")
                return false;
            }
        }
    }
    ...
    // - thisStr: the variable string
    // - strChars: the constant string content
    // - strLen: the constant string length
    // - cmpMode: Ordinal or OrdinalIgnoreCase

    // Split the block at the call site and replace the call with a local variable
    BasicBlock*    prevBb;
    DebugInfo      debugInfo    = stmt->GetDebugInfo();
    const unsigned resultLclNum = SplitAtTreeAndReplaceItWithLocal(this, block, stmt, call, &prevBb, &block);
    *pBlock = block;
    ...
    // Store the variable string to a temp for reuse
    GenTree* varStrArg         = call->gtArgs.GetUserArgByIndex(0)->GetNode();
    unsigned varStrTmp         = lvaGrabTemp(true DEBUGARG("varStr for StringComparison"));
    lvaTable[varStrTmp].lvType = TYP_REF;

    // Build the SIMD comparison using the simple impExpandHalfConstEquals overload
    compCurBB = prevBb;
    GenTree* cmpResult = impExpandHalfConstEquals(gtNewLclvNode(varStrTmp, TYP_REF)->AsLclVarCommon(),
                                                  (WCHAR*)strChars, strLen, OFFSETOF__CORINFO_String__chars, cmpMode);
    compCurBB = nullptr;

    if (cmpResult == nullptr)
    {
        JITDUMP("StringComparison: impExpandHalfConstEquals failed\n")
        return false;
    }

    // Build control flow:
    //   prevBb -> nullCheckBb -> lengthCheckBb -> fastpathBb -> block
    //                 |               |
    //                 +---------------+----------> block (early exit with false)

    BasicBlock* nullCheckBb = fgNewBBafter(BBJ_COND, prevBb, true);
    nullCheckBb->SetFlags(BBF_INTERNAL);
    nullCheckBb->bbCodeOffs = nullCheckBb->bbCodeOffsEnd = block->bbCodeOffsEnd;

    fgInsertStmtAtEnd(nullCheckBb, fgNewStmtFromTree(gtNewStoreLclVarNode(resultLclNum, gtNewFalse()), debugInfo));
    fgInsertStmtAtEnd(nullCheckBb, fgNewStmtFromTree(gtNewTempStore(varStrTmp, gtCloneExpr(varStrArg)), debugInfo));

    GenTree* nullCond = gtNewOperNode(GT_EQ, TYP_INT, gtNewLclvNode(varStrTmp, TYP_REF), gtNewNull());
    nullCond->gtFlags |= GTF_RELOP_JMP_USED;
    fgInsertStmtAtEnd(nullCheckBb, fgNewStmtFromTree(gtNewOperNode(GT_JTRUE, TYP_VOID, nullCond), debugInfo));

    BasicBlock* lengthCheckBb = fgNewBBafter(BBJ_COND, nullCheckBb, true);
    lengthCheckBb->SetFlags(BBF_INTERNAL);
    lengthCheckBb->bbCodeOffs = lengthCheckBb->bbCodeOffsEnd = block->bbCodeOffsEnd;

    GenTree* lenCond = gtNewOperNode(GT_LT, TYP_INT,
        gtNewArrLen(TYP_INT, gtNewLclvNode(varStrTmp, TYP_REF), OFFSETOF__CORINFO_String__stringLen),
        gtNewIconNode(strLen));
    lenCond->gtFlags |= GTF_RELOP_JMP_USED;
    fgInsertStmtAtEnd(lengthCheckBb, fgNewStmtFromTree(gtNewOperNode(GT_JTRUE, TYP_VOID, lenCond), debugInfo));

    BasicBlock* fastpathBb = fgNewBBafter(BBJ_ALWAYS, lengthCheckBb, true);
    fastpathBb->SetFlags(BBF_INTERNAL);
    fastpathBb->bbCodeOffs = fastpathBb->bbCodeOffsEnd = block->bbCodeOffsEnd;
    fgInsertStmtAtEnd(fastpathBb, fgNewStmtFromTree(gtNewStoreLclVarNode(resultLclNum, cmpResult), debugInfo));

    // Wire up control flow
    fgRedirectEdge(prevBb->TargetEdgeRef(), nullCheckBb);
    nullCheckBb->inheritWeight(prevBb);

    nullCheckBb->SetTrueEdge(fgAddRefPred(block, nullCheckBb));
    nullCheckBb->SetFalseEdge(fgAddRefPred(lengthCheckBb, nullCheckBb));

    lengthCheckBb->SetTrueEdge(fgAddRefPred(block, lengthCheckBb));
    lengthCheckBb->SetFalseEdge(fgAddRefPred(fastpathBb, lengthCheckBb));

    fastpathBb->SetTargetEdge(fgAddRefPred(block, fastpathBb));
    block->inheritWeight(prevBb);
    ...
    JITDUMP("StringComparison: successfully expanded!\n")
    return true;
}
