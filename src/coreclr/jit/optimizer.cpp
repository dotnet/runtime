        ...
        for (BasicBlock* const predBlock : block->PredBlocks())
        {
            ...
            switch (bb->GetBBJumpKind())
            {
                case BBJ_COND:
                    ...
                    FALLTHROUGH;
                case BBJ_NONE:
                    noway_assert(bb->GetBBNext() == block);
                    break;
...
private:
    ...
    unsigned int PositionNum(BasicBlock* block)
    {
        if (block->bbNum > oldBlockMaxNum)
        {
            ...
            // block.
            assert(block->bbPreds->getSourceBlock() == block->GetBBPrev());
...
bool Compiler::optCanonicalizeLoop(unsigned char loopInd)
{
    ...
    if (h->KindIs(BBJ_ALWAYS) && (h->bbJumpDest == t) && (h->bbFlags & BBF_KEEP_BBJ_ALWAYS))
    {
        // Insert new head
...
bool Compiler::optCanonicalizeLoopCore(unsigned char loopInd, LoopCanonicalizationOption option)
{
    ...
    assert(h->GetBBNext() == newT);
    assert(newT->GetBBNext() == t);
    ...
    if (option == LoopCanonicalizationOption::Current)
    {
             ...
             childLoop = optLoopTable[childLoop].lpSibling)
        {
            if ((optLoopTable[childLoop].lpEntry == origE) && (optLoopTable[childLoop].lpHead == h) &&
                newT->KindIs(BBJ_NONE) && (newT->GetBBNext() == origE))
            {
    ...
    for (unsigned lnum = optLoopCount - 1; lnum != ~0U; --lnum)
    {
        ...
        // Create the unrolled loop statement list.
        {
            ...
            for (lval = lbeg; iterToUnroll > 0; iterToUnroll--)
            {
                ...
                // every iteration.
                for (BasicBlock* block = loop.lpTop; block != loop.lpBottom->GetBBNext(); block = block->GetBBNext())
                {
            ...
            for (BasicBlock* const block : loop.LoopBlocks())
            {
                ...
                {
                    if ((block == bottom) && (succ == bottom->GetBBNext()))
                    {
                        continue;
                    }
    ...
    for (BasicBlock* const predBlock : entry->PredBlocks())
    {
        ...
        switch (predBlock->GetBBJumpKind())
        {
            case BBJ_NONE:
                ...
                noway_assert((entry == top) && (predBlock == head) && (predBlock->GetBBNext() == preHead));
            ...
            case BBJ_COND:
                if (predBlock->bbJumpDest == entry)
                {
                    ...
                    noway_assert(predBlock->GetBBNext() != preHead);
                }
                else
                {
                    noway_assert((entry == top) && (predBlock == head) && (predBlock->GetBBNext() == preHead));
