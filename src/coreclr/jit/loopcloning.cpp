...
bool Compiler::optDeriveLoopCloningConditions(FlowGraphNaturalLoop* loop, LoopCloneContext* context)
{
    ...
    if ((stride > 1) && isIncreasingLoop && needsOverflowGuard)
    {
        ...
        if (iterInfo->HasConstLimit)
        {
            ...
            if (limitVal > maxLimitBase64)
            {
                JITDUMP("> Stride %lld: const limit %lld exceeds overflow bound %lld\n", (long long)stride,
        ...
        else if (iterInfo->HasInvariantLocalLimit)
        {
            ...
            {
                JITDUMP("Stride>1 overflow guard trivially holds (offset %lld)\n", (long long)offset);
            }
            else if (maxLimitBase64 < 0)
            {
                JITDUMP("> Stride %lld, offset %lld: overflow guard unsatisfiable\n", (long long)stride,
                        (long long)offset);
                return false;
            }
            else
            {
                ...
                {
                    JITDUMP("> Stride %lld: limit var V%02u type does not match IV\n", (long long)stride,
                            limitLcl);
                    return false;
                }
        ...
        else if (iterInfo->HasArrayLengthLimit && largeStride)
        {
            ...
            {
                JITDUMP("Stride>1 overflow guard trivially holds for arr.Length (offset %lld)\n", (long long)offset);
            }
            else if (maxLimitBase64 < 0)
            {
                JITDUMP("> Stride %lld, offset %lld: arr.Length overflow guard unsatisfiable\n", (long long)stride,
                        (long long)offset);
                return false;
            }
            else
