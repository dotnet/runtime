...
public:
    ...
    class RegisterSegmentIterator
    {
        ABIPassingSegment* m_segment;
        ABIPassingSegment* m_end;

    public:
        RegisterSegmentIterator(ABIPassingSegment* segment, ABIPassingSegment* segmentEnd)
            : m_segment(segment)
            , m_end(segmentEnd)
        {
        }

        ABIPassingSegment& operator*()
        {
            return *m_segment;
        }
        ...
        RegisterSegmentIterator& operator++()
        {
            do
            {
                m_segment++;
            } while ((m_segment != m_end) && !m_segment->IsPassedInRegister());

            return *this;
        }

        RegisterSegmentIterator& operator++(int)
        {
            (*this)++;
            return *this;
        }

        bool operator==(RegisterSegmentIterator other)
        {
    ...
    class RegisterSegmentsIterable
    {
        ABIPassingSegment* m_segment;
        ABIPassingSegment* m_segmentEnd;

    public:
        RegisterSegmentsIterable(ABIPassingSegment* segment, ABIPassingSegment* end)
            : m_segment(segment)
            , m_segmentEnd(end)
        {
        }

        RegisterSegmentIterator begin()
        {
            return RegisterSegmentIterator(m_segment, m_segmentEnd);
        }

        RegisterSegmentIterator end()
        {
            return RegisterSegmentIterator(m_segmentEnd, m_segmentEnd);
        }
    };

    RegisterSegmentsIterable RegisterSegments();
};
