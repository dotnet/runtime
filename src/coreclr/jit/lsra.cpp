        ...
        else if (currentInterval->isUpperVector)
        {
...
#elif defined(TARGET_ARM64)
                else if (varTypeIsSIMDVL(lclVarInterval->registerType))
                {
            ...
            else if (refType == RefTypeUpperVectorRestore)
            {
                ...
                if (lclVarInterval->isPartiallySpilled)
                {
                    ...
                    if (varTypeIsSIMDVL(lclVarInterval->registerType))
                    {
                        // TODO-VL: Need to do this for allocateRegistersMinimal too?
                        allocate = false;
                    }
                                          ...
                                          BasicBlock*  block)
{
    ...
    if (restoreReg == REG_NA)
    {
        ...
        assert((refPosition->assignedReg() != REG_NA) || (varTypeIsSIMDVL(restoreLcl->TypeGet())));
