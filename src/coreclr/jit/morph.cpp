...
void CallArgs::ArgsComplete(Compiler* comp, GenTreeCall* call)
{
    ...
    for (CallArg& arg : Args())
    {
        ...
        /* If the argument tree contains a store (GTF_ASG) then the argument and and
           every earlier argument (except constants) must be evaluated into temps since
           there may be other arguments that follow and they may use the changed value.
           ...
           For the case of stores, we only know that there exist some store someplace
           in the tree. We don't know what is being stored so we are very conservative
           here and assume that any local variable could have been redefined.
         */
            ...
            else
            {
                ...
                if (argx->gtOper == GT_MKREFANY)
                {
                    ...
                    if ((argx->AsOp()->gtOp2->gtFlags & GTF_ALL_EFFECT) == 0)
                    {
                        ...
                        // In the early argument evaluation, place a store to the temp
...
//
    ...
    if (op2 != nullptr)
    {
        ...
        // merge the state of the current assertions table with that of
        // the exit of the "then" part.
        if (isQmarkColon)
        {
            noway_assert(optLocalAssertionProp);
...
bool Compiler::fgCheckStmtAfterTailCall()
{
    ...
    // For debuggable code, it would be a store of the call to a temp.
    ...
    if (nextMorphStmt != nullptr)
    {
        ...
        else
        {
            ...
            // We can have a chain of stores from the call result to
