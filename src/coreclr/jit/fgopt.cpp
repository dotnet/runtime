...
bool Compiler::fgRemoveUnreachableBlocks(CanRemoveBlockBody canRemoveBlock)
{
    ...
    for (BasicBlock* const block : Blocks())
    {
        ...
        if (block->bbFlags & BBF_DONT_REMOVE)
        {
            ...
            {
                noway_assert(block->bbNext->getBBJumpKind() == BBJ_ALWAYS);
    ...
    if (opts.IsOSR())
    {
        ...
        if (osrEntry->hasTryIndex())
        {
            ...
            if (inNestedTry || osrEntryMidTry)
            {
                ...
                assert(fgFirstBB->getBBJumpKind() == BBJ_ALWAYS);
...
bool Compiler::fgCanCompactBlocks(BasicBlock* block, BasicBlock* bNext)
{
    ...
    if (block->getBBJumpKind() != BBJ_NONE)
    {
        return false;
    }
...
void Compiler::fgCompactBlocks(BasicBlock* block, BasicBlock* bNext)
{
    ...
    switch (bNext->getBBJumpKind())
    {
        ...
        case BBJ_EHCATCHRET:
            ...
            if (bNext->getBBJumpKind() == BBJ_COND && bNext->bbJumpDest != bNext->bbNext)
            {
...
{
    noway_assert(block->getBBJumpKind() == BBJ_COND && block->bbJumpDest == block->bbNext);
...
bool Compiler::fgOptimizeBranchToEmptyUnconditional(BasicBlock* block, BasicBlock* bDest)
{
    ...
    assert(bDest->getBBJumpKind() == BBJ_ALWAYS);
...
bool Compiler::fgOptimizeEmptyBlock(BasicBlock* block)
{
    ...
    switch (block->getBBJumpKind())
    {
        ...
        case BBJ_ALWAYS:
            ...
            if (bPrev->getBBJumpKind() != BBJ_NONE)
            {
                break;
            }
        ...
        case BBJ_NONE:
             ...
             */
            {
                ...
                if ((succBlock != nullptr) && !BasicBlock::sameEHRegion(block, succBlock))
                {
                    ...
                    for (BasicBlock* const predBlock : block->PredBlocks())
                    {
                        if (predBlock->getBBJumpKind() == BBJ_EHCATCHRET)
                        {
...
{
    assert(block->getBBJumpKind() == BBJ_SWITCH);
    ...
    REPEAT_SWITCH:;
        ...
        if (bDest->isEmpty() && (bDest->getBBJumpKind() == BBJ_ALWAYS) &&
...
bool Compiler::fgBlockIsGoodTailDuplicationCandidate(BasicBlock* target, unsigned* lclNum)
{
    *lclNum = BAD_VAR_NUM;
    ...
    if (target->getBBJumpKind() != BBJ_COND)
    {
        return false;
    }
...
bool Compiler::fgOptimizeUncondBranchToSimpleCond(BasicBlock* block, BasicBlock* target)
{
    ...
    if (opts.IsOSR())
    {
        assert(target->getBBJumpKind() == BBJ_COND);
...
bool Compiler::fgOptimizeBranchToNext(BasicBlock* block, BasicBlock* bNext, BasicBlock* bPrev)
{
    ...
    else
    {
        ...
        noway_assert(block->getBBJumpKind() == BBJ_COND);
...
bool Compiler::fgOptimizeBranch(BasicBlock* bJump)
{
    ...
    if (bJump->getBBJumpKind() != BBJ_ALWAYS)
    {
        return false;
    }
    ...
    if (bDest->getBBJumpKind() != BBJ_COND)
    {
        return false;
    }
...
bool Compiler::fgExpandRarelyRunBlocks()
{
    ...
    auto newRunRarely = [](BasicBlock* block, BasicBlock* bPrev) {
        ...
        if ((bPrev->bbFlags & BBF_KEEP_BBJ_ALWAYS) != 0)
        {
            ...
            bPrevPrev = tmpbb;
#else
            if (tmpbb->getBBJumpKind() == BBJ_CALLFINALLY)
            {
                bPrevPrev = tmpbb;
            }
#endif
...
bool Compiler::fgReorderBlocks(bool useProfile)
{
    ...
    for (bPrev = fgFirstBB, block = bPrev->bbNext; block != nullptr; bPrev = block, block = block->bbNext)
    {
            ...
            ((bDest == nullptr) || bDest->hasProfileWeight()))
        {
            ...
            if (forwardBranch)
            {
                if (bPrev->getBBJumpKind() == BBJ_ALWAYS)
                {
                ...
                else // (bPrev->getBBJumpKind() == BBJ_COND)
                {
                    noway_assert(bPrev->getBBJumpKind() == BBJ_COND);
                    //
        ...
        CHECK_FOR_RARE:;
            ...
            if (!isRare)
            {
                if ((bDest == block->bbNext) && (block->getBBJumpKind() == BBJ_RETURN) &&
                    (bPrev->getBBJumpKind() == BBJ_ALWAYS))
                {
        ...
        noway_assert((bEnd->getBBJumpKind() != BBJ_CALLFINALLY) || (bEnd->bbFlags & BBF_RETLESS_CALL));
        ...
        if (insertAfterBlk == nullptr)
        {
            ...
            else
            {
                ...
                //
                {
                    ...
                    if ((bEnd->getBBJumpKind() == BBJ_ALWAYS) && (!isRare || bEnd->bbJumpDest->isRunRarely()) &&
    ...
    do
    {
        ...
        REPEAT:;
            ...
            if (bDest != nullptr)
            {
                ...
                //
                if ((block->getBBJumpKind() == BBJ_COND) &&   // block is a BBJ_COND block
                    (bNext != nullptr) &&                     // block is not the last block
                    (bNext->bbRefs == 1) &&                   // No other block jumps to bNext
                    (bNext->getBBJumpKind() == BBJ_ALWAYS) && // The next block is a BBJ_ALWAYS block
            ...
            if (block->countOfInEdges() == 0 && bPrev->getBBJumpKind() == BBJ_CALLFINALLY)
            {
                assert(bPrev->isBBCallAlwaysPair());
                noway_assert(!(bPrev->bbFlags & BBF_RETLESS_CALL));
                noway_assert(block->getBBJumpKind() == BBJ_ALWAYS);
                bPrev = block;
                continue;
            }
#endif // defined(FEATURE_EH_FUNCLETS) && defined(TARGET_ARM)
    ...
    auto tailMerge = [&](BasicBlock* block) -> bool {
        ...
        while (i < (predInfo.Height() - 1))
        {
            ...
            for (int j = 0; j < matchedPredInfo.Height(); j++)
            {
                ...
                bool const isFallThrough = (predBlock->getBBJumpKind() == BBJ_NONE);
...
bool Compiler::fgTryOneHeadMerge(BasicBlock* block, bool early)
{
    ...
    if ((block->getBBJumpKind() != BBJ_COND) || (block->bbNext == block->bbJumpDest))
    {
        return false;
    }
