...
{
    m_flowFound           = false;
    BasicBlock* thenBlock = m_startBlock->GetBBNext();
...
void OptIfConversionDsc::IfConvertDump()
{
    ...
    for (BasicBlock* dumpBlock = m_startBlock->GetBBNext(); dumpBlock != m_finalBlock;
...
bool OptIfConversionDsc::optIfConvert()
{
    ...
    if (!IfConvertCheckStmts(m_startBlock->GetBBNext(), &m_thenOperation))
    {
        return false;
    }
    ...
    m_comp->fgRemoveAllRefPreds(m_startBlock->GetBBNext(), m_startBlock);
