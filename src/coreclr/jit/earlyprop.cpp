...
GenTree* Compiler::optPropGetValueRec(unsigned lclNum, unsigned ssaNum, optPropKind valueKind, int walkDepth)
{
    ...
    if (ssaDefStore != nullptr)
    {
        ...
        {
            unsigned valueLclNum = defValue->AsLclVar()->GetLclNum();
            unsigned valueSsaNum = defValue->AsLclVar()->GetSsaNum();

            defValue = optPropGetValueRec(valueLclNum, valueSsaNum, valueKind, walkDepth + 1);
        }
        else
        {
