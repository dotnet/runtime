...
bool Compiler::optValnumCSE_Locate()
{
    ...
    for (BasicBlock* const block : Blocks())
    {
        ...
        noway_assert(!block->CheckFlag(BBF_MARKED));
...
void Compiler::optValnumCSE_InitDataFlow()
{
    ...
    for (unsigned inx = 0; inx < optCSECandidateCount; inx++)
    {
        ...
        while (lst != nullptr)
        {
            ...
            if (!block->CheckFlag(BBF_HAS_CALL))
            {
...
public:
    ...
    bool EndMerge(BasicBlock* block)
    {
        ...
        //
        if (block->CheckFlag(BBF_NO_CSE_IN))
        {
        ...
        //
        if (!block->CheckFlag(BBF_HAS_CALL) || BitVecOps::IsEmpty(m_comp->cseLivenessTraits, block->bbCseIn))
        {
        ...
        if (m_comp->verbose)
        {
            ...
            printf("  :: cseIn     = %s\n", genES2str(m_comp->cseLivenessTraits, block->bbCseIn));
            if (block->CheckFlag(BBC_HAS_CALL) &&
...
void Compiler::optEnsureClearCSEInfo()
{
    for (BasicBlock* const block : Blocks())
    {
        assert(!block->CheckFlag(BBF_MARKED));
