        ...
        for (unsigned cnt = 0; cnt < m_pCompiler->optCSECandidateCount; cnt++)
        {
            ...
            if (!Compiler::Is_Shared_Const_CSE(dsc->csdHashKey))
            {
                printf(FMT_CSE ", {$%-3x, $%-3x} defCnt=%d useCnt=%d: [def=%3f, use=%3f, cost=%3u%s]\n        :: ",
                       dsc->csdIndex, dsc->csdHashKey, dsc->defExcSetPromise, dsc->csdDefCount, dsc->csdUseCount, def,
                       use, cost, dsc->csdLiveAcrossCall ? ", call" : "      ");
            }
            else
            {
                ...
                printf(FMT_CSE ", {K_%p} defCnt=%d useCnt=%d: [def=%3f, use=%3f, cost=%3u%s]\n        :: ",
                       dsc->csdIndex, dspPtr(kVal), dsc->csdDefCount, dsc->csdUseCount, def, use, cost,
