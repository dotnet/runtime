    ...
    unsafe partial class CorInfoImpl
    {
        ...
        private uint FilterNamedIntrinsicMethodAttribs(uint attribs, MethodDesc method)
        {
            ...
            if ((attribs & (uint)CorInfoFlag.CORINFO_FLG_JIT_INTRINSIC) != 0)
            {
                ...
                string isaName = InstructionSetSupport.GetHardwareIntrinsicId(_compilation.TypeSystemContext.Target.Architecture, (MetadataType)method.OwningType);
                fIsPlatformHWIntrinsic = !string.IsNullOrEmpty(isaName);
                ...
                if (_TARGET_X86_ || _TARGET_AMD64_)
                {
                    ...
                    // but we don't know what the target machine will support. This only applies if the baseline support is less than
                    // the instruction support added by CorInfoImpl.getJitFlags
                    //
                    ...
                    if (!JitConfigProvider.Instance.InstructionSetSupport.IsInstructionSetSupported("Popcnt"))
                    ...
                    if (fIsPlatformHWIntrinsic)
                    {
                        ...
                        if (JitConfigProvider.Instance.InstructionSetSupport.IsInstructionSetSupported(isaName))
                        {
                            if ((enclosingClassName == null) || (className == "X64"))
                            {
                                // If it's anything related to an explicitly supported instruction set we can expand unconditionally since this is
                                // a baseline requirement of CoreCLR.
                                fTreatAsRegularMethodCall = false;
                            }
                        }
                        else if (InstructionSetSupport.DoesInstructionSetImplyOtherInstructionSet(_compilation.TypeSystemContext.Target.Architecture, isaName, "Avx"))
                        {
                    ...
                    else if (!JitConfigProvider.Instance.InstructionSetSupport.IsInstructionSetSupported("Sse41"))
                    {
