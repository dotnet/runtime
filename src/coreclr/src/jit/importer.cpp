    ...
    GenTreeCall* call, CORINFO_METHOD_HANDLE methHnd, CORINFO_SIG_INFO* sig, unsigned mflags, BasicBlock* block)
{
    ...
    else if (IsTargetAbi(CORINFO_CORECLR_ABI) &&
                (mflags & CORINFO_FLG_PINVOKE) != 0 &&
                (mflags & CORINFO_FLG_FORCEINLINE) != 0 &&
                opts.jitFlags->IsSet(JitFlags::JIT_FLAG_IL_STUB) &&
                opts.ShouldUsePInvokeHelpers())
    {
        // The raw PInvoke call in IL stubs generated by crossgen2 must be inlined, otherwise we would end up with
        // a stub that recursively calls itself, and end up with a stack overflow.
        // Crossgen2 sets the flag to use PInvoke helpers, and compiles the raw PInvoke call as a regular call
        // instruction instead of a calli.

        // The signature of the raw PInvoke call should never need any marshaling.
