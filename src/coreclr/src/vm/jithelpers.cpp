...
#ifdef TARGET_X86
        IMAGE_DOS_HEADER *pDOS = (IMAGE_DOS_HEADER *)GetModuleBase();
        _ASSERTE(pDOS->e_magic == VAL16(IMAGE_DOS_SIGNATURE) && pDOS->e_lfanew != 0);

        IMAGE_NT_HEADERS *pNT = (IMAGE_NT_HEADERS*)((LPBYTE)GetModuleBase() + VAL32(pDOS->e_lfanew));
#ifdef HOST_64BIT
...
#pragma warning(pop)
                    ...
                    // mscoree.dll)
                    if (dynamicHlpFunc->pfnHelper >= (LPBYTE*)GetModuleBase() && dynamicHlpFunc->pfnHelper < (LPBYTE*)GetModuleBase() + VAL32(pNT->OptionalHeader.SizeOfImage))
                    {
