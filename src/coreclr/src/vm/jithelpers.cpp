...
void JIT_UncommonPatchpoint(int ilOffset)
{
    ...
    {
        // Invalid patchpoints are fatal, for uncommon patchpoints
        //
        if ((ppInfo->m_flags & PerPatchpointInfo::patchpoint_invalid) == PerPatchpointInfo::patchpoint_invalid)
        {
            LOG((LF_TIEREDCOMPILATION, LL_FATALERROR, "Jit_UncommonPatchpoint: invalid patchpoint [%d] (0x%p) in Method=0x%pM (%s::%s) at offset %d\n",
                    ppId, ip, pMD, pMD->m_pszDebugClassName, pMD->m_pszDebugMethodName, ilOffset));
            EEPOLICY_HANDLE_FATAL_ERROR(COR_E_EXECUTIONENGINE);
        }
        ...
        if ((oldFlags & PerPatchpointInfo::patchpoint_triggered) == PerPatchpointInfo::patchpoint_triggered)
        {
            LOG((LF_TIEREDCOMPILATION, LL_INFO1000, "Jit_Uncommonatchpoint: AWAITING OSR method for patchpoint [%d] (0x%p)\n", ppId, ip));
        ...
        if (!triggerTransition)
        {
            LOG((LF_TIEREDCOMPILATION, LL_INFO1000, "Jit_UncommonPatchpoint: (lost race) AWAITING OSR method for patchpoint [%d] (0x%p)\n", ppId, ip));
            __SwitchToThread(0, backoffs++);
            continue;
        }
        ...
        // Invoke the helper to build the OSR method
        //
        // TODO: may not want to optimize this part of the method, if it's truly uncommon
        ...
        //
        LOG((LF_TIEREDCOMPILATION, LL_INFO10, "Jit_UncommonPatchpoint: patchpoint [%d] (0x%p) TRIGGER\n", ppId, ip));
        PCODE newMethodCode = HCCALL3(JIT_Patchpoint_Framed, pMD, codeInfo, ilOffset);

        // If that failed, mark the patchpoint as invalid.
        // This is fatal, for uncommon patchpoints
        //
        if (newMethodCode == NULL)
        {
            STRESS_LOG3(LF_TIEREDCOMPILATION, LL_WARNING, "Jit_UncommonPatchpoint: patchpoint (0x%p) OSR method creation failed,"
                " marking patchpoint invalid for Method=0x%pM il offset %d\n", ip, pMD, ilOffset);
            InterlockedOr(&ppInfo->m_flags, (LONG)PerPatchpointInfo::patchpoint_invalid);
            EEPOLICY_HANDLE_FATAL_ERROR(COR_E_EXECUTIONENGINE);
            break;
