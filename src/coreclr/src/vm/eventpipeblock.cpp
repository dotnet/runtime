...
EventPipeMetadataBlockV2::EventPipeMetadataBlockV2(unsigned int maxBlockSize) :
    EventPipeBlock(maxBlockSize)
{
    // the total size of the block needs to be written into the header size field
    // and old parsers interpret that field as a signed short
    _ASSERTE(maxBlockSize + GetHeaderSize() <= 0x7FFF);
}

bool EventPipeMetadataBlockV2::CanStoreMetadataBlob(const SString& providerName, unsigned int metadataBlobLength)
{
    LIMITED_METHOD_CONTRACT;
    ...
    unsigned int totalSize =
        sizeof(unsigned int) + /* metadataSize */
        sizeof(unsigned int) + /* metadataId */
        (providerName.GetCount() + 1) * sizeof(WCHAR) + 
        metadataBlobLength;

    return (m_pWritePointer + totalSize < m_pEndOfTheBuffer);
}

void EventPipeMetadataBlockV2::WriteMetadataBlob(unsigned int metadataId, const SString& providerName, BYTE* pMetadataBlob, unsigned int metadataBlobLength)
{
    LIMITED_METHOD_CONTRACT;

    _ASSERTE(CanStoreMetadataBlob(providerName, metadataBlobLength));
    unsigned int providerNameSize = (providerName.GetCount() + 1) * sizeof(WCHAR);
    unsigned int totalSize =
        sizeof(unsigned int) + /* metadataSize */
        sizeof(unsigned int) + /* metadataId */
        providerNameSize +
    ...
    memcpy(m_pWritePointer, (BYTE*)providerName.GetUnicode(), providerNameSize);
    m_pWritePointer += providerNameSize;

    memcpy(m_pWritePointer, pMetadataBlob, metadataBlobLength);
    m_pWritePointer += metadataBlobLength;
}
