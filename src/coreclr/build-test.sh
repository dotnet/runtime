...
precompile_coreroot_fx()
{
    ...
    for fileToPrecompile in ${filesToPrecompile}; do
        ...
        if [[ "$__DoCrossgen" != 0 ]]; then
            commandLine="$__CrossgenExe" /Platform_Assemblies_Paths "$overlayDir" "$filename" 1> "$filename".stdout 2> "$filename".stderr
        fi

        if [[ "$__DoCrossgen2" != 0 ]]; then
            commandLine="$overlayDir"/crossgen2/crossgen2 "$crossgen2References" -O --inputbubble --out "$outputDir"/"$(basename $filename)" "$filename" 1> "$filename".stdout 2> "$filename".stderr
        fi
        ...
        if [[ "$exitCode" != 0 ]]; then
            if grep -q -e '0x80131018' "$filename".stderr; then
                ...
                echo Unable to precompile "$filename", exit code is "$exitCode": "$commandLine"
                cat "$filename".stdout
                cat "$filename".stderr
