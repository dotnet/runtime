...
build_Tests()
{
    ...
    if [[ "$__CopyNativeTestBinaries" == 1 ]]; then
        ...
        build_MSBuild_projects "Tests_Managed" "$__ProjectDir/tests/build.proj" "Managed tests build (build tests)" "/t:CopyAllNativeProjectReferenceBinaries" "/p:CopyNativeTestBinariesOnly=false"  "/bl:${__RepoRootDir}/artifacts/log/${__BuildType}/copy_native_test_binaries${__RuntimeFlavor}.binlog"
