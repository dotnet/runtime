...
build_Tests()
{
    ...
    if [[ "$__SkipNative" != 1 ]]; then
        build_native "$__BuildArch" "$__TestDir" "$__ProjectRoot" "$__NativeTestIntermediatesDir" "CoreCLR test component" "/p:GenerateRunScript=false"
    ...
    if [[ "$__CopyNativeTestBinaries" == 1 ]]; then
        ...
        build_MSBuild_projects "Tests_Managed" "$__ProjectDir/tests/build.proj" "Managed tests build (build tests)" "/t:CopyAllNativeProjectReferenceBinaries" "/p:GenerateRunScript=false"
