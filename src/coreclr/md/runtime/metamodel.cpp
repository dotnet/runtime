...
#define MiniMdCodedToken(x) {MINIPAL_LENGTHOF(CMiniMdBase::mdt##x), CMiniMdBase::mdt##x, #x},
...
#undef MiniMdTable
#define MiniMdTable(x) { { r##x##Cols, MINIPAL_LENGTHOF(r##x##Cols), x##Rec::COL_KEY, 0 }, r##x##ColNames, #x},
...
const CMiniTableDefEx g_Table_GenericParamV1_1 = { { rGenericParamV1_1Cols, MINIPAL_LENGTHOF(rGenericParamV1_1Cols), GenericParamV1_1Rec::COL_KEY, 0 }, rGenericParamV1_1ColNames, "GenericParamV1_"};
...
#define _CBTKN(cRecs,tkns) cbRID((cRecs) << m_cb[MINIPAL_LENGTHOF(tkns)])
                                    ...
                                    // should we write the data into the structure
{
    ...
    _ASSERTE(MINIPAL_LENGTHOF(pCols) >= pTable->m_cCols);
    ...
    for (ULONG ixCol = 0; ixCol < pTable->m_cCols; ++ixCol)
    {
        ...
        if (pCols[ixCol].m_Type <= iCodedTokenMax)
        {
            ...
            _ASSERTE(iCdTkn < MINIPAL_LENGTHOF(g_CodedTokens));
    ...
    RID    *pFoundRid)  // RID of custom value, or 0.
{
    ...
    ULONG   ulTarget = encodeToken(rid,tkObj,mdtHasCustomAttribute, MINIPAL_LENGTHOF(mdtHasCustomAttribute)); // encoded token representing target.
    ...
    for(;;)
    {
        ...
        tkFound = decodeToken(tkFound, mdtCustomAttributeType, MINIPAL_LENGTHOF(mdtCustomAttributeType));
        if (tkFound == tkType)
        {
            *pFoundRid = ixCur;
            return S_OK;
