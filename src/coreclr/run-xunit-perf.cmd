...
set HERE=%CD%
set CORECLR_REPO=E:\Git\coreclr
set CORECLR_OVERLAY=%CORECLR_REPO%\bin\tests\Windows_NT.x64.Debug\Tests\Core_Root
set CORECLR_PERF=%CORECLR_REPO%\bin\tests\Windows_NT.x64.Debug\Jit\Performance\CodeQuality
set RUNLOG=%HERE%\perfrun.log

if NOT EXIST %CORECLR_OVERLAY% (
  echo Can't find test overlay directory '%CORECLR_OVERLAY%'
  echo Please build and run debug CoreCLR tests
  exit /B 1
)
...
xcopy /sy %CORECLR_REPO%\bin\tests\Windows_NT.x64.Debug\Tests\Core_Root\* . > %RUNLOG%
