...
VOID InitLogging()
{
    ...
    if (fileName != 0)
    {
        if (SUCCEEDED(szLogFileName.ReSizeNoThrow(minipal_u16_strlen((const CHAR16_T*)fileName) + 32)))
        {
        ...
        (LogFileHandle == INVALID_HANDLE_VALUE))
    {
        ...
        if (LogFileHandle == INVALID_HANDLE_VALUE && minipal_u16_strlen((const CHAR16_T*)szLogFileName.Ptr()) + 3 <= szLogFileName.Size())
        {
            WCHAR* ptr = szLogFileName.Ptr() + minipal_u16_strlen((const CHAR16_T*)szLogFileName.Ptr()) + 1;
            ptr[-1] = W('.');
            ptr[0] = W('0');
            ptr[1] = 0;
