...
void Exception::GetMessage(SString<EncodingUnicode> &result)
{
    WRAPPER_NO_CONTRACT;
...
void HRMsgException::GetMessage(SString<EncodingUnicode> &result)
{
    WRAPPER_NO_CONTRACT;
...
void SEHException::GetMessage(SString<EncodingUnicode> &string)
{
    WRAPPER_NO_CONTRACT;
    ...
    else
    {
        ...
        {
            string.Printf(W("Exception code 0x%.8x"), m_exception.ExceptionCode);
        }
        else
        {
            // If we don't have a valid exception code, then give a generic message that's a little nicer than
            // "code 0x00000000".
            string.Append(W("Unknown exception"));
        }
    }
}
...
//------------------------------------------------------------------------------
void DelegatingException::GetMessage(SString<EncodingUnicode> &result)
{
    WRAPPER_NO_CONTRACT;
    ...
    else
    {
        ...
        result.Append(W("Unknown exception"));
    }
...
void DECLSPEC_NORETURN ThrowHR(HRESULT hr, SString<EncodingUnicode> const &msg)
{
    WRAPPER_NO_CONTRACT;
...
void DECLSPEC_NORETURN ThrowHR(HRESULT hr, UINT uText)
{
    ...
    SString<EncodingUnicode> sExceptionText;

    // We won't check the return value here. If it fails, we'll just
    // throw the HR
    LoadResource(sExceptionText, CCompRC::Error, uText);
...
//-------------------------------------------------------------------------------------------
void GetHRMsg(HRESULT hr, SString<EncodingUnicode> &result, BOOL bNoGeekStuff/* = FALSE*/)
{
    CONTRACTL
    {
        GC_NOTRIGGER;
...
//-------------------------------------------------------------------------------------------
void GenerateTopLevelHRExceptionMessage(HRESULT hresult, SString<EncodingUnicode> &result)
{
    CONTRACTL
    {
        GC_NOTRIGGER;
