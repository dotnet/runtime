...
static CAllowedObjectTypes aotAnyMutex(anyMutexTypeIds, MINIPAL_LENGTHOF(anyMutexTypeIds));
    ...
    IN LPCWSTR lpName)
{
    ...
    if (lpName != nullptr)
    {
        int bytesWritten = WideCharToMultiByte(CP_ACP, 0, lpName, -1, utf8Name, MINIPAL_LENGTHOF(utf8Name), nullptr, nullptr);
        if (bytesWritten == 0)
        {
