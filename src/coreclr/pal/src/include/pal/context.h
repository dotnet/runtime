...
#else // !TARGET_FREEBSD (Linux ppc64le)
...
#endif // TARGET_FREEBSD vs Linux ppc64le
...
/* FreeBSD ppc `struct reg` has no MSR slot. MSR is not consumed by CoreCLR
   on this path, so route reads and writes through a per-thread throwaway
   lvalue (ASSIGN_CONTROL_REGS copies Msr in both directions). */
static thread_local unsigned long BSDppc_MsrScratch __attribute__((unused));
#define BSDREG_Msr(reg)     (BSDppc_MsrScratch)
