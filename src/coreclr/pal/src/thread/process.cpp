...
Note:
  This is a thin wrapper that delegates to the minipal process API.
  Security attributes and most creation flags are not supported.
           ...
           OUT LPPROCESS_INFORMATION lpProcessInformation)
{
    ...
    BOOL bRet = FALSE;
    LPCWSTR lpCmd = lpCommandLine ? lpCommandLine : lpApplicationName;
    ...
    {
        minipal_process_info info;
        bool ok = minipal_create_process_w(
            (const CHAR16_T*)lpCmd,
        ...
        if (!ok)
        {
            ERROR("minipal_create_process_w failed\n");
            palError = ERROR_ACCESS_DENIED;
            goto done;
        }

        // Register the child process in the PAL object manager so that
        // WaitForSingleObject, GetExitCodeProcess, etc. work correctly.
        ...
        // pobjProcess is invalidated by the above call
        pobjProcess = NULL;
        ...
        if (NO_ERROR != palError)
        {
            ERROR("Unable to get process local data\n");
            goto done;
        }

        pLocalData->dwProcessId = (DWORD)info.process_id;
        ...
        if (NO_ERROR != palError)
        {
            ERROR("Unable to create dummy thread handle\n");
            goto done;
        }
        ...
        if (lpProcessInformation != NULL)
        {
            ...
            lpProcessInformation->dwProcessId = (DWORD)info.process_id;
        ...
        // minipal handles are not needed; the PAL manages lifetime via its own handles.
        minipal_close_process_handle(info.process_handle);
        minipal_close_process_handle(info.thread_handle);
    ...
    if (NO_ERROR != palError && palError != ERROR_INVALID_PARAMETER)
    {
