...
Not a full Android experience is available - it's only possible to publish for Bionic RIDs: linux-bionic-arm, linux-bionic-arm64 and linux-bionic-x64. Publishing for Android RIDs (android-arm/android-arm64/android-x64) is not possible. This limited experience corresponds to building with [Android NDK](https://developer.android.com/ndk) from Native code - the limitations are similar. Interop with Java needs to be done manually through JNI, if necessary.
...
* Publish the project:
  ...
  ```
* You should have a binary under `bin\Release\net11.0\<rid>\publish`. Copy it to an Android device. Either `adb push` or using some GUI.
