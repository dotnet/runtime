...
namespace Internal.JitInterface
{
    ...
    internal static class LoongArch64PassStructInRegister
    {
        public static uint GetLoongArch64PassStructInRegisterFlags(TypeDesc typeDesc)
        {
            ...
            {
                return (uint)LAFlags.STRUCT_NO_FLOAT_FIELD;
            }
                ...
                switch (field.FieldType.Category)
                {
                    case TypeFlags.Double:
                    {
                        if (numIntroducedFields == 1)
                            floatFieldFlags = (uint)LAFlags.STRUCT_FLOAT_FIELD_ONLY_ONE;
                        else if (fieldIndex == 0)
                            floatFieldFlags = (uint)LAFlags.STRUCT_FIRST_FIELD_DOUBLE;
                        else if ((floatFieldFlags & (uint)LAFlags.STRUCT_FLOAT_FIELD_FIRST) != 0)
                            floatFieldFlags = floatFieldFlags ^ (uint)LAFlags.STRUCT_MERGE_FIRST_SECOND_8;
                        else
                            floatFieldFlags |= (uint)LAFlags.STRUCT_SECOND_FIELD_DOUBLE;
                    }
                    break;
                    ...
                    case  TypeFlags.Single:
                    {
                        if (numIntroducedFields == 1)
                            ...
                            floatFieldFlags = (uint)LAFlags.STRUCT_FLOAT_FIELD_FIRST;
                        else if ((floatFieldFlags & (uint)LAFlags.STRUCT_FLOAT_FIELD_FIRST) != 0)
                            floatFieldFlags ^= (uint)LAFlags.STRUCT_MERGE_FIRST_SECOND;
                        else
                            floatFieldFlags |= (uint)LAFlags.STRUCT_FLOAT_FIELD_SECOND;
                    }
                    break;
                    ...
                    //case TypeFlags.SzArray:
                    {
                        ...
                        {
                            if ((floatFieldFlags2 & (uint)LAFlags.STRUCT_FLOAT_FIELD_ONLY_ONE) != 0)
                                floatFieldFlags = (uint)LAFlags.STRUCT_FLOAT_FIELD_FIRST;
                            if (field.FieldType.GetElementSize().AsInt == 8)
                                floatFieldFlags |= (uint)LAFlags.STRUCT_FIRST_FIELD_SIZE_IS8;
                        }
                        else
                        {
                            ...
                            if ((floatFieldFlags2 & (uint)LAFlags.STRUCT_FLOAT_FIELD_ONLY_ONE) != 0)
                                floatFieldFlags |= (uint)LAFlags.STRUCT_MERGE_FIRST_SECOND;
                            if (field.FieldType.GetElementSize().AsInt == 8)
                                floatFieldFlags |= (uint)LAFlags.STRUCT_SECOND_FIELD_SIZE_IS8;

                            floatFieldFlags2 = floatFieldFlags & ((uint)LAFlags.STRUCT_FLOAT_FIELD_FIRST | (uint)LAFlags.STRUCT_FLOAT_FIELD_SECOND);
                            if (floatFieldFlags2 == 0)
                                floatFieldFlags = (uint)LAFlags.STRUCT_NO_FLOAT_FIELD;
                            else if (floatFieldFlags2 == ((uint)LAFlags.STRUCT_FLOAT_FIELD_FIRST | (uint)LAFlags.STRUCT_FLOAT_FIELD_SECOND))
                                floatFieldFlags ^= ((uint)LAFlags.STRUCT_FLOAT_FIELD_ONLY_TWO | (uint)LAFlags.STRUCT_FLOAT_FIELD_FIRST | (uint)LAFlags.STRUCT_FLOAT_FIELD_SECOND);
                        }
                    }
                    break;
                    ...
                    default:
                    {
                        if (field.FieldType.GetElementSize().AsInt == 8)
                        {
                            ...
                            floatFieldFlags = (floatFieldFlags & (uint)LAFlags.STRUCT_FLOAT_FIELD_FIRST) > 0 ? floatFieldFlags : (uint)LAFlags.STRUCT_NO_FLOAT_FIELD;
                        break;
                    }
                }
