...
int verbMerge::AppendFileRaw(FILE* fpOut, LPCWSTR fileFullPath, unsigned char* buffer, size_t bufferSize)
{
    ...
    FILE* fpIn = fopen(fileNameAsChar, "rb");
    if (fpIn == NULL)
    {
...
// AppendFile: append the file named by 'fileFullPath' to the output file referred to by 'fpOut'. The 'fpOut'
// handle is assumed to be open, and the file position is assumed to be at the correct spot for writing, to append.
//
// 'buffer' is memory that can be used to do reading/buffering.
//
...
int verbMerge::DoWork(const char* nameOfOutputFile, const char* pattern, bool recursive, bool dedup, bool stripCR)
{
    ...
    FILE* fpOut = fopen(nameOfOutputFile, "wb");
    if (fpOut == NULL)
    {
...
CLEAN_UP:
    ...
    if (result != 0)
    {
        ...
        if (st != 0)
        {
            LogError("Failed to delete file after MCS /merge failed. errno=%d", errno);
        }
    }
    else
    {
