...
void Logger::OpenLogFile(char* logFilePath)
{
    ...
    {
        if (fopen_s(&s_logFile, logFilePath, "wb") != 0)
        {
        ...
        {
            fprintf(stderr, "WARNING: [Logger::OpenLogFile] Failed to open log file '%s'. errno=%d\n",
                    logFilePath, errno);
        }
    }
}
...
void Logger::CloseLogFile()
{
    if (s_logFile != NULL)
    {
        ...
        if (fgetsize(s_logFile) == 0 && s_logFilePath != nullptr)
        {
