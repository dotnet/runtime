...
minipal_critsect      Logger::s_critSec;
...
void Logger::Initialize()
{
    if (!s_initialized)
    {
        minipal_critsect_init(&s_critSec);
        s_initialized = true;
    }
}
...
void Logger::Shutdown()
{
    if (s_initialized)
    {
        minipal_critsect_destroy(&s_critSec);
        CloseLogFile();
        s_initialized = false;
    }
}
    ...
    const char* function, const char* file, int line, LogLevel level, va_list argList, const char* msg)
{
    ...
    minipal_critsect_enter(&s_critSec);
...
CleanUp:
    ...
    minipal_critsect_leave(&s_critSec);
    delete[] fullMsg;
}
