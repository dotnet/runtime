...
void MethodContext::recGetTlsRootInfo(CORINFO_CONST_LOOKUP* result)
{
    if (GetTlsRootInfo == nullptr)
        GetTlsRootInfo = new LightWeightMap<DWORD, Agnostic_CORINFO_CONST_LOOKUP>();

    DWORD key = 0;
    Agnostic_CORINFO_CONST_LOOKUP value = SpmiRecordsHelper::StoreAgnostic_CORINFO_CONST_LOOKUP(result);
    GetTlsRootInfo->Add(key, value);
    DEBUG_REC(dmpGetTlsRootInfo(key, result));
}

void MethodContext::dmpGetTlsRootInfo(DWORD key, const Agnostic_CORINFO_CONST_LOOKUP& value)
{
    printf("GetTlsRootInfo key %u, result-%s", key, SpmiDumpHelper::DumpAgnostic_CORINFO_CONST_LOOKUP(value).c_str());   
}
void MethodContext::repGetTlsRootInfo(CORINFO_CONST_LOOKUP* result)
{
    ...
    *result = SpmiRecordsHelper::RestoreCORINFO_CONST_LOOKUP(value);
}

void MethodContext::recGetTlsIndexInfo(CORINFO_CONST_LOOKUP* result)
{
    if (GetTlsIndexInfo == nullptr)
        GetTlsIndexInfo = new LightWeightMap<DWORD, Agnostic_CORINFO_CONST_LOOKUP>();

    DWORD                         key   = 0;
    Agnostic_CORINFO_CONST_LOOKUP value = SpmiRecordsHelper::StoreAgnostic_CORINFO_CONST_LOOKUP(result);
    GetTlsIndexInfo->Add(key, value);
    DEBUG_REC(dmpGetTlsIndexInfo(key, result));
}

void MethodContext::dmpGetTlsIndexInfo(DWORD key, const Agnostic_CORINFO_CONST_LOOKUP& value)
{
    printf("GetTlsIndexInfo key %u, result-%s", key, SpmiDumpHelper::DumpAgnostic_CORINFO_CONST_LOOKUP(value).c_str());
}
...
void MethodContext::recGetThreadStaticBaseSlowInfo(CORINFO_CONST_LOOKUP* result)
{
    if (GetThreadStaticBaseSlowInfo == nullptr)
        GetThreadStaticBaseSlowInfo = new LightWeightMap<DWORD, Agnostic_CORINFO_CONST_LOOKUP>();

    DWORD                         key   = 0;
    Agnostic_CORINFO_CONST_LOOKUP value = SpmiRecordsHelper::StoreAgnostic_CORINFO_CONST_LOOKUP(result);
    GetThreadStaticBaseSlowInfo->Add(key, value);
    DEBUG_REC(dmpGetThreadStaticBaseSlowInfo(key, result));
}

void MethodContext::dmpGetThreadStaticBaseSlowInfo(DWORD key, const Agnostic_CORINFO_CONST_LOOKUP& value)
{
    printf("GetThreadStaticBaseSlowInfo key %u, result-%s", key, SpmiDumpHelper::DumpAgnostic_CORINFO_CONST_LOOKUP(value).c_str());
}
...
void MethodContext::recGetEnsureClassCtorRunAndReturnThreadStaticBaseHelper(CORINFO_CLASS_HANDLE  cls, CORINFO_CONST_LOOKUP* addr, CORINFO_CONST_LOOKUP* targetSymbol, int size)
{
    if (GetEnsureClassCtorRunAndReturnThreadStaticBaseHelper == nullptr)
        GetEnsureClassCtorRunAndReturnThreadStaticBaseHelper = new LightWeightMap<DWORDLONG, Agnostic_GetClassCtorInitializationInfo>();

    Agnostic_GetClassCtorInitializationInfo value;
    ZeroMemory(&value, sizeof(value));
    value.addr       = SpmiRecordsHelper::StoreAgnostic_CORINFO_CONST_LOOKUP(addr);
    value.targetAddr = SpmiRecordsHelper::StoreAgnostic_CORINFO_CONST_LOOKUP(targetSymbol);
    value.size = size;

    DWORDLONG key = CastHandle(cls);

    GetEnsureClassCtorRunAndReturnThreadStaticBaseHelper->Add(key, value);
    DEBUG_REC(dmpGetEnsureClassCtorRunAndReturnThreadStaticBaseHelper(key, result));
}

void MethodContext::dmpGetEnsureClassCtorRunAndReturnThreadStaticBaseHelper(
    DWORDLONG key,
    const Agnostic_GetClassCtorInitializationInfo& value)
{
    printf("GetEnsureClassCtorRunAndReturnThreadStaticBaseHelper key %016" PRIX64 ", addr-%s, targetAddr-%s, size-%d", key,
           SpmiDumpHelper::DumpAgnostic_CORINFO_CONST_LOOKUP(value.addr).c_str(),
           SpmiDumpHelper::DumpAgnostic_CORINFO_CONST_LOOKUP(value.targetAddr).c_str(), value.size);
}

int MethodContext::repGetEnsureClassCtorRunAndReturnThreadStaticBaseHelper(CORINFO_CLASS_HANDLE  cls,
                                                                           CORINFO_CONST_LOOKUP* addr,
                                                                           CORINFO_CONST_LOOKUP* targetSymbol)
{
    DWORDLONG                     key   = CastHandle(cls);
    Agnostic_GetClassCtorInitializationInfo value =
        LookupByKeyOrMiss(GetEnsureClassCtorRunAndReturnThreadStaticBaseHelper, key, ": key %016" PRIX64 "", key);

    DEBUG_REP(dmpGetEnsureClassCtorRunAndReturnThreadStaticBaseHelper(key, value));

    *addr         = SpmiRecordsHelper::RestoreCORINFO_CONST_LOOKUP(value.addr);
    *targetSymbol = SpmiRecordsHelper::RestoreCORINFO_CONST_LOOKUP(value.targetAddr);
