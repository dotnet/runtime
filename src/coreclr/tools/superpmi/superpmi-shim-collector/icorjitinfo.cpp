...
CORINFO_METHOD_HANDLE interceptor_ICJI::getAwaitReturnCall(CORINFO_METHOD_HANDLE callerHandle, CORINFO_CONTEXT_HANDLE callerContext, CORINFO_LOOKUP* instArg)
{
    mc->cr->AddCall("getAwaitReturnCall");
    CORINFO_METHOD_HANDLE result = original_ICorJitInfo->getAwaitReturnCall(callerHandle, callerContext, instArg);
    mc->recGetAwaitReturnCall(callerHandle, callerContext, instArg, result);
    return result;
}
