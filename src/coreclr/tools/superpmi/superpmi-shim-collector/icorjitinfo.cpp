...
CORINFO_METHOD_HANDLE interceptor_ICJI::getAwaitReturnCall(CORINFO_METHOD_HANDLE callerHandle, bool transparent, CORINFO_LOOKUP* instArg)
{
    mc->cr->AddCall("getAwaitReturnCall");
    CORINFO_METHOD_HANDLE result = original_ICorJitInfo->getAwaitReturnCall(callerHandle, transparent, instArg);
    mc->recGetAwaitReturnCall(callerHandle, transparent, instArg, result);
    return result;
}
