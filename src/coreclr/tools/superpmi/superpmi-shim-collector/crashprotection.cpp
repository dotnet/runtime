...
namespace
{
	...
	public:
		...
		SyncThreadStarter(ThreadFunc func, TPayload&& payload) : m_func{func}, m_payload{std::move(payload)}, m_started{false} {
			int err;
...
CrashProtection::Init()
{
	...
	s_self = std::make_unique<CrashProtection>(commands.ExtractWriter(), completion.ExtractReader(), std::move(threadData));
