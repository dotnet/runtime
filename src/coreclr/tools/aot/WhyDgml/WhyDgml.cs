...
class Program
{
    static void Main(string[] args)
    {
        if (args.Length != 2)
        {
            ...
            Console.WriteLine();
            Console.WriteLine("You can use wildcard * in node_name");
            return;
        }
        ...
        {
            int matchedCount = 0;
            foreach (var key in nameToNode.Keys)
            {
                var isMatched = Regex.IsMatch(key, WildCardToRegular(goal));
                if (isMatched)
                {
                    matchedCount++;
