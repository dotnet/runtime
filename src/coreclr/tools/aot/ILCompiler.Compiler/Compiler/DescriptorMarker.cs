    ...
    internal sealed class DescriptorMarker : ProcessLinkerXmlBase
    {
        ...
        private void MarkAndPreserve(TypeDesc type, XPathNavigator nav, TypePreserve preserve)
        {
            ...
            foreach (var member in members)
            {
                ...
                {
                    case MethodDesc m:
                        RootingHelpers.TryGetDependenciesForReflectedMethod(_dependencies, _factory, m, reason);
                        break;
                    case FieldDesc field:
                        RootingHelpers.TryGetDependenciesForReflectedField(_dependencies, _factory, field, reason);
                        break;
                    case MetadataType nestedType:
                        RootingHelpers.TryGetDependenciesForReflectedType(_dependencies, _factory, nestedType, reason);
                        break;
                    case PropertyPseudoDesc property:
                        if (property.GetMethod != null)
                            RootingHelpers.TryGetDependenciesForReflectedMethod(_dependencies, _factory, property.GetMethod, reason);
                        if (property.SetMethod != null)
                            RootingHelpers.TryGetDependenciesForReflectedMethod(_dependencies, _factory, property.SetMethod, reason);
                        break;
                    case EventPseudoDesc @event:
                        if (@event.AddMethod != null)
                            RootingHelpers.TryGetDependenciesForReflectedMethod(_dependencies, _factory, @event.AddMethod, reason);
                        if (@event.RemoveMethod != null)
                            RootingHelpers.TryGetDependenciesForReflectedMethod(_dependencies, _factory, @event.RemoveMethod, reason);
                        break;
                    default:
        ...
        protected override void ProcessType(TypeDesc type, XPathNavigator nav)
        {
            ...
            RootingHelpers.TryGetDependenciesForReflectedType(_dependencies, _factory, type, "member marked via descriptor");
