...
struct JitInterfaceCallbacks
{
    ...
    uint32_t (* getNonGCThreadLocalFieldInfo)(void * thisHandle, CorInfoExceptionClass** ppException, CORINFO_FIELD_HANDLE field);
    uint32_t (* getGCThreadLocalFieldInfo)(void * thisHandle, CorInfoExceptionClass** ppException, CORINFO_FIELD_HANDLE field);
...
{
    CorInfoExceptionClass* pException = nullptr;
    uint32_t temp = _callbacks->getNonGCThreadLocalFieldInfo(_thisHandle, &pException, field);
    if (pException != nullptr) throw pException;
    return temp;
}
...
{
    CorInfoExceptionClass* pException = nullptr;
    uint32_t temp = _callbacks->getGCThreadLocalFieldInfo(_thisHandle, &pException, field);
    if (pException != nullptr) throw pException;
    return temp;
}
