...
bool gc_heap::virtual_commit (void* address, size_t size, int bucket, int h_number, bool* hard_limit_exceeded_p)
{
    ...
    // In addition if large pages or never-decommit is enabled, we set commit_succeeded_p
    // to true because memory is already committed (and VirtualCommit would be a no-op).
    bool commit_succeeded_p = ((h_number >= 0) ? ((use_large_pages_p || never_decommit_p) ? true :
...
bool gc_heap::virtual_decommit (void* address, size_t size, int bucket, int h_number)
{
    ...
    // With large pages or never-decommit, VirtualDecommit on heap memory is a no-op.
    // All such callers should either skip the decommit or handle stale data themselves
    // (decommit_region does the latter by calling reduce_committed_bytes directly and
    // clearing memory).
    assert ((!use_large_pages_p && !never_decommit_p) || bucket == recorded_committed_bookkeeping_bucket);
...
{
    if (settings.concurrent || use_large_pages_p || never_decommit_p || (settings.pause_mode == pause_no_gc))
    {
        return;
    }
...
bool gc_heap::decommit_step (uint64_t step_milliseconds)
{
    ...
    if (use_large_pages_p || never_decommit_p)
    {
    ...
    assert(!use_large_pages_p && !never_decommit_p);
...
size_t gc_heap::decommit_region (heap_segment* region, int bucket, int h_number)
{
    ...
    if (use_large_pages_p || never_decommit_p)
    {
        // VirtualDecommit is a no-op for large pages / never-decommit so skip it and
    ...
    bool require_clearing_memory_p = !decommit_succeeded_p || use_large_pages_p || never_decommit_p;
    ...
    {
        uint8_t* clear_end = (use_large_pages_p || never_decommit_p) ? heap_segment_used (region) : heap_segment_committed (region);
