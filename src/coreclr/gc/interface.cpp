...
HRESULT GCHeap::Initialize()
{
    ...
    // Objects on the LOH/POH are padded so that their data is aligned on this boundary.
    // Only powers of 2 bigger than the alignment the heap already provides are meaningful.
    size_t alignment = (size_t)GCConfig::GetUOHDataAlignment();
    uoh_data_alignment = (((alignment & (alignment - 1)) == 0) &&
                          (alignment > AlignQword (1)) &&
                          (alignment <= max_uoh_data_alignment)) ? alignment : 0;
