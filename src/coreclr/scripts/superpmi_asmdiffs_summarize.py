...
def append_diff_file(f, platform, arch, file_name, full_file_path):
    """ Append a single summary file to the consolidated diff file.

    Args:
        f : File we are appending to
        platform (string): OS platform we ran on
        arch (string): OS architecture we ran on
...
To reproduce these diffs on {3} {4}:
```
superpmi.py asmdiffs -target_os {0} -target_arch {1} -arch {4}
```

""".format(diff_platform, diff_arch, file_name, platform, arch))
        ...
        for dirpath, _, files in os.walk(diff_summary_dir):
            for file_name in files:
                if file_name.startswith("superpmi_") and file_name.endswith(".md"):
                    ...
                    if append_diff_file(f, platform, arch, file_name, full_file_path):
