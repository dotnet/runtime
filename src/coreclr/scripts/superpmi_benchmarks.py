...
def build_and_run(coreclr_args, output_mch_name):
    ...
    tfm = "net9.0"
    env["PERFLAB_TARGET_FRAMEWORKS"] = tfm
    ...
    if is_windows:
        ...
        env["NUGET_EXPERIMENTAL_CHAIN_BUILD_RETRY_POLICY"] = "9,2000"
    ...
    for try_num in range(num_tries):
        ...
        (_, _, return_code) = run_command(
            ...
            _exit_on_fail=exit_on_fail, _env=env)
        if return_code == 0:
            # It succeeded!
            break
    ...
    run_command(
        [dotnet_exe, "build", project_file, "--configuration", "Release",
         ...
         "-o", artifacts_directory], _exit_on_fail=True, _env=env)
