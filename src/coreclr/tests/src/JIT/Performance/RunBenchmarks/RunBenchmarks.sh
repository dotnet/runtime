...
# *** Currently we can build test cases only on Windows, so "win" is hard-coded in the variables.
BENCHMARK_ROOT_DIR="$CORECLR_ROOT/artifacts/tests/win.$ARCH.$BUILD/JIT/Performance/CodeQuality"
BENCHMARK_SRC_DIR="$CORECLR_ROOT/tests/src/JIT/Performance/RunBenchmarks"
BENCHMARK_HOST="$CORERUN $CORECLR_ROOT/artifacts/tests/win.$ARCH.$BUILD/JIT/Performance/RunBenchmarks/RunBenchmarks/RunBenchmarks.exe"
