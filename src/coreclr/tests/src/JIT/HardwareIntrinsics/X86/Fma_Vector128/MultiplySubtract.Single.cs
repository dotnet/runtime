...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleTernaryOpTest__MultiplySubtractSingle
    {
        ...
        {
            var result = typeof(Fma).GetMethod(nameof(Fma.MultiplySubtract), new Type[] { typeof(Vector128<Single>), typeof(Vector128<Single>), typeof(Vector128<Single>) })
        ...
        private void ValidateResult(Single[] firstOp, Single[] secondOp, Single[] thirdOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!Succeeded)
            {
                Console.WriteLine($"{nameof(Fma)}.{nameof(Fma.MultiplySubtract)}<Single>(Vector128<Single>, Vector128<Single>, Vector128<Single>): {method} failed:");
