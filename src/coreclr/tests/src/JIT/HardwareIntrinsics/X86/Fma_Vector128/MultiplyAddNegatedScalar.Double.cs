...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleTernaryOpTest__MultiplyAddNegatedScalarDouble
    {
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleTernaryOpTest__MultiplyAddNegatedScalarDouble testClass)
            {
        ...
        {
            var result = typeof(Fma).GetMethod(nameof(Fma.MultiplyAddNegatedScalar), new Type[] { typeof(Vector128<Double>), typeof(Vector128<Double>), typeof(Vector128<Double>) })
        ...
        private void ValidateResult(Double[] firstOp, Double[] secondOp, Double[] thirdOp, Double[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!Succeeded)
            {
                Console.WriteLine($"{nameof(Fma)}.{nameof(Fma.MultiplyAddNegatedScalar)}<Double>(Vector128<Double>, Vector128<Double>, Vector128<Double>): {method} failed:");
