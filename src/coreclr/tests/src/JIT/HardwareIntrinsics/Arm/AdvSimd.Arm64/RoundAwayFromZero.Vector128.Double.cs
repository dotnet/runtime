...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    public sealed unsafe class SimpleUnaryOpTest__RoundAwayFromZero_Vector128_Double
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleUnaryOpTest__RoundAwayFromZero_Vector128_Double testClass)
            {
            ...
            public void RunStructFldScenario_Load(SimpleUnaryOpTest__RoundAwayFromZero_Vector128_Double testClass)
            {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd.Arm64).GetMethod(nameof(AdvSimd.Arm64.RoundAwayFromZero), new Type[] { typeof(Vector128<Double>) })
        ...
        private void ValidateResult(Double[] firstOp, Double[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (BitConverter.DoubleToInt64Bits(Helpers.RoundAwayFromZero(firstOp[i])) != BitConverter.DoubleToInt64Bits(result[i]))
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd.Arm64)}.{nameof(AdvSimd.Arm64.RoundAwayFromZero)}<Double>(Vector128<Double>): {method} failed:");
