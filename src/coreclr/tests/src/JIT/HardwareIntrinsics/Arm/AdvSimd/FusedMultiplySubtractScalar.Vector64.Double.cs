...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void FusedMultiplySubtractScalar_Vector64_Double()
        {
    ...
    public sealed unsafe class SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Double
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Double testClass)
            {
                var result = AdvSimd.FusedMultiplySubtractScalar(_fld1, _fld2, _fld3);

                Unsafe.Write(testClass._dataTable.outArrayPtr, result);
                testClass.ValidateResult(_fld1, _fld2, _fld3, testClass._dataTable.outArrayPtr);
            }

            public void RunStructFldScenario_Load(SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Double testClass)
            {
                fixed (Vector64<Double>* pFld1 = &_fld1)
                fixed (Vector64<Double>* pFld2 = &_fld2)
                fixed (Vector64<Double>* pFld3 = &_fld3)
                {
                    var result = AdvSimd.FusedMultiplySubtractScalar(
        ...
        static SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Double()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetDouble(); }
        ...
        public SimpleTernaryOpTest__FusedMultiplySubtractScalar_Vector64_Double()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.FusedMultiplySubtractScalar), new Type[] { typeof(Vector64<Double>), typeof(Vector64<Double>), typeof(Vector64<Double>) })
        ...
        public void RunLclVarScenario_UnsafeRead()
        {
            ...
            var result = AdvSimd.FusedMultiplySubtractScalar(op1, op2, op3);
        ...
        private void ValidateResult(Double[] firstOp, Double[] secondOp, Double[] thirdOp, Double[] result, [CallerMemberName] string method = "")
        {
            ...
            if (BitConverter.DoubleToInt64Bits(Helpers.FusedMultiplySubtract(firstOp[0], secondOp[0], thirdOp[0])) != BitConverter.DoubleToInt64Bits(result[0]))
            {
                succeeded = false;
            }
            else
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.FusedMultiplySubtractScalar)}<Double>(Vector64<Double>, Vector64<Double>, Vector64<Double>): {method} failed:");
