...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    public sealed unsafe class ExtractVectorTest__ExtractVector64_Single_1
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario_Load(ExtractVectorTest__ExtractVector64_Single_1 testClass)
            {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.ExtractVector64), new Type[] { typeof(Vector64<Single>), typeof(Vector64<Single>), typeof(byte) })
        ...
        private void ValidateResult(Single[] firstOp, Single[] secondOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (BitConverter.SingleToInt32Bits(Helpers.ExtractVector(firstOp, secondOp, ElementIndex, i)) != BitConverter.SingleToInt32Bits(result[i]))
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.ExtractVector64)}<Single>(Vector64<Single>, Vector64<Single>, 1): {method} failed:");
