...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void Floor_Vector64_Single()
        {
            var test = new SimpleUnaryOpTest__Floor_Vector64_Single();
    ...
    public sealed unsafe class SimpleUnaryOpTest__Floor_Vector64_Single
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleUnaryOpTest__Floor_Vector64_Single testClass)
            {
                var result = AdvSimd.Floor(_fld1);

                Unsafe.Write(testClass._dataTable.outArrayPtr, result);
                testClass.ValidateResult(_fld1, testClass._dataTable.outArrayPtr);
            }

            public void RunStructFldScenario_Load(SimpleUnaryOpTest__Floor_Vector64_Single testClass)
            {
                fixed (Vector64<Single>* pFld1 = &_fld1)
                {
                    var result = AdvSimd.Floor(
        ...
        static SimpleUnaryOpTest__Floor_Vector64_Single()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetSingle(); }
        ...
        public SimpleUnaryOpTest__Floor_Vector64_Single()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.Floor), new Type[] { typeof(Vector64<Single>) })
        ...
        public void RunClassLclFldScenario()
        {
            ...
            var test = new SimpleUnaryOpTest__Floor_Vector64_Single();
            var result = AdvSimd.Floor(test._fld1);
        ...
        private void ValidateResult(Single[] firstOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (BitConverter.SingleToInt32Bits(Helpers.Floor(firstOp[i])) != BitConverter.SingleToInt32Bits(result[i]))
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.Floor)}<Single>(Vector64<Single>): {method} failed:");
