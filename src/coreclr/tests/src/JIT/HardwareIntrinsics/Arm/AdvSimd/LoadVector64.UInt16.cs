...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    public sealed unsafe class LoadUnaryOpTest__LoadVector64_UInt16
    {
        ...
        public void RunReflectionScenario_Load()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.LoadVector64), new Type[] { typeof(UInt16*) })
        ...
        private void ValidateResult(UInt16[] firstOp, UInt16[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.LoadVector64)}<UInt16>(Vector64<UInt16>): {method} failed:");
