...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void CompareGreaterThan_Vector64_Single()
        {
            var test = new SimpleBinaryOpTest__CompareGreaterThan_Vector64_Single();
    ...
    public sealed unsafe class SimpleBinaryOpTest__CompareGreaterThan_Vector64_Single
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleBinaryOpTest__CompareGreaterThan_Vector64_Single testClass)
            {
        ...
        static SimpleBinaryOpTest__CompareGreaterThan_Vector64_Single()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetSingle(); }
        ...
        public SimpleBinaryOpTest__CompareGreaterThan_Vector64_Single()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.CompareGreaterThan), new Type[] { typeof(Vector64<Single>), typeof(Vector64<Single>) })
        ...
        private void ValidateResult(Single[] left, Single[] right, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (BitConverter.SingleToInt32Bits(Helpers.CompareGreaterThan(left[0], right[0])) != BitConverter.SingleToInt32Bits(result[0]))
            {
                succeeded = false;
            }
            else
            {
                ...
                {
                    if (BitConverter.SingleToInt32Bits(Helpers.CompareGreaterThan(left[i], right[i])) != BitConverter.SingleToInt32Bits(result[i]))
                    {
                        succeeded = false;
                        break;
                    }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.CompareGreaterThan)}<Single>(Vector64<Single>, Vector64<Single>): {method} failed:");
