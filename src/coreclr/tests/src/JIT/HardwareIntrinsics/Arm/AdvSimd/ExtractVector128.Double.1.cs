...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void ExtractVector128_Double_1()
        {
    ...
    public sealed unsafe class ExtractVectorTest__ExtractVector128_Double_1
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(ExtractVectorTest__ExtractVector128_Double_1 testClass)
            {
            ...
            public void RunStructFldScenario_Load(ExtractVectorTest__ExtractVector128_Double_1 testClass)
            {
        ...
        static ExtractVectorTest__ExtractVector128_Double_1()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetDouble(); }
        ...
        public ExtractVectorTest__ExtractVector128_Double_1()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.ExtractVector128), new Type[] { typeof(Vector128<Double>), typeof(Vector128<Double>), typeof(byte) })
        ...
        private void ValidateResult(Double[] firstOp, Double[] secondOp, Double[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (BitConverter.DoubleToInt64Bits(Helpers.ExtractVector(firstOp, secondOp, ElementIndex, i)) != BitConverter.DoubleToInt64Bits(result[i]))
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.ExtractVector128)}<Double>(Vector128<Double>, Vector128<Double>, 1): {method} failed:");
