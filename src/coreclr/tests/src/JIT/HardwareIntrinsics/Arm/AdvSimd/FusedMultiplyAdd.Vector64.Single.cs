...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void FusedMultiplyAdd_Vector64_Single()
        {
    ...
    public sealed unsafe class SimpleTernaryOpTest__FusedMultiplyAdd_Vector64_Single
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleTernaryOpTest__FusedMultiplyAdd_Vector64_Single testClass)
            {
            ...
            public void RunStructFldScenario_Load(SimpleTernaryOpTest__FusedMultiplyAdd_Vector64_Single testClass)
            {
        ...
        static SimpleTernaryOpTest__FusedMultiplyAdd_Vector64_Single()
        {
            for (var i = 0; i < Op1ElementCount; i++) { _data1[i] = TestLibrary.Generator.GetSingle(); }
        ...
        public SimpleTernaryOpTest__FusedMultiplyAdd_Vector64_Single()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.FusedMultiplyAdd), new Type[] { typeof(Vector64<Single>), typeof(Vector64<Single>), typeof(Vector64<Single>) })
        ...
        private void ValidateResult(Single[] firstOp, Single[] secondOp, Single[] thirdOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            {
                if (BitConverter.SingleToInt32Bits(Helpers.FusedMultiplyAdd(firstOp[i], secondOp[i], thirdOp[i])) != BitConverter.SingleToInt32Bits(result[i]))
                {
                    succeeded = false;
                    break;
                }
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.FusedMultiplyAdd)}<Single>(Vector64<Single>, Vector64<Single>, Vector64<Single>): {method} failed:");
