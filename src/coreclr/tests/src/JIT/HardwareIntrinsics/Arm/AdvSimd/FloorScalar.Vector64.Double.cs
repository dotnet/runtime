...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    public sealed unsafe class SimpleUnaryOpTest__FloorScalar_Vector64_Double
    {
        ...
        private struct TestStruct
        {
            ...
            public void RunStructFldScenario(SimpleUnaryOpTest__FloorScalar_Vector64_Double testClass)
            {
            ...
            public void RunStructFldScenario_Load(SimpleUnaryOpTest__FloorScalar_Vector64_Double testClass)
            {
        ...
        public void RunReflectionScenario_UnsafeRead()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.FloorScalar), new Type[] { typeof(Vector64<Double>) })
        ...
        private void ValidateResult(Double[] firstOp, Double[] result, [CallerMemberName] string method = "")
        {
            ...
            if (BitConverter.DoubleToInt64Bits(Helpers.Floor(firstOp[0])) != BitConverter.DoubleToInt64Bits(result[0]))
            {
                succeeded = false;
            }
            else
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.FloorScalar)}<Double>(Vector64<Double>): {method} failed:");
