...
namespace JIT.HardwareIntrinsics.Arm
{
    ...
    {
        private static void LoadVector64_SByte()
        {
            var test = new LoadUnaryOpTest__LoadVector64_SByte();
    ...
    public sealed unsafe class LoadUnaryOpTest__LoadVector64_SByte
    {
        ...
        public LoadUnaryOpTest__LoadVector64_SByte()
        {
            Succeeded = true;
        ...
        public void RunReflectionScenario_Load()
        {
            ...
            var result = typeof(AdvSimd).GetMethod(nameof(AdvSimd.LoadVector64), new Type[] { typeof(SByte*) })
        ...
        private void ValidateResult(SByte[] firstOp, SByte[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!succeeded)
            {
                TestLibrary.TestFramework.LogInformation($"{nameof(AdvSimd)}.{nameof(AdvSimd.LoadVector64)}<SByte>(Vector64<SByte>): {method} failed:");
