...
std::vector<unsigned char> MethodContext::gatherData()
{
    // Calculate file size. We include an end '42' canary at the end.
...
#include "crlwmlist.h"
    ...
    unsigned char* buff2 = data.data(); // used by macro
...
#include "crlwmlist.h"
    ...
    buff2[buffIndex++] = '4';
    buff2[buffIndex++] = '2';
    Assert(buffIndex == totalLen);

    return data;
}

static std::vector<unsigned char> applyCompression(const std::vector<unsigned char>&& data, MethodContextCompression compression)
{
    switch (compression)
    {
    case MethodContextCompression::None:
    {
        return std::move(data);
    }
    case MethodContextCompression::Brotli:
    {
        ...
        std::unique_ptr<BrotliEncoderState, BrotliDeleter> brotli(BrotliEncoderCreateInstance(nullptr, nullptr, nullptr));
    ...
    default:
    {
        Assert(!"Unreachable compression case");
        return std::move(data);
    }
    }
}

unsigned int MethodContext::saveToFile(HANDLE hFile, MethodContextCompression compression)
{
    std::vector<unsigned char> uncompressedData = gatherData();
    Assert(uncompressedData.size() <= UINT32_MAX);
    unsigned int uncompressedDataSize = static_cast<unsigned int>(uncompressedData.size());
    std::vector<unsigned char> compressedData = applyCompression(std::move(uncompressedData), compression);

    Assert(compressedData.size() <= UINT32_MAX);
    unsigned int compressedDataSize = static_cast<unsigned int>(compressedData.size());

    unsigned int totalSize = 0;
    totalSize += 2; // leading magic cookie 'm', 'c'
    totalSize += 4; // 4-byte version (version 2+)
    totalSize += 1; // 1-byte compression kind (version 2+)
    totalSize += 4; // 4-byte length of upcoming data (might be compressed)
    totalSize += 4; // 4-byte uncompressed length (version 2+)
    totalSize += compressedDataSize;

    std::vector<unsigned char> finalData(totalSize);
    unsigned int index = 0;
    auto write = [&finalData, &index](auto value) {
        Assert(index + sizeof(value) <= finalData.size());
        memcpy(&finalData[index], &value, sizeof(value));
        index += sizeof(value);
    };
    write((char)'m');
    write((char)'c');
    write((int)CURRENT_MC_VERSION);
    write((char)compression);
    write((unsigned int)compressedDataSize);
    write((unsigned int)uncompressedDataSize);
    memcpy(&finalData[index], compressedData.data(), compressedDataSize);
    index += compressedDataSize;
    Assert(index == finalData.size());

    DWORD bytesWritten;
    WriteFile(hFile, finalData.data(), totalSize, &bytesWritten, nullptr);
    return bytesWritten;
}

// This code can't exist in a function with C++ objects needing destruction. Returns true on success
// (and sets *ppmc with new MethodContext), false on failure. This function frees the blob.
// static
bool MethodContext::Initialize(int mcIndex, const MethodContextBlobInfo& info, unsigned char* blob, /* OUT */ MethodContext** ppmc)
{
    MethodContext* mc = new MethodContext();
    mc->index         = mcIndex;
    *ppmc             = mc;
    bool result = mc->Initialize(mcIndex, info, blob);
    delete[] blob;
    return result;
}
...
bool MethodContext::Initialize(int mcIndex, const MethodContextBlobInfo& info, unsigned char* blob)
{
    bool result = true;
    ...
    {
        MethodContext* pThis;
        const MethodContextBlobInfo* info;
        unsigned char* blob;
    } param;
    param.pThis = this;
    param.info  = &info;
    ...
    {
        pParam->pThis->MethodInitHelperBlob(*pParam->info, pParam->blob);
    }
...
static std::vector<unsigned char> applyDecompression(unsigned char* data, size_t dataSize, MethodContextCompression compression, size_t decompressedLength)
{
    switch (compression)
    {
    case MethodContextCompression::None:
    {
        return std::vector<unsigned char>(data, data + dataSize);
    }
    case MethodContextCompression::Brotli:
    {
        struct BrotliDeleter
        ...
        std::unique_ptr<BrotliDecoderState, BrotliDeleter> brotli(BrotliDecoderCreateInstance(nullptr, nullptr, nullptr));

        std::vector<unsigned char> decompressed(decompressedLength);
        BrotliDecoderResult result = BrotliDecoderDecompress(dataSize, data, &decompressedLength, decompressed.data());
        Assert(result == BROTLI_DECODER_RESULT_SUCCESS);
        Assert(decompressedLength == decompressed.size());
        return decompressed;
    }
    default:
    {
        Assert(!"Unreachable decompression case");
        return std::vector<unsigned char>();
    }
    }
}

MethodContextBlobInfo MethodContext::ReadBlobInfoFromFile(HANDLE hFile)
{
    DWORD        bytesRead;
    char         buff[512];
    int version = 0;
    MethodContextBlobInfo blobInfo;

    AssertCode(ReadFile(hFile, buff, 2 + sizeof(int), &bytesRead, NULL) == TRUE,
               EXCEPTIONCODE_MC); // Read Magic number and version
    AssertCodeMsg((buff[0] == 'm') && (buff[1] == 'c'), EXCEPTIONCODE_MC, "Didn't find magic number");
    memcpy(&version, &buff[2], sizeof(int));
    if (version != CURRENT_MC_VERSION)
    {
        // Assume old style uncompressed mc file without version field for smooth transition.
        // The minimum total size is > CURRENT_MC_VERSION so this is unambiguous.
        // 2 bytes: 'mc'
        // 4 bytes: dataLen
        // dataLen bytes: uncompressed data
        // 2 bytes: '42' (canary) that we include as part of the blob
        unsigned int dataLen = static_cast<unsigned int>(version);
        blobInfo.CompressedBlobSize = dataLen + 2;
        blobInfo.DecompressedBlobSize = dataLen + 2;
        blobInfo.Compression = MethodContextCompression::None;
        return blobInfo;
    }

    // 2 bytes: 'mc'
    // 4 bytes: version
    // 1 bytes: compression kind
    // 4 bytes: compressedDataLen
    // 4 bytes: uncompressedDataLen
    // compressedDataLen bytes: compressed data

    char remainingFields[9];
    AssertCode(ReadFile(hFile, remainingFields, 9, &bytesRead, NULL) == TRUE, EXCEPTIONCODE_MC);
    blobInfo.Compression = static_cast<MethodContextCompression>(remainingFields[0]);
    memcpy(&blobInfo.CompressedBlobSize, &remainingFields[1], 4);
    memcpy(&blobInfo.DecompressedBlobSize, &remainingFields[5], 4);
    return blobInfo;
}

void MethodContext::MethodInitHelperFile(HANDLE hFile)
{
    MethodContextBlobInfo info = ReadBlobInfoFromFile(hFile);
    std::vector<unsigned char> compressedData(info.CompressedBlobSize);
    DWORD bytesRead;
    AssertCode(ReadFile(hFile, compressedData.data(), info.CompressedBlobSize, &bytesRead, NULL) == TRUE, EXCEPTIONCODE_MC);

    MethodInitHelperBlob(info, compressedData.data());
}

void MethodContext::MethodInitHelperBlob(const MethodContextBlobInfo& info, unsigned char* blob)
{
    std::vector<unsigned char> decompressedData = applyDecompression(blob, info.CompressedBlobSize, info.Compression, info.DecompressedBlobSize);
    MethodInitHelperUncompressed(decompressedData.data(), decompressedData.size());
}

void MethodContext::MethodInitHelperUncompressed(unsigned char* buff2, size_t totalLen)
{
    unsigned int   buffIndex = 0;
    ...
    while (buffIndex < totalLen - 2) // trailing canary
    {
