...
HRESULT GetCurrentModuleFileName(__out_ecount(*pcchBuffer) LPWSTR pBuffer, __inout DWORD* pcchBuffer)
{
    ...
    DWORD ret = WszGetModuleFileName(NULL, appPath, MINIPAL_LENGTHOF(appPath));

    if ((ret == MINIPAL_LENGTHOF(appPath)) || (ret == 0))
    {
...
BOOL IsCurrentModuleFileNameInAutoExclusionList()
{
    ...
    DWORD cchAppName = MINIPAL_LENGTHOF(wszAppName);
                                     ...
                                     BOOL*      pfAuto)
{
    ...
    *pcchDebuggerString = valueSize / sizeof(WCHAR);
    ...
    if (pfAuto != NULL)
    {
        ...
        DWORD cchAppName = MINIPAL_LENGTHOF(wzAppName);
        long  iValue;
...
BOOL LaunchJITDebugger()
{
    ...
    WCHAR debugger[1000];
    GetDebuggerSettingInfo(debugger, MINIPAL_LENGTHOF(debugger), NULL);
