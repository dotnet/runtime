...
void MethodStatsEmitter::Emit(int methodNumber, MethodContext* mc, ULONGLONG firstTime, ULONGLONG secondTime)
{
    if (hStatsFile != INVALID_HANDLE_VALUE)
    {
        ...
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 'h') != NULL || strchr(statsTypes, 'H') != NULL)
        {
            ...
            charCount += sprintf_s(rowData + charCount, MINIPAL_LENGTHOF(rowData) - charCount, "%s,", md5Hash);
        }
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 'n') != NULL || strchr(statsTypes, 'N') != NULL)
        {
            charCount += sprintf_s(rowData + charCount, MINIPAL_LENGTHOF(rowData) - charCount, "%d,", methodNumber);
        }
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 'i') != NULL || strchr(statsTypes, 'I') != NULL)
        {
            ...
            charCount += sprintf_s(rowData + charCount, MINIPAL_LENGTHOF(rowData) - charCount, "%d,", info.ILCodeSize);
        }
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 'a') != NULL || strchr(statsTypes, 'A') != NULL)
        {
            ...
            charCount += sprintf_s(rowData + charCount, MINIPAL_LENGTHOF(rowData) - charCount, "%d,", codeSize);
        }
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 't') != NULL || strchr(statsTypes, 'T') != NULL)
        {
            charCount +=
                sprintf_s(rowData + charCount, MINIPAL_LENGTHOF(rowData) - charCount, "%llu,%llu,", firstTime, secondTime);
        }
...
void MethodStatsEmitter::SetStatsTypes(char* types)
{
    ...
    if (hStatsFile != INVALID_HANDLE_VALUE)
    {
        ...
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 'h') != NULL || strchr(statsTypes, 'H') != NULL)
            charCount += sprintf_s(rowHeader + charCount, MINIPAL_LENGTHOF(rowHeader) - charCount, "HASH,");
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 'n') != NULL || strchr(statsTypes, 'N') != NULL)
            charCount += sprintf_s(rowHeader + charCount, MINIPAL_LENGTHOF(rowHeader) - charCount, "METHOD_NUMBER,");
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 'i') != NULL || strchr(statsTypes, 'I') != NULL)
            charCount += sprintf_s(rowHeader + charCount, MINIPAL_LENGTHOF(rowHeader) - charCount, "IL_CODE_SIZE,");
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 'a') != NULL || strchr(statsTypes, 'A') != NULL)
            charCount += sprintf_s(rowHeader + charCount, MINIPAL_LENGTHOF(rowHeader) - charCount, "ASM_CODE_SIZE,");
        if (strchr(statsTypes, '*') != NULL || strchr(statsTypes, 't') != NULL || strchr(statsTypes, 'T') != NULL)
            charCount += sprintf_s(rowHeader + charCount, MINIPAL_LENGTHOF(rowHeader) - charCount, "Time1,Time2,");
