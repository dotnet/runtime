...
void ControllerStackInfo::GetStackInfo(
    ...
    CONTEXT *pContext,
    bool suppressUMChainFromCRLToCOMMethodFrameGeneric
    )
{
    ...
    m_suppressUMChainFromCRLToCOMMethodFrameGeneric = suppressUMChainFromCRLToCOMMethodFrameGeneric;
...
StackWalkAction ControllerStackInfo::WalkStack(FrameInfo *pInfo, void *data)
{
    ...
    //
    // pInfo->fIgnoreThisFrameIfSuppressingUMChainFromCRLToCOMMethodFrameGeneric has been
    // set by TrackUMChain to help us remember that the current frame we're looking at is
    // CRLToCOMMethodFrameGeneric (we can't rely on looking at pInfo->frame to check
    // this), and i->m_suppressUMChainFromCRLToCOMMethodFrameGeneric has been set by the
    ...
    if(i->m_suppressUMChainFromCRLToCOMMethodFrameGeneric &&
        (pInfo->chainReason == CHAIN_ENTER_UNMANAGED) &&
        (pInfo->fIgnoreThisFrameIfSuppressingUMChainFromCRLToCOMMethodFrameGeneric))
    {
        return SWA_CONTINUE;
    }
                                         ...
                                         TRIGGER_WHY tyWhy)
{
    ...
    if (module == NULL && managed)
    {
        ...
        else if ((patch->trace.GetTraceType() == TRACE_FRAME_PUSH) && (thread->GetFrame()->IsTransitionToNativeFrame()))
        {
            ...
            // do too much work to determine the destination of a CRLToCOMMethodFrame. (See issue 87103.)
            //
