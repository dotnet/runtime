...
void DoAssertOnType(DebuggerIPCEventType event, int count)
{
    ...
    if ((event & 0x0300) == 0x0100)
    {
        ...
        {
            char        tmpStr[256];
            _snprintf_s(tmpStr, MINIPAL_LENGTHOF(tmpStr), _TRUNCATE, "%s == %d, break now!",
...
void Debugger::SendRawEvent(const DebuggerIPCEvent * pManagedEvent)
{
    ...
    EX_TRY
    {
        ...
        RaiseException(CLRDBG_NOTIFICATION_EXCEPTION_CODE, dwFlags, MINIPAL_LENGTHOF(rgData), rgData);
...
HRESULT Debugger::Startup(void)
{
...
#endif // !TARGET_UNIX
    {
        ...
        if (dwRegVal > 0)
        {
            ...
            {
                int iProc = GetRandomInt(MINIPAL_LENGTHOF(g_pStressProcs));
