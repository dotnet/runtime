...
//
// Arguments:
//      vmAssembly - new assembly to add
...
// static
void CordbAppDomain::AssemblyEnumerationCallback(VMPTR_Assembly vmAssembly, void * pThis)
{
    ...
    pAppDomain->LookupOrCreateAssembly(vmAssembly);
}
...
//---------------------------------------------------------------------------------------
// Lookup (or create) the CordbAssembly for the given VMPTR_Assembly
//
// Arguments:
//     vmAssembly - CLR token for the Assembly.
//
// Returns:
...
//
CordbModule* CordbAppDomain::LookupOrCreateModule(VMPTR_Module vmModule, VMPTR_Assembly vmAssembly)
{
    INTERNAL_API_ENTRY(this);
    CordbModule * pModule;
    ...
    _ASSERTE(!vmAssembly.IsNull() || !vmModule.IsNull());

    // check to see if the module is present in this app domain
    pModule = m_modules.GetBase(vmAssembly.IsNull() ? VmPtrToCookie(vmModule) : VmPtrToCookie(vmAssembly));
    if (pModule != NULL)
    {
        return pModule;
    }
...
CordbModule* CordbAppDomain::LookupOrCreateModule(VMPTR_Assembly vmAssembly)
{
    ...
    return LookupOrCreateModule(VMPTR_Module::NullPtr(), vmAssembly);
}
...
// static
void CordbAppDomain::ModuleEnumerationCallback(VMPTR_Assembly vmModule, void * pUserData)
{
    CONTRACTL
    {
        THROWS;
