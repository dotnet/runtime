...
namespace cdac
{
    namespace
    {
        ...
        const uint32_t MaxPointerDataCount = 1 << 20;
    ...
    bool DataDescriptor::LoadMerged(ReadMemoryCallback readMemory, void* context, uint64_t address, int depth, std::string& error)
    {
        ...
        for (size_t i = 0; i < m_descriptorAddresses.size(); i++)
        {
            if (m_descriptorAddresses[i] == address)
            {
                return true;
            }
        }
        ...
        if (pointerDataCount > MaxPointerDataCount)
        {
            error = "pointer_data_count out of range";
            return false;
        }
