...
namespace cdac
{
    Target::Target(const DataDescriptor* descriptor,
                   ...
                   void* readContext,
                   ReadThreadContextCallback readThreadContext)
        : m_descriptor(descriptor)
        , m_readMemory(readMemory)
        , m_readContext(readContext)
        , m_readThreadContext(readThreadContext)
    {
    }
    ...
    bool Target::TryReadThreadContext(uint64_t osThreadId, ThreadContext& threadContext) const
    {
        if (m_readThreadContext == nullptr)
        {
            return false;
        }
        return m_readThreadContext(m_readContext, osThreadId, threadContext);
    }
