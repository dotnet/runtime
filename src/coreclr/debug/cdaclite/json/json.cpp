...
namespace json
{
    ...
    bool Value::TryGetUInt64(uint64_t& out) const
    {
        if (type == Type::Number)
        {
            ...
            if (number >= 0.0 && number <= static_cast<double>(UINT64_MAX))
            {
    ...
    namespace
    {
        ...
        private:
            ...
            bool ParseNumber(Value& value)
            {
                ...
                if (!AtEnd() && (*m_cur == 'e' || *m_cur == 'E'))
                {
                    isFloat = true;
                    m_cur++;
                    if (!AtEnd() && (*m_cur == '+' || *m_cur == '-'))
                    {
                        m_cur++;
                    }
                    if (AtEnd() || *m_cur < '0' || *m_cur > '9')
                ...
                value.rawNumber.assign(start, static_cast<size_t>(m_cur - start));

                char* numberEnd = nullptr;
                errno = 0;
                value.number = strtod(value.rawNumber.c_str(), &numberEnd);
                if (errno != 0 || numberEnd == value.rawNumber.c_str() || *numberEnd != '\0')
                {
                    return Fail("invalid number");
                }
                if (!isFloat)
