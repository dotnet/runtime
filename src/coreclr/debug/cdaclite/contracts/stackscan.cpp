...
namespace contracts
{
    namespace
    {
        ...
        const int32_t RangeSectionFlag_Interpreter = 0x08;
        ...
        public:
            ...
            uint64_t EnumCodeForIP(uint64_t ip)
            {
                ...
                uint64_t methodDescPtr = ((int32_t)rangeSection.Flags & RangeSectionFlag_Interpreter) != 0
                    ? EnumInterpreterCodeAt(rangeSection, ip)
                    : EnumJitCodeAt(rangeSection, ip);
                if (methodDescPtr != 0)
                {
            ...
            void ProcessCodePointer(uint64_t value)
            {
                if (value == 0 || AlreadyProcessed(value))
                {
                    return;
                }
                uint64_t methodDesc = EnumCodeForIP(value);
                if (methodDesc != 0)
                {
                    NoteCapturedMethod(methodDesc);
                }
            }
        ...
        private:
            ...
            void EmitReadableSpan(const char* kind, uint64_t start)
            {
                if (start == 0)
                {
                    return;
                ...
                uint8_t probe = 0;
                if (!m_target.ReadBuffer(start, &probe, sizeof(probe)))
                {
                    return;
                }
                uint64_t end = (start + StackScanPageSize - 1) & ~(StackScanPageSize - 1);
                if (end < start)
                {
                    return;
                }
                while (end <= UINT64_MAX - StackScanPageSize &&
                       m_target.ReadBuffer(end, &probe, sizeof(probe)))
                {
                    end += StackScanPageSize;
                }
                Emit(kind, start, end - start);
            }

            void EmitUnwindData(const data::RangeSection& rangeSection, uint64_t runtimeFunctions, uint32_t count, uint32_t stride)
            {
                for (uint32_t i = 0; i < count; i++)
                {
                    data::RuntimeFunction runtimeFunction;
                    if (!m_target.TryRead(runtimeFunctions + (uint64_t)i * stride, runtimeFunction))
                    {
                        break;
                    }

#if defined(TARGET_AMD64)
                    uint32_t unwindData = runtimeFunction.UnwindData;
                    if ((unwindData & 0x1) != 0)
                    {
                        uint64_t owningFunction = rangeSection.RangeBegin + (unwindData & ~0x1u);
                        if (!m_target.TryRead(owningFunction, runtimeFunction))
                        {
                            continue;
                        }
                        unwindData = runtimeFunction.UnwindData;
                    }
                    uint64_t unwindAddress = rangeSection.RangeBegin + unwindData;
                    uint8_t unwindHeader[4];
                    if (!m_target.ReadBuffer(unwindAddress, unwindHeader, sizeof(unwindHeader)))
                    {
                        continue;
                    }
                    uint64_t unwindSize = 4 + (uint64_t)unwindHeader[2] * 2 + sizeof(uint32_t);
                    unwindSize = (unwindSize + sizeof(uint32_t) - 1) & ~(sizeof(uint32_t) - 1);
                    Emit("unwind-data", unwindAddress, unwindSize);
#elif defined(TARGET_X86)
                    Emit("unwind-data", rangeSection.RangeBegin + runtimeFunction.UnwindData, sizeof(uint32_t));
#elif defined(TARGET_ARM) || defined(TARGET_ARM64) || defined(TARGET_LOONGARCH64) || defined(TARGET_RISCV64)
                    if ((runtimeFunction.UnwindData & 0x3) != 0)
                    {
                        continue;
                    }
                    uint64_t unwindAddress = rangeSection.RangeBegin + runtimeFunction.UnwindData;
                    uint32_t header[2] = {};
                    if (!m_target.ReadBuffer(unwindAddress, header, sizeof(header[0])))
                    {
                        continue;
                    }
#if defined(TARGET_ARM)
                    uint32_t unwindWords = header[0] >> 28;
                    uint32_t epilogScopes = (header[0] >> 23) & 0x1f;
#else
                    uint32_t unwindWords = header[0] >> 27;
                    uint32_t epilogScopes = (header[0] >> 22) & 0x1f;
#endif
                    uint64_t unwindSize = sizeof(uint32_t);
...
#endif
                    {
                        unwindSize += sizeof(uint32_t);
                        if (!m_target.ReadBuffer(
                                unwindAddress + sizeof(uint32_t),
                                &header[1],
                                sizeof(header[1])))
                        {
                            continue;
                        }
                        unwindWords = (header[1] >> 16) & 0xff;
                        epilogScopes = header[1] & 0xffff;
                    }
                    if ((header[0] & (1 << 21)) == 0)
                    {
                        unwindSize += (uint64_t)sizeof(uint32_t) * epilogScopes;
                    }
                    unwindSize += (uint64_t)sizeof(uint32_t) * unwindWords + sizeof(uint32_t);
                    Emit("unwind-data", unwindAddress, unwindSize);
#endif
                }
            }
            ...
            uint64_t EnumJitCodeAt(const data::RangeSection& rangeSection, uint64_t ip)
            {
                    ...
                    m_target.TryGetTypeSize("RuntimeFunction", rfStride) && rfStride != 0)
                {
                    ...
                    EmitUnwindData(rangeSection, unwindInfosAddr, codeHeader.NumUnwindInfos, rfStride);
                }

                // The blob is variable-length. Emit its complete readable allocation rather than
                // truncating it at a fixed cap.
                if (codeHeader.GCInfo != 0)
                {
                    EmitReadableSpan("gc-info", codeHeader.GCInfo);
                }

                // Without decoding GC info here, use the containing code range's exact end rather
                // than a fixed-size cap that can truncate large methods.
                if (rangeSection.RangeEndOpen > codeStart)
                {
                    Emit("jit-code", codeStart, rangeSection.RangeEndOpen - codeStart);
                }

                return codeHeader.MethodDesc;
            }

            uint64_t EnumInterpreterCodeAt(const data::RangeSection& rangeSection, uint64_t ip)
            {
                data::CodeHeapListNode heapNode;
                if (!m_target.TryRead(rangeSection.HeapList, heapNode) ||
                    ip < heapNode.StartAddress || ip > heapNode.EndAddress)
                {
                    return 0;
                }
                ...
                uint64_t codeHeaderIndirect = codeStart - m_target.PointerSize();
                uint64_t codeHeaderAddress = 0;
                if (!m_target.TryReadPointer(codeHeaderIndirect, codeHeaderAddress) ||
                    codeHeaderAddress <= m_stubCodeBlockLast)
                {
                    return 0;
                }
                Emit("code-header-ptr", codeHeaderIndirect, m_target.PointerSize());

                data::InterpreterRealCodeHeader codeHeader;
                if (!m_target.TryRead(codeHeaderAddress, codeHeader))
                {
                    return 0;
                }
                if (codeHeader.GCInfo != 0)
                {
                    EmitReadableSpan("interpreter-gc-info", codeHeader.GCInfo);
                }
                if (rangeSection.RangeEndOpen > codeStart)
                {
                    Emit("interpreter-code", codeStart, rangeSection.RangeEndOpen - codeStart);
                }
                return codeHeader.MethodDesc;
            }
                ...
                if (emitNativeCode)
                {
                        ...
                        m_target.TryReadPointer(nativeCodeSlotAddr, pCode) && pCode != 0)
                    {
                        ...
                        if (rsAddr != 0)
                        {
                            ...
                            {
                                if (((int32_t)rs.Flags & RangeSectionFlag_Interpreter) != 0)
                                {
                                    EnumInterpreterCodeAt(rs, pCode);
                                }
                                else
                                {
            ...
            uint64_t EnumR2RMethodDesc(const data::RangeSection& rangeSection, uint64_t ip)
            {
                ...
                Emit(
                    "r2r-runtime-functions",
                    r2r.RuntimeFunctions,
                    (uint64_t)r2r.NumRuntimeFunctions * rfStride);
                ...
                EmitUnwindData(
                    rangeSection,
                    r2r.RuntimeFunctions + (uint64_t)index * rfStride,
                    1,
                    rfStride);
                data::RuntimeFunction runtimeFunction;
                if (m_target.TryRead(r2r.RuntimeFunctions + (uint64_t)index * rfStride, runtimeFunction))
                {
                    uint32_t codeEndRva = 0;
#if defined(TARGET_AMD64)
                    codeEndRva = runtimeFunction.EndAddress;
#else
                    if (index + 1 < r2r.NumRuntimeFunctions)
                    {
                        m_target.TryReadUInt32(
                            r2r.RuntimeFunctions + (uint64_t)(index + 1) * rfStride,
                            codeEndRva);
                    }
                    else
                    {
                        codeEndRva = static_cast<uint32_t>(rangeSection.RangeEndOpen - imageBase);
                    }
#endif
                    if (codeEndRva > runtimeFunction.BeginAddress)
                    {
                        Emit(
                            "r2r-code",
                            imageBase + runtimeFunction.BeginAddress,
                            codeEndRva - runtimeFunction.BeginAddress);
                    }
                }
            ...
            uint64_t ResolveFrameMethodDesc(uint64_t frameAddr, uint64_t identifier)
            {
                ...
                if (m_target.TryGetGlobalValue("StubDispatchFrameIdentifier", id) && id == identifier)
                {
                    ...
                    uint64_t representativeMT = 0;
                    uint32_t representativeSlot = 0;
                    if (!m_target.TryReadFieldPointer(frameAddr, "StubDispatchFrame", "RepresentativeMTPtr", representativeMT) ||
                        representativeMT == 0 ||
                        !m_target.TryReadFieldUInt32(frameAddr, "StubDispatchFrame", "RepresentativeSlot", representativeSlot))
                    {
                        return 0;
                    }

                    uint64_t eeClassOrCanon = 0;
                    if (!m_target.TryReadFieldPointer(representativeMT, "MethodTable", "EEClassOrCanonMT", eeClassOrCanon))
                    {
                        return 0;
                    }
                    uint64_t canonicalMT = eeClassOrCanon & ~(uint64_t)0x3;
                    if ((eeClassOrCanon & 0x3) == 0)
                    {
                        if (!m_target.TryReadFieldPointer(canonicalMT, "EEClass", "MethodTable", canonicalMT))
                        {
                            return 0;
                        }
                    }

                    uint64_t numVirtualsAddress = 0;
                    uint16_t numVirtuals = 0;
                    if (!m_target.TryGetFieldAddress(canonicalMT, "MethodTable", "NumVirtuals", numVirtualsAddress) ||
                        !m_target.TryReadUInt16(numVirtualsAddress, numVirtuals))
                    {
                        return 0;
                    }

                    uint32_t methodTableSize = 0;
                    if (!m_target.TryGetTypeSize("MethodTable", methodTableSize))
                    {
                        return 0;
                    }
                    if (representativeSlot < numVirtuals)
                    {
                        uint64_t indirectionAddress = canonicalMT + methodTableSize +
                            (uint64_t)(representativeSlot >> 3) * ptrSize;
                        uint64_t slots = 0;
                        uint64_t entryPoint = 0;
                        if (m_target.TryReadPointer(indirectionAddress, slots) &&
                            slots != 0 &&
                            m_target.TryReadPointer(
                                slots + (uint64_t)(representativeSlot & 7) * ptrSize,
                                entryPoint))
                        {
                            uint64_t entryPointMethodDesc = EnumCodeForIP(entryPoint);
                            if (entryPointMethodDesc != 0)
                            {
                                return entryPointMethodDesc;
                            }
                        }
                    }

                    uint64_t methodDesc = FindIntroducedMethodForSlot(canonicalMT, representativeSlot);
                    if (methodDesc != 0 || representativeSlot >= numVirtuals)
                    {
                        return methodDesc;
                    }

                    std::set<uint64_t> visitedMethodTables;
                    uint64_t currentMT = canonicalMT;
                    while (visitedMethodTables.insert(currentMT).second)
                    {
                        uint64_t parentMT = 0;
                        if (!m_target.TryReadFieldPointer(currentMT, "MethodTable", "ParentMethodTable", parentMT) ||
                            parentMT == 0)
                        {
                            break;
                        }
                        currentMT = GetCanonicalMethodTable(parentMT);
                        if (currentMT == 0)
                        {
                            break;
                        }
                        methodDesc = FindIntroducedMethodForSlot(currentMT, representativeSlot);
                        if (methodDesc != 0)
                        {
                            return methodDesc;
                        }
            ...
            uint64_t GetCanonicalMethodTable(uint64_t methodTable)
            {
                uint64_t eeClassOrCanon = 0;
                if (!m_target.TryReadFieldPointer(methodTable, "MethodTable", "EEClassOrCanonMT", eeClassOrCanon) ||
                    eeClassOrCanon == 0)
                {
                    return 0;
                }
                uint64_t canonicalMT = eeClassOrCanon & ~(uint64_t)0x3;
                if ((eeClassOrCanon & 0x3) == 0 &&
                    !m_target.TryReadFieldPointer(canonicalMT, "EEClass", "MethodTable", canonicalMT))
                {
                    return 0;
                }
                return canonicalMT;
            ...
            uint64_t FindIntroducedMethodForSlot(uint64_t methodTable, uint32_t slot)
            {
                uint64_t eeClass = 0;
                if (!m_target.TryReadFieldPointer(methodTable, "MethodTable", "EEClassOrCanonMT", eeClass) ||
                    (eeClass & 0x3) != 0)
                {
                    return 0;
                }
                ...
                uint64_t chunk = 0;
                if (!m_target.TryReadFieldPointer(eeClass, "EEClass", "MethodDescChunk", chunk))
                {
                    return 0;
                }

                uint32_t chunkSize = 0;
                if (!m_target.TryGetTypeSize("MethodDescChunk", chunkSize))
                {
                    return 0;
                }

                std::set<uint64_t> visitedChunks;
                while (chunk != 0 && visitedChunks.insert(chunk).second)
                {
                    data::MethodDescChunk methodDescChunk;
                    if (!m_target.TryRead(chunk, methodDescChunk))
                    {
                        break;
                    }
                    ...
                    uint8_t count = 0;
                    if (!m_target.TryGetFieldAddress(chunk, "MethodDescChunk", "Count", countAddress) ||
                        !m_target.TryReadUInt8(countAddress, count))
                    {
                        return 0;
                    }

                    uint64_t methodDesc = chunk + chunkSize;
                    for (uint32_t i = 0; i <= count; i++)
                    {
                        uint64_t slotAddress = 0;
                        uint16_t methodSlot = 0;
                        if (!m_target.TryGetFieldAddress(methodDesc, "MethodDesc", "Slot", slotAddress) ||
                            !m_target.TryReadUInt16(slotAddress, methodSlot))
                        {
                            return 0;
                        }
                        if (methodSlot == slot)
                        ...
                        uint32_t methodDescSize = MethodDescSize(m_target, methodDesc);
                        if (methodDescSize == 0)
                        {
                            return 0;
                        }
                        methodDesc += methodDescSize;
                    }
                    chunk = methodDescChunk.Next;
                }
                return 0;
            }
    ...
    int EnumerateStackScanRegions(const Target& target, RegionCallback sink, void* sinkContext)
    {
        ...
        for (int i = 0; threadAddr != 0 && i < MaxThreads; i++)
        {
            ...
            if (thread.OSId != 0 && target.TryReadThreadContext(thread.OSId, threadContext))
            {
                enumerator.ProcessCodePointer(threadContext.instructionPointer);
                for (uint32_t r = 0; r < threadContext.registerValueCount; r++)
                {
                    enumerator.ProcessCodePointer(threadContext.registerValues[r]);
                }
            }
            ...
            if (stackLow != 0 && stackHigh > stackLow)
            {
                ...
                for (;;)
                {
                    ...
                    {
                        uint64_t value = 0;
                        memcpy(&value, pageBuf + off, (size_t)ptrSize);
                        if (value == 0)
                        {
                            continue;
                        }
