...
namespace contracts
{
    ...
    int EnumerateStressLogRegions(const Target& target, RegionCallback, void*)
    {
        ...
        uint64_t enabled = 0;
        if (!target.TryGetGlobalValue(GlobalStressLogEnabled, enabled) || enabled == 0)
        {
            return 0;
        }
        ...
        uint64_t chunkHeaderSize = 8u * target.PointerSize();
        if (chunkSize > UINT32_MAX - chunkHeaderSize)
        {
            return -1;
        }
        ...
        uint32_t chunkEmitSize = static_cast<uint32_t>(chunkSize + chunkHeaderSize);
