...
namespace contracts
{
    ...
    int EnumerateBootstrapRegions(const Target& target, RegionCallback sink, void* sinkContext)
    {
        ...
        if (target.TryReadGlobalPointer("Debugger", debuggerAddr) && debuggerAddr != 0)
        {
            ...
            uint64_t hijackFunctions = 0;
            uint64_t maxHijackFunctions = 0;
            uint32_t memoryRangeSize = 0;
            if (target.TryReadFieldPointer(debuggerAddr, "Debugger", "RgHijackFunction", hijackFunctions) &&
                hijackFunctions != 0 &&
                target.TryGetGlobalValue("MaxHijackFunctions", maxHijackFunctions) &&
                target.TryGetTypeSize("MemoryRange", memoryRangeSize) &&
                memoryRangeSize != 0 &&
                maxHijackFunctions <= UINT32_MAX / memoryRangeSize)
            {
                target.EmitMemory(
                    hijackFunctions,
                    static_cast<uint32_t>(maxHijackFunctions) * memoryRangeSize);
                emitted++;
            }
        }
