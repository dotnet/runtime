...
{
    char memPath[128];
    snprintf(memPath, sizeof(memPath), "/proc/%u/mem", m_pid);
    ...
    if (disablePagemapUse.IsSet() && disablePagemapUse.TryAsInteger(10, val) && val == 0)
    {
        ...
        snprintf(pagemapPath, sizeof(pagemapPath), "/proc/%u/pagemap", m_pid);
...
{
    char taskPath[128];
    snprintf(taskPath, sizeof(taskPath), "/proc/%u/task", m_pid);
...
{
    char auxvPath[128];
    snprintf(auxvPath, sizeof(auxvPath), "/proc/%u/auxv", m_pid);
