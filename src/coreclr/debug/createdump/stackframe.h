...
public:
    ...
    StackFrame(uint64_t moduleAddress, uint64_t returnAddress, uint64_t stackPointer, IXCLRDataMethodInstance* pMethod, uint32_t nativeOffset, uint64_t token, uint32_t ilOffset) :
        m_moduleAddress(moduleAddress),
        m_returnAddress(returnAddress),
        m_stackPointer(stackPointer),
        m_nativeOffset(nativeOffset),
    ...
    StackFrame(const StackFrame& frame) :
        ...
        m_returnAddress(frame.m_returnAddress),
