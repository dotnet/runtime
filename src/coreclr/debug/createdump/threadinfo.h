...
private:
    ...
    std::set<StackFrame>::iterator m_beginRepeat;   // beginning of stack overflow repeated frame sequence
    std::set<StackFrame>::iterator m_endRepeat;     // end of repeated frame sequence
...
public:
    ...
    inline int ThreadInfo::NumRepeatedFrames() const { return m_repeatedFrames;  }
    inline bool IsBeginRepeat(std::set<StackFrame>::iterator& iterator) const { return m_repeatedFrames > 0 && iterator == m_beginRepeat; }
    inline bool IsEndRepeat(std::set<StackFrame>::iterator& iterator) const { return  m_repeatedFrames > 0 && iterator == m_endRepeat; }
