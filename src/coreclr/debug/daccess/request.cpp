...
HRESULT ClrDataAccess::GetObjectComWrappersData(CLRDATA_ADDRESS objAddr, CLRDATA_ADDRESS *rcw, unsigned int count, CLRDATA_ADDRESS *mowList, unsigned int *pNeeded)
{
    ...
    if (rcw != nullptr)
    {
        ...
        MethodTable* pComObjectBaseMT = (&g_CoreLib)->GetClassIfExist(CLASS__COM_OBJECT_BASE);
        MethodTable* pCurrentMT = pComObjectBaseMT != nullptr ? object->GetMethodTable() : nullptr;
        while (pCurrentMT != nullptr && pCurrentMT != pComObjectBaseMT)
        {
        ...
        {
            FieldDesc* pWrapperField = (&g_CoreLib)->GetField(FIELD__COM_OBJECT_BASE__NATIVE_OBJECT_WRAPPER);
