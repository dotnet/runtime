    ...
    if (!block || block->firstElement == 0)
    {
        ...
        else
        {
            ...
            newBlock = (HashInstanceKeyBlock*)
                ClrVirtualAlloc(NULL, HASH_INSTANCE_BLOCK_ALLOC_SIZE*MINIPAL_LENGTHOF(m_hash), MEM_COMMIT, PAGE_READWRITE);
        }
        if (!newBlock)
        {
            return NULL;
        ...
        {
            for (DWORD j = 0; j < MINIPAL_LENGTHOF(m_hash); j++)
            {
                m_hash[j] = newBlock;
...
DacInstanceManager::ClearEnumMemMarker(void)
{
    ...
    for (i = 0; i < MINIPAL_LENGTHOF(m_hash); i++)
    {
        HashInstanceKeyBlock* block = m_hash[i];
        while (block)
        {
                                   ...
                                   bool* isAlternate)
{
    ...
    if (FAILED(hr) && isNGEN)
    {
        ...
        if (wcscpy_s(ngenImageName, MINIPAL_LENGTHOF(ngenImageName), uniPath) != 0)
        {
            goto ErrExit;
        }
        if (wcscpy_s(uniPath, MINIPAL_LENGTHOF(uniPath), ngenImageName) != 0)
        {
            goto ErrExit;
        }
        // Transform NGEN image name into IL Image name
        if (!GetILImageNameFromNgenImage(ilExtension, uniPath, MINIPAL_LENGTHOF(uniPath)))
        {
            goto ErrExit;
        }
#endif//FEATURE_CORESYSTEM
