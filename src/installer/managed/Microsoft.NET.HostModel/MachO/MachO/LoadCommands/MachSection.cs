...
{
    internal sealed class MachSection
    {
        ...
        internal MachSection(MachObjectFile objectFile, string segmentName, string sectionName)
            : this(objectFile, segmentName, sectionName, null)
        {
        }

        internal MachSection(MachObjectFile objectFile, string segmentName, string sectionName, Stream stream)
        ...
        /// </summary>
        internal string SectionName { get; private init; }
        ...
        /// </remarks>
        internal string SegmentName { get; private init; }
        ...
        /// </summary>
        internal uint Log2Alignment { get; set; }

        /// <summary>
        /// Gets the file offset to relocation entries of this section.
        /// </summary>
        internal uint RelocationOffset => RelocationData?.FileOffset ?? 0u;

        /// <summary>
        /// Gets or sets the number of relocation entries of this section.
        /// </summary>
        internal uint NumberOfRelocationEntries => (uint)((RelocationData?.Size ?? 0u) / 8);

        internal uint Flags { get; set; }

        [EditorBrowsable(EditorBrowsableState.Advanced)]
        internal uint Reserved1 { get; set; }

        [EditorBrowsable(EditorBrowsableState.Advanced)]
        internal uint Reserved2 { get; set; }

        [EditorBrowsable(EditorBrowsableState.Advanced)]
        internal uint Reserved3 { get; set; }

        internal MachSectionAttributes Attributes
        {
            get => (MachSectionAttributes)(Flags & ~0xffu);
            set => Flags = (Flags & 0xffu) | (uint)value;
        }

        internal MachSectionType Type
        {
            get => (MachSectionType)(Flags & 0xff);
            set => Flags = (Flags & ~0xffu) | (uint)value;
        }

        internal bool IsInFile => Size > 0 && Type != MachSectionType.ZeroFill && Type != MachSectionType.GBZeroFill && Type != MachSectionType.ThreadLocalZeroFill;

        internal bool HasContentChanged { get; set; }

        internal MachLinkEditData RelocationData
        {
            get
            {
        ...
        internal IList<MachRelocation> Relocations
        {
            get
            {
