...
public class ComputeWasmPublishAssets : Task
{
    ...
    public bool WasmEnableEventPipe { get; set; }
...
#pragma warning disable CA1864 // Prefer the 'IDictionary.TryAdd(TKey, TValue)' method. Dictionary.TryAdd() not available in .Net framework.
            if (AssetsComputingHelper.ShouldFilterCandidate(candidate, TimeZoneSupport, InvariantGlobalization, LoadFullICUData, CopySymbols, customIcuCandidateFilename, EnableThreads, WasmEnableEventPipe, EmitSourceMap, out var reason))
            {
