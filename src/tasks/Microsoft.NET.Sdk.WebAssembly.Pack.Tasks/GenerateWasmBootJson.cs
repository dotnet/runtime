    ...
    public void WriteBootJson(Stream output, string entryAssemblyName)
    {
        ...
        if (Resources != null)
        {
            ...
            foreach (var resource in Resources)
            {
                ...
                {
                    if (TryGetLazyLoadedAssembly($"{fileName}.dll", out _) || TryGetLazyLoadedAssembly($"{fileName}.webcil", out _) || TryGetLazyLoadedAssembly($"{fileName}{Utils.WebcilInWasmExtension}", out _))
                    {
