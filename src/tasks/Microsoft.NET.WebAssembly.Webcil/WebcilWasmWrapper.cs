...
//
// The entire wasm module is going to be unchanging, except for the data section which has 2
// segments.  segment 0 is passive, 4 bytes, and contains the length of the webcil payload.  segment 1 is of a
// variable size and contains the webcil payload.
//
// Two wrapper versions are produced. The unchanging parts of each are stored as a "prefix" and "suffix"
// containing the bytes of the WAT module below, split into the parts that come before and after the data
// section.
//
// Version 0 (Mono) keeps the payload in a passive segment that the host copies with getWebcilPayload:
//
// (module
...
//    memory.init 1))
//
// Version 2 (CoreCLR) is self-installing: the payload is an active segment that the engine copies to the
// host-supplied __memory_base at instantiation, so the host calls nothing after instantiating it:
//
// (module
//  (import "webcil" "memory" (memory 1))
//  (import "webcil" "__memory_base" (global $memoryBase i32))
//  (global (export "webcilVersion") i32 (i32.const 2))
//  (func (export "getWebcilSize") (param $destPtr i32)
//    local.get $destPtr
//    i32.const 0
//    i32.const 4
//    memory.init 0)
//  (data "\0f\00\00\00")                      ;; data segment 0: payload size as a 4 byte LE uint32
//  (data (global.get $memoryBase) "webcil Payload\cc")) ;; data segment 1: webcil payload
public class WebcilWasmWrapper
{
    private readonly Stream _webcilPayloadStream;
    private readonly uint _webcilPayloadSize;
    private readonly bool _selfInstalling;

    public WebcilWasmWrapper(Stream webcilPayloadStream)
        : this(webcilPayloadStream, selfInstalling: false)
    {
    }

    public WebcilWasmWrapper(Stream webcilPayloadStream, bool selfInstalling)
    {
        ...
        _selfInstalling = selfInstalling;
    }
    ...
    private
        ...
        byte[]
#endif
        WasmWrapperPrefix => _selfInstalling ? s_selfInstallingWasmWrapperPrefix : s_wasmWrapperPrefix;

    //
    // Everything from the above version 0 wat module before the data section
    //
    ...
    //
    // Everything from the above version 0 wat module after the data section
    //
    // extracted by wasm-reader -s wrapper.wasm
    private static
#if NET
    ...
    //
    // Everything from the above version 2 wat module before the data section. The data section is last,
    // so there is no suffix.
    //
    // generated by `wasm-tools parse` and `wasm-tools strip -a`
    private static
#if NET
        ReadOnlyMemory<byte>
#else
        byte[]
#endif
        s_selfInstallingWasmWrapperPrefix = new byte[] {
        0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x01, 0x60, 0x01, 0x7f, 0x00, 0x02, 0x2a, 0x02, 0x06, 0x77, 0x65, 0x62, 0x63, 0x69, 0x6c, 0x06, 0x6d, 0x65, 0x6d, 0x6f, 0x72, 0x79,
        0x02, 0x00, 0x01, 0x06, 0x77, 0x65, 0x62, 0x63, 0x69, 0x6c, 0x0d, 0x5f, 0x5f, 0x6d, 0x65, 0x6d, 0x6f, 0x72, 0x79, 0x5f, 0x62, 0x61, 0x73, 0x65, 0x03, 0x7f, 0x00, 0x03, 0x02, 0x01, 0x00, 0x06,
        0x06, 0x01, 0x7f, 0x00, 0x41, 0x02, 0x0b, 0x07, 0x21, 0x02, 0x0d, 0x77, 0x65, 0x62, 0x63, 0x69, 0x6c, 0x56, 0x65, 0x72, 0x73, 0x69, 0x6f, 0x6e, 0x03, 0x01, 0x0d, 0x67, 0x65, 0x74, 0x57, 0x65,
        0x62, 0x63, 0x69, 0x6c, 0x53, 0x69, 0x7a, 0x65, 0x00, 0x00, 0x0c, 0x01, 0x02, 0x0a, 0x0e, 0x01, 0x0c, 0x00, 0x20, 0x00, 0x41, 0x00, 0x41, 0x04, 0xfc, 0x08, 0x00, 0x00, 0x0b,
    };

    // The version 2 payload segment's offset expression: global.get 0 (the imported __memory_base), end.
    private static readonly byte[] s_selfInstallingPayloadOffsetExpr = { 0x23, 0x00, 0x0b };

    private void WriteWasmHeader(Stream outputStream)
    {
#if NET
        outputStream.Write(WasmWrapperPrefix.Span);
#else
        outputStream.Write(WasmWrapperPrefix, 0, WasmWrapperPrefix.Length);
#endif
    }

    private void WriteWasmSuffix(Stream outputStream)
    {
        if (_selfInstalling)
            return;
#if NET
    ...
    // 1 byte to encode the data segment mode
    private const uint SegmentCodeSize = 1;

    private uint PayloadSegmentHeaderSize => SegmentCodeSize + (_selfInstalling ? (uint)s_selfInstallingPayloadOffsetExpr.Length : 0);
    ...
    private void WriteDataSection(BinaryWriter writer)
    {
        ...
        //   segment 1 has its mode byte (plus, when active, its offset expression), a uleb128 encoding of the
        //   webcilPayloadSize, and the payload. don't count the size of the payload yet
        checked
        {
            dataSectionSize += PayloadSegmentHeaderSize + (uint)ulebWebcilPayloadSize.Length;
        }
        ...
        uint payloadOffset = (uint)WasmWrapperPrefix.Length + 1 + (uint)putativeULEBDataSectionSize.Length + dataSectionSize ;
        ...
        if (_selfInstalling)
        {
            writer.Write((byte)0); // active segment, memory 0
            writer.Write(s_selfInstallingPayloadOffsetExpr);
        }
        else
        {
            writer.Write((byte)1); // passive segment
