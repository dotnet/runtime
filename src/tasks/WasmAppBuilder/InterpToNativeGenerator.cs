...
public class InterpToNativeGenerator : Task
{
    ...
    public static void Emit(StreamWriter w)
    {
        ...
        w.WriteLine("*/");
        w.WriteLine("");
        w.WriteLine("#include <mono/mono/mini/mini.h>");
        w.WriteLine("");
        ...
        foreach (var c in signatures)
        {
            ...
            w.WriteLine("\tgpointer* iargs = mono_wasm_interp_method_args_get_iargs (margs);");
        ...
        w.WriteLine("mono_wasm_install_interp_to_native_invokes ({");
    ...
    private sealed class EmitCtx
    {
        ...
        public string Emit(char c)
        {
            switch (c)
            {
                case 'I':
                    iarg += 1;
                    return $"(int)(gssize)iargs [{iarg - 1}]";
                case 'F':
                    farg += 1;
