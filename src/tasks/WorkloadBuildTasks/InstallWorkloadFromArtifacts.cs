...
namespace Microsoft.Workload.Build.Tasks
{
    public class InstallWorkloadFromArtifacts : Task
    {
        ...
        public override bool Execute()
        {
            ...
            // FIXME: clear out the entries for all the nupkgs found in artifacts
            // can't figure out the packname vs version in that!!
        ...
        private bool InstallWorkloadManifest(string name, string version, string nugetConfigContents)
        {
            ...
            PackageInstaller installer = new(nugetConfigContents, Log);
            ...
            if (!installer.Install(errorOnMissing: true, pkgRef))
                return false;
