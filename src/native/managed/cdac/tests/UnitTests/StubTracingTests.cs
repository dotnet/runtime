...
public class StubTracingTests
{
    ...
    public void ArmCallCountingStubUsesInstructionPointer()
    {
        ...
        AddPointerGlobal(
            builder,
            arch,
            Constants.Globals.DACNotifyCompilationFinished,
            globalAddress: 0x2000,
            value: 0);
        AddPointerGlobal(
            builder,
            arch,
            Constants.Globals.ThePreStubPatchLabel,
            globalAddress: 0x2100,
            value: 0);
    ...
    private static Target CreateTarget(
        ...
        MockMemorySpace.HeapFragment[]? memory = null)
    {
        TestPlaceholderTarget.Builder builder = new TestPlaceholderTarget.Builder(arch)
            .AddMockContract(executionManager)
            .AddContract<IStubTracing>("c1");
    ...
    private static void AddPointerGlobal(
        TestPlaceholderTarget.Builder builder,
        MockTarget.Architecture arch,
        string name,
        ulong globalAddress,
        ulong value)
    {
        TargetTestHelpers helpers = new(arch);
        byte[] pointer = new byte[helpers.PointerSize];
        helpers.WritePointer(pointer, value);
        builder.MemoryBuilder.AddHeapFragment(new MockMemorySpace.HeapFragment
        {
        ...
        builder.AddGlobals((name, globalAddress));
    }
}
