...
public unsafe class DacDbiImplTests
{
    ...
    // ---- Tier 2/3 ResolveTypeReference tests (resolution-scope walk + by-name metadata search) ----
    //
    // These exercise the cDAC-only resolution path (no legacy DAC). The Loader and EcmaMetadata
    // contracts are mocked so that the referencing module's TypeRef cache always misses (Tier 1),
    // forcing the implementation to walk the TypeRef resolution scope to a module and search that
    // module's metadata (TypeDefs / ExportedType forwarders / nested types) by name.

    private const uint MdtTypeRef = (uint)EcmaMetadataUtils.TokenType.mdtTypeRef;
    private const uint MdtAssemblyRef = 0x23u << 24;
    ...
    [Theory]
    [ClassData(typeof(MockTarget.StdArch))]
    public void ResolveTypeReference_TypeRef_NotFoundInTargetModule_ReturnsClassNotLoaded(MockTarget.Architecture arch)
    {
        // Referencing module references "NS.Missing" in AssemblyRef "Target".
        var (refReader, refProvider) = BuildMetadata(mb =>
        {
            AssemblyReferenceHandle asmRef = mb.AddAssemblyReference(mb.GetOrAddString("Target"), new Version(1, 0, 0, 0), default, default, default, default);
            mb.AddTypeReference(asmRef, mb.GetOrAddString("NS"), mb.GetOrAddString("Missing"));
        });

        // Target module defines no matching type and no forwarder.
        var (targetReader, targetProvider) = BuildMetadata(_ => { });

        const ulong refAsmPtr = 0x100, refManifest = 0x9001, targetModPtr = 0x4000;
        ModuleHandle refHandle = Mod(0x1000), targetHandle = Mod(0x2000);
