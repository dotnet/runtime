...
public class GCMemoryRegionTests
{
    ...
    [Theory]
    [ClassData(typeof(MockTarget.StdArch))]
    public void GetGCFreeRegions_ServerHeapInlineFreeRegions(MockTarget.Architecture arch)
    {
        TargetTestHelpers helpers = new(arch);
        MockMemorySpace.Builder builder = new(helpers);
        MockMemorySpace.BumpAllocator allocator = builder.CreateAllocator(0x0010_0000, 0x0100_0000);
        ...
        var types = new Dictionary<DataType, Target.TypeInfo>
        {
            [DataType.GCHeap] = new() { Fields = MakeFields(
                ("FreeRegions", 0, DataType.pointer)), Size = CountFreeRegionKinds * regionFreeListSize },
            [DataType.HeapSegment] = new() { Fields = MakeFields(
                ("Allocated", 0, DataType.pointer),
                ("Committed", ptrSize, DataType.pointer),
                ("Reserved", 2 * ptrSize, DataType.pointer),
        ...
        var heapFragment = allocator.Allocate(CountFreeRegionKinds * regionFreeListSize, "Heap");
        Span<byte> heapSpan = builder.BorrowAddressRange(heapFragment.Address, heapFragment.Data.Length);
        helpers.WritePointer(heapSpan.Slice(0, ptrSize), new TargetPointer(CountFreeRegionKinds));
        helpers.WritePointer(heapSpan.Slice(5 * ptrSize, ptrSize), segFragment.Address);

        var heapTableFragment = allocator.Allocate((ulong)ptrSize, "HeapTable");
        Span<byte> heapTableSpan = builder.BorrowAddressRange(heapTableFragment.Address, heapTableFragment.Data.Length);
        helpers.WritePointer(heapTableSpan, heapFragment.Address);

        var heapsFragment = allocator.Allocate((ulong)ptrSize, "Heaps");
        Span<byte> heapsSpan = builder.BorrowAddressRange(heapsFragment.Address, heapsFragment.Data.Length);
        helpers.WritePointer(heapsSpan, heapTableFragment.Address);

        var numHeapsFragment = allocator.Allocate(sizeof(int), "NumHeaps");
        Span<byte> numHeapsSpan = builder.BorrowAddressRange(numHeapsFragment.Address, numHeapsFragment.Data.Length);
        helpers.Write(numHeapsSpan, 1);

        var globals = BuildGlobals(
            new()
            {
                [Constants.Globals.Heaps] = heapsFragment.Address,
                [Constants.Globals.NumHeaps] = numHeapsFragment.Address,
            },
            new());

        var readContext = builder.GetMemoryContext();
        IGC gc = CreateGCContract(arch, types, globals,
            [(Constants.Globals.GCIdentifiers, "server,regions,background")],
            readContext.ReadFromTarget);
