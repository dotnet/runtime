...
public class DacDbiLoaderDumpTests : DumpTestBase
{
    ...
    [ConditionalTheory]
    [MemberData(nameof(TestConfigurations))]
    public unsafe void GetSymbolsBuffer_ReturnsValidResult(TestConfiguration config)
    {
        InitializeDumpTest(config);
        DacDbiImpl dbi = CreateDacDbi();
        ILoader loader = Target.Contracts.Loader;
        ...
        foreach (ModuleHandle module in GetAllModules())
        {
            ...
            if (symbolFormat == 0 /* kSymbolFormatNone */)
            {
    ...
    [ConditionalTheory]
    [MemberData(nameof(TestConfigurations))]
    public unsafe void GetSymbolsBuffer_InvalidModule(TestConfiguration config)
    {
        InitializeDumpTest(config);
        DacDbiImpl dbi = CreateDacDbi();
        ...
        DacDbiTargetBuffer targetBuffer;
        int symbolFormat;
        int hr = dbi.GetSymbolsBuffer(0, &targetBuffer, &symbolFormat);
        Assert.NotEqual(System.HResults.S_OK, hr);
    }
}
