...
/// <summary>
/// Paired cDAC/DAC proxy for ISOSStressLogThreadEnum.
/// </summary>
[GeneratedComClass]
internal sealed unsafe partial class SOSStressLogThreadEnumProxy
    : ShimProxy, ICustomQueryInterface, ISOSStressLogThreadEnum
{
    private readonly ISOSEnum? _cdacEnum;
    private readonly ISOSEnum? _legacyEnum;
    private readonly ISOSStressLogThreadEnum? _cdacStressLogThreadEnum;
    private readonly ISOSStressLogThreadEnum? _legacyStressLogThreadEnum;

    internal SOSStressLogThreadEnumProxy(ValidationSession session, object? cdacObject, object? dacObject)
        : base(session, cdacObject, dacObject)
    {
        _cdacEnum = cdacObject as ISOSEnum;
        _legacyEnum = dacObject as ISOSEnum;
        _cdacStressLogThreadEnum = cdacObject as ISOSStressLogThreadEnum;
        _legacyStressLogThreadEnum = dacObject as ISOSStressLogThreadEnum;
    }
    ...
    public CustomQueryInterfaceResult GetInterface(ref Guid iid, out nint ppv)
    {
        ...
        if (iid == typeof(ISOSStressLogThreadEnum).GUID)
            return Support(_cdacStressLogThreadEnum, _legacyStressLogThreadEnum);
    ...
    #region ISOSStressLogThreadEnum
    int ISOSStressLogThreadEnum.Next(uint count, SOSThreadStressLogData[] values, uint* pFetched)
    {
        using ShimCall shimCall = ShimCall.Enter();
        int hr = _cdacStressLogThreadEnum is not null ? _cdacStressLogThreadEnum.Next(count, values, pFetched) : HResults.E_NOTIMPL;
        return hr;
    }

    #endregion ISOSStressLogThreadEnum
