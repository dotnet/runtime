...
/// <summary>
/// Paired cDAC/DAC proxy for ISOSStackRefEnum.
/// </summary>
[GeneratedComClass]
internal sealed unsafe partial class SOSStackRefEnumProxy
    : ShimProxy, ICustomQueryInterface, ISOSStackRefEnum
{
    private readonly ISOSEnum? _cdacEnum;
    private readonly ISOSEnum? _legacyEnum;
    private readonly ISOSStackRefEnum? _cdacStackRefEnum;
    private readonly ISOSStackRefEnum? _legacyStackRefEnum;

    internal SOSStackRefEnumProxy(ValidationSession session, object? cdacObject, object? dacObject)
        : base(session, cdacObject, dacObject)
    {
        _cdacEnum = cdacObject as ISOSEnum;
        _legacyEnum = dacObject as ISOSEnum;
        _cdacStackRefEnum = cdacObject as ISOSStackRefEnum;
        _legacyStackRefEnum = dacObject as ISOSStackRefEnum;
    }
    ...
    public CustomQueryInterfaceResult GetInterface(ref Guid iid, out nint ppv)
    {
        ...
        if (iid == typeof(ISOSStackRefEnum).GUID)
            return Support(_cdacStackRefEnum, _legacyStackRefEnum);
    ...
    #region ISOSStackRefEnum
    int ISOSStackRefEnum.Next(uint count, SOSStackRefData[] refs, uint* pFetched)
    {
        using ShimCall shimCall = ShimCall.Enter();
        int hr = _cdacStackRefEnum is not null ? _cdacStackRefEnum.Next(count, refs, pFetched) : HResults.E_NOTIMPL;
        return hr;
    }
    ...
    {
        using ShimCall shimCall = ShimCall.Enter();
        int hr = _cdacStackRefEnum is not null ? _cdacStackRefEnum.EnumerateErrors(ppEnum) : HResults.E_NOTIMPL;
        return hr;
    }

    #endregion ISOSStackRefEnum
