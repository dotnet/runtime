...
public sealed unsafe partial class ClrDataExceptionState : IXCLRDataExceptionState
{
    ...
    int IXCLRDataExceptionState.IsSameState(ExceptionRecord64* exRecord, uint contextSize, byte* cxRecord)
        => _legacyImpl is not null ? _legacyImpl.IsSameState(exRecord, contextSize, cxRecord) : HResults.E_NOTIMPL;
    int IXCLRDataExceptionState.IsSameState2(uint flags, ExceptionRecord64* exRecord, uint contextSize, byte* cxRecord)
