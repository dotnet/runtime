      ...
      ISOSDacInterface16
{
    ...
    int ISOSDacInterface.GetCCWInterfaces(ClrDataAddress ccw, uint count, [In, MarshalUsing(CountElementName = nameof(count)), Out] DacpCOMInterfacePointerData[]? interfaces, uint* pNeeded)
    {
        ...
        try
        {
            ...
            Contracts.IBuiltInCOM builtInCOMContract = _target.Contracts.BuiltInCOM; // E_NOTIMPL if not defined (non-Windows)
        ...
        if (_legacyImpl is not null)
        {
            ...
            if (hr == HResults.S_OK)
            {
                Debug.Assert(numWritten == pNeededLocal, $"cDAC count: {numWritten}, DAC count: {pNeededLocal}");
