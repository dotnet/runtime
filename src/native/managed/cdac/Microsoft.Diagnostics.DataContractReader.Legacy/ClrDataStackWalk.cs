...
public sealed unsafe partial class ClrDataStackWalk : IXCLRDataStackWalk
{
    ...
    private bool MoveNextLegacyVisible()
    {
        ...
        ulong stackPointerBeforeFiltering = stackWalk.GetStackPointer(_dataFrames.Current).Value;

        do
        {
            _stackSizeSkipped = unchecked(stackWalk.GetStackPointer(_dataFrames.Current).Value - stackPointerBeforeFiltering);
    ...
    int IXCLRDataStackWalk.GetStackSizeSkipped(ulong* stackSizeSkipped)
    {
        ...
        try
        {
            if (stackSizeSkipped is null)
                throw new NullReferenceException("The stack size output pointer is null.");
