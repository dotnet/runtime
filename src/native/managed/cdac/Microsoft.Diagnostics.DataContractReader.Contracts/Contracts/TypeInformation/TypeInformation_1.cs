...
internal sealed class TypeInformation_1 : ITypeInformation
{
    private readonly Target _target;
    private readonly Dictionary<ModuleHandle, SignatureTypeInfoProvider> _providers = [];

    internal TypeInformation_1(Target target)
    {
        _target = target;
    }
    ...
    public MethodSignature<SignatureTypeInfo> DecodeMethodSignature(MethodDescHandle methodDesc)
    {
        ...
        SignatureTypeInfoProvider provider = GetProvider(moduleHandle);
    ...
    public SignatureTypeInfo GetFieldTypeInfo(TargetPointer fieldDesc, SignatureTypeInfo owningType)
    {
        IRuntimeTypeSystem rts = _target.Contracts.RuntimeTypeSystem;
        ...
        RuntimeSignatureDecoder<SignatureTypeInfo, SignatureTypeContext> decoder =
            new(GetProvider(moduleHandle), _target, metadataReader, context);
    ...
    private SignatureTypeInfoProvider GetProvider(ModuleHandle moduleHandle)
    {
        if (_providers.TryGetValue(moduleHandle, out SignatureTypeInfoProvider? provider))
        {
            return provider;
        }

        provider = new SignatureTypeInfoProvider(_target, moduleHandle);
        _providers[moduleHandle] = provider;
        return provider;
    }
