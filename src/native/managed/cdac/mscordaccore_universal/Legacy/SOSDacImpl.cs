      ...
      ISOSDacInterface16
{
    ...
    int ISOSDacInterface.GetGCHeapData(void* data)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetGCHeapData(data)) : HResults.E_NOTIMPL;
    ...
    int ISOSDacInterface.GetGCHeapList(uint count, [In, MarshalUsing(CountElementName = "count"), Out] ClrDataAddress[] heaps, uint* pNeeded)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetGCHeapList(count, heaps, pNeeded)) : HResults.E_NOTIMPL;
    ...
    int ISOSDacInterface.GetMethodDescPtrFromFrame(ClrDataAddress frameAddr, ClrDataAddress* ppMD)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetMethodDescPtrFromFrame(frameAddr, ppMD)) : HResults.E_NOTIMPL;
    ...
    int ISOSDacInterface.TraverseModuleMap(int mmt, ClrDataAddress moduleAddr, void* pCallback, void* token)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.TraverseModuleMap(mmt, moduleAddr, pCallback, token)) : HResults.E_NOTIMPL;
