...
internal sealed unsafe partial class ClrDataTask : IXCLRDataTask
{
    ...
    int IXCLRDataTask.GetProcess(/*IXCLRDataProcess*/ void** process)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetProcess(process)) : HResults.E_NOTIMPL;
    int IXCLRDataTask.GetCurrentAppDomain(/*IXCLRDataAppDomain*/ void** appDomain)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetCurrentAppDomain(appDomain)) : HResults.E_NOTIMPL;
    int IXCLRDataTask.GetUniqueID(ulong* id)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetUniqueID(id)) : HResults.E_NOTIMPL;
    ...
    int IXCLRDataTask.IsSameObject(IXCLRDataTask* task)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.IsSameObject(task)) : HResults.E_NOTIMPL;
    int IXCLRDataTask.GetManagedObject(/*IXCLRDataValue*/ void** value)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetManagedObject(value)) : HResults.E_NOTIMPL;
    int IXCLRDataTask.GetDesiredExecutionState(uint* state)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetDesiredExecutionState(state)) : HResults.E_NOTIMPL;
    int IXCLRDataTask.SetDesiredExecutionState(uint state)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.SetDesiredExecutionState(state)) : HResults.E_NOTIMPL;
    ...
    int IXCLRDataTask.GetOSThreadID(uint* id)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetOSThreadID(id)) : HResults.E_NOTIMPL;
    int IXCLRDataTask.GetContext(uint contextFlags, uint contextBufSize, uint* contextSize, byte* contextBuffer)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetContext(contextFlags, contextBufSize, contextSize, contextBuffer)) : HResults.E_NOTIMPL;
    ...
    int IXCLRDataTask.GetCurrentExceptionState(/*IXCLRDataExceptionState*/ void** exception)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetCurrentExceptionState(exception)) : HResults.E_NOTIMPL;
    ...
    int IXCLRDataTask.GetName(uint bufLen, uint* nameLen, char* nameBuffer)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetName(bufLen, nameLen, nameBuffer)) : HResults.E_NOTIMPL;
    int IXCLRDataTask.GetLastExceptionState(/*IXCLRDataExceptionState*/ void** exception)
        => _legacyImpl is not null ? FallbackHelper.Fallback(() => _legacyImpl.GetLastExceptionState(exception)) : HResults.E_NOTIMPL;
}
