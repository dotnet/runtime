...
{
    private static readonly Dictionary<DataType, AbstractTarget.TypeInfo> TestTypes = new()
    {
        // Size and fields
        [DataType.Thread] = new(){
            Size = 56,
    ...
    """);
        ...
        fixed (byte* jsonPtr = json)
        {
            ...
            foreach ((DataType type, AbstractTarget.TypeInfo info) in TestTypes)
            {
                {
                    // By known type
                    AbstractTarget.TypeInfo actual = target.GetTypeInfo(type);
                    Assert.Equal(info.Size, actual.Size);
                    Assert.Equal(info.Fields, actual.Fields);
                }
                {
                    // By name
                    AbstractTarget.TypeInfo actual = target.GetTypeInfo(type.ToString());
                    Assert.Equal(info.Size, actual.Size);
                    Assert.Equal(info.Fields, actual.Fields);
                }
            }
