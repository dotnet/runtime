...
{
    static Array IData<Array>.Create(AbstractTarget target, TargetPointer address)
        => new Array(target, address);

    public Array(AbstractTarget target, TargetPointer address)
    {
        AbstractTarget.TypeInfo type = target.GetTypeInfo(DataType.Array);
