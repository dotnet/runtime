...
// Browser/WASMFS uses emscripten's musl libc, which defines DT_* to the standard
// Linux values and reports correct d_type (see ConvertDirent), so it is covered by
// the plain defined(DT_UNKNOWN) check like any other Unix.
#if defined(DT_UNKNOWN) && !defined(TARGET_WASI)
...
static void ConvertDirent(const struct dirent* entry, DirectoryEntry* outputEntry)
{
    ...
    // WASI's dirent d_type is likewise treated as unreliable here.
    // (Browser/WASMFS is excluded: its getdents64 reports correct d_type, so we
    // trust it below and avoid an extra stat per directory entry.)
