...
int32_t SystemNative_PortAssociate(intptr_t port, intptr_t pFileObj, char *dirPath, TimeSpec *mtime, int32_t events, intptr_t cookie)
{
#if HAVE_SUNOS_PORTFS
    struct file_obj *fo;
...
int32_t SystemNative_PortGet(intptr_t port, int32_t *events, intptr_t *cookie, TimeSpec *tmo)
{
#if HAVE_SUNOS_PORTFS
    ...
    *cookie = (intptr_t)pe.portev_user;  // Return the cookie we set
    return (0);
#else
...
int32_t SystemNative_PortSend(intptr_t port, int32_t evflags, intptr_t cookie)
{
#if HAVE_SUNOS_PORTFS
    int32_t pfd = ToFileDescriptor(port);
