...
int32_t SystemNative_PortAssociate(intptr_t port, intptr_t pFileObj, const char *dirPath, TimeSpec *mtime, int32_t events, uintptr_t cookie)
{
    ...
    // kernel returns EINVAL if this is NULL -- no check required here
    ...
    rv = port_associate(pfd, PORT_SOURCE_FILE, (uintptr_t)fo,
            events, (void *)cookie);
    // Note that the kernel does NOT reference fo->fo_name after this.
    // Setting this to NULL so that's clear in code analysis.
    fo->fo_name = NULL;
    return rv;
#else
