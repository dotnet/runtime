...
import { dotnetApi, dotnetRuntimeExports } from "../cross-module";
import { Int32Ptr, VoidPtr, VoidPtrNull } from "../types";
...
export function SystemJS_GetLocaleInfo(culture: number, cultureLength: number, locale: number, localeLength: number, dst: number, dstMaxLength: number, dstLength: Int32Ptr): VoidPtr {
    ...
    try {
        const localeNameOriginal = dotnetRuntimeExports.utf16ToString(<any>locale, <any>(locale + 2 * localeLength));
        ...
        if (!localeName && localeNameOriginal) {
            ...
            dotnetRuntimeExports.stringToUTF16(dst, dst + 2 * localeNameOriginal.length, localeNameOriginal);
            dotnetApi.setHeapI32( dstLength, localeNameOriginal.length);
            return VoidPtrNull;
        }
        const cultureNameOriginal = dotnetRuntimeExports.utf16ToString(<any>culture, <any>(culture + 2 * cultureLength));
        ...
        dotnetRuntimeExports.stringToUTF16(dst, dst + 2 * result.length, result);
        dotnetApi.setHeapI32(dstLength, result.length);
        return VoidPtrNull;
    } catch (ex: any) {
        dotnetApi.setHeapI32(dstLength, -1);
        return dotnetRuntimeExports.stringToUTF16Ptr(ex.toString());
    }
