...
if /i [%1] == [-project] ( set __Project=%2&&shift&&shift&goto Arg_Loop)

if /i [%1] == [-flavor] ( set __BuildFlavor=%2&&shift&&shift&goto Arg_Loop)
...
echo Commencing build of native components for project %__Project%
echo.
...
set "__SourcesDir=%__sourceRootDir%\%__Project%"

set "__IntermediatesDir=%__artifactsDir%\obj\native\external\%__Project%\%__outConfig%\%__BuildFlavor%"
...
if %__CMakeBinDir% == "" (
    set "__CMakeBinDir=%__artifactsDir%\bin\native\external\%__outConfig%\%__BuildFlavor%"
)
...
set MSBUILD_EMPTY_PROJECT_CONTENT= ^
 ...
 ^^^</Project^^^>
echo %MSBUILD_EMPTY_PROJECT_CONTENT% > "%__artifactsDir%\obj\native\external\Directory.Build.props"
echo %MSBUILD_EMPTY_PROJECT_CONTENT% > "%__artifactsDir%\obj\native\external\Directory.Build.targets"

:: Regenerate the VS solution

call "%__repoRoot%\eng\native\gen-buildsys.cmd" "%__SourcesDir%" "%__IntermediatesDir%" %__VSVersion% %__BuildArch% %__TargetOS% %__ExtraCmakeParams%
if NOT [%errorlevel%] == [0] goto :Failure
...
echo Done building Native components for project %__Project%
exit /B 0

:Failure
:: Build failed
echo Failed to generate native component build project for %__Project%!
exit /b 1
