...
void utils_get_directory(const pal_char_t* path, pal_char_t* out_dir, size_t out_dir_len)
{
    ...
    if (last_sep == NULL)
    {
        ...
        snprintf(out_dir, out_dir_len, _X("%s/"), buf);
        return;
    }
...
void utils_get_filename(const pal_char_t* path, pal_char_t* out_name, size_t out_name_len)
{
    ...
    const pal_char_t* last_sep = strrchr(path, '/');
...
void utils_get_runtime_id(pal_char_t* out_rid, size_t out_rid_len)
{
    ...
    if (pal_getenv(_X("DOTNET_RUNTIME_ID"), rid_env, sizeof(rid_env)))
    {
    ...
    snprintf(out_rid, out_rid_len, _X("%s-%s"),
...
void utils_get_host_version_description(pal_char_t* out_desc, size_t out_desc_len)
{
    ...
    {
        snprintf(out_desc, out_desc_len, _X("%s %s"), _STRINGIFY(HOST_VERSION), commit_maybe);
    }
    else
    {
        snprintf(out_desc, out_desc_len, _X("%s"), _STRINGIFY(HOST_VERSION));
    }
}
...
void utils_get_dotnet_root_env_var_for_arch(pal_char_t* out_name, size_t out_name_len)
{
    ...
    snprintf(out_name, out_name_len, _X("%s_%s"), DOTNET_ROOT_ENV_VAR, arch_upper);
}
