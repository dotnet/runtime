...
export function browserHostInitializeCoreCLR():void {
    if (CoreCLRInitialized) {
        return;
    }
    // int browserHostInitializeCoreCLR(void)
    const res = Module.ccall("browserHostInitializeCoreCLR", "number") as number;
    if (res != 0) {
...
export function browserHostResolveMain(exitCode:number):void {
    runMainPromiseController.resolve(exitCode);
}

export function browserHostRejectMain(reason:any):void {
    runMainPromiseController.reject(reason);
}
