...
enum StatusCode
{
    ...
    AppArgNotManagedFailure             = 0x80008095,   // [internal usage only] File exists but is not a managed .dll or .exe
    AppHostExeNotBoundFailure           = 0x80008096,   // `apphost` failed to determine which application to run
    FrameworkMissingFailure             = 0x80008097,   // Failed to find a compatible framework version
    HostApiFailed                       = 0x80008098,   // Host command failed
    HostApiBufferTooSmall               = 0x80008099,   // Buffer provided to a host API is too small to fit the requested value
    // unused                           = 0x8000809a,
    AppPathFindFailure                  = 0x8000809b,   // Application path imprinted in `apphost` doesn't exist
    SdkResolveFailure                   = 0x8000809c,   // Failed to find the requested SDK
    FrameworkCompatFailure              = 0x8000809d,   // Application has multiple references to the same framework which are not compatible
    FrameworkCompatRetry                = 0x8000809e,   // [internal usage only]
    // unused                           = 0x8000809f,
    BundleExtractionFailure             = 0x800080a0,   // Error extracting single-file bundle
    BundleExtractionIOError             = 0x800080a1,   // Error reading or writing files during single-file bundle extraction
    LibHostDuplicateProperty            = 0x800080a2,   // The application's `.runtimeconfig.json` contains a runtime property which is produced by the hosting layer
    HostApiUnsupportedVersion           = 0x800080a3,   // Feature which requires certain version of the hosting layer was used on a version which doesn't support it
    HostInvalidState                    = 0x800080a4,   // Current state is incompatible with the requested operation
    HostPropertyNotFound                = 0x800080a5,   // Property requested by `hostfxr_get_runtime_property_value` doesn't exist
    HostIncompatibleConfig              = 0x800080a6,   // Host configuration is incompatible with existing host context
    HostApiUnsupportedScenario          = 0x800080a7,   // Hosting API does not support the requested scenario
    HostFeatureDisabled                 = 0x800080a8,   // Support for a requested feature is disabled
};
