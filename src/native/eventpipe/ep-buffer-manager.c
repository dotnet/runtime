	...
	ep_timestamp_t before_timestamp)
{
	...
	EventPipeBuffer *current_buffer = NULL;
	bool done = false;
	EP_SPIN_LOCK_ENTER (&buffer_manager->rt_lock, section1)
		buffer_list = ep_thread_session_state_get_buffer_list (thread_session_state);
		EP_ASSERT (buffer_list != NULL);
	...
	EventPipeStackContents *stack)
{
	...
	EP_ASSERT (ep_thread_is_rundown_thread (current_thread) || ep_thread_get_session_write_in_progress (current_thread) == ep_session_get_index (session));
