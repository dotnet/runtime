	...
	EventPipeStackContents *stack_contents)
{
	...
	characters_written = ep_rt_utf8_string_snprintf (buffer, MINIPAL_LENGTHOF (buffer), "{\"Time\" : \"%f\", \"Metric\" : \"1\",\n\"Stack\": [\n\"", millis_since_trace_start);
	if (characters_written > 0 && characters_written < (int32_t)MINIPAL_LENGTHOF (buffer))
	...
	for (uint32_t i = 0; i < ep_stack_contents_get_length (stack_contents); ++i) {
		...
		if (!ep_rt_method_get_simple_assembly_name (method, assembly_name, MINIPAL_LENGTHOF (assembly_name))) {
			assembly_name [0] = '?';
			assembly_name [1] = 0;
		}

		if (!ep_rt_method_get_full_name (method, method_name, MINIPAL_LENGTHOF (method_name))) {
			method_name [0] = '?';
			method_name [1] = 0;
		}

		characters_written = ep_rt_utf8_string_snprintf (buffer, MINIPAL_LENGTHOF (buffer), "\"%s!%s\",\n", assembly_name, method_name);
		if (characters_written > 0 && characters_written < (int32_t)MINIPAL_LENGTHOF (buffer))
			json_file_write_string (json_file, buffer);
	}

	characters_written = ep_rt_utf8_string_snprintf (buffer, MINIPAL_LENGTHOF (buffer), "\"Thread (%" PRIu64 ")\"]},", ep_rt_thread_id_t_to_uint64_t (thread_id));
	if (characters_written > 0 && characters_written < (int32_t)MINIPAL_LENGTHOF (buffer))
		json_file_write_string (json_file, buffer);
}
