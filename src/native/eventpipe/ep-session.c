	...
	EventPipeSession *session)
{
	...
	for (dn_list_it_t it = dn_list_begin (ep_session_provider_list_get_providers (providers)); !dn_list_it_end (it); it = dn_list_it_next (it)) {
		...
		for (int32_t i = 0; i < tracepoints->size; ++i) {
			...
			tracepoint->write_index = (uint32_t *)malloc (sizeof (uint32_t));
			EP_ASSERT(tracepoint->write_index != NULL);
			tracepoint->enabled = (uint32_t *)malloc (sizeof (uint32_t));
			EP_ASSERT(tracepoint->enabled != NULL);
		...
		if (default_tracepoint_name != NULL) {
			...
			default_tracepoint->write_index = (uint32_t *)malloc (sizeof (uint32_t));
			EP_ASSERT(default_tracepoint->write_index != NULL);
			default_tracepoint->enabled = (uint32_t *)malloc (sizeof (uint32_t));
			EP_ASSERT(default_tracepoint->enabled != NULL);
