...
namespace JIT.HardwareIntrinsics.X86
{
    ...
    public sealed unsafe class SimpleTernaryOpTest__MultiplyAddNegatedSingle
    {
        ...
        {
            var result = typeof(Fma).GetMethod(nameof(Fma.MultiplyAddNegated), new Type[] { typeof(Vector256<Single>), typeof(Vector256<Single>), typeof(Vector256<Single>) })
        ...
        private void ValidateResult(Single[] firstOp, Single[] secondOp, Single[] thirdOp, Single[] result, [CallerMemberName] string method = "")
        {
            ...
            if (!Succeeded)
            {
                Console.WriteLine($"{nameof(Fma)}.{nameof(Fma.MultiplyAddNegated)}<Single>(Vector256<Single>, Vector256<Single>, Vector256<Single>): {method} failed:");
