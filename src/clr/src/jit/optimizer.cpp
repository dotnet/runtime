...
void                Compiler::optOptimizeInducIndexChecks(unsigned loopNum, arraySizes arrayDesc[])
{
    noway_assert(loopNum < optLoopCount);

    JITDUMP("Considering loop %d for induction variable range checks.\n", loopNum);

    LoopDsc *       loop = optLoopTable + loopNum;
    
    const unsigned LOOP_LEN_NOCONST = ((unsigned)-1);
    unsigned        constLoopBound = LOOP_LEN_NOCONST;
    ...
    JITDUMP("  (possible) iter var is V%02u.\n", ivLclNum);
    ...
    if (!(loop->lpFlags & LPFLG_CONST_INIT))
    {
        JITDUMP("  Rejected V%02u as iter var because of non-constant init.\n", ivLclNum);
        return;
    }

    int            posBias = loop->lpConstInit;

    if (posBias < 0)
    {
        JITDUMP("  Rejected V%02u as iter var because of negative init.\n", ivLclNum);
        return;
    }
    ...
    {
        JITDUMP("  Rejected V%02u as iter var because of stride different from 1.\n", ivLclNum);
        return;
    }
    ...
    if (end->bbJumpKind != BBJ_COND)
    {
        JITDUMP("  Gave up on loop: couldn't find termination test.\n");
        return;
    }
    ...
    GenTreePtr conds = optFindLoopTermTest(end);

    if  (conds == NULL)
    {
        JITDUMP("  Gave up on loop: couldn't find loop condition.\n");
        return;
    }

    /* Get to the condition node from the statement tree */

    noway_assert(conds->gtOper == GT_STMT);
    noway_assert(conds->gtStmt.gtStmtExpr->gtOper == GT_JTRUE);
    // pointer to the node containing the loop test
    noway_assert(conds->gtStmt.gtStmtExpr->gtOp.gtOp1 == loop->lpTestTree);     

    /* if test isn't less than or less than equal, forget it */
    if (loop->lpTestOper() != GT_LT && loop->lpTestOper() != GT_LE)
    {
        JITDUMP("  Gave up on loop: loop condition in wrong form.\n");
        return;
    }

#ifdef DEBUG
    GenTreePtr op1 = loop->lpIterator();
    noway_assert((op1->gtOper==GT_LCL_VAR) && (op1->gtLclVarCommon.gtLclNum==ivLclNum));
#endif

    /* op2 is the loop termination test */
    GenTreePtr op2 = loop->lpLimit();
    ...
    bool isLessThanOrEqual = (loop->lpTestOper() == GT_LE);
...
AGAIN:
    ...
    case GT_ADD:
        ...
        if (rc->gtOp.gtOp2->IsCnsIntOrI()
            && rc->gtOp.gtOp2->gtIntCon.gtIconVal < 0
            && rc->gtOp.gtOp1->gtOper == GT_ARR_LENGTH)
        {
            // If less than or equal make sure the negative constant is at least 1
            ssize_t ivalTemp = -rc->gtOp.gtOp2->gtIntCon.gtIconVal;
            if ((ivalTemp < 0x7fffffff) && (!isLessThanOrEqual || ivalTemp > 0))
            {
                negBias = (int)ivalTemp;
    ...
    case GT_SUB:
        ...
        if (rc->gtOp.gtOp2->IsCnsIntOrI()
            && (size_t)rc->gtOp.gtOp2->gtIntCon.gtIconVal < 0x7fffffff
            && (!isLessThanOrEqual || (size_t)rc->gtOp.gtOp2->gtIntCon.gtIconVal > 0)
            && rc->gtOp.gtOp1->gtOper == GT_ARR_LENGTH)
        {
            negBias = (int)rc->gtOp.gtOp2->gtIntCon.gtIconVal;
            op2 = rc = rc->gtOp.gtOp1;
            goto AGAIN;
        }
        break;
    ...
    case GT_LCL_VAR:
        ...
        if  (!varDscRc)
        {
            JITDUMP("  Gave up on loop: can't prove loop condition constant (depends on untracked var).\n");
            return;
        }

        // If address-exposed, quit.
        if (varDscRc->lvAddrExposed)
        {
            JITDUMP("  Gave up on loop: can't prove loop condition constant (depends on address-exposed var).\n");
            return;
        }

        /* if altered, then quit */
        if (optIsVarAssgLoop(loopNum, rc->gtLclVarCommon.gtLclNum))
        {
            JITDUMP("  Gave up on loop: can't prove loop condition constant (depends on var modified in loop).\n");
            return;
        }
    ...
    case GT_CNS_INT:
        /* save the const loop limit */
        ival = rc->gtIntCon.gtIconVal;
        // If less than or equal make sure the negative constant is at most max - 1.
        if (ival < 0x7fffffff && (!isLessThanOrEqual || ival < 0x7ffffffe))
            constLoopBound = (unsigned)rc->gtIntCon.gtIconVal;
        break;

    default:
        JITDUMP("  Gave up on loop: loop condition in unrecognized form.\n");
        return;
    }

    if (op2->gtOper  == GT_LCL_VAR)
    {
        VARSET_TP VARSET_INIT_NOCOPY(killedVars, VarSetOps::MakeEmpty(this));
        bool isLhsRhsKilled = false;

        GenTreePtr opInitFound = NULL;
        BasicBlock* block = head;
        int iterCount = 0;
        while (block != NULL && opInitFound == NULL && !isLhsRhsKilled)
        {
            opInitFound = optFindLocalInit(block, op2, &killedVars, &isLhsRhsKilled);
            flowList* preds = block->bbPreds;
            // Exactly one predecessor.
            block = (preds != NULL && preds->flNext == NULL) ? preds->flBlock : NULL;
            if ((block == head) || (++iterCount >= 10))
            {
                break;
            }
        }
    ...
    if (constLoopBound == LOOP_LEN_NOCONST)
    {
        bool isArrLenVarNumFound = false;
        if (op2)
        {
            switch (op2->gtOper)
            {
            // The default case (< or <= array.length)
            case GT_ARR_LENGTH:
                if (op2->gtArrLen.ArrRef()->gtOper == GT_LCL_VAR)
                {
                    // Don't allow <= if there isn't any neg bias .
                    if (loop->lpTestOper() == GT_LT || (loop->lpTestOper() == GT_LE && negBias > 0))
                    {
                        arrayLclNum = op2->gtArrLen.ArrRef()->gtLclVarCommon.gtLclNum;
                        isArrLenVarNumFound = true;
                    }
                }
                break;

            // Cases where we have (< or <= array.length + negative_K) or (< or <= array.length - positive_K)
            case GT_SUB:
            case GT_ADD:
                {
                    GenTreePtr arrOpLen = op2->gtOp.gtOp1;
                    GenTreePtr arrOpConst = op2->gtOp.gtOp2;

                    // Op1 should be arr length
                    if (arrOpLen->gtOper != GT_ARR_LENGTH ||
                        arrOpLen->gtArrLen.ArrRef()->gtOper != GT_LCL_VAR ||
                        !arrOpConst->IsCnsIntOrI())
                    {
                        break;
                    }

                    // If GT_ADD, negate the constant and turn it into positive value.
                    ssize_t ivalTemp = (op2->gtOper == GT_ADD) ? (-arrOpConst->gtIntCon.gtIconVal)
                                                               : arrOpConst->gtIntCon.gtIconVal;
                    // Cannot remove range check if:
                    // 1. A negative constant for sub and positive constant for add.
                    // 2. A value >= 0x7fffffff for constant.
                    // 3. If the loop condition is GT_LE, the constant value should be >= 1. i.e., arrLen + (-1).
                    if ((ivalTemp < 0) || (ivalTemp >= 0x7fffffff) || (isLessThanOrEqual && ivalTemp <= 0))
                    {
                        break;
                    }
                    negBias = static_cast<int>(ivalTemp);
                    op2 = arrOpLen;
                    arrayLclNum = arrOpLen->gtArrLen.ArrRef()->gtLclVarCommon.gtLclNum;
                    isArrLenVarNumFound = true;
                }
                break;
...
#endif
        {
            ...
            if  (!varDscArr ||
                optIsVarAssgLoop(loopNum, (unsigned)(varDscArr - lvaTable)))
            {
#if FANCY_ARRAY_OPT
                arrayLclNum = CONST_ARRAY_LEN;
#else
                JITDUMP("  Gave up on loop: loop condition array var not tracked or modified in loop.\n");
                return;
#endif
            }
        }
    ...
    {
        // If constLoopBound > all the array dimensions we know, we can't remove the range check.
        bool isRemovable = false;
        for (unsigned i = 0; i < MAX_ARRAYS; i++)
        {
            if (static_cast<int>(constLoopBound) <= arrayDesc[i].arrayDim)
            {
                isRemovable = true;
                break;
            }   
        ...
        if (!isRemovable)
        {
            JITDUMP("  Gave up on loop: constant loop bound not removable.\n");
            return;
        }
    }
            ...
            for (GenTreePtr tree = stmt->gtStmtList; tree; tree = tree->gtNext)
            {
                ...
                if (tree->OperKind() & GTK_ASGOP)
                {
                    if (tree->gtOp.gtOp1->gtOper == GT_LCL_VAR
                        && tree->gtOp.gtOp1->gtLclVarCommon.gtLclNum == ivLclNum)
                        return;
                }

                // To facilitate removal of the range check, look for its parent,
                // which should be GT_COMMA
                if (tree->gtOper == GT_COMMA && tree->gtOp.gtOp1->OperGet() == GT_ARR_BOUNDS_CHECK)
                {
                    int size;
                    int delta = 0;

                    GenTreeBoundsChk* bndsChk = tree->gtOp.gtOp1->AsBoundsChk();
                    GenTreePtr arr = bndsChk->GetArray();
                    if (arr == NULL)
                    {
                        continue;
                    }
                    ...
                    if (constLoopBound != LOOP_LEN_NOCONST)
                    {
                        for (unsigned i = 0; i < MAX_ARRAYS; i++)
                        {
                            if  (arr->gtLclVarCommon.gtLclNum == arrayDesc[i].arrayVar)
                            {
                                // retrieve array size
                                size = arrayDesc[i].arrayDim;
                                if (size >= (int)constLoopBound)
                                {
                                    delta = size - (int)constLoopBound;
                                    goto CHK_INDX;
                                }
                            }
                        }

                        //does not match known arrays, so quit
                        continue;
                    }

                    if  (arr->gtLclVarCommon.gtLclNum != arrayLclNum)
                    {
#if FANCY_ARRAY_OPT
                        if  (arrayLclNum == CONST_ARRAY_LEN)
                        {
                            ...
                            noway_assert(arr->gtLclVarCommon.gtLclNum < lvaCount);
                            arrayDsc = lvaTable + arr->gtLclVarCommon.gtLclNum;

                            if  (arrayDsc->lvKnownDim)
                            {
                                if  (optIsNoMore(loopLim, arrayDsc->lvKnownDim, (loop->lpTestOper() == GT_LE)))
                                {
                                    index = (*pnop)->gtOp.gtOp1;

                                    // TODO: Allow "i+1" and things like that
...
CHK_INDX:
                    ...
                    */
                    if (index->gtOper == GT_SUB
                        && index->gtOp.gtOp2->IsCnsIntOrI())
                    {
                        ssize_t ival = index->gtOp.gtOp2->gtIntCon.gtIconVal;
                 ...
                 /* allow add of constant to induction variable
                    ...
                    */
                    if (index->gtOper == GT_ADD
                        && index->gtOp.gtOp2->IsCnsIntOrI())
                    {
                        // GT_ADD with -ve or +ve offsets
                        ssize_t ival = index->gtOp.gtOp2->gtIntCon.gtIconVal;
                        if ((ival >= 0 && ival <= negBias)|| (ival < 0 && -ival <= posBias) ||((ival >=0 )&& (ival <= delta)))
                            index = index->gtOp.gtOp1;
                    }
...
RMV:
                    ...
                    if (!(index->gtOper == GT_LCL_VAR
                        && index->gtLclVarCommon.gtLclNum == ivLclNum))
                        continue;
...
void                Compiler::optOptimizeIndexChecks()
{
    ...
    /*
      * we track MAX_ARRAYS arrays
      * unsigned        arrayVar;
      * long            arrayDim = DUMMY_INIT(~0);
      */
      
    arraySizes arrayDesc[MAX_ARRAYS];
    unsigned arrayIndx = 0;
    ...
    unsigned indx;
    for (indx=0; indx<MAX_ARRAYS;indx++)
    {
        arrayDesc[indx].arrayVar = NO_ARR_VAR;
        arrayDesc[indx].arrayDim = DUMMY_INIT(~0);
    }

    /* Walk all the basic blocks in the function. We don't need to worry about
    ...
    for (BasicBlock * block = fgFirstBB; block; block = block->bbNext)
    {
        ...
        if  (!(block->bbFlags & BBF_HAS_NEWARRAY))
            continue;
...
#if JIT_FEATURE_SSA_SKIP_DEFS
        for (stmt = block->FirstNonPhiDef(); stmt; stmt = stmt->gtNextStmt)
#else
        for (stmt = block->firstStmt(); stmt; stmt = stmt->gtNextStmt)
#endif
        {
            for (tree = stmt->gtStmtList; tree; tree = tree->gtNext)
            {
                switch (tree->gtOper)
                {
                    GenTreePtr      op1;
                    GenTreePtr      op2;
                    GenTreePtr      arrayDimNode;
                ...
                case GT_ASG:
                    ...
                    // If the variable has its address taken, we cannot remove the range check
                    //
                    if (lvaTable[op1->gtLclVarCommon.gtLclNum].lvAddrExposed)
                    {
                        break;
                    }
#if ASSERTION_PROP
                    // If the variable has multiple defs we cannot remove the range check
                    // 
                    if (!lvaTable[op1->gtLclVarCommon.gtLclNum].lvSingleDef)
                    {
                        break;
                    }
#endif
                    ...
                    noway_assert(op2->gtOper == GT_CALL);

                    fgArgTabEntryPtr arg1TabEntry;  arg1TabEntry = gtArgEntryByArgNum(op2, 1);
                    arrayDimNode = arg1TabEntry->node;

                    PREFAST_ASSUME(arrayDimNode != NULL);
                    
                    if (arrayDimNode->IsCnsIntOrI())
                    {
                        /* We have a constant-sized array */
                       arrayDesc[arrayIndx].arrayVar = op1->gtLclVarCommon.gtLclNum;
                       size_t arrayDim = arrayDimNode->gtIntCon.gtIconVal;
                       if (arrayDim > 0x7fffffff)
                           break;
                       arrayDesc[arrayIndx].arrayDim = (int)arrayDim;
                       arrayIndx = (arrayIndx + 1) % MAX_ARRAYS;
                       arrayInit = true;
                    }
#if FANCY_ARRAY_OPT
                    else
                    {
                        ...
                        GenTreePtr  tmp = arrayDimNode;
                        if  (tmp->gtOper == GT_ADD &&
                             tmp->gtOp.gtOp2->IsCnsIntOrI())
                            tmp = tmp->gtOp.gtOp1;

                        if  (tmp->gtOper != GT_LCL_VAR)
                            break;

                        noway_assert(tmp->gtLclVarCommon.gtLclNum < lvaCount);
                        arrayDsc = lvaTable + tmp->gtLclVarCommon.gtLclNum;
                    ...
                    noway_assert(op1->gtLclVarCommon.gtLclNum < lvaCount);
                    arrayDsc = lvaTable + op1->gtLclVarCommon.gtLclNum;
                    ...
                    arrayDsc->lvKnownDim = arrayDimNode;
#endif
                ...
                case GT_COMMA:
#if FANCY_ARRAY_OPT
                    // TODO-CQ: Figure out what to do for this - is FANCY_ARRAY_OPT ever enabled???
                    if  (op2 != nullptr && op2->OperGet() == GT_IND && (op2->gtFlags & GTF_IND_RNGCHK))
#else
                    if  ((tree->gtOp.gtOp1->OperGet() == GT_ARR_BOUNDS_CHECK) && arrayInit)
#endif
                    {
                        GenTreeBoundsChk* bndsChk = tree->gtOp.gtOp1->AsBoundsChk();
                        GenTreePtr arr = bndsChk->GetArray();
                        ...
                        if  (arr == NULL || arr->gtOper != GT_LCL_VAR)
                            break;

                        GenTreePtr ind = bndsChk->gtIndex;
                        if (!ind->IsCnsIntOrI())
                            break;
                        ...
                        for (indx=0; indx<MAX_ARRAYS;indx++)
                        {
                            if  (arr->gtLclVarCommon.gtLclNum == arrayDesc[indx].arrayVar)
                            {
                                //noway_assert(arrayInit);
                                size = arrayDesc[indx].arrayDim;
                                break;
                             }
                        }
                        ...
                        if  (ind->gtIntConCommon.IconValue() < 0)
                            break;
                        if  (ind->gtIntConCommon.IconValue()  >= size)
                            break;
    ...
    for (unsigned i=0; i < optLoopCount; i++)
    {
        ...
        if (!(optLoopTable[i].lpFlags & LPFLG_REMOVED))
           optOptimizeInducIndexChecks(i, arrayDesc);
    }
}
