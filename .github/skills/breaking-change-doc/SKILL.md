...
- **Automated**: `.github/workflows/breaking-change-doc-dispatch.yml` dispatches
  the [GitHub Agentic Workflow](https://github.github.com/gh-aw/) when a PR
  labeled `needs-breaking-change-doc-created` is merged (or the label is added
  to an already-merged PR). The agentic workflow can also be run manually via
  `workflow_dispatch` with an optional `suppress_output` flag for dry-run
  inspection.
...
| File | Purpose |
|------|---------|
| `.github/workflows/breaking-change-doc-dispatch.yml` | Dispatches the gh-aw workflow on PR merge/label |
| `.github/workflows/breaking-change-doc.md` | gh-aw workflow — runs via `workflow_dispatch` |
