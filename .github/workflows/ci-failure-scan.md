...
**Match-count gate.** Reject the emit if the body lacks `<!-- ci-scan-match-count: <N> hits in failure.log -->` with `N >= 1`. Record `skipped: KBE match-count gate (N=<count or absent>)` and stop. Rationale, log-source caveats, and native-assert handling live in check #7 of the shared instructions.
