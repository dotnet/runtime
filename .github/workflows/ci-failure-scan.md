...
**Pre-emit gate (hard reject).** Before calling `create_issue`, scan the proposed body for the literal marker `<!-- ci-scan-match-count: <N> hits in failure.log -->` produced by KBE-verification check #7. If absent, or if `N == 0`, do NOT emit. Record `skipped: KBE body missing positive match-count marker` and stop. A KBE without a verified positive count is guaranteed Build Analysis noise; the BA Report block at the bottom of such an issue will stay at `0 / 0 / 0` indefinitely, the agent will re-file it on the next scan as a "recurrence", and maintainers will eventually close it without action (see #128544 -> #128827 for the failure mode this gate exists to prevent).

For JIT, runtime, and build-level asserts specifically: regex vs substring is not the lever. Both match against the same set of logs BA indexes; switching `ErrorMessage` -> `ErrorPattern` does not reach a different log file. The lever is the array form (KBE check #7), which pairs an anchor present in the xunit log (test workitem name) with the assert text — at least one anchor will hit even when BA's log set does not include the raw console capture. If neither anchor greps positive in the available failure log, treat as `skipped: native assert not in indexed log source` per check #7 and do NOT file the KBE.
