...
**Full-leg failure detection.** Before scoring individual signatures within a single leg, check the leg's work-item failure ratio. When > 80% of a leg's work items fail (e.g., all arm32 NativeAOT tests crash with SIGBUS/segfault, the whole run on the floor), treat this as a leg-level regression, not N independent test bugs. File ONE leg-scoped KBE keyed to `(definition_id, leg)` using the most representative signature (shared crash signal such as the common exit code, signal, or assertion), apply both `Known Build Error` and `blocking-clean-ci` labels, and SKIP the individual per-test signatures in that leg with `skipped: leg-level failure filed as #aw_<id>`. This prevents a per-test KBE flood and duplicate closures when the failure is a single leg-level break rather than N test bugs. Do not split a leg-level failure across multiple per-test KBEs.
