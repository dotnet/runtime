...
**Match-count gate.** Reject the emit unless the KBE body contains the visible verification field defined by check #7 of the shared instructions, with `N >= 1`. Treat an absent field as `N=0` and record the same skip reason check #7 uses: `skipped: signature did not match failure.log (N=<count>)`. Rationale, log-source caveats, and native-assert handling live in check #7.
