...
Keep the `COPILOT_GITHUB_TOKEN` expression on a single line: with gh-aw v0.82.6+ it is also emitted into a `Check for OAuth tokens` activation step whose generated YAML is invalid when the expression spans multiple lines. `gh-aw compile` automatically wires `pat_pool` into the activation and agent jobs' `needs:` graph because of the `needs.pat_pool.` references within the `engine.env` property.
